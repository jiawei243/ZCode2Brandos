...
Agent CLI 与运行时源码位于 [apps/wbrand-cli/](apps/wbrand-cli/)，作为普通目录随本仓库一起克隆，无需单独拉取或初始化 Git submodule。
...
需要独立开发数据目录时，可设置 `WBRAND_DATA_BASE_DIR`。例如在 macOS / Linux 中：
...
Agent 源码修改后，执行 `pnpm --filter @wbrand/cli... build` 并重启服务。需要验证完整发行包时，按下方“WBrand 命令行版”打包章节解压运行。

### WBrand 命令行版

命令行发行包包含 TUI、Web 和 Agent，统一使用 `wbrand` 启动：无参数进入 TUI；第一个参数为 `--web` 时启动 Web；其他参数交给现有 Agent CLI 处理。两种模式都在本机运行，无需 Electron。
...
直接启动通用 Web 服务的 HTTP 入口时，通过 `WBRAND_SERVER_AUTH_TOKEN` 配置 API／WebSocket 认证；通过程序接口创建服务时，使用 `authToken` 选项。

构建方式见下方打包章节。`pnpm build:wbrand` 只生成发行包，不会替换 `PATH` 中已有的 `wbrand`。如果命令仍指向旧安装或其他源码目录，macOS / Linux 可用 `command -v wbrand` 检查，Windows 可用 `where.exe wbrand` 检查。
...
这个入口直接运行 Agent CLI，不经过发行包的 `--web` 分流。开发 Web 用 `pnpm dev:web`；验证统一的 `wbrand` 命令，用下方解压后的 `bin/wbrand.mjs`。
...
安装：双击打开产物 DMG，将 WBrand 拖入"应用程序"。本地构建未签名，首次打开若被 macOS 拦截，执行：

```bash
sudo xattr -rd com.apple.quarantine /Applications/WBrand.app
```

### WBrand 命令行版

构建入口为 `pnpm build:wbrand`。脚本会依次构建 CLI/TUI、后端和 Web，收集 TUI 的原生库、worker 与运行时依赖，再组装发行包；运行发行包仍需要 Node.js，版本以 `mise.toml` 为准。

打包前必须设置下载根地址 `WBRAND_DIST_BASE_URL`（可放在 `.env`、`.env.local` 或环境变量中），也可以通过 `--base-url` 传入。以下地址是占位示例，发布时替换为实际托管地址：

```bash
pnpm build:wbrand --base-url https://downloads.example.com/wbrand/

# 已配置 WBRAND_DIST_BASE_URL 时
pnpm build:wbrand
...
默认版本取根目录 `package.json`，输出目录为 `dist/wbrand/`：

- `releases/<version>/wbrand-<version>.tar.gz`：运行包。
- `releases/<version>/sha256.txt`：校验摘要。
- `latest.json`、`install.sh`：版本索引和安装脚本。

完整目录可上传到配置的下载根地址。安装脚本从该地址下载运行包，默认安装到 `~/.wbrand/runtime`，并在 `~/.local/bin` 创建 `wbrand` 命令。安装目录可通过 `WBRAND_DIST_HOME` 修改，命令目录可通过 `WBRAND_DIST_BIN_DIR` 修改。
