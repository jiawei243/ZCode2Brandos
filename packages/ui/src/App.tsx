...
import type { GitChangeSourceId, WorkspacePurpose } from "@wbrand/shared";
import { useWBrandStore } from "@/store/StoreProvider.js";
import { getVisibleTaskMetas, useWBrandSessionStore } from "@/store/wbrandSessionStore.js";
...
import { WBRAND_PRODUCT_DOCS_URL } from "@/lib/productDocs.js";
...
import { useWorkspaceShellWBrandState } from "@/app-shell/useWorkspaceShellWBrandState.js";
...
export function App({
  ...
  const { intl, locale, setLocale } = useWBrandIntl();
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellWBrandState, reloadSessionDisabled } = useWorkspaceShellWBrandState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellWBrandState.activeTaskId;
  ...
  const draftSessionId = useWBrandSessionStore(
  ...
  const draftFocusVersion = workspaceShellWBrandState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useWBrandStore((s) => s.notificationEnabled);
  ...
  const theme = useWBrandStore((s) => s.theme);
  const setTheme = useWBrandStore((s) => s.setTheme);
  const {
    isMacFullscreen,
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellWBrandState.selectedProvider,
    intl,
  });
  ...
  const handleOpenProductDocs = useCallback(() => {
    platform.openExternal(WBRAND_PRODUCT_DOCS_URL);
  }, [platform]);
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      createSource?: import("@wbrand/shared").SessionCreateSource,
    ) => {
      const store = useWBrandSessionStore.getState();
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellWBrandState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellWBrandState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginsOverview: (params) => services.wbrandAgentService.getPluginsOverview(params),
      addPluginMarketplace: (params) => services.wbrandAgentService.addPluginMarketplace(params),
      updatePluginMarketplace: (params) =>
        services.wbrandAgentService.updatePluginMarketplace(params),
      installPlugin: (params) => services.wbrandAgentService.installPlugin(params),
      listPlugins: (params) => services.wbrandAgentService.listPlugins(params),
      getPluginReferenceCatalog: (params) =>
        services.wbrandAgentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.wbrandAgentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useWBrandSessionStore.getState();
  ...
  return (
    <>
      ...
      <WorkspaceShellLayout
        ...
        workspaceShellWBrandState={workspaceShellWBrandState}
        theme={theme}
