...
import type { IBroadcastService } from "@wbrand/services";
import { createWBrandStore, type WBrandStore, type WBrandState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<WBrandStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<WBrandStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createWBrandStore(broadcastService, {
 ...
 */
export function useWBrandStore<T>(selector: (state: WBrandState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useWBrandStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useWBrandStore（store 耦合剥离配套）。
 *
 ...
 */
export function useWBrandStoreWithDefault<T>(
  selector: (state: WBrandState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
