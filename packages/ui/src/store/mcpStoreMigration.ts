import type { McpServerConfig, NativeMcpServerRecord } from "@wbrand/shared";
...
function isWBrandAgentUserServer(server: NativeMcpServerRecord): boolean {
  return (
    server.source === "wbrandagentmcp" &&
    server.scope === "user" &&
    (!server.location || server.location.source === "wbrand")
  );
}

export async function importLegacyCommonServersToWBrandAgent(
  ...
  const existingNames = new Set(
    nativeServers.filter(isWBrandAgentUserServer).map((server) => server.name),
  );
  let importedCount = 0;
  let skippedCount = 0;
...
export async function migrateStoredCommonMcpToWBrandAgent(
  platform: McpPlatformService | null,
  nativeServers: NativeMcpServerRecord[],
  ...
  const migration = await importLegacyCommonServersToWBrandAgent(
    ...
    legacyServers,
    nativeServers,
    "localStorage:wbrand-mcp-config",
  );
  if (migration.completed) {
    // 只有确认写入 wbrand agent 目录后才清理旧数据，避免 Web 端没有 desktop bridge 时丢配置。
    clearLegacyCommonMcpServers();
  }
  ...
  logger.info(
    `[mcpStore] migrated ${migration.importedCount} legacy common MCP servers to wbrand agent config`,
  );
