/**
 * WBrand Session Store 选择器与内部辅助函数
 *
 * 从 wbrandSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { WBrandTaskRuntimeStatus, WBrandTaskMeta } from "@wbrand/shared";
...
import {
  ...
  type WBrandSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceWBrandUIState,
} from "./wbrandSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceWBrandUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceWBrandUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: WBrandSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceWBrandUIState) => WorkspaceWBrandUIState,
  workspaceIdentity?: string,
): Pick<WBrandSessionStoreState, "workspaces"> {
  const workspaceKey = resolveWorkspaceStateKey(workspacePath, workspaceIdentity);
  const current =
  ...
  if (nextWorkspaceState === current) {
    // 单 WBrand Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceWBrandUIState,
  taskId: string,
): TaskRuntimeState {
...
export function getTaskUiState(workspaceState: WorkspaceWBrandUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceWBrandUIState
    | Partial<Pick<WorkspaceWBrandUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
): WBrandTaskMeta | null {
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, WBrandTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<WBrandTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceWBrandState(
  state: WBrandSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceWBrandUIState, "workspaceInit">,
): WorkspaceInitState {
