...
import type { AttachmentRef } from "@wbrand/shared/wbrand-protocol-v4";
...
declare global {
  interface Window {
    __wbrandComposerAttachmentUploadStoreE2E?: typeof useComposerAttachmentUploadStore;
    __wbrandCurrentComposerAttachmentScopeKeyE2E?: string;
  }
}

if (shouldExposeE2EStoreBridge()) {
  // E2E 只暴露当前唯一附件 owner，供 scope 切换用例准备状态；不再把附件塞回旧 Session Store。
  window.__wbrandComposerAttachmentUploadStoreE2E = useComposerAttachmentUploadStore;
}

export function exposeComposerAttachmentScopeKeyForE2E(scopeKey: string): void {
  if (window.__wbrandComposerAttachmentUploadStoreE2E) {
    window.__wbrandCurrentComposerAttachmentScopeKeyE2E = scopeKey;
  }
}
