...
import type { Hook, HookConfig } from "@wbrand/shared";
import type { IHooksService } from "@wbrand/services";
...
function buildWBrandHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildWBrandHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "wbrand") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildWBrandHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
