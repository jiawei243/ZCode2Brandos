...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToWBrandAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToWBrandAgent(params.provider);
