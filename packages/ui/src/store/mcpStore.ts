...
import { convertToWBrandAgentMcpServer } from "@wbrand/shared";
...
import {
  importLegacyCommonServersToWBrandAgent,
  migrateStoredCommonMcpToWBrandAgent,
...
interface McpStoreState {
  ...
  statusSnapshots: Record<string, WBrandMcpServerStatusSnapshot>;
  ...
  addWBrandAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  updateWBrandAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  deleteWBrandAgentMcpServer: (name: string, projectPath?: string) => void;
  ...
  beginServerStatusListRefresh: (mode?: WBrandMcpListMode) => number;
  ...
  getServer: (id: string) => WBrandMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForWBrand: (provider: string) => WBrandAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<WBrandMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 WBrand Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.wbrand/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 WBrand Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToWBrandAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addWBrandAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateWBrandAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteWBrandAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "wbrandagentmcp" : source;
    ...
    addWBrandAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("wbrandagentmcp", name, config, projectPath),
    updateWBrandAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("wbrandagentmcp", name, config, projectPath),
    deleteWBrandAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("wbrandagentmcp", name, projectPath),
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "wbrandagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForWBrand: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, WBrandMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "wbrandagentmcp") continue;
      ...
      const result: WBrandAgentMcpServer[] = [];
      for (const server of candidates.values()) {
        const wbrandAgentServer = convertToWBrandAgentMcpServer(server.name, server.config);
        if (wbrandAgentServer) result.push(wbrandAgentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "wbrandagentmcp";
    ...
    migrateLegacyCommonMcp: async () => {
      ...
      const migration = await importLegacyCommonServersToWBrandAgent(
        mcpPlatformService,
        result.servers ?? {},
