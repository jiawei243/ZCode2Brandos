...
import type { WBrandTaskListItem } from "@wbrand/services";
import type { WBrandTaskMeta } from "@wbrand/shared";
import { matchesTaskListMembershipKind } from "@wbrand/shared/wbrand-protocol-v4";
import { mergeTaskMetaCandidates } from "@/lib/wbrandTaskMetaMerge.js";
import { compareWBrandTaskListItems } from "@/lib/taskListOrdering.js";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: WBrandTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: WBrandTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: WBrandTaskMeta;
    nextTask: WBrandTaskMeta;
    previousState: TaskListMembershipState;
    nextState: TaskListMembershipState;
  }) => void;
  setTaskUnreadOverlay: (
    task: Pick<WBrandTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">,
...
type CachedTaskListItem = WBrandTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: WBrandTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, WBrandTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareWBrandTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
