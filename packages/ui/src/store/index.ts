...
import type { IBroadcastService, BroadcastMessage } from "@wbrand/services";
import type { OAuthProviderId, UserInfo } from "@wbrand/shared";
import type { CodingPlanResetType } from "@wbrand/shared";
...
const CODE_PREVIEW_SETTINGS_KEY = "wbrand-code-preview-settings";
const PERFORMANCE_MODE_STORAGE_KEY = "wbrand-performance-mode";
 ...
 */
export function createWBrandStore(
  ...
  const useStore = create<WBrandState>()((set, get) => ({
    ...
    theme: normalizeThemePreference((readSafeLocalStorage("wbrand-theme") as Theme) || "zai-dark"),
    setTheme: (theme: Theme) => {
      const normalizedTheme = normalizeThemePreference(theme);
      writeSafeLocalStorage("wbrand-theme", normalizedTheme);
    ...
    locale: readSafeLocalStorage("wbrand-locale") || "zh-CN",
    setLocale: (locale: string) => {
      writeSafeLocalStorage("wbrand-locale", locale);
      set({ locale });
    },
...
export type WBrandStore = ReturnType<typeof createWBrandStore>;
