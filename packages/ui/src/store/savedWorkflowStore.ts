...
import {
  ...
  type WBrandSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: WBrandSavedWorkflowEntry[];
  invalid: WBrandSavedWorkflowInvalidEntry[];
  runs: WBrandSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: WBrandAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: WBrandAgentSavedWorkflowTarget,
  agentService: IWBrandAgentService,
): Promise<{ list: WBrandWorkflowsListResult; runs: WBrandSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as WBrandSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: WBrandAgentSavedWorkflowTarget | null,
