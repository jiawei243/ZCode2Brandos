...
} from "@/store/wbrandSessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/wbrandSessionStoreSelectors.js";
...
function normalizeThoughtLevelConfigOption(option: WBrandConfigOption): WBrandConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: WBrandConfigOption[]): WBrandConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly WBrandConfigOption[]): WBrandConfigOption[] {
...
function isModeConfigOption(option: WBrandConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<WBrandConfigOption["options"]>;
}): WBrandConfigOption {
...
function replaceModeConfigOption(
  currentOptions: WBrandConfigOption[] | null,
  nextModeOption: WBrandConfigOption | null,
...
function updateActiveTaskConfigOptions(
  current: WorkspaceWBrandUIState,
  updater: (options: WBrandConfigOption[] | null) => WBrandConfigOption[] | null,
): Pick<WorkspaceWBrandUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceWBrandUIState,
  taskId: string,
): { configOptions: WBrandConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: WBrandTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as WBrandTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToWBrandAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToWBrandAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToWBrandAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
