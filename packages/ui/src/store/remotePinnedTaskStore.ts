...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    wbrandTaskService: Pick<IWBrandTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: WBrandTaskMeta) => void;
...
function sortPinnedTasks(tasks: WBrandTaskMeta[]): WBrandTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, wbrandTaskService }) {
    ...
    try {
      const items = await wbrandTaskService.listPinnedTasks({
        workspacePath,
