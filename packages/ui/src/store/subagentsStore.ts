...
import type { ISubagentsService } from "@wbrand/services";
...
interface SubagentsStoreState {
  ...
  loadedProvider: WBrandProvider | null;
  agents: AgentSummary[];
  ...
  initialize: (
    ...
    providerOrSubagentsService: WBrandProvider | ISubagentsService,
...
export const useSubagentsStore = create<SubagentsStoreState>((set, get) => ({
  ...
  async initialize(
    ...
    const provider = normalizeAgentProviderToWBrandAgent(
      hasProvider ? providerOrSubagentsService : WBRAND_AGENT_PROVIDER,
    );
  ...
  async refresh(subagentsService: ISubagentsService, workspaceIdentity?: string) {
    ...
    const provider = normalizeAgentProviderToWBrandAgent(get().provider);
  ...
  async createAgent(
    ...
    try {
      const result = await subagentsService.createAgent({
        config,
        provider: normalizeAgentProviderToWBrandAgent(provider),
      });
      await get().refresh(subagentsService, workspaceIdentityFromState);
      return result.agent;
    } catch (error) {
