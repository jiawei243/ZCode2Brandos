...
import { getDefaultWorkspaceState } from "@/store/wbrandSessionStoreTypes.js";
...
function arePermissionOptionsEqual(
  left: WBrandPermissionRequest["options"],
  right: WBrandPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: WBrandPermissionRequest,
  right: WBrandPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useWBrandChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToWBrandAgent(params.provider),
