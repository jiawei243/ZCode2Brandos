...
import type {
  ...
  WBrandPluginDiagnostic,
  WBrandPluginInfo,
  WBrandPluginMarketplaceSummary,
  WBrandPluginScope,
  WBrandPluginsDescribeResult,
} from "@wbrand/shared";
import type { IPluginManagementService } from "@wbrand/services";
...
export interface PluginDescribeEntry {
  ...
  data?: WBrandPluginsDescribeResult;
  error?: string;
}

// 设置页「插件管理」的数据源: 经 IPluginManagementService 薄服务由 wbrand-cli 提供 (list + enable/disable)。
// UI 不再直触 IWBrandAgentService，plugins/* 旧协议词的消费收拢到服务实现一处。
// 与已 retired 的 marketplace pluginStore 无关, 故单独建一个精简 store。
export interface PluginManagementState {
  workspacePath: string | null;
  workspaceIdentity: string | null;
  configScope: WBrandPluginScope | null;
  ...
  diagnostics: WBrandPluginDiagnostic[];
  loading: boolean;
  error: string | null;
  /**
...
function throwForErrorDiagnostic(diagnostics: WBrandPluginDiagnostic[]): void {
