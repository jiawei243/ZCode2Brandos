/* oxlint-disable eslint(max-lines) -- WBrand Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * WBrand Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 wbrandSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
  ...
  type WBrandWorkspaceInitStatus,
  type WBrandTaskRuntimeStatus,
  type WBrandProvider,
  type WBrandTaskMeta,
  type WBrandContextCacheUsage,
  type WBrandConfigOption,
  type WBrandSlashCommand,
  type WBrandPermissionRequest,
  type WBrandElicitationRequest,
  type WBrandBackgroundTaskControlItem,
  type WBrandSessionActiveTurnKind,
...
export interface WorkspaceInitState {
  status: WBrandWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: WBrandTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 WBrand Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: WBrandBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: WBrandPermissionRequest | null;
  pendingPermissionRequests: WBrandPermissionRequest[];
  elicitationRequest: WBrandElicitationRequest | null;
  pendingElicitationRequests: WBrandElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: WBrandUiError | null;
}
...
export interface WorkspaceWBrandUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * WBrand Agent 的 workspace 初始化状态。
   *
   * 单 WBrand Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  taskConfigOptionsByTaskId: Record<string, WBrandConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, WBrandTaskMeta>;
  ...
  /** 当前 workspace 选中的 WBrand Agent provider */
  ...
  /** WBrand Agent configOptions（模型、模式、思考级别等） */
  configOptions: WBrandConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: WBrandTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface WBrandSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceWBrandUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceWBrandUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: WBrandPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: WBrandElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
...
const FALLBACK_PROVIDER: WBrandProvider = WBRAND_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceWBrandUIState {
  // 单 WBrand Agent 迁移后默认 provider 必须收敛到 glm。
