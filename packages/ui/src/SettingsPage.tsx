...
export function SettingsPage({
  ...
  const { intl, localePreference, setLocalePreference } = useWBrandIntl();
  ...
  const theme = useWBrandStore((state) => state.theme);
  const setTheme = useWBrandStore((state) => state.setTheme);
  const codePreviewSettings = useWBrandStore((state) => state.codePreviewSettings);
  const setCodePreviewSettings = useWBrandStore((state) => state.setCodePreviewSettings);
  const uiFontSizePx = useWBrandStore((state) => state.uiFontSizePx);
  const setUiFontSizePx = useWBrandStore((state) => state.setUiFontSizePx);
  const notificationEnabled = useWBrandStore((state) => state.notificationEnabled);
  const setNotificationEnabled = useWBrandStore((state) => state.setNotificationEnabled);
  const notificationSoundEnabled = useWBrandStore((state) => state.notificationSoundEnabled);
  const setNotificationSoundEnabled = useWBrandStore((state) => state.setNotificationSoundEnabled);
  ...
  const setNewUserOnboardingOpen = useWBrandStore((state) => state.setNewUserOnboardingOpen);
  ...
  const [wbrandInteractionBehavior, setWBrandInteractionBehavior] =
    useState<WBrandInteractionBehavior>("queue");
  ...
  useEffect(() => {
    services.settingService
      ...
      .then((settings: AppSettings) => {
        ...
        setWBrandInteractionBehavior(settings.wbrandInteractionBehavior ?? "queue");
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    setWBrandInteractionBehavior(sharedSettings.wbrandInteractionBehavior ?? "queue");
  ...
  const handleWBrandInteractionBehaviorChange = useCallback(
    async (behavior: WBrandInteractionBehavior) => {
      await runSettingsActionAsync({
        featureId: "settings.conversation",
        action: "change_interaction_behavior",
        trigger: "select",
        operation: () => updateSharedSettings({ wbrandInteractionBehavior: behavior }),
        completed: { resultSource: "shared_settings", valueAfter: behavior },
      });
      setWBrandInteractionBehavior(behavior);
    },
    [updateSharedSettings],
  );
                      ...
                      <div className="space-y-8">
                        {activeSection === "general" ? (
                          <GeneralSectionContent
                            ...
                            wbrandInteractionBehavior={wbrandInteractionBehavior}
