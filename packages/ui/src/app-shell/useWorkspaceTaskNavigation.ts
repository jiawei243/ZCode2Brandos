...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useWBrandSessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useWBrandSessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useWBrandSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useWBrandSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useWBrandSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useWBrandSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useWBrandSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useWBrandSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.wbrandTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    ...
    while (entry) {
      ...
      const navWorkspaceState = useWBrandSessionStore
        .getState()
