...
import type { WBrandProvider } from "@wbrand/shared";
...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: WBrandProvider | null }) => {
      ...
      const wbrandSessionStore = useWBrandSessionStore.getState();
      const latestWorkspaceState = wbrandSessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: WBrandProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      if (shouldPrepareWorkspace) {
        wbrandSessionStore.setConfigOptionsStatus(workspaceAbsPath, "loading", workspaceIdentity);
        // 草稿态点击 reload 后若不清空旧错误，输入区会继续显示上一轮失败提示，
        // 用户会误判本次重建仍失败。这里在新一轮重建开始时先清空草稿错误。
        wbrandSessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        wbrandSessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.wbrandTaskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          const prepareResult = await prepareWorkspaceWithWBrandSessionService({
            ...
            workspaceIdentity,
            provider,
            wbrandSessionService: services.wbrandSessionService,
          });

          const latestAfterPrepare = wbrandSessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = wbrandSessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              wbrandSessionStore.setConfigOptions(
                workspaceAbsPath,
                prepareResult.configOptions ?? [],
                workspaceIdentity,
              );
              wbrandSessionStore.setConfigOptionsStatus(
                workspaceAbsPath,
                "ready",
                workspaceIdentity,
              );
              wbrandSessionStore.setSlashCommands(
                workspaceAbsPath,
        ...
        if (shouldPrepareWorkspace) {
          wbrandSessionStore.setConfigOptionsStatus(workspaceAbsPath, "error", workspaceIdentity);
          // 草稿态下 reload 前会先清空旧错误；如果失败后不回填 draftError，
          // 聊天区只剩 toast，用户看不到可重试的详细报错。这里统一回填标准化错误到输入区。
          wbrandSessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }
