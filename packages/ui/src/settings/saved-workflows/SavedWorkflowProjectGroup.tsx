...
export function SavedWorkflowProjectGroup({
  ...
  const agentService = services.wbrandAgentService;
  ...
  const launch = useCallback(
    async (entry: WBrandSavedWorkflowEntry, args: Record<string, unknown>) => {
  ...
  const handleCardPromote = useCallback(
    (entry: WBrandSavedWorkflowEntry) => void handlePromote(entry),
    [handlePromote],
  );
