...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type WBrandSavedWorkflowRun } from "@wbrand/shared";
import type { WBrandAgentSavedWorkflowTarget } from "@wbrand/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: WBrandAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: WBrandSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
