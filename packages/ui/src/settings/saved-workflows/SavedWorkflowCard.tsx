...
function SavedWorkflowLastRunBadge({
  ...
  run: WBrandSavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: WBrandSavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: WBrandSavedWorkflowEntry) => void;
  onRun: (entry: WBrandSavedWorkflowEntry) => void;
  onRevise: (entry: WBrandSavedWorkflowEntry) => void;
  onCopyPath: (entry: WBrandSavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: WBrandSavedWorkflowEntry) => void;
  onDelete: (entry: WBrandSavedWorkflowEntry) => void;
}
