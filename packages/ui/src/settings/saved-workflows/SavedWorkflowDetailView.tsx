...
import {
  ...
  type WBrandSavedWorkflowEntry,
  type WBrandSavedWorkflowMeta,
  type WBrandSavedWorkflowRun,
  type WBrandWorkflowsGetResult,
} from "@wbrand/shared";
import type { IWBrandAgentService, WBrandAgentSavedWorkflowTarget } from "@wbrand/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<WBrandWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: WBrandSavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: WBrandAgentSavedWorkflowTarget;
  agentService: IWBrandAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: WBrandSavedWorkflowEntry | undefined;
  runs: readonly WBrandSavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: WBrandSavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: WBrandSavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: WBrandSavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: WBrandSavedWorkflowMeta = {
      description,
