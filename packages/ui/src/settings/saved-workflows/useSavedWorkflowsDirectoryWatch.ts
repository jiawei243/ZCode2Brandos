...
import type { IFileWatcherService } from "@wbrand/services";
...
function savedWorkflowsDirectoryPath(workspacePath: string): string {
  ...
  return `${trimmed}${separator}.wbrand${separator}workflows`;
}
 ...
 *
 * 项目组传 `workspacePath`（拼出 `<ws>/.wbrand/workflows`）；全局组传 `directory`（协议 list 回的
 * 绝对目录，即 `~/.wbrand/workflows`），二者择一——`directory` 优先。
 */
export function useSavedWorkflowsDirectoryWatch({
  ...
  useEffect(() => {
    ...
    void fileWatcherService
      ...
      .catch((error: unknown) => {
        logger.debug("[SavedWorkflows] 监听 .wbrand/workflows 失败（目录可能尚不存在）", {
          path: directoryPath,
