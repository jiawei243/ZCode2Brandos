...
import type { WBrandSavedWorkflowRun, WBrandSavedWorkflowRunStatus } from "@wbrand/shared";
...
): Map<string, WBrandSavedWorkflowRun> {
  const byName = new Map<string, WBrandSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: WBrandSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: WBrandSavedWorkflowRun, now: number): number {
