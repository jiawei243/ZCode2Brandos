...
import type { CodingPlanWebviewLocale } from "@wbrand/shared";
...
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "wbrand:coding-plan:report-context";
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeWBrandEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeWBrandEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeWBrandEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__wbrandLang__ 供 wbrandBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          ...
          wbrandjwttoken: credentials.wbrandJwtToken?.trim() || null,
          "oauth:bigmodel:access_token": null,
        }
      : {
          "oauth:zai:access_token": null,
          // wbrandjwttoken 是 wbrand-plan 域通用凭证（BigModel OAuth callback 同样落盘），
  ...
  return `(() => {
  ${storageUpdates}
  const wbrandTheme = ${JSON.stringify(theme)};
  document.documentElement.classList.toggle("dark", wbrandTheme === "zai-dark");
  document.documentElement.classList.toggle("theme-zai-light", wbrandTheme === "zai-light");
  document.documentElement.classList.toggle("theme-zai-dark", wbrandTheme === "zai-dark");
  localStorage.setItem("wbrand-theme", wbrandTheme);
  localStorage.setItem("wbrand:coding-plan:embedded", "app");
  // 写入当前 App locale，供官网 wbrandBridge.getLang() 读取。
  // 注意：这是注入 webview 执行的原始 JS，不能用 TS 语法（如 as any）。
  window.__wbrandLang__ = ${JSON.stringify(resolvedLocale)};
  const wbrandReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__wbrandReportContext__ = wbrandReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(wbrandReportContext));
  window.dispatchEvent(new CustomEvent("wbrand-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: wbrandReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__wbrandReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "wbrand-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__wbrandLang__ 并派发 wbrand-coding-plan-lang-change 事件，
 * 官网侧（wbrandBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__wbrandLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("wbrand-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  // wbrandjwttoken 对两个 provider 都加载：它是 wbrand-plan 域通用凭证，
  // BigModel OAuth callback 同样落盘（见 resolveBigModelStartPlanWbrandJwt）。
  return provider === "zai"
    ? [`oauth:${ZAI_PROVIDER_ID}:access_token`, "wbrandjwttoken"]
    : [`oauth:${BIGMODEL_PROVIDER_ID}:access_token`, "wbrandjwttoken"];
}
