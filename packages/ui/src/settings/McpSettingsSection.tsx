 ...
 * Supports the unified WBrand Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToWBrandAgentMcpServer,
...
import { isWBrandAgentMcpStatusModeUnsupportedError, type IMcpSyncService } from "@wbrand/services";
...
const DEFAULT_MCP_SOURCE: ServerScope = "wbrandagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 wbrandAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isWBrandAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "wbrandagentmcp" || !server.enabled) {
      continue;
    }
    ...
    if (!wbrandAgentServer) {
      continue;
    }
    seenNames.add(server.name);
    result.push(wbrandAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: WBrandAgentMcpServer[]): {
  mode: "status";
  mcpServers: WBrandAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: WBrandMcpServer[]): string {
  return servers
    .filter((server) => server.source === "wbrandagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: WBrandPluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, WBrandAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<WBrandMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForWBrand("wbrand");
  ...
  async function handleSave(form: FormState, prev?: WBrandMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: WBrandMcpServer) {
  ...
  function handleEdit(server: WBrandMcpServer) {
  ...
  function handleOpenAuthorization(server: WBrandMcpServer) {
