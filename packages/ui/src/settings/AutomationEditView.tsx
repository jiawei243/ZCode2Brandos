...
import {
  ...
  type WBrandAutomationScheduleRule,
} from "@wbrand/shared";
import {
...
interface AutomationEditViewProps {
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: WBrandAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: WBrandAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: WBrandAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: WBRAND_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<WBrandAutomationScheduleRule | undefined>(
    () =>
