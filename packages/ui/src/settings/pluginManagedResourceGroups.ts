import type {
  ...
  WBrandPluginInfo,
  WBrandPluginComponentKind,
  WBrandPluginsDescribeResult,
} from "@wbrand/shared";
import { isPluginCommand, isUserCommand, WBRAND_COMMAND_AGENT_SOURCE } from "@wbrand/shared";
...
const COMPONENT_KIND_ORDER: WBrandPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: WBrandPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<WBrandPluginComponentKind, PluginComponentDisplayGroup>();
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === WBRAND_COMMAND_AGENT_SOURCE &&
...
function toPluginMcpServerDisplayName(plugin: WBrandPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: WBrandPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: WBrandMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
