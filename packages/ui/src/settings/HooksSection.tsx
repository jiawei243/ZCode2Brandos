...
import { invalidateDeferredDraftSessionForRuntimeChange } from "@/lib/wbrandDraftSkillInvalidation.js";
...
function isEditableHook(hook: Hook): boolean {
  ...
  return hook.editable ?? (!hook.location || hook.location.source === "wbrand");
}

// workspace-hook-trust：editable=false 且 source=wbrand 的行是「上游/祖先 wbrand.json
// 里的只读工作区 Hook」。它们不是外部格式兼容导入源，塞进 Legacy 会让 Import 按钮
// 必然失败（importHook 拒绝 source=wbrand），也违反「只读但可逐条 Trust」的约定。
// 这类行应留在 Installed 分组，由信任状态门控 Switch，走行内 Trust 流程。
function isReadOnlyWBrandHook(hook: Hook): boolean {
  return hook.editable === false && (hook.location?.source ?? "wbrand") === "wbrand";
}

function isInCompatibilitySection(hook: Hook): boolean {
  return !isEditableHook(hook) && !isReadOnlyWBrandHook(hook);
}
...
function buildPluginHookRows(
  plugins: readonly Pick<WBrandPluginInfo, "enabled" | "hookDetails" | "id" | "name">[],
  installedPlugins: readonly Pick<WBrandInstalledPluginSummary, "id" | "scope">[],
...
export function HooksSection({ workspacePath, workspaceIdentity }: HooksSectionProps) {
  ...
  const wbrandSessionService = useWBrandSessionService(
  ...
  const editableHooks = useMemo(
    () =>
      hooksState.hooks.filter(
        (hook) =>
          (isEditableHook(hook) || isReadOnlyWBrandHook(hook)) &&
  ...
  const invalidateDraft = useCallback(
    ...
    [targetWorkspaceIdentity, targetWorkspacePath, wbrandSessionService],
  );
