...
import type { WBrandPluginComponentKind, WBrandPluginsDescribeResult } from "@wbrand/shared";
...
const SECTION_ORDER: WBrandPluginComponentKind[] = ["mcp", "skill", "command", "agent", "hook"];

const SECTION_TITLE_IDS: Record<WBrandPluginComponentKind, string> = {
...
const SECTION_ICONS: Record<WBrandPluginComponentKind, typeof Server> = {
  mcp: Server,
  skill: WandSparkles,
  command: Terminal,
  agent: Bot,
...
export function PluginStoreDetailView({
  ...
  return (
    <div className="space-y-8" data-testid="plugin-store-detail" data-plugin-id={item.id}>
      ...
      {/* 组件分区：已安装走权威枚举（WBrandPluginInfo.components），候选走 plugins/describe 按需拉取。 */}
      {componentsLoading ? (
        <div
...
function InfoSection({
  ...
  item: StorePluginItem;
  describeMetadata: WBrandPluginsDescribeResult["metadata"];
}) {
  const { intl } = useWBrandIntl();
