...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<WBrandAvailablePluginSummary, "name" | "listing">[],
  name: string,
): WBrandPluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: WBrandPluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: WBrandPluginStoreListing;
  summary?: WBrandAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: WBrandPluginInfo;
  installedMeta?: WBrandInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<WBrandInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [WBRAND_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly WBrandPluginMarketplaceSummary[],
  locale: string,
): WBrandPluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  marketplaces: WBrandPluginMarketplaceSummary[];
  marketplaceAvailabilityKnown: boolean;
  availablePlugins: WBrandAvailablePluginSummary[];
  installedPlugins: WBrandInstalledPluginSummary[];
  plugins: WBrandPluginInfo[];
  restorableBuiltins: WBrandAvailablePluginSummary[];
}): StorePluginItem[] {
