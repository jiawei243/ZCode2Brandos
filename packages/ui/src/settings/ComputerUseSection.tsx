...
//  - 顶部一个总开关：开/关 wbrand-cua 插件（连带其 MCP server 与 skill 一起启用/禁用）。
...
import type { CuaOsSupport, CuaPermissionKind, RemoteTarget } from "@wbrand/shared";
import {
  DesktopCommandIds,
  isRemoteWorkspaceIdentity,
  WBRAND_CUA_OFFICIAL_PLUGIN_ID,
} from "@wbrand/shared";
import { isCuaPermissionStatusAvailable, type CuaPermissionRestartOptions } from "@wbrand/services";
...
export function ComputerUseSection({
  ...
  // 总开关 = wbrand-cua 插件启用态（读自插件管理 store；切换即同步启用/禁用插件及其 MCP + skill）。
  ...
  const cuaPlugin = plugins.find((plugin) => plugin.id === WBRAND_CUA_OFFICIAL_PLUGIN_ID);
  const cuaEnabled = cuaPlugin?.enabled ?? false;
  const cuaToggling = togglingPluginId === WBRAND_CUA_OFFICIAL_PLUGIN_ID;
  ...
  useEffect(() => {
    ...
    // 复用 Plugins 分区同一条初始化路径，确保 store 已加载 wbrand-cua 的 enabled 态。
  ...
  // 重启 Helper 后验证仍持续 stale → 显示"重启 WBrand"兜底按钮。accessibility 变 granted 时自愈清除。
  ...
  // 兜底:重启 Helper 后仍持续 stale 时,用户可一键重启 WBrand(复用 OAuth 登出同款 RelaunchApp)。
  // 新 WBrand 进程会干净地重新拉起 Helper,绕过当前进程里可能卡住的重启机制(孤儿/socket/状态污染)。
  ...
  // 自愈:accessibility 在后续任一次查询里变成 granted 时,清除"重启 WBrand"兜底(说明问题已解决)。
  useEffect(() => {
  ...
  // 保留重新授权入口与“重启 WBrand”兜底，避免把不可由单次 Helper 重启修复的状态误导成已解决。
  ...
  return (
    <div className="space-y-4">
      {/* 总开关：开/关 wbrand-cua 插件（同步其 MCP + skill） */}
      <SettingsGroupCard>
        <SettingsRow
