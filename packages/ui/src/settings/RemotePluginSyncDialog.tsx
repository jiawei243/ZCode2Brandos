...
import type {
  ...
  RemoteTarget,
  WBrandPluginOptionValue,
  WBrandPluginInfo,
  WBrandPluginMarketplaceSummary,
  WBrandPluginUserConfigOption,
  WBrandPluginsOverviewResult,
} from "@wbrand/shared";
import type { IPluginSyncService, IWBrandAgentService } from "@wbrand/services";
...
interface RemotePluginSyncPluginOptions {
  configuredOptions: Record<string, WBrandPluginOptionValue>;
  userConfig: Record<string, WBrandPluginUserConfigOption>;
}
...
interface RemotePluginSyncDialogProps {
  ...
  localWBrandAgentService?: RemotePluginSyncAgentService | null;
  remoteWBrandAgentService?: RemotePluginSyncAgentService | null;
...
function buildMarketplaceRemotePluginSyncCandidates(
  overview: WBrandPluginsOverviewResult,
  plugins: readonly WBrandPluginInfo[],
): RemotePluginSyncCandidate[] {
...
function buildMarketplaceRemotePluginStatuses(
  candidates: readonly RemotePluginSyncCandidate[],
  remoteOverview: WBrandPluginsOverviewResult,
...
function componentTypesFromPluginInfo(plugin: WBrandPluginInfo | undefined): string[] {
  return plugin?.components?.map((component) => component.kind) ?? [];
}

function buildRemotePluginSyncPluginOptions(
  plugin: WBrandPluginInfo | undefined,
...
function serializeMarketplaceSourceInput(
  marketplace: WBrandPluginMarketplaceSummary,
): string | undefined {
...
async function loadRemotePluginSyncCandidates(params: {
  ...
  if (!params.localWBrandAgentService || !params.remoteWBrandAgentService) {
  ...
  const [localOverview, localPluginsResult, remoteOverview] = await Promise.all([
    params.localWBrandAgentService.getPluginsOverview({
      workspacePath: params.localWorkspacePath,
    }),
    params.localWBrandAgentService.listPlugins({
      workspacePath: params.localWorkspacePath,
    }),
    params.remoteWBrandAgentService.getPluginsOverview({
...
function isPathPluginOption(option: WBrandPluginUserConfigOption | undefined): boolean {
  return option?.type === "file" || option?.type === "directory";
}

function isPortablePluginOptionValue(
  value: WBrandPluginOptionValue,
  localOption: WBrandPluginUserConfigOption | undefined,
  remoteOption: WBrandPluginUserConfigOption | undefined,
): boolean {
...
function buildPortablePluginOptionsSyncPlan(
  ...
  remotePlugin: WBrandPluginInfo,
): { options: Record<string, WBrandPluginOptionValue>; skippedCount: number } {
  const options: Record<string, WBrandPluginOptionValue> = {};
  let skippedCount = 0;
...
function arePluginOptionsEqual(
  left: Record<string, WBrandPluginOptionValue>,
  right: Record<string, WBrandPluginOptionValue>,
): boolean {
...
async function syncPortablePluginOptions(
  ...
  const remoteWBrandAgentService = params.remoteWBrandAgentService;
  if (!remoteWBrandAgentService) {
    emitRemotePluginSyncProgress(
      params,
      row,
      "syncing",
  ...
  const remotePlugins = await awaitRemotePluginSyncStep(
    ...
    () => remoteWBrandAgentService.listPlugins(workspace),
  );
  ...
  await awaitRemotePluginSyncStep(
    ...
    () =>
      remoteWBrandAgentService.configurePlugin({
        ...workspace,
...
async function syncMarketplaceRemotePlugin(
  ...
  try {
    throwIfRemotePluginSyncStopped(params, row);
    if (!params.remoteWBrandAgentService || !marketplacePlugin) {
    ...
    const installResult = await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        remoteWBrandAgentService.installPlugin({
          ...workspace,
    ...
    await awaitRemotePluginSyncStep(
      ...
      () =>
        remoteWBrandAgentService.setPluginEnabled({
          ...workspace,
          enabled: candidate.enabled,
          pluginId: candidate.pluginId,
          scope: REMOTE_PLUGIN_SYNC_CONFIG_SCOPE,
...
async function prepareRemoteMarketplaceSource(
  params: {
    ...
    onItemProgress?: (event: RemotePluginSyncProgressEvent) => void;
    remotePluginSyncService: IPluginSyncService;
    remoteWBrandAgentService: RemotePluginSyncAgentService;
    selectedMarketplacePluginNames: ReadonlyMap<string, string[]>;
    signal?: AbortSignal;
    stopControl?: RemotePluginSyncStopControl;
  },
  ...
  if (marketplaceSourceInput) {
    await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        params.remoteWBrandAgentService.addPluginMarketplace({
          ...workspace,
          operationId,
      ...
      const result = await syncSelectedRemotePlugins({
        ...
        stopControl: {
          cancelOperation: async (operationId) => {
            if (!remoteWBrandAgentService?.cancelPluginOperation) {
              return;
            }
            await remoteWBrandAgentService.cancelPluginOperation({ operationId });
          },
