...
import { WBrandAboutLogo } from "@/components/ui/WBrandAboutLogo.js";
...
function LoginPanel({ active, onComplete }: LoginPanelProps) {
  ...
  const user = useWBrandStore((s) => s.user);
  const oauthError = useWBrandStore((s) => s.oauthError);
  const setOAuthError = useWBrandStore((s) => s.setOAuthError);
  const oauthSuccessSeq = useWBrandStore((s) => s.oauthSuccessSeq);
  const lastOAuthSuccessProvider = useWBrandStore((s) => s.lastOAuthSuccessProvider);
  const loginEntryRequest = useWBrandStore((s) => s.loginEntryRequest);
  const clearLoginEntryRequest = useWBrandStore((s) => s.clearLoginEntryRequest);
  const markLoginEntryAttemptStatus = useWBrandStore((s) => s.markLoginEntryAttemptStatus);
...
function LoginPanelLogo() {
  return (
    ...
    <div
      ...
      role="img"
    >
      <WBrandAboutLogo className="h-auto w-10" />
    </div>
  );
}
