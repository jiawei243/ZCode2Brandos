...
import type { WBrandGroupedTaskViewNode, WBrandTaskGroupColor } from "@wbrand/services";
import {
...
export function GroupItem({
  ...
  getTaskRemoteSessionId: (task: WBrandTaskMeta) => string | undefined;
  getTaskWorkspaceLabel: (task: WBrandTaskMeta) => string;
  onSelectTask: (workspacePath: string, taskId: string, workspaceIdentity?: string) => void;
  onCloseTask: (task: WBrandTaskMeta) => void;
  ...
  onUpdateGroupColor: (groupId: string, color: WBrandTaskGroupColor) => void;
  ...
  const handleGroupColorChange = useCallback(
    (color: string) => {
      onUpdateGroupColor(node.group.id, color as WBrandTaskGroupColor);
    },
