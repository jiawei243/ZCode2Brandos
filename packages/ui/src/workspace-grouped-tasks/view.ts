...
import type { WBrandGroupedTaskView, WBrandGroupedTaskViewNode } from "@wbrand/services";
import type { WBrandTaskMeta } from "@wbrand/shared";
import type { GroupedDraftTaskPlacement } from "@/store/wbrandSessionStoreTypes.js";
import { taskKey } from "@/workspace-grouped-tasks/ids.js";

function cloneView(view: WBrandGroupedTaskView): WBrandGroupedTaskView {
  return {
...
function cloneNodes(view: WBrandGroupedTaskView): WBrandGroupedTaskViewNode[] {
  return [...view.nodes];
}

function cloneGroupNode(
  node: Extract<WBrandGroupedTaskViewNode, { type: "group" }>,
): Extract<WBrandGroupedTaskViewNode, { type: "group" }> {
...
function filterGroupedViewByTaskKeys(
  ...
  let changed = false;
  const nodes: WBrandGroupedTaskView["nodes"] = [];
...
function getGroupedTaskGroupIds(view: WBrandGroupedTaskView): string[] {
...
function getTaskGroupIdInView(view: WBrandGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function findTopLevelTaskIndex(view: WBrandGroupedTaskView, targetTaskKey: string): number {
  return view.nodes.findIndex(
    (node) => node.type === "task" && taskKey(node.task) === targetTaskKey,
  );
}

function findGroupIndex(view: WBrandGroupedTaskView, groupId: string): number {
  return view.nodes.findIndex((node) => node.type === "group" && node.group.id === groupId);
}

function findGroupIdByTaskKey(view: WBrandGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function getNodeIdAfterGroup(
  view: WBrandGroupedTaskView,
  groupId: string,
): WBrandGroupedTaskViewNode | null {
...
function insertTaskBeforeTopLevelNode(
  view: WBrandGroupedTaskView,
  task: WBrandTaskMeta,
  beforeNode: WBrandGroupedTaskViewNode | null,
): WBrandGroupedTaskView {
  const nodes = cloneNodes(view);
  const insertIndex =
    beforeNode === null
