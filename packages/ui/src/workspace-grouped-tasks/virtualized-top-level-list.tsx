...
import type { WBrandGroupedTaskViewNode } from "@wbrand/services";
...
function getTopLevelNodeKey(node: WBrandGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: WBrandGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: WBrandGroupedTaskViewNode, index: number) => ReactNode;
}) {
