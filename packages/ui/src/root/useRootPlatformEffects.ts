...
export function useRootPlatformEffects({
  ...
  initialWorkspacePurpose?: import("@wbrand/shared").WorkspacePurpose;
  ...
  isDesktop?: boolean;
  locale: ReturnType<typeof import("@/i18n/IntlProvider.js").useWBrandIntl>["locale"];
  tabs: WindowTabState[];
  ...
  intl: ReturnType<typeof import("@/i18n/IntlProvider.js").useWBrandIntl>["intl"];
  ...
  useEffect(() => {
    ...
    const disposeNotificationClick = platform.onTaskNotificationClick((taskId: string) => {
      ...
      const workspaces = useWBrandSessionStore.getState().workspaces;
  ...
  useEffect(() => {
    ...
    void baseServices.conversationShareService
      ...
      .then((result) => {
        ...
        const sessionStore = useWBrandSessionStore.getState();
  ...
  useEffect(() => {
    ...
    syncActiveSession();
    return useWBrandSessionStore.subscribe(syncActiveSession);
