...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: WBrandTaskMeta,
    overrides?: Partial<WBrandTaskMeta>,
  ): WBrandTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.wbrandTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.wbrandTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 wbrandTaskMetaMerge：归档后只有旧列表状态被更新。
