...
import type { WBrandGroupedTaskView, WBrandTaskGroupColor } from "@wbrand/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type WBrandTaskMeta } from "@wbrand/shared";
...
function getGroupedTaskViewSignature(view: WBrandGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const groupedDraftTask = useWBrandSessionStore(
    (state) =>
      selectWorkspaceWBrandState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useWBrandSessionStore(
    (state) =>
      selectWorkspaceWBrandState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useWBrandSessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<WBrandGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<WBrandGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: WBrandTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: WBrandTaskMeta, groupId: string | null) => {
  ...
  const handleSubmitRenameTask = useCallback(async () => {
    ...
    try {
      const meta = await workspaceServices.services.wbrandTaskService.renameTask({
        taskId: task.taskId,
  ...
  const handleMarkTaskAsUnread = useCallback(
    (task: WBrandTaskMeta) => {
      ...
      void workspaceServices.services.wbrandTaskService
        .setTaskUnread({
          taskId: task.taskId,
          workspacePath: task.workspacePath,
          unread: true,
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: WBrandTaskGroupColor) => {
  ...
  const setViewWithGroupedTaskAnimation = useCallback(
    (nextView: WBrandGroupedTaskView) => {
