...
import { invalidateDeferredDraftSessionForSkillChange } from "@/lib/wbrandDraftSkillInvalidation.js";
...
const EMPTY_PINNED_TASKS: WBrandTaskMeta[] = [];
...
export const WorkspaceSidebarItem = memo(function WorkspaceSidebarItem({
  ...
  const workspaceWBrandState = useWBrandSessionStore((state) =>
    selectWorkspaceWBrandState(state, tab.workspacePath, tab.workspaceIdentity),
  );
  const activeTaskId = workspaceWBrandState.activeTaskId;
  const removeTaskState = useWBrandSessionStore((state) => state.removeTaskState);
  const upsertOptimisticTaskListItem = useWBrandSessionStore(
    (state) => state.upsertOptimisticTaskListItem,
  );
  const removeOptimisticTaskListItem = useWBrandSessionStore(
    (state) => state.removeOptimisticTaskListItem,
  );
  const setTaskUnreadIndicator = useWBrandSessionStore((state) => state.setTaskUnreadIndicator);
  ...
  const wbrandTaskService = services.wbrandTaskService;
  const taskItemsRef = useRef(taskItems);
  taskItemsRef.current = taskItems;
  const workspaceWBrandStateRef = useRef(workspaceWBrandState);
  workspaceWBrandStateRef.current = workspaceWBrandState;
  ...
  const handleRemoveWorkspace = useCallback(async () => {
    ...
    if (
      hasRunningWorkspaceChat({
        workspaceState: workspaceWBrandStateRef.current,
  ...
  const handleRenameTask = useCallback(
    async (taskId: string, title: string) => {
      ...
      let meta: WBrandTaskMeta;
      try {
        meta = await wbrandTaskService.renameTask({
          taskId,
          workspacePath: tab.workspacePath,
          title,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleSetTaskPinned = useCallback(
    async (taskId: string, pinned: boolean) => {
      ...
      try {
        const meta = await wbrandTaskService.setTaskPinned({
          taskId,
          workspacePath: tab.workspacePath,
          pinned,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleArchiveTask = useCallback(
    async (taskId: string) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await wbrandTaskService.archiveTask({
        taskId,
        workspacePath: tab.workspacePath,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
      });
  ...
  const handleSetTaskUnread = useCallback(
    async (taskId: string, unread: boolean) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await wbrandTaskService.setTaskUnread({
        taskId,
        workspacePath: tab.workspacePath,
        unread,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  return (
    <li ref={itemRef} style={itemStyle} className="space-y-2">
      ...
      <RemoteSyncDialogs
        ...
        remoteWBrandAgentService={services.wbrandAgentService}
        remoteTarget={tab.remoteTarget}
        ...
        onPluginsSynced={async () => {
          await refreshWorkspacePluginCapabilitiesAfterRemoteSync({
            ...
            workspaceIdentity: tab.workspaceIdentity,
            workspacePath: tab.workspacePath,
            wbrandAgentService: services.wbrandAgentService,
            wbrandSessionService: services.wbrandSessionService,
          });
        }}
      />
