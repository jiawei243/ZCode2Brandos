import type { WBrandWorkspaceTaskListChanged } from "@wbrand/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<WBrandWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<WBrandWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<WBrandWorkspaceTaskListChanged, "reason">,
): boolean {
