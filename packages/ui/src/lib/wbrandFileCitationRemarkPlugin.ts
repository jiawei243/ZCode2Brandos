...
import { extractWBrandFileCitations } from "@/lib/wbrandFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractWBrandFileCitations(value);
...
export function createWBrandFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function wbrandFileCitationRemarkPlugin() {
    return (tree: unknown) => {
