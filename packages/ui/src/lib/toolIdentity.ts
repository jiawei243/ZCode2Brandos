import {
  getWBrandToolFamilyForName,
  isTodoPlanToolName,
  isWBrandFileContentWriteToolName,
  normalizeWBrandToolName,
  type WBrandKnownToolName,
  type WBrandToolFamily,
} from "@wbrand/shared";
...
export interface ToolCallIdentity {
  toolName: WBrandKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getWBrandToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    wbrand: readNestedString(raw, ["_meta", "wbrand", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawWBrand = normalizeLegacyToken(rawNames.wbrand);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.wbrand, source: "raw-wbrand-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.wbrand ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 WBrand Agent 投影会用
  ...
  if (
    // 当前 WBrand Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isWBrandFileContentWriteToolName(identity.toolName)) {
    return true;
  }
