import type { WBrandConfigOption, WBrandTaskMeta } from "@wbrand/shared";
import { getWBrandAgentModeSelectOptions } from "@wbrand/shared";
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<WBrandTaskMeta, "provider" | "model">,
): string | null {
...
function createRecoveredModelOption(modelValue: string): WBrandConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): WBrandConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): WBrandConfigOption {
  return {
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<WBrandTaskMeta, "provider" | "model"> &
    Partial<Pick<WBrandTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly WBrandConfigOption[];
}): WBrandConfigOption[] {
