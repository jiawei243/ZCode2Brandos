...
import { normalizeWBrandUiError } from "@/lib/wbrandUiError.js";

export const MODEL_CONFIG_MISSING_UI_ERROR_CODE = "model_config_missing";

export type ModelConfigMissingUiError = WBrandError & {
...
export function isProviderNotReadyError(error: unknown): boolean {
  return normalizeUnknownError(error).code === WBRAND_AGENT_PROVIDER_NOT_READY_CODE;
}
...
): WBrandError & { detail?: string } {
  const normalizedError = normalizeWBrandUiError(
