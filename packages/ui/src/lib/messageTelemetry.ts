...
interface FinalizePromptTelemetryInput {
  ...
  status: string;
  finishedAt: number;
  fileChanges?: readonly WBrandPersistedFileChange[];
  usage?: WBrandUsage;
  errorType?: string;
  errorMsg?: string;
...
function resolvePromptTelemetryModelProvider(params: {
  ...
  // ghost supplier 代表尚未解析到稳定 custom provider 的临时隔离态，provider 维度统一回退到当前 WBrand Agent provider。
...
export function buildPromptTelemetryExtraDetail(params: {
  ...
  return {
    ...
    // agent 字段取 WBrand Agent provider；本仓库没有独立 session.agentId。
...
const COMPACTION_TERMINAL_STATUSES: readonly WBrandTimelineStatus[] = [
  "completed",
  "failed",
  "interrupted",
];

function isCompactionTerminalStatus(status: WBrandTimelineStatus): boolean {
...
export function buildCompactionTelemetryExtraDetail(params: {
  timeline: WBrandContextCompactionTimelineMeta;
  provider?: WBrandProvider;
...
function buildPromptUsageTelemetryExtraDetail(
  ...
  return {
    ...
    cache_write_input_tokens: String(usage.cachedWriteInputTokens ?? 0),
    // WBrand Agent 链路未透出 tool use prompt token，成功态显式补 0 保持 extraDetail 字段集合完整。
...
function mergePromptUsage(left: WBrandUsage | null, right: WBrandUsage): WBrandUsage {
  if (!left) {
    return {
...
function collectFileChangeMetrics(fileChanges: readonly WBrandPersistedFileChange[] | undefined) {
...
export function recordPromptModelRequestStarted(
  taskId: string,
  event: Extract<WBrandStreamEvent, { type: "task_network_debug_status" }>,
