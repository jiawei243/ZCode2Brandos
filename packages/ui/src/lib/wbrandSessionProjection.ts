/* oxlint-disable eslint(max-lines) -- WBrand session 到当前聊天 projection 的迁移桥需要同时保持 snapshot 和 event 映射一致。 */
import {
...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    ...
    // WBrand Protocol 必须收到严格的 providerId/modelId 结构。
    return {
...
export function wbrandSessionSettingsToConfigOptions(
...
export function wbrandWorkspacePresentationToConfigOptions(
  ...
  return [
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableWBrandMode(mode),
      options: getWBrandAgentModeSelectOptions(),
    },
  ];
}
...
function resolveSettingsThoughtLevelCurrentValue(
  thoughtLevel: WBrandSessionSettingsState["thoughtLevel"],
): string | undefined {
  ...
  const defaultThoughtLevel =
    thoughtLevel.defaultLevel && thoughtLevelValues.has(thoughtLevel.defaultLevel)
      ...
      : undefined;
  // WBrand Protocol 的 defaultLevel 是模型事实，current 为空时表示用户尚未显式修改。
...
function normalizeAvailableWBrandMode(mode: WBrandSessionMode): string {
  return WBRAND_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

function getWBrandAgentModeSelectOptions(): NonNullable<WBrandConfigOption["options"]> {
