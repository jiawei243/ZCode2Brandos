...
// sessions-index 只补实时 activity/detail，最终拼出旧 WBrandGroupedTaskView 同形结果。
...
function taskKeyOf(task: WBrandTaskMeta): string {
...
function taskOrderMapKey(task: WBrandTaskMeta): string {
...
function nodeMapKey(node: WBrandGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, WBrandGroupedTaskViewStructureMember>,
): WBrandTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: WBrandTaskListItem; addedAt: number; key: string }> = [];
...
interface BuildGroupedTaskViewParams {
  ...
  taskIndexItems: WBrandTaskMeta[];
  /** sessions-index 派生的会话 activity/detail，只 enrich 命中的持久行。 */
  sessions: WBrandTaskMeta[];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, WBrandTaskListItem>();
  ...
  const membersByGroupId = new Map<string, WBrandGroupedTaskViewStructureMember[]>();
  ...
  const nodes: WBrandGroupedTaskViewNode[] = structure.groups.map((group) => {
