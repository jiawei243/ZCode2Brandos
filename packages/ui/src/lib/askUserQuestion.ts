type WBrandUserQuestionAnswers = Record<string, unknown>;
...
interface AskUserQuestionData {
  ...
  answers?: WBrandUserQuestionAnswers;
}
...
function readQuestions(input: unknown): unknown[] {
  ...
  // WBrand Agent 的单题输入与交互请求的多题输入共用展示管线。
  return typeof input.question === "string" && Array.isArray(input.options) ? [input] : [];
}

function readAnswers(input: unknown): WBrandUserQuestionAnswers | undefined {
  if (!isPlainRecord(input)) {
    return undefined;
  }
...
function readNestedAskUserQuestionAnswers(input: unknown): WBrandUserQuestionAnswers | undefined {
...
function parseWBrandAskUserQuestionOutput(
  output: unknown,
  input: unknown,
): WBrandUserQuestionAnswers | undefined {
...
export function readAskUserQuestionAnswers(value: {
  ...
  const parsedWBrandOutputAnswers = parseWBrandAskUserQuestionOutput(
    value.output,
    readAskUserQuestionInput(value),
  );
  if (parsedWBrandOutputAnswers) {
    return parsedWBrandOutputAnswers;
  }
  ...
  if (isPlainRecord(value.raw)) {
    ...
    const parsedRawWBrandOutputAnswers = parseWBrandAskUserQuestionOutput(
      value.raw.rawOutput,
      readAskUserQuestionInput(value),
    );
    if (parsedRawWBrandOutputAnswers) {
      return parsedRawWBrandOutputAnswers;
    }
    const nestedRawAnswers = readNestedAskUserQuestionAnswers(value.raw.output);
    if (nestedRawAnswers) {
      return nestedRawAnswers;
    }
    const parsedRawWBrandAnswers = parseWBrandAskUserQuestionOutput(
      value.raw.output,
      readAskUserQuestionInput(value),
    );
    if (parsedRawWBrandAnswers) {
      return parsedRawWBrandAnswers;
    }
...
export function getAskUserQuestionAnswerText(
  ...
  answers: WBrandUserQuestionAnswers | undefined,
  noAnswerText: string,
) {
