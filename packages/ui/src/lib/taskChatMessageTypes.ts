// re-home 产物（为删除 wbrandChatMessages 铺路，模式同 wbrand-task-types-core）。
// 本文件承载旧消息转换层（wbrandChatMessages/wbrandChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（wbrandChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  WBrandAssistantMessagePart,
  WBrandAssistantMessageFeedback,
  WBrandPromptAttachment,
  WBrandTimelineMeta,
  WBrandTaskSnapshotBodyRef,
  WBrandTaskSnapshotToolFieldRef,
  WBrandTaskSnapshotToolSlice,
} from "@wbrand/shared";
...
export interface TaskChatToolCall {
  ...
  /** WBrand 固定工具名；kind 仍保留给旧 WBrand Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = WBrandAssistantMessagePart;
