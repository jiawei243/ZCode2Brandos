...
import { extractConversationPreviewFileReferences } from "@wbrand/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@wbrand/shared";
import {
  extractWBrandFileCitationDirectives,
  resolveWBrandFileCitationPreviewKind,
} from "@/lib/wbrandFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractWBrandFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 wbrand-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
