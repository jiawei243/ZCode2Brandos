...
> &
  Partial<Pick<IWBrandTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<WBrandTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: WBrandTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: WBrandTaskMeta[][]): WBrandTaskMeta[] {
  const itemByEntityKey = new Map<string, WBrandTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, WBrandTaskMeta>();
