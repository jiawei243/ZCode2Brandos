import type { CuaAccessibilitySettingsResult } from "@wbrand/shared";

function normalizeToolName(value: string | null | undefined): string {
  return value?.trim().toLowerCase().replace(/_/g, "-") ?? "";
}

export function isWBrandCuaToolName(value: string | null | undefined): boolean {
  const normalized = normalizeToolName(value);
  // server key 段为 computer-use。feat: mcp__computer-use__*；
  // main v3.5.3 的 plugin MCP 命名约定给 plugin server 加 namespace：
  // mcp__plugin_wbrand-cua_computer-use__*（归一化后 server 段前是单连字符 cua-computer-use，
...
function didReturnFromCuaPermissionSettings(
  ...
  // main 只有在整组 staged pane 都完成并观察到 WBrand 应用级返回后才置 true。renderer focus
