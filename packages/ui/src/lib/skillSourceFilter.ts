...
function resolveSkillSourceType(skillPath: string): SkillSourceType {
  ...
  if (normalized.includes("/.wbrand/skills/")) {
    return "glm";
  }
  if (normalized.includes("/.wbrand/cli/plugins/cache/")) {
    return "glm";
  }
  return "unknown";
}
...
function isWbrandSkill(skill: { id?: string; path: string; scope?: string }): boolean {
  return (
    // plugin skill 的真实路径在 CLI plugin cache 下，不在 `.wbrand/skills`。
...
export function filterSkillsForProvider<T extends { path: string; id?: string; scope?: string }>(
  skills: T[],
  _legacyProvider: WBrandProvider,
): T[] {
  return skills.filter(isWbrandSkill);
}
