import type { WBrandError, TraceId } from "@wbrand/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@wbrand/shared/wbrand-protocol-v4";

export interface WBrandUiError extends WBrandError {
...
interface NormalizeWBrandUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_WBRAND_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "WBrand session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // WBrand Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // wbrand-cli 会把模型/网络错误摘要放在 data.wbrand.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "wbrand", "error", "message"],
    ["data", "wbrand", "error", "detail"],
    ["data", "wbrand", "error", "details"],
  ];
...
export function normalizeWBrandUiError(
  error: unknown,
  options: NormalizeWBrandUiErrorOptions = {},
): WBrandUiError {
  const candidates = collectMessageCandidates(error);
  // wbrand-cli 已经把 provider/network 根因放进 detail 或 data.wbrand.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_WBRAND_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) => candidate !== primaryMessage && !GENERIC_WBRAND_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "wbrand", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "wbrand", "error", "underlyingErrorDetail"],
  ]);
