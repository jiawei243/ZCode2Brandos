...
import type { OnboardingRecordEntry } from "@wbrand/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useWBrandStore((state) => state.newUserOnboardingOpen);
  const setRequested = useWBrandStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useWBrandStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useWBrandStore((state) => state.interfaceMode);
  ...
  const requestOnboardingDialog = useWBrandStore((state) => state.requestOnboardingDialog);
