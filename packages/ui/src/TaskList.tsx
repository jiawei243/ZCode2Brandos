...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@wbrand/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<WBrandTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<WBrandTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<WBrandTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<WBrandTaskMeta | null>;
  ...
  const startDraft = useWBrandSessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: WBrandTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareWBrandTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
