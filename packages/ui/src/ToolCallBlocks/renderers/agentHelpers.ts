...
type AgentIntl = ReturnType<typeof useWBrandIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "wbrand", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "wbrand", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "wbrand", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "wbrand", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const wbrand = meta && isPlainRecord(meta.wbrand) ? meta.wbrand : null;
  const wbrandBackgroundAgent =
    wbrand && isPlainRecord(wbrand.backgroundAgent) ? wbrand.backgroundAgent : null;
  const taskNotification =
    wbrand && isPlainRecord(wbrand.taskNotification) ? wbrand.taskNotification : null;
  const backgroundAgent = wbrandBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "wbrand", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "wbrand", "taskNotification", "summary"]);
