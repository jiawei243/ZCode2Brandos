...
import type { WBrandTaskPendingInteraction } from "@wbrand/shared";
import { cn } from "@/components/lib/utils.js";

interface TaskInteractionBadgeProps {
  interaction?: WBrandTaskPendingInteraction;
...
function isAskUserQuestionInteraction(
  interaction: WBrandTaskPendingInteraction | undefined,
): boolean {
...
export function TaskInteractionBadge({
  ...
  const style =
    progress !== undefined
      ? ({
          "--wbrand-interaction-progress": String(progress),
          "--wbrand-interaction-remaining-ms": `${remainingMs}ms`,
        } as CSSProperties)
      : undefined;
  ...
  const badgeContent = (
    <>
      {progress !== undefined ? (
        <span
          aria-hidden="true"
          className="wbrand-task-interaction-countdown-fill absolute inset-0 origin-left bg-interaction-ask-fill transition-opacity group-hover/interaction-badge:opacity-0 group-focus-visible/interaction-badge:opacity-0 motion-reduce:transition-none"
        />
      ) : null}
      {canSnooze ? (
