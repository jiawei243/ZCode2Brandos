...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@wbrand/shared/wbrand-protocol-v4";
...
function readPermissionRuleScopes(option: WBrandPermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: WBrandPermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: WBrandPermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<WBrandProvider, Record<string, string>>
> = {
  glm: {
    // GLM/WBrand Agent 通过 WBrand Agent 发来的项目级记忆授权文案是英文原文。
...
function getProviderOptionNameMessageIds(
  provider: WBrandProvider | undefined,
  name: string,
...
function getPermissionDisplayReason(request: WBrandPermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: WBrandPermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: WBrandPermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  // 当前 WBrand Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // WBrand Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
