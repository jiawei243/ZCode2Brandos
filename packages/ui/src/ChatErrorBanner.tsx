 ...
 *
 * 显示 WBrand Agent 链路中的错误，带 traceId 方便排查。
 */
...
const LOCALIZED_ERROR_CODES = new Set([
  ...
  "WBRAND_RUNTIME_MODEL_UNAVAILABLE",
  "WBRAND_BIGMODEL_TEAM_PLAN_MEMBER_REQUIRED",
]);
...
function isModelConfigMissingError(error: Pick<WBrandUiError, "code" | "message">): boolean {
...
export function resolveChatErrorBannerDisplayMessage(
  ...
  if (isSuspiciousEmptyModelResultMessage(error.message)) {
    return intl.formatMessage({ id: "wbrand.error.modelSuspiciousEmpty" });
  }

  return error.code && LOCALIZED_ERROR_CODES.has(error.code)
    ? intl.formatMessage({ id: `wbrand.error.${error.code}` })
    : error.message;
}

export function shouldSuppressChatErrorBanner(
  error: Pick<WBrandUiError, "code" | "message">,
): boolean {
  // 只有历史恢复残留的模型不可用提示才隐藏；当前发送/草稿报错需要展示，
  // 否则 registry 移除模型后用户会看到“请求没返回”而没有任何可操作反馈。
  return Boolean(
    error.code === "WBRAND_RUNTIME_MODEL_UNAVAILABLE" &&
