import type { WBrandModelTrajectoryMessage } from "@wbrand/services";

export function trajectoryToolOutputs(message: WBrandModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: WBrandModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: WBrandModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: WBrandModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
