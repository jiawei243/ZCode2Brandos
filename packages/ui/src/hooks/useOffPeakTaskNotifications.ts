...
import type { WBrandOffPeakTaskStatus } from "@wbrand/shared";
import type { IOffPeakTaskService } from "@wbrand/services";
...
function notifiableStatus(status: WBrandOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, WBrandOffPeakTaskStatus>());
