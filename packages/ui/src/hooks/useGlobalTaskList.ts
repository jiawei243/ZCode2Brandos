...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): WBrandTaskListWorkspaceScope[] {
  const scopes = new Map<string, WBrandTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useWBrandSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceWBrandState(
            state,
            tab.workspacePath,
