...
import { isRemoteWorkspaceIdentity, WBRAND_CUA_OFFICIAL_PLUGIN_ID } from "@wbrand/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/wbrandSessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 wbrandSessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 wbrand-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useWBrandSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === WBRAND_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === WBRAND_CUA_OFFICIAL_PLUGIN_ID,
