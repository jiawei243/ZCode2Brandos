...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly WBrandTaskMeta[];
  optimisticTasks: readonly WBrandTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareWBrandTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useWBrandSessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useWBrandSessionStore.getState();
