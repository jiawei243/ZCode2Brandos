...
import { mergeTaskWithOptimisticMeta } from "@/lib/wbrandTaskMetaMerge.js";
import {
...
function applyPromotedGroupPlacements(
  ...
  >,
  optimisticTaskByKey: ReadonlyMap<string, WBrandTaskMeta>,
): WBrandGroupedTaskView {
  let nextView = view;
...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: WBrandTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, WBrandTaskMeta>();
  const placementTaskByKey = new Map<string, WBrandTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, WBrandTaskMeta[]>();
  const topMissingTasks: WBrandTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: WBrandGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: WBrandGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: WBrandGroupedTaskView["nodes"][number],
  next: WBrandGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: WBrandGroupedTaskView,
  next: WBrandGroupedTaskView,
): WBrandGroupedTaskView {
...
function nodeToTopLevelRef(
  node: WBrandGroupedTaskView["nodes"][number],
): WBrandGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
const groupedViewCacheBySignature = new Map<string, WBrandGroupedTaskView>();

function readCachedGroupedView(signature: string): WBrandGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: WBrandGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<WBrandGroupedTaskView>(
  ...
  const clearPromotedGroupedDraftTask = useWBrandSessionStore(
  ...
  const displayedViewRef = useRef<WBrandGroupedTaskView>({ nodes: [] });
  ...
  const [remoteDataLoader] = useState(
    () =>
      new GroupedRemoteDataSingleFlight<{
        structure: WBrandGroupedTaskViewStructure;
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.wbrandTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.wbrandTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: WBrandTaskMeta[] = [];
    const settledRootTasks: WBrandTaskMeta[] = [];
    ...
    void services.wbrandTaskService
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.wbrandTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.wbrandTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<WBrandTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.wbrandTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.wbrandTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: WBrandGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.wbrandTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.wbrandTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: WBrandTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.wbrandTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: WBrandGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.wbrandTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.wbrandTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: WBrandGroupedTaskView = {
      ...
      try {
        ...
        await services.wbrandTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.wbrandTaskService, view],
  );
