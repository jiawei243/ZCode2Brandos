...
function areResolvedTaskMetasEqual(left: WBrandTaskMeta | null, right: WBrandTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: WBrandTaskMeta | null) {
  const stableTaskMetaRef = useRef<WBrandTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useWBrandSessionStore((state) =>
    selectWorkspaceWBrandState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 wbrandTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
