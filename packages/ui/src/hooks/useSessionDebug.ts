...
import type { SessionDebugSnapshot } from "@wbrand/shared";
...
export function useSessionDebug({
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const data = await wbrandAgentService.readSessionDebug({
          workspacePath,
          workspaceIdentity,
          sessionId: taskId,
        });
        ...
        if (!disposed)
          setResult((previous) => ({
            key: scopeKey,
            service: wbrandAgentService,
            data:
              previous?.key === scopeKey && previous.service === wbrandAgentService
                ? previous.data
                : null,
            error: true,
          }));
  ...
  }, [enabled, scopeKey, taskId, workspaceIdentity, workspacePath, wbrandAgentService]);
  const current = result?.key === scopeKey && result.service === wbrandAgentService ? result : null;
