...
function supportsTaskNativeSessionLogFile(_provider: WBrandProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 wbrandTaskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: WBrandProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const wbrandTaskService = useWBrandTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, wbrandTaskService, providerHint, taskId, workspaceIdentity, workspacePath]);
