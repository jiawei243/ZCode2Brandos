...
import type { WBrandSessionEndedSubagent, WBrandSessionSubagentsResult } from "@wbrand/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: WBrandSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly WBrandSessionEndedSubagent[],
  incoming: readonly WBrandSessionEndedSubagent[],
): WBrandSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const wbrandAgentService = services.wbrandAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<WBrandSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof wbrandAgentService?.listSessionSubagents !== "function") {
      ...
      return wbrandAgentService.listSessionSubagents({
