...
interface PluginReferenceCatalogState {
  entries: WBrandPluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<WBrandPluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<WBrandPluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<WBrandPluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.wbrandAgentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<WBrandPluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<WBrandPluginsReferenceCatalogResult>> | undefined;
