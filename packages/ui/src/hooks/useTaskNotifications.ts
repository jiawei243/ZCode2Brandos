...
import type { IPlatformService, TaskNotificationPayload } from "@wbrand/shared";
import type { ConversationSnapshot, SessionSummary } from "@wbrand/shared/wbrand-protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, wbrandAgentService);
    const syncState = () => {
      setIndexState({
        signature,
