...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, WBrandPersistedFileSnapshot>,
  file: WBrandTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: WBrandPersistedFileChange | null;
  summary: WBrandTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, WBrandPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 WBrand Agent 已持久化的单轮文件快照，
