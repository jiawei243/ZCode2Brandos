...
import { APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL, type AppSettings } from "@wbrand/shared";
...
export function useSettings() {
  const { broadcastService, settingService, wbrandAgentService } = useServices();
  ...
  const update = useCallback(
    async (patch: Partial<AppSettings>) => {
      ...
      if (
        ...
        const syncResults = await Promise.allSettled([
          wbrandAgentService.syncAppRuntimePreferences(preferences),
        ]);
    ...
    [broadcastService, settingService, settingsStore, wbrandAgentService, platform, refresh],
  );
