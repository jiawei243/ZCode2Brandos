...
import type { IWBrandTaskService } from "@wbrand/services";
import type { WBrandTaskSnapshot } from "@wbrand/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IWBrandTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<WBrandTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IWBrandTaskService["getTaskSnapshotWithEtag"]>[0];

const wbrandTaskServiceProxyCache = new WeakMap<IWBrandTaskService, IWBrandTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IWBrandTaskService,
  Map<string, { etag: string; snapshot: WBrandTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "wbrand-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: WBrandTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: WBrandTaskSnapshot }>();
...
function createWBrandTaskServiceProxy(service: IWBrandTaskService): IWBrandTaskService {
...
/** 获取 WBrand task wrapper 服务实例 */
export function useWBrandTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IWBrandTaskService {
  // WBrand task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.wbrandTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = wbrandTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createWBrandTaskServiceProxy(rawService);
  wbrandTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
