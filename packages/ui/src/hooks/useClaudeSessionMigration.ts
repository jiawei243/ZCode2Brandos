...
import type { WBrandImportSessionsResult, WBrandImportableSessionCandidate } from "@wbrand/shared";
import { logger } from "@/logger.js";
import { useWBrandTaskService } from "@/hooks/useWBrandTaskService.js";
...
export function useClaudeSessionMigration(params: {
  ...
  const wbrandTaskService = useWBrandTaskService(
  ...
  const [candidates, setCandidates] = useState<WBrandImportableSessionCandidate[]>([]);
  const [selectedSessionIds, setSelectedSessionIds] = useState<string[]>([]);
  const [scanError, setScanError] = useState<string | null>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [lastImportResult, setLastImportResult] = useState<WBrandImportSessionsResult | null>(null);
  ...
  const scan = useCallback(async () => {
    ...
    try {
      ...
      const nextCandidates = await wbrandTaskService.scanImportableClaudeSessions({
  ...
  const importSessions = useCallback(
    async (sessionIds: string[]) => {
      ...
      try {
        ...
        const result = await wbrandTaskService.importClaudeSessions({
          workspacePath: effectiveWorkspacePath,
          ...(effectiveWorkspaceIdentity ? { workspaceIdentity: effectiveWorkspaceIdentity } : {}),
          sessionIds,
        });
