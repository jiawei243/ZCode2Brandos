...
import { createUuid } from "@wbrand/shared";
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@wbrand/shared";
...
export function useAppPanels(options: {
  ...
  const { wbrandAgentService, wbrandSessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await wbrandAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
