...
function isConfigSelectValueInOptions(option: WBrandConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: WBrandConfigOption): string | undefined {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 WBrand Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<WBrandConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: WBrandProvider | undefined,
  entry: Pick<WBrandConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return WBRAND_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return WBRAND_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
