...
import { StoreProvider, useWBrandStore } from "@/store/StoreProvider.js";
...
import { consumeWbrandJwtInvalidRestartMarker } from "@/root/wbrandJwtInvalidRestartMarker.js";
...
function RootInner({
  ...
  const user = useWBrandStore((state) => state.user);
  ...
  const setIsRestoringOAuthSession = useWBrandStore((state) => state.setIsRestoringOAuthSession);
  const setOAuthError = useWBrandStore((state) => state.setOAuthError);
  const oauthPollingActive = useWBrandStore((state) => state.oauthPollingActive);
  const setOAuthPollingActive = useWBrandStore((state) => state.setOAuthPollingActive);
  const markOAuthSuccess = useWBrandStore((state) => state.markOAuthSuccess);
  const {
  ...
  const [welcomeScreenOpenReason, setWelcomeScreenOpenReason] =
    useState<WelcomeScreenOpenReason | null>(() =>
      consumeWbrandJwtInvalidRestartMarker() ? "session-expired" : null,
    );
  const [providerFamilyDomainMigrationComplete, setProviderFamilyDomainMigrationComplete] =
    useState(false);
  const loginEntryRequest = useWBrandStore((state) => state.loginEntryRequest);
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel === APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL) {
        ...
        void services.wbrandAgentService.syncAppRuntimePreferences(parsed.data).catch((error) => {
  ...
  }, [refreshAppSettings, services.broadcastService, services.wbrandAgentService]);

  useEffect(() => {
    if (!appSettings) {
      return;
    }
    void services.wbrandAgentService
  ...
  const totalUnreadTaskCount = useWBrandSessionStore((state) =>
  ...
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免未连接账号前启动 WBrand session。
  ...
  if (isStartupRenderBlocked) {
    ...
    return (
      <RootShell>
        ...
        {/* HTML 启动壳已经展示 WBrand SVG，但 React 接管 root 后旧壳会被整棵替换。
