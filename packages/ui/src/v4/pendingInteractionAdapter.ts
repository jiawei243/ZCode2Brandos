...
/** v4 permission payload → 旧 PermissionDialog 可消费的 WBrandPermissionRequest。 */
export function pendingPermissionToLegacyRequest(
  ...
  const advertisedOptions = payload.fullAccessOption
    ...
    : payload.options;
  const options: WBrandPermissionOption[] = advertisedOptions.map((option) => ({
