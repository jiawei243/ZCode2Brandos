  ...
  const revalidateAndPrependPlugin = useCallback(
    async (current: DraftSuggestedPluginFlow, requestVersion: number) => {
      await invalidateDeferredDraftSessionForSkillChange({
        wbrandSessionService: resolution.services.wbrandSessionService,
        workspacePath,
        workspaceIdentity,
    ...
    [
      ...
      resolution.services.wbrandSessionService,
      targetParams,
      workspaceIdentity,
      workspacePath,
    ],
      ...
      if (
        ...
        (kind === "install" &&
          (!flow.result?.pluginName ||
            flow.result.marketplace !== WBRAND_OFFICIAL_PLUGIN_MARKETPLACE_ID ||
