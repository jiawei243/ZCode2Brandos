...
function resolveLegacyConversationModelValue(params: {
  ...
  if (!configProvider || !modelName || configProvider === WBRAND_AGENT_PROVIDER) {
...
/** V4 config.provider 是实际模型 provider id；agentProvider 表示 WBrand 运行时。 */
export function buildV4ConversationPromptTelemetryExtraDetail(params: {
  agentProvider?: WBrandProvider;
  configProvider?: string | null;
  modelName?: string | null;
  askMode?: string | null;
  ...
  const agentProvider = params.agentProvider ?? WBRAND_AGENT_PROVIDER;
  ...
  return {
    ...
    // agent 表示 WBrand 运行时，不能用模型 provider id 替代。
    agent: agentProvider,
  };
}
