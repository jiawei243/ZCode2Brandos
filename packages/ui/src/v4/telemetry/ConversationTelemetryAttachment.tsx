...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@wbrand/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@wbrand/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.wbrandAgentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.wbrandAgentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.wbrandAgentService as object | null | undefined;
