...
// 复用 treemappingActivity 的既有解析规则，不复活 wbrandChatMessages。
//
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(wbrandAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [wbrandAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
