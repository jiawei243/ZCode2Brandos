 ...
 * 公开页 bundle 体积与无 Desktop 宿主（window.wbrand / PlatformProvider / tab store）的运行环境；
...
import { getCompactToolCallStatusMessageId, type Locale } from "@wbrand/shared";
import type {
  ArtifactRow,
  AssistantTextRow,
  ConversationRow,
...
import { WBrandIntlProvider, useWBrandIntl } from "@/i18n/IntlProvider.js";
...
function resolveImportedArtifactPath(
  ...
  // 取舍：这里只校验形状（.wbrand-share/<dir>/shared-artifacts/<file> 四段），刻意不把
  ...
  // .wbrand-share/*/shared-artifacts/ 内，无越权读放大。
...
export function ConversationShareReadonlyTimeline({
  ...
  const labels: ReadonlyLabels =
    locale === "zh-CN"
      ? {
          ...
          unsupportedRows: "部分内容需要更新 WBrand 查看",
        }
      : {
          history: "Reasoning",
          ...
          unsupportedRows: "Some content requires a newer version of WBrand",
        };
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <WBrandIntlProvider initialLocale={locale}>
