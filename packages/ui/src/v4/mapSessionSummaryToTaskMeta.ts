// sessions-index 的 SessionSummary → 侧栏实时 detail WBrandTaskMeta 映射。
...
import type { TraceId, WBrandProvider, WBrandTaskMeta } from "@wbrand/shared";
import type { SessionSummary } from "@wbrand/shared/wbrand-protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): WBrandTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
interface MapSessionSummaryOptions {
  ...
  previous?: WBrandTaskMeta;
}

/**
 * SessionSummary → 实时 detail WBrandTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as WBrandProvider } : {}),
      ...(status ? { status } : {}),
