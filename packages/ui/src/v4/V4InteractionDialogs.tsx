...
function buildV4ElicitationProgressKey(request: WBrandElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useWBrandStoreWithDefault((state) => state.notificationEnabled, true);
  const localElicitationDraft = useWBrandSessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useWBrandSessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: WBrandPermissionOption, feedback?: string) => {
