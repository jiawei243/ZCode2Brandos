import { BufferWriter, serialize } from "@wbrand/rpc";
...
interface AttachmentUploadAgent {
  ...
  attachmentCommitV4(params: WBrandAgentAttachmentTerminalParams): Promise<V4AttachmentPutResult>;
...
function measureAttachmentChannelRequestBytes(method: string, params: unknown): number {
  ...
  serialize(writer, [100, 2_147_483_647, ServiceChannels.WBrandAgent, method]);
...
export async function uploadAttachmentTransaction(
  ...
  const beginParams: WBrandAgentAttachmentBeginParams = {
    ...common,
  ...
  try {
    ...
    for (let chunkIndex = begin.nextChunkIndex; chunkIndex < totalChunks; chunkIndex += 1) {
      ...
      const chunkParams: WBrandAgentAttachmentChunkParams = {
        ...common,
        chunkIndex,
    ...
    const terminal = common satisfies WBrandAgentAttachmentTerminalParams;
