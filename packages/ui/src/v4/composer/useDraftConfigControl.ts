...
import { WBRAND_AGENT_PROVIDER, resolveExecutionState } from "@wbrand/shared";
...
import type { SessionConfigState } from "@wbrand/shared/wbrand-protocol-v4";
import type { IModelSelectionService } from "@wbrand/services";
...
import { submissionModeSchema } from "@wbrand/shared/wbrand-protocol-v4";
import { prepareWorkspaceWithWBrandSessionService } from "@/hooks/useWorkspacePrepare.js";
import { useWBrandSessionService } from "@/hooks/useWBrandSessionService.js";
...
function shouldHydrateWorkspaceCatalog(params: {
  configOptions: readonly WBrandConfigOption[];
  sessionId: string | null;
  slashCommands: readonly WBrandSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const wbrandSessionService = useWBrandSessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithWBrandSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useWBrandSessionStore.getState();
