...
import type { SessionCreateSource } from "@wbrand/shared";
...
import { localizeConversationShareUrl } from "@wbrand/shared";
import type {
...
import { prepareWorkspaceWithWBrandSessionService } from "@/hooks/useWorkspacePrepare.js";
import {
...
import { parseModelPickerValue } from "@/lib/wbrandSessionProjection.js";
...
import { useWBrandSessionStore } from "@/store/wbrandSessionStore.js";
import {
...
import type { GroupedDraftTaskState } from "@/store/wbrandSessionStoreTypes.js";
import {
...
export function SessionPane({
  ...
  const [sendSubmissionError, setSendSubmissionError] = useState<WBrandUiError | null>(null);
  ...
  const workspaceConfigOptions = useWBrandSessionStore(
  ...
  useEffect(() => {
    ...
    useWBrandSessionStore.getState().invalidateDraftRuntime(workspacePath, workspaceIdentity);
  ...
  const promoteGroupedDraftTask = useWBrandSessionStore((state) => state.promoteGroupedDraftTask);
  ...
  const theme = useWBrandStoreWithDefault((state) => state.theme, "system");
  const codePreviewSettings = useWBrandStoreWithDefault(
  ...
  const dispatchCommand = useCallback(
    async (
      ...
      const groupedDraftTask =
        type === "createSession"
          ? useWBrandSessionStore.getState().getWorkspaceState(workspacePath, workspaceIdentity)
              .groupedDraftTask
          : null;
    ...
    async (reference?: ConversationSelectionReference, forceNew = false) => {
      ...
      try {
        ...
        if (targetChildSessionId) {
          try {
            await wbrandSessionService.readSession({
              workspacePath,
              ...(workspaceIdentity ? { workspaceIdentity } : {}),
  ...
  const dispatchSendText = useCallback(
    (text: string, options?: ConversationComposerSendOptions) => {
      const createSource = useWBrandSessionStore
        .getState()
    ...
    async (
      ...
      try {
        ...
        setSendSubmissionError({
          code: runtimeModelUnavailable ? "WBRAND_RUNTIME_MODEL_UNAVAILABLE" : "SEND_FAILED",
  ...
  const handleRecoverCustomModelSelection = useCallback(
    async (value: string, sourceModel: ModelSelectionSource | null) => {
      ...
      const displayProvider = provider ?? WBRAND_AGENT_PROVIDER;
      ...
      try {
        await wbrandTaskService.restartWorkspaceProcess({
          workspacePath,
          workspaceIdentity,
        ...
        const prepareResult = await prepareWorkspaceWithWBrandSessionService({
          workspacePath,
          workspaceIdentity,
          provider: displayProvider,
          wbrandSessionService,
          ...
          >
            <ConversationTimeline
              ...
              headerSlot={
                ...
                // rows 为空，但只读块必须留下来显示「需要更新 WBrand」，不能整块消失。
                importedShare &&
