 ...
 * Modified by WBrand: local integration, formatting and adaptations.
...
import type { EditorInfo, FileStat, OpenInEditorOptions } from "@wbrand/shared";
...
import { createWBrandFileCitationRemarkPlugin } from "@/lib/wbrandFileCitationRemarkPlugin.js";
import { windowsFileLinkEscapeRemarkPlugin } from "@/lib/windowsFileLinkEscapeRemarkPlugin.js";
import { projectWBrandFileCitations } from "@/lib/wbrandFileCitation.js";
import { rewriteMarkdownArtifactImageSources } from "@wbrand/shared";
...
export type MessageResponseProps = {
  ...
  /** 仅 Assistant 正文开启：把完整 wbrand-file-citation 投影为现有文件链接。 */
  renderWBrandFileCitations?: boolean;
};
...
export function buildMessageStreamdownRenderKey(params: {
  ...
  return [
    ...
    params.renderWBrandFileCitations ? "citations" : "plain",
...
export const messageResponsePropsAreEqual = (
  ...
  nextProps.renderWBrandFileCitations === prevProps.renderWBrandFileCitations &&
...
export const MessageResponse = memo(
  ({
    ...
    onOpenFileLink,
    onOpenExternalUrl,
    renderWBrandFileCitations = false,
    workspacePath,
    workspaceHomePath,
    workspaceIdentity,
    ...
    const projectedCitationMarkdown = useMemo(
      () =>
        renderWBrandFileCitations
          ? projectWBrandFileCitations(rawMarkdown, { streaming: renderStreaming }).visibleText
          : rawMarkdown,
      [rawMarkdown, renderStreaming, renderWBrandFileCitations],
    );
    ...
    const messageRemarkPlugins = useMemo<PluggableList>(
      () => [
        ...
        ...(renderWBrandFileCitations && workspacePath
          ? [createWBrandFileCitationRemarkPlugin(workspacePath, workspaceHomePath)]
          : []),
      ],
      [renderWBrandFileCitations, workspaceHomePath, workspacePath],
    );
    ...
    const boundaryResetKey = useMemo(
      () =>
        renderStreaming
          ...
          : `${streamdownMode}:${renderWBrandFileCitations ? "citations" : "plain"}:${hashMarkdownCacheKey(targetMarkdown)}`,
      [renderStreaming, renderWBrandFileCitations, streamdownMode, targetMarkdown],
    );
