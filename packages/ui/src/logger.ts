import { formatLogPrefix } from "@wbrand/shared";
...
function log(level: LogLevel, ...args: unknown[]) {
  ...
  // ui 包单独 typecheck 时拿不到 desktop renderer 注入的 window.wbrand 声明，
  ...
  if (level !== "debug" && typeof window !== "undefined") {
    ...
    (window as DesktopLogBridgeWindow).wbrand?.log?.(level, args);
  }
}
...
export function logMemoryDiagnostics(line: string): void {
  ...
  if (typeof window !== "undefined") {
    const bridge = (window as DesktopLogBridgeWindow).wbrand?.log;
    if (bridge) {
      bridge("info", [line]);
      return;
    }
