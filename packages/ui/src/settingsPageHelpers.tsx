...
export type { Locale, LocalePreference } from "@wbrand/shared";
...
export function GeneralSectionContent({
  ...
  onWBrandInteractionBehaviorChange: (behavior: WBrandInteractionBehavior) => Promise<void>;
  ...
  return (
    <div className="space-y-4">
      ...
      <SettingsGroupCard>
        <SettingsRow
          label={intl.formatMessage({ id: "settings.wbrandInteractionBehavior" })}
          description={intl.formatMessage({
            id: "settings.wbrandInteractionBehaviorDescription",
          })}
          control={
            <Select
              value={wbrandInteractionBehavior}
              onValueChange={(value) => {
                void onWBrandInteractionBehaviorChange(value as WBrandInteractionBehavior);
              }}
            >
              <SelectTrigger size="lg" className="w-[260px] min-w-0 justify-between">
                <SelectValue />
              ...
              <SelectContent>
                {WBRAND_INTERACTION_BEHAVIOR_OPTIONS.map((behavior) => (
                  <SelectItem key={behavior} value={behavior}>
                    {intl.formatMessage({
                      id: `settings.wbrandInteractionBehavior.option.${behavior}`,
                    })}
                  </SelectItem>
                ))}
              </SelectContent>
