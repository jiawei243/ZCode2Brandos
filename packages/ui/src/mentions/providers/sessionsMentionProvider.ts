// composer parity：`#` 会话候选的数据源从旧 wbrandSessionStore/taskQueryCache/remote*
...
import type { WBrandProvider, WBrandTaskMeta } from "@wbrand/shared";
...
function getSessionLabel(task: WBrandTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: WBrandTaskMeta): string {
  const raw = task.workspacePath.trim() || task.workspaceIdentity?.trim() || "";
  return raw.split(/[\\/]/).filter(Boolean).at(-1) ?? raw;
}

function mapTaskToMentionItem(task: WBrandTaskMeta, provider: WBrandProvider): SessionMentionItem {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, WBrandTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.wbrandAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.wbrandAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.wbrandAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.wbrandAgentService,
        },
      ];
    }
