...
import { WBRAND_AGENT_PROVIDER, resolveWBrandRuntimeEnv } from "@wbrand/shared";
import { createHash, randomUUID } from "node:crypto";
import { existsSync } from "node:fs";
import { cp, lstat, mkdir, mkdtemp, readdir, readFile, readlink, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { dirname, join, resolve } from "node:path";
import type { IRemoteBackend } from "@wbrand/server/remote/backend.js";
import {
  buildRemoteExecutableReplaceCommand,
  buildRemoteMoveCommand,
  type DeployLoggers,
  waitForClose,
} from "@wbrand/server/remote/deployShared.js";
import {
  buildWriteLiteralFileCommand,
  quotePosixPathArg,
} from "@wbrand/server/remote/posixShell.js";
import { createTarGzArchive } from "@wbrand/server/remote/localTarGz.js";
import {
  buildRemoteAgentBundleWrapper,
  isRemoteAgentBundleWrapperCurrent,
  REMOTE_AGENT_BUNDLE_NAME,
} from "@wbrand/server/remote/wbrandAgentBundleWrapper.js";
import {
  deployRemoteAgentWrapper,
  isWslBackend,
} from "@wbrand/server/remote/wbrandAgentWrapperDeploy.js";
import {
...
} from "@wbrand/server/remote/wbrandAgentOfficialPluginAssets.js";
import { repairLegacyRemoteOfficialPluginDirectoryPermissions } from "@wbrand/server/remote/wbrandAgentOfficialPluginPermissionRepair.js";

const DEV_AGENT_BUNDLE_RELATIVE_PATH = "apps/wbrand-cli/packages/cli/dist/wbrand.cjs";
const DEV_AGENT_BUNDLE_ENV = "WBRAND_REMOTE_DEV_AGENT_BUNDLE";
const REMOTE_DEV_AGENT_BUNDLE_NAME = REMOTE_AGENT_BUNDLE_NAME;
const REMOTE_DEV_AGENT_VERSION_FILE_NAME = ".dev-version";

interface DeployDevelopmentWBrandAgentRuntimeParams {
...
function shouldUseDevelopmentAgentBundle(): boolean {
  if (resolveWBrandRuntimeEnv(process.env) !== "development") {
    return false;
  }
...
async function hashDevelopmentOfficialPluginPackage(
  ...
  const manifestPath = join(packageRoot, ".wbrand-plugin", "plugin.json");
  if (!existsSync(manifestPath)) {
    throw new Error(`[wbrand-agent-deploy] missing official plugin manifest: ${manifestPath}`);
  }
...
function assertDevelopmentOfficialPluginRequiredAssets(
  ...
  for (const requiredRelativePath of REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS) {
    ...
    if (!existsSync(requiredPath)) {
      throw new Error(
        `[wbrand-agent-deploy] missing official plugin required asset: ${requiredPath}`,
      );
    }
  }
}
...
async function shouldSkipDevelopmentWBrandAgentDeploy(params: {
  backend: IRemoteBackend;
  remoteVersionFile: string;
...
async function stageDevelopmentOfficialPluginPackages(params: {
  ...
  for (const packageName of REMOTE_AGENT_OFFICIAL_PLUGIN_PACKAGE_NAMES) {
    const sourceRoot = join(params.repoRoot, "apps", "wbrand-cli", "packages", packageName);
...
async function uploadDevelopmentOfficialPluginPackages(params: {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "wbrand-remote-agent-packages-"));
    ...
    const cleanupRemoteStaging = async (): Promise<void> => {
      try {
        ...
        params.loggers.logWarn(
          `[wbrand-agent-deploy] ${WBRAND_AGENT_PROVIDER}: 清理 owner staging 失败 (${ownerSuffix}): ${String(error)}`,
        );
      }
    };
    params.loggers.log(`[wbrand-agent-deploy] ${WBRAND_AGENT_PROVIDER}: 开发态上传官方插件资源`);
    try {
...
export async function deployDevelopmentWBrandAgentRuntime(
  backend: IRemoteBackend,
  params: DeployDevelopmentWBrandAgentRuntimeParams,
  loggers: DeployLoggers,
): Promise<boolean> {
  ...
  if (
    await shouldSkipDevelopmentWBrandAgentDeploy({
      backend,
  ...
  // 开发态 SSH 远端过去只会部署本地 wbrand.cjs，不会携带 packages/*-plugin。
  // builtin plugin seed 依赖 agent 包旁边的官方插件源资源，所以 dev 部署需要同步 bundle 与插件资源。
  // 本地修改 apps/wbrand-cli 后，远端测试仍运行滞后的发布包。这里改为上传 dev 启动时刚构建的
  // dist/wbrand.cjs，并用远端已部署的 node 包一层 wrapper 启动，保证 agent 仍运行在目标机器内。
  loggers.log(
    `[wbrand-agent-deploy] ${WBRAND_AGENT_PROVIDER}: 开发态上传本地 wbrand.cjs ${devVersion.slice(0, 12)}`,
  );
  ...
  loggers.log(
    `[wbrand-agent-deploy] ${WBRAND_AGENT_PROVIDER}: 开发态部署完成 ${devVersion.slice(0, 12)}`,
  );
  return true;
}
