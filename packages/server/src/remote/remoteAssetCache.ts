...
import { fileExists } from "@wbrand/server/remote/deployShared.js";
import { extractTarGzArchive } from "@wbrand/server/remote/localTarGz.js";
import {
...
async function ensureRemoteComponentDirFromCdnInternal(
  ...
  if (!forceRefresh && initialMissingPaths) {
    ...
    // wbrand.cjs 的 glm cache 后，再补传 packages 会一直复用残缺 cache。
...
export async function parseRemoteAssetManifestFromResponse(
  ...
  for (const [index, rawComponent] of rawComponents.entries()) {
    ...
    if (!mountRule) {
      ...
      // 当前客户端只认识 WBrand Agent 与基础运行时，未知组件应跳过，不能阻断当前组件下载。
      continue;
    }
...
async function isValidReleaseDir(releaseDir: string, platformArch: string): Promise<boolean> {
  ...
  if (!(await fileExists(releaseDir, "server", "wbrand-server.cjs"))) {
    return false;
  }
...
export function resolveFallbackRemoteAssetCacheDir(): string {
  return join(tmpdir(), "wbrand-remote-assets-cache");
}
