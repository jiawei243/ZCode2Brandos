...
import type { IRemoteBackend, RemoteEnvironment } from "@wbrand/server/remote/backend.js";
import {
  REMOTE_BASE,
  type DeployLoggers,
...
import type { RemoteAssetInstaller } from "@wbrand/server/remote/remoteAssetInstaller.js";
import { buildWriteLiteralFileCommand } from "@wbrand/server/remote/posixShell.js";
import { deployDevelopmentWBrandAgentRuntime } from "@wbrand/server/remote/wbrandAgentDevDeploy.js";
import {
...
} from "@wbrand/server/remote/remoteAssetLiveIdentity.js";

const REMOTE_AGENT_RUNTIME_BASE = `${REMOTE_BASE}/agents`;

export interface DeployWBrandAgentRuntimeOptions extends RemoteAssetDeployOptions {
  platformArch: string;
...
function isSelectedWBrandAgentComponent(
  componentId: string,
...
async function shouldSkipWBrandAgentDeploy(params: {
  ...
  // wrapper 在、但 wbrand.cjs 缺失（被清理 / 旧原生二进制部署残留）时也要重新部署。
  ...
  params.loggers.log(
    `[wbrand-agent-deploy] ${WBRAND_AGENT_PROVIDER}: 制品 SHA ${params.expectedArtifactSha256} 已部署，跳过`,
  );
  return true;
}
...
/**
 * 部署 WBrand Agent runtime 到远程机器。
 *
 * 生产态只用 manifest SHA 判断制品是否变化；语义版本不参与跳过决策。
 */
export async function deployWBrandAgentRuntime(
  backend: IRemoteBackend,
  env: RemoteEnvironment,
  options: DeployWBrandAgentRuntimeOptions,
  loggers: DeployLoggers,
): Promise<void> {
  const provider = WBRAND_AGENT_PROVIDER;
  const runtime = WBRAND_AGENT_RUNTIME;
  const componentId = provider;
  if (!isSelectedWBrandAgentComponent(componentId, options.selectedResourcePackageIds)) {
    loggers.log(`[wbrand-agent-deploy] ${provider}: 未选择资源包 ${componentId}，跳过检查和部署`);
    return;
  }

  // binaryName 指 wrapper 可执行文件名（如 wbrand-agent / wbrand-agent.exe）——
  // 一个调用远端 node 执行 wbrand.cjs 的壳脚本。
  const binaryName = runtime.resolveEntrySegments(env.platform).at(-1);
  if (!binaryName) {
    loggers.logWarn(`[wbrand-agent-deploy] ${provider}: 无法解析 agent 入口名称，跳过部署`);
    return;
  }
  ...
  if (
    await deployDevelopmentWBrandAgentRuntime(
      backend,
      {
  ...
  try {
    ...
    loggers.logWarn(
      `[wbrand-agent-deploy] ${provider}: 读取 manifest SHA 失败，将重新部署: ${String(error)}`,
    );
  }

  if (
    await shouldSkipWBrandAgentDeploy({
      backend,
      remoteBinaryPath,
      remoteBundlePath,
      runtimeResourceDir: runtime.bundledResourceDir,
  ...
  loggers.log(`[wbrand-agent-deploy] ${provider}: 开始部署 v${runtime.version}...`);
  ...
  if (permissionRepairSucceeded) {
    ...
    await installOfficialPluginPackages();
    // 2) packages 替换成功后再安装编译产物 wbrand.cjs（跨平台同一份，glm 组件里就是它）。
    await installBundle();
  }
  // 3) 写入 wrapper（即 resolver 期望的 wbrand-agent），用远端已部署的 node 执行 wbrand.cjs。
  ...
  loggers.log(`[wbrand-agent-deploy] ${provider}: 部署完成 v${runtime.version}`);
}
