...
import { type DeployLoggers, waitForClose } from "@wbrand/server/remote/deployShared.js";
...
export async function repairLegacyRemoteOfficialPluginDirectoryPermissions(params: {
  ...
  params.loggers.logWarn(
    `[wbrand-agent-deploy] ${WBRAND_AGENT_PROVIDER}: 检查并修复旧 builtin plugin 目录权限 ${params.remoteOfficialPluginDir}`,
  );
  ...
  try {
    ...
    params.loggers.logWarn(
      `[wbrand-agent-deploy] ${WBRAND_AGENT_PROVIDER}: 修复旧 builtin plugin 目录权限失败，将继续尝试替换 packages: ${
