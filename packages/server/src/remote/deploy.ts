...
import { deployWBrandAgentRuntime } from "./wbrandAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@wbrand/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { checkServerBundleRequiredMarkers } from "@wbrand/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@wbrand/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@wbrand/server/remote/wbrandAgentOfficialPluginAssets.js";
import {
  ensureRemoteReleaseDirFromCdn,
  selectRemoteAssetManifestComponents,
  type RemoteAssetManifestRef,
} from "@wbrand/server/remote/remoteAssetCache.js";
import {
...
} from "@wbrand/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@wbrand/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@wbrand/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@wbrand/server/remote/remoteDeployLock.js";
import type { RemoteAssetNetworkPort } from "@wbrand/server/remote/remoteAssetNetwork.js";
...
/**
 * Deploy the wbrand server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/wbrand-server.cjs 可启动，不代表随包工具仍存在。
      ...
      await deployWBrandAgentRuntime(
        backend,
        env,
        {
          ...assetDeployOptions,
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/wbrand-server.cjs",
      remotePath: `${REMOTE_BASE}/wbrand-server.cjs`,
    ...
    // 部署 WBrand Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployWBrandAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/wbrand-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", WBRAND_VERSION);
    if (version !== WBRAND_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${WBRAND_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", WBRAND_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/wbrand-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/wbrand.cjs`);
