...
import type { RemoteEnvironment } from "@wbrand/server/remote/backend.js";
import {
  REMOTE_BASE,
  type DeployLoggers,
  waitForClose,
...
} from "@wbrand/server/remote/remoteAssetCdn.js";
  ...
  if (options.mockCdnDir) {
    try {
      const content = await readFile(
        join(options.mockCdnDir, "releases", WBRAND_VERSION, `manifest-${platformArch}.json`),
        "utf8",
      );
      return {
