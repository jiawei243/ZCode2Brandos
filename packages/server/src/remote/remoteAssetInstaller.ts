...
import type { RemoteAssetInstallMode } from "@wbrand/shared";
...
import type {
  ...
  RemoteDownloadTool,
  RemoteSha256Tool,
} from "@wbrand/server/remote/remoteAssetPreflight.js";
import {
  resolveRemoteAssetFetch,
  type RemoteAssetNetworkPort,
} from "@wbrand/server/remote/remoteAssetNetwork.js";
...
function buildStaleRemoteStagingCleanupCommand(parentDir: string, patterns: string[]): string {
  ...
  return [
    ...
    // 新连接只回收超过 24 小时的 WBrand owner staging，避免误删当前 owner 或正常时长内的活跃部署。
...
export class LocalUploadAssetInstaller implements RemoteAssetInstaller {
  ...
  async installDirectory(params: {
    ...
    const localTarPath = join(
      tmpdir(),
      `wbrand-remote-${params.componentId}-${Date.now()}-${randomUUID()}.tar.gz`,
    );
