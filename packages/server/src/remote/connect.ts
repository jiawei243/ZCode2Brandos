import { SocketProtocol, ChannelClient } from "@wbrand/rpc";
import type { IServiceAccessor } from "@wbrand/services";
import { RemoteServiceAccess } from "@wbrand/client";
import {
...
/**
 * Connect to a remote wbrand server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 wbrand-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    'WBRAND_SERVER_RUNTIME_ROOT="$HOME/.wbrand/server"',
  ];
  ...
  if (appVersion) {
    ...
    envParts.push(`${WBRAND_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${WBRAND_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${WBRAND_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${WBRAND_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ~/.wbrand/server/node ~/.wbrand/server/wbrand-server.cjs`;
}
