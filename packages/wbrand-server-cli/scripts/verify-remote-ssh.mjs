 ...
 *
 * 前置：docker 可用；已运行 `pnpm --filter @wbrand/server-cli stage --target <target>`。
...
const HOST_CAPABILITY_HEADER = "x-wbrand-rpc-host-capability";
...
const releaseArchive = join(packageRoot, "dist-release", `wbrand-server-${target}.tar.gz`);
const containerName = `wbrand-server-verify-${process.pid}`;
const imageTag = "wbrand-server-verify-sshd:ubuntu22";
...
async function main() {
  ...
  await run("ls", [releaseArchive], { quiet: true }).catch(() => {
    throw new Error(
      `Release archive missing: ${releaseArchive}; run pnpm --filter @wbrand/server-cli stage --target ${target}`,
    );
  });

  // 一次性 ssh 密钥，容器仅信任本次运行生成的公钥。
  const workDir = await mkdtemp(join(tmpdir(), "wbrand-server-verify-"));
  ...
  await ssh(
    `mkdir -p /root/wbrand-server && tar -xzf /root/wbrand-server-${target}.tar.gz -C /root/wbrand-server --strip-components=1`,
  );
  ...
  await ssh(
    "cd /root/wbrand-server/runtime && ./node -e \"const pty=require('node-pty');const p=pty.spawn('/bin/echo',['pty-ok'],{cols:80,rows:24});let o='';p.onData(d=>o+=d);p.onExit(()=>{process.exit(o.includes('pty-ok')?0:1)})\"",
  );
  log("node-pty ok");

  log("verify remote lifecycle status/stop");
  const { stdout: statusOutput } = await ssh("/root/wbrand-server/bin/wbrand status --json");
  assert(
    JSON.parse(statusOutput.trim().split("\n").pop()).state === "ready",
    "remote status ready",
  );
  await ssh("/root/wbrand-server/bin/wbrand stop --json");
  const { stdout: stoppedOutput } = await ssh("/root/wbrand-server/bin/wbrand status --json");
