...
interface StageOptions {
  ...
  distDir: string;
  /** 既有 CLI/Agent bundle（wbrand.cjs，自包含 CJS） */
  ...
  /** 未被 pnpm 链接到 node_modules 的 workspace 包（例如 @wbrand/tui）。 */
...
# 由 wbrand-server staging 生成：定位发行根后用随包 Node 启动 Server CLI。
...
/** 组装 `wbrand-server-<os>-<arch>/` 发行目录；只做本地组装，不上传、不发布。 */
export async function stageRelease(options: StageOptions): Promise<StagedRelease> {
  ...
  const releaseName = `wbrand-server-${options.target}`;
  ...
  await cp(options.agentBundlePath, join(runtimeDir, "wbrand.cjs"), { dereference: true });
  ...
  const launcherPath = join(
    releaseDir,
    "bin",
    options.target.startsWith("win32-") ? "wbrand.cmd" : "wbrand",
  );
  ...
  const componentSpecs = [
    ...
    { id: "agent-runtime", paths: ["runtime/wbrand.cjs", "runtime/licenses/agent"] },
    ...(plugins.length > 0
      ? [
          {
            id: "official-plugins",
