...
import { createServiceLogger } from "@wbrand/services/node";
import {
...
function exposeWebSocket(
  ...
  services.exposeOnChannelServer(
    server,
    scope ? new Map([[IWBrandAgentService.channelName, scope.service]]) : new Map(),
  );
  socket.onClose(() => {
    void scope?.dispose();
    rawServer.dispose();
...
export async function createCoreHttpServer(
  ...
  const info: ServerRemoteInfo = {
    serverId: options.serverId ?? hostname() ?? "wbrand-server",
  ...
  app.use("/ws/host", async (context, next) => {
    const capability = context.req.header(WBRAND_RPC_HOST_CAPABILITY_HEADER);
