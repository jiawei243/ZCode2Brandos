...
import { IWBrandAgentService } from "@wbrand/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitWBrandBuiltinProviderConfigFilePath =
    process.env[WBRAND_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const wbrandBuiltinProviderConfigFilePath = explicitWBrandBuiltinProviderConfigFilePath
    ? explicitWBrandBuiltinProviderConfigFilePath
    : typeof __WBRAND_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeWBrandBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __WBRAND_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!wbrandBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 WBrand Built-in Provider Config，且未设置 ${WBRAND_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IWBrandAgentService));
