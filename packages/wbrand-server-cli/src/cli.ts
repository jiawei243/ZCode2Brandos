...
import { WBRAND_VERSION } from "@wbrand/shared";
import {
...
export function serviceRegistrationEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.WBRAND_SERVER_SKIP_SERVICE_REGISTRATION !== "1";
}
...
async function runServe(
  ...
  if (shouldRegisterService(daemonRequested, supervisorProcess)) {
    ...
    if (existing && (existing.state === "ready" || existing.state === "starting")) {
      if ((existing.serviceRegistered && !legacyIdentityMigration) || startupMode === "fallback") {
        if (json) stdout(io, existing);
          ...
          stdout(
            io,
            `WBrand Server ${existing.state} at ${existing.host ?? ""}:${existing.port ?? ""}`,
          );
        return 0;
      }
    ...
    if (!serviceStarted) {
      const child = fork(
        ...
        [...SERVER_SERVICE_ARGS, "--server-root", layout.serverRoot],
        {
          ...
          env: {
            ...process.env,
            WBRAND_DATA_BASE_DIR: layout.dataBaseDir,
            WBRAND_SERVER_ROOT: layout.serverRoot,
          },
        },
      );
      child.unref();
    ...
    const started = await waitForPersistedStatus(
      ...
      () => {
        if (childEarlyExit) {
          throw new Error(
            `WBrand Server daemon exited before ready (code=${childEarlyExit.code ?? "null"} signal=${childEarlyExit.signal ?? "none"}); check ${layout.statusFile} for details`,
          );
        }
      },
      serviceStarted,
      ...
      launch: (generation, release) => {
        ...
        return fork(corePath, [String(generation)], {
          ...
          env: {
            ...inheritedEnv,
            ...releaseWiring,
            ...(runtimeRoot ? { WBRAND_SERVER_RUNTIME_ROOT: runtimeRoot } : {}),
          },
  ...
  else stdout(io, `WBrand Server ${status.state} at ${status.host ?? ""}:${status.port ?? ""}`);
...
async function runControl(
  ...
  try {
    ...
    if (command === "status") {
      ...
      result = persisted.status ?? createStoppedServerStatus(WBRAND_VERSION);
...
async function runUninstall(
  ...
  const first = await (io.confirm?.("Type DELETE to uninstall WBrand Server: ") ??
    Promise.resolve(""));
...
async function finishUninstall(
  ...
  try {
    ...
    // WBrand allowlist，并把保留项写入结果供用户审计。先落盘 marker，再释放 lock，
    // 让并发 Supervisor 在删除 run 目录的最后窗口也会 fail-closed。
    await writeFile(
      layout.uninstalledFile,
      `${JSON.stringify({ uninstalled: true, uninstalledAt: Date.now(), version: WBRAND_VERSION, preservedPaths }, null, 2)}\n`,
...
async function delegateLegacyCli(argv: readonly string[], io: CliIO): Promise<number> {
  const candidate =
    process.env.WBRAND_LEGACY_CLI_ENTRY?.trim() ||
    join(dirname(fileURLToPath(import.meta.url)), "wbrand.cjs");
  try {
    await access(candidate);
  } catch {
    stdout(io, argv.length ? `Unknown command: ${argv[0]}` : "WBrand TUI");
