...
export async function prepareOnlineUpdate(layout: ServerLayout): Promise<UpdatePreparation> {
  const catalogUrl = process.env.WBRAND_SERVER_RELEASE_MANIFEST_URL?.trim();
  if (!catalogUrl) {
    const pending = await new ReleaseManager(layout).readPending();
    if (!pending) {
      throw new Error(
        "WBrand Server release source is not configured and no offline pending release is available",
      );
    }
  ...
  const temporaryDir = await mkdtemp(join(tmpdir(), "wbrand-server-update-"));
...
async function prepareComponentUpdate(input: {
  ...
  const temporaryDir = await mkdtemp(join(tmpdir(), "wbrand-server-components-"));
