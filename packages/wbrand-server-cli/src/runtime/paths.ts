...
function getDefaultServerDataRoot(): string {
  const configured = process.env.WBRAND_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".wbrand", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\wbrand-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".wbrand") {
