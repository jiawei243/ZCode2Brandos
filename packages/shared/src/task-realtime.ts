...
// wbrand-task-types-core.ts。本文件与旧 realtime 总线组同生命周期。
import type {
...
import type { WBrandPermissionResponse } from "./wbrand-protocol-legacy-types.js";
import type { WorkspaceHookReviewDecision } from "./wbrand-protocol-v4/workspace-hook-review.js";
...
export type TaskOwnerCommandRequest =
  ...
  | {
      ...
      taskId: string;
      runId: string;
      taskCommand: Extract<WBrandTaskRuntimeCommand, { type: "send_prompt" }>;
    }
  | {
      commandRequestId: string;
      ...
      runId: string;
      commandId: string;
      clientMode: Extract<WBrandTaskClientMode, "web-remote-replayable">;
    }
  | {
      commandRequestId: string;
...
export type TaskOwnerCommandResult =
  | { commandRequestId: string; success: true; taskCommand?: WBrandTaskRuntimeCommand }
  | {
      commandRequestId: string;
      success: false;
      error: string;
