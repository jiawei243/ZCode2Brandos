export const WBRAND_KNOWN_TOOL_NAMES = [
  "Read",
  "Write",
  "Edit",
  "ApplyPatch",
...
export type WBrandKnownToolName = (typeof WBRAND_KNOWN_TOOL_NAMES)[number];

export type WBrandToolFamily =
  | "file-read"
  | "file-write"
  | "shell"
  | "search"
...
const TOOL_FAMILY_BY_NAME: Record<WBrandKnownToolName, WBrandToolFamily> = {
  Read: "file-read",
  Write: "file-write",
  Edit: "file-write",
...
const TOOL_NAME_BY_LOWER = new Map<string, WBrandKnownToolName>(
  WBRAND_KNOWN_TOOL_NAMES.map((toolName) => [toolName.toLowerCase(), toolName]),
);

export function normalizeWBrandToolName(
  value: string | null | undefined,
): WBrandKnownToolName | null {
...
export function getWBrandToolFamilyForName(
  value: string | null | undefined,
): WBrandToolFamily | null {
  const toolName = normalizeWBrandToolName(value);
  return toolName ? TOOL_FAMILY_BY_NAME[toolName] : null;
}

export function isWBrandToolFamily(
  value: string | null | undefined,
  family: WBrandToolFamily,
): boolean {
  return getWBrandToolFamilyForName(value) === family;
}

export function isWBrandFileContentWriteToolName(value: string | null | undefined): boolean {
  return normalizeWBrandToolName(value) === "Write";
}
