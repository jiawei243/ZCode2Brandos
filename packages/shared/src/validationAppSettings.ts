...
import { normalizeWBrandEndpointOrigin } from "./wbrandEndpoint.js";
import {
...
const wbrandInteractionBehaviorSchema = z.enum(["queue", "guide"]);
...
const wbrandEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeWBrandEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeWBrandEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("wbrandEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = wbrandEndpointOriginSchema.safeParse(raw.wbrandEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, wbrandEndpointOrigin: parsed.data };
  }
  const { wbrandEndpointOrigin: _wbrandEndpointOrigin, ...next } = raw;
...
const appSettingsObjectSchema = z.object({
  ...
  wbrandInteractionBehavior: wbrandInteractionBehaviorSchema.default("queue"),
  ...
  wbrandEndpointOrigin: wbrandEndpointOriginSchema.optional(),
});
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeWBrandEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
...
export const appSettingsPatchSchema = z.object({
  ...
  wbrandInteractionBehavior: wbrandInteractionBehaviorSchema.optional(),
