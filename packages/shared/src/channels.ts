...
export const ServiceChannels = {
  ...
  /** 跨窗口广播 */
  Broadcast: "broadcast",
  /** WBrand task wrapper 服务 */
  WBrandTask: "wbrand-task",
  /** 窗口 Host 聚合 workspace/task 投影与列表写路由 */
  WindowController: "window-controller",
  /** WBrand Protocol agent 服务 */
  WBrandAgent: "wbrand-agent",
  /** WBrand session 应用服务 */
  WBrandSession: "wbrand-session",
  ...
  /** 插件管理服务 */
  Plugins: "plugins",
  /** 设置页插件管理服务（UI 平台能力面收敛，不再直触 wbrandAgentService） */
...
export const PlatformChannels = {
  /** 打开系统目录选择框 */
  SelectDirectory: "wbrand:select-directory",
  /** 打开系统文件选择框 */
  SelectFile: "wbrand:select-file",
  /** 打开系统多文件选择框 */
  SelectFiles: "wbrand:select-files",
  /** Renderer → Main：写入宿主 ~/.wbrand 临时文本附件 */
  CreateTempTextAttachment: "wbrand:create-temp-text-attachment",
  /** Renderer → Main：通过原生另存为对话框保存文件 */
  SaveFile: "wbrand:save-file",
  /** Renderer → Main：用 Chromium 打印引擎把当前页面 print 媒体版面导出为 PDF */
  PrintToPdf: "wbrand:print-to-pdf",
  /** Main → Renderer：转发远程连接过程日志 */
  RemoteConnectionLog: "wbrand:remote-connection-log",
  /** Main → Renderer：远程 workspace session 已关闭 */
  RemoteSessionClosed: "wbrand:remote-session-closed",
  /** 检查目录是否已在其他窗口打开，如果是则激活该窗口 */
  ActivateOrSetWorkspace: "wbrand:activate-or-set-workspace",
  /** 建立 SSH 远程连接 */
  ConnectRemote: "wbrand:connect-remote",
  /** 取消当前窗口正在进行中的远程连接 */
  CancelPendingRemoteConnection: "wbrand:cancel-pending-remote-connection",
  /** Renderer → Main：绑定远程 logical session 的 canonical workspace context */
  BindRemoteWorkspaceSessionContext: "wbrand:bind-remote-workspace-session-context",
  /** 释放当前窗口里的远程 session */
  DisposeRemoteSession: "wbrand:dispose-remote-session",
  /** Renderer → Main：检查本机 Docker daemon 是否可用 */
  IsDockerAvailable: "wbrand:is-docker-available",
  /** Renderer → Main：列出本机可用的 WSL 发行版 */
  ListWSLDistros: "wbrand:list-wsl-distros",
  /** Renderer → Main：列出当前可连接的 Docker 容器 */
  ListDockerContainers: "wbrand:list-docker-containers",
  /** Renderer → Main：列出 SSH config 里可用于快速填表的 alias */
  ListSSHConfigAliases: "wbrand:list-ssh-config-aliases",
  /** Renderer → Main：从用户目录加载 CLI MCP 配置 */
  LoadMcpFromUserDirectory: "wbrand:load-mcp-from-user-directory",
  /** Renderer → Main：保存 CLI MCP 配置到用户目录 */
  SaveMcpToUserDirectory: "wbrand:save-mcp-to-user-directory",
  /** Renderer 日志转发到 main 进程统一存储 */
  Log: "wbrand:log",
  /** Renderer → Main：同步当前窗口所有 tab 的 workspace 路径 */
  SyncWindowTabs: "wbrand:sync-window-tabs",
  /** Renderer → Main：同步当前窗口的未读 task 数 */
  SyncWindowUnreadCount: "wbrand:sync-window-unread-count",
  /** Renderer → Main：当前窗口 active task，只更新 Main 的临时焦点映射。 */
  SyncActiveTaskSession: "wbrand:sync-active-task-session",
  /** Renderer → Main：同步 main 进程需即时感知的应用设置 */
  SyncAppSettings: "wbrand:sync-app-settings",
  /** Renderer → Main：快捷键设置页录制态开关；true = main 暂时摘除可配置菜单 accelerator */
  SetShortcutRecordingActive: "wbrand:set-shortcut-recording-active",
  /** Main → Renderer：聚焦到指定 workspace 路径的 tab */
  FocusTab: "wbrand:focus-tab",
  /** Main → Renderer：菜单触发新建 tab */
  NewTab: "wbrand:new-tab",
  /** Main → Renderer：菜单或快捷键请求关闭当前上下文 */
  CloseActiveContextRequest: "wbrand:close-active-context-request",
  /** Main → Renderer：内置 webview 请求打开新的浏览器 tab */
  OpenBrowserUrl: "wbrand:open-browser-url",
  /** Main → Renderer：agent 首次 browser 命令建好受控 view，通知 renderer 自动开 browser-use tab */
  BrowserViewReady: "wbrand:browser-view-ready",
  /** Main → Renderer：agent 正在操作某个 browser-use tab，renderer 临时显示状态图标 */
  BrowserViewOperation: "wbrand:browser-view-operation",
  /** Main → Renderer：browser visibility capability 显示/隐藏 IAB 右侧面板 */
  BrowserViewVisibility: "wbrand:browser-view-visibility",
  /** Main → Renderer：Agent 设置/重置目标 tab viewport */
  BrowserViewViewportChanged: "wbrand:browser-view-viewport-changed",
  /** Main → Renderer：截图前请求 owner renderer 准备后台 guest 合成表面。 */
  BrowserViewScreenshotSurfacePrepare: "wbrand:browser-view-screenshot-surface-prepare",
  /** Renderer → Main：目标 guest 连续两个 animation frame 的 viewport 已稳定。 */
  BrowserViewScreenshotSurfaceReady: "wbrand:browser-view-screenshot-surface-ready",
  /** Main → Renderer：截图结束或准备失败，释放临时后台合成层。 */
  BrowserViewScreenshotSurfaceRelease: "wbrand:browser-view-screenshot-surface-release",
  /** Main → Renderer：agent close 命令 detach 受控 guest 后，通知 renderer 卸载对应 tab */
  BrowserViewCloseTab: "wbrand:browser-view-close-tab",
  /** Main → Renderer：预算淘汰时卸载 guest，但保留 logical tab shell。 */
  BrowserViewSuspend: "wbrand:browser-view-suspend",
  /** Main → Renderer：为 suspended shell 重新挂载 guest。 */
  BrowserViewRestore: "wbrand:browser-view-restore",
  /** Main → Renderer：菜单触发新建任务 */
  NewTask: "wbrand:new-task",
  /** Main → Renderer：菜单触发打开工作区 */
  OpenWorkspace: "wbrand:open-workspace",
  /** Main → Renderer：deep link 直接打开指定本地工作区目录 */
  OpenWorkspacePath: "wbrand:open-workspace-path",
  /** Main → Renderer：打开内置反馈对话框 */
  OpenFeedbackDialog: "wbrand:open-feedback-dialog",
  /** Main → Renderer：打开我的工单面板 */
  OpenTicketsPanel: "wbrand:open-tickets-panel",
  /** Main → Renderer：窗口全屏状态变化 */
  WindowFullscreenChanged: "wbrand:window-fullscreen-changed",
  /** Renderer → Main：读取窗口最大化状态与系统原生圆角能力 */
  GetDesktopWindowChromeState: "wbrand:get-desktop-window-chrome-state",
  /** Main → Renderer：窗口最大化状态与系统原生圆角能力变化 */
  DesktopWindowChromeStateChanged: "wbrand:desktop-window-chrome-state-changed",
  /** Main → Renderer：原生窗口控制区安全边距变化 */
  WindowControlsOverlayChanged: "wbrand:window-controls-overlay-changed",
  /** Preload → Main：preload 已同步读到当前窗口控制区安全边距 */
  WindowControlsOverlayReady: "wbrand:window-controls-overlay-ready",
  /** 获取资源管理器快照（CPU / 内存，按基础服务、内置插件、社区插件归类） */
  GetResourceUsageSnapshot: "wbrand:get-resource-usage-snapshot",
  SetResourceUsageSamplingActive: "wbrand:set-resource-usage-sampling-active",
  /** 打开资源管理器窗口（其他窗口触发） */
  OpenResourceManager: "wbrand:open-resource-manager",
  /** 资源管理器「存储」tab：开始扫描本机 .wbrand 占用（main 持有 StorageService，Worker 线程遍历） */
  StorageStartScan: "wbrand:storage-start-scan",
  /** 资源管理器「存储」tab：取消扫描 */
  StorageCancelScan: "wbrand:storage-cancel-scan",
  /** 资源管理器「存储」tab：读取最近一次快照 */
  StorageGetSnapshot: "wbrand:storage-get-snapshot",
  /** 资源管理器「存储」tab：按类别清理 */
  StorageClean: "wbrand:storage-clean",
  /** 资源管理器「存储」tab：在系统文件管理器中定位数据根内的路径 */
  StorageRevealPath: "wbrand:storage-reveal-path",
  /** Main → 资源管理器 renderer：扫描进度快照推送 */
  StorageScanProgress: "wbrand:storage-scan-progress",
  /** Renderer → Main：打开外部 URL（用于 OAuth 跳转浏览器） */
  OpenExternal: "wbrand:open-external",
  /** Renderer → Main：查询当前语言下是否存在可用的用户社群入口 */
  CanOpenCommunity: "wbrand:can-open-community",
  /** Renderer → Main：在系统文件管理器中打开路径 */
  OpenInFileManager: "wbrand:open-in-file-manager",
  /** Renderer → Main：使用系统默认应用打开本地文件 */
  OpenExternalFile: "wbrand:open-external-file",
  /** Renderer → Main：打开 WBrand Computer Use 权限引导 */
  OpenCuaPermissionOnboarding: "wbrand:open-cua-permission-onboarding",
  /** Renderer → Main：取消当前 renderer 发起的一次权限引导 participant */
  CancelCuaPermissionOnboarding: "wbrand:cancel-cua-permission-onboarding",
  /**
   * Renderer → Main：预热并缓存已验证的 Helper 路径 + bundle 指纹。
   * 必须在拖拽浮窗挂载时调用 —— dragstart 链路里不允许任何异步 I/O。
   */
  PrepareCuaHelperPermissionDrag: "wbrand:prepare-cua-helper-permission-drag",
  /** Renderer → Main：把已验证的 Helper.app 同步拖出到 macOS 权限列表 */
  StartCuaHelperPermissionDrag: "wbrand:start-cua-helper-permission-drag",
  /**
   ...
   * 立刻消失可能打断正在进行的拖拽。
   */
  NotifyCuaHelperPermissionDragEnded: "wbrand:notify-cua-helper-permission-drag-ended",
  /** Renderer → Main：上报 OAuth state 用于 deep link 路由 */
  OAuthRegisterState: "wbrand:oauth-register-state",
  /** Main → Renderer：转发 deep link URL */
  OAuthCallback: "wbrand:oauth-callback",
  /** Main → Renderer：转发支付 deep link URL */
  PaymentCallback: "wbrand:payment-callback",
  /** Main → Renderer：外部分享页请求导入 share code。 */
  ShareImport: "wbrand:share-import",
  /** Renderer → Main：OAuth 回调已处理完成，可继续后置启动流程 */
  OAuthCallbackHandled: "wbrand:oauth-callback-handled",
  /** Renderer → Main：renderer 已就绪，可接收缓存的 deep link */
  RendererReady: "wbrand:renderer-ready",
  /** Renderer → Main：同步当前 renderer 的 telemetry 上下文 */
  SyncTelemetryContext: "wbrand:sync-telemetry-context",
  /** Renderer → Main：通过统一 telemetry 层上报业务事件 */
  ReportTelemetryEvent: "wbrand:report-telemetry-event",
  /** Renderer → Main：上报 ARMS 自定义事件 */
  ReportArmsCustomEvent: "wbrand:report-arms-custom-event",
  /** Renderer → Main：读取 Renderer 用户操作 Trace 灰度配置。 */
  GetRendererActionTraceConfig: "wbrand:get-renderer-action-trace-config",
  /** Main → Renderer：Renderer 用户操作 Trace 灰度配置变化。 */
  RendererActionTraceConfigChanged: "wbrand:renderer-action-trace-config-changed",
  /** Renderer → Main：发送已结束的 ui_action batch。 */
  ReportRendererActionTraceBatch: "wbrand:report-renderer-action-trace-batch",
  /** Renderer → Main：主窗口 renderer 每 60 秒的 heap 读数，单向 send，不需要回执。 */
  ReportRendererHeapSample: "wbrand:report-renderer-heap-sample",
  ReportLocalTtftBatch: "wbrand:report-local-ttft-batch",
  /** E2E preload → Main：读取 sendCustom 最终参数的内存 ring。 */
  ReadFinalArmsCustomEventsE2E: "wbrand:e2e:read-final-arms-custom-events",
  /** E2E preload → Main：清空 sendCustom 最终参数的内存 ring。 */
  ClearFinalArmsCustomEventsE2E: "wbrand:e2e:clear-final-arms-custom-events",
  /** E2E preload → Main：配置只针对目标 event name 的真实网络抑制。 */
  ConfigureFinalArmsCustomEventsE2E: "wbrand:e2e:configure-final-arms-custom-events",
  /** Renderer → Main：触发任务完成/失败的系统通知 */
  ShowTaskNotification: "wbrand:show-task-notification",
  /** Main → Preload：通知 renderer 播放任务通知提示音 */
  TaskNotificationSound: "wbrand:task-notification-sound",
  /** Main → Preload：用户点击了系统通知，携带 taskId 让 renderer 跳转到对应任务 */
  TaskNotificationClick: "wbrand:task-notification-click",
  /** Renderer → Main：导出日志（打包 ~/.wbrand/v2 及外部 agent 日志为 zip 并在 Finder 中显示） */
  ExportLogs: "wbrand:export-logs",
  /** Renderer → Main：截取当前窗口作为反馈附件 */
  CaptureWindowScreenshot: "wbrand:capture-window-screenshot",
  /**
   ...
   */
  BrowserViewAttachGuest: "wbrand:browser-view-attach-guest",
  /** Renderer → Main：重建 `<webview>` 前主动断开旧 guest 的 CDP。 */
  BrowserViewDetachGuest: "wbrand:browser-view-detach-guest",
  /** Renderer → Main：用户显式关闭 Browser tab。 */
  BrowserViewCloseTabFromRenderer: "wbrand:browser-view-close-tab-from-renderer",
  /** Renderer → Main：上报 Browser tab residency/display facts。 */
  BrowserViewReportResidency: "wbrand:browser-view-report-residency",
  /** Renderer → Main：指定 generation 的 guest 已卸载。 */
  BrowserViewSuspendReady: "wbrand:browser-view-suspend-ready",
  /** Renderer → Main：用户访问 suspended tab，请求恢复。 */
  BrowserViewEnsureResident: "wbrand:browser-view-ensure-resident",
  /** Renderer → Main：读取 workspace/task 的持久化 logical shells。 */
  BrowserViewRestoreTabs: "wbrand:browser-view-restore-tabs",
  /** Renderer → Main：自由尺寸拖拽/开关回写目标 guest viewport */
  BrowserViewUpdateViewport: "wbrand:browser-view-update-viewport",
  /** Embedded Browser preload → Main：在网页原生 Dialog 创建前同步请求可信系统框。 */
  EmbeddedBrowserJavaScriptDialog: "wbrand:embedded-browser-javascript-dialog",
  /** Renderer → Main：把自动发现的本机 Chrome Profile 数据一次性导入内置浏览器分区。 */
  ImportChromeBrowserData: "wbrand:import-chrome-browser-data",
  /** Renderer → Main：清理内置浏览器缓存或全部站点数据。 */
  ClearEmbeddedBrowserData: "wbrand:clear-embedded-browser-data",
  /** Main → Renderer：通知有新版本已下载完毕，可以重启安装 */
  UpdateReady: "wbrand:update-ready",
  /** Main → Renderer：用户手动点击"检查更新"后的结果反馈（toast 用） */
  UpdateCheckResult: "wbrand:update-check-result",
  /** Main → Renderer：自动更新持续状态变化（菜单 UI 用） */
  UpdateStateChanged: "wbrand:update-state-changed",
  /** Renderer → Main：主动获取当前自动更新状态（菜单打开时补偿事件丢失） */
  GetUpdateState: "wbrand:get-update-state",
  /** Renderer → Main：开始下载当前已发现的自动更新 */
  DownloadUpdate: "wbrand:download-update",
  /** Renderer → Main：取消当前正在下载的自动更新 */
  CancelUpdateDownload: "wbrand:cancel-update-download",
  /** Renderer → Main：打开独立自动更新窗口 */
  OpenUpdateStatusWindow: "wbrand:open-update-status-window",
  /** Renderer → Main：读取自动更新偏好 */
  GetAutoUpdatePreferences: "wbrand:get-auto-update-preferences",
  /** Renderer → Main：写入“自动下载并安装更新”偏好 */
  SetAutoDownloadAndInstallUpdates: "wbrand:set-auto-download-and-install-updates",
  /** Renderer → Main：查询桌面端正在运行的会话数量 */
  GetDesktopSessionActivity: "wbrand:get-desktop-session-activity",
  /** Renderer → Main：读取当前窗口页面缩放档位 */
  GetDesktopZoomLevel: "wbrand:get-desktop-zoom-level",
  /** Main → Renderer：当前窗口页面缩放档位变化 */
  DesktopZoomLevelChanged: "wbrand:desktop-zoom-level-changed",
  /** Renderer → Main：读取开发态 stdio tap proxy 开关状态 */
  GetWBrandStdioTapDevState: "wbrand:get-wbrand-stdio-tap-dev-state",
  /** Main → Renderer：本地 setting.json 已由 main 进程更新 */
  SettingsChanged: "wbrand:settings-changed",
  /** Main → Renderer：应用语言已切换 */
  ApplicationLocaleChanged: "wbrand:application-locale-changed",
  /** Renderer → Main：读取宿主系统语言 */
  GetSystemLocale: "wbrand:get-system-locale",
  /** Main → Renderer：更新安装后的版本说明 */
  PostUpdateReleaseNotes: "wbrand:post-update-release-notes",
  /** Renderer → Main：确认版本说明已读 */
  AcknowledgePostUpdateReleaseNotes: "wbrand:ack-post-update-release-notes",
  /** Renderer → Main：跳过当前已发现的自动更新版本 */
  SkipUpdateVersion: "wbrand:skip-update-version",
  /** Renderer → Main：用户确认重启安装更新 */
  QuitAndInstallUpdate: "wbrand:quit-and-install-update",
  /** Renderer → Main：获取系统中已安装的编辑器/终端列表（含图标） */
  GetInstalledEditors: "wbrand:get-installed-editors",
  /** Renderer → Main：按 bundle id 获取系统应用图标 */
  GetApplicationIcon: "wbrand:get-application-icon",
  /** Renderer → Main：用指定编辑器打开路径 */
  OpenInEditor: "wbrand:open-in-editor",
  /** Renderer → Main：执行桌面窗口级命令 */
  ExecuteDesktopCommand: "wbrand:execute-desktop-command",
  /** Renderer → Main：同步应用菜单语言，用于重建原生菜单 */
  SetApplicationLocale: "wbrand:set-application-locale",
  /** Renderer → Main：同步标题栏亮暗色，用于原生窗口控制按钮配色 */
  SetTitleBarTheme: "wbrand:set-title-bar-theme",
  /** Renderer → Main：迁移旧版 Common MCP 配置 */
  MigrateLegacyCommonMcp: "wbrand:migrate-legacy-common-mcp",
  /** Renderer → Main：获取当前设备的稳定标识符（deviceMid） */
  GetDeviceId: "wbrand:get-device-id",
} as const;
...
export const EmbeddedBrowserWebviewChannels = {
  ...
  WheelBoundary: "wbrand:embedded-browser-wheel-boundary",
} as const;
...
/**
 * Electron `<webview>`（partition=persist:wbrand-coding-plan）的 `sendToHost` / `ipc-message` 频道。
 * 官网页通过 preload 注入的 window.wbrandBridge 调用，不经过 main process。
 */
export const CodingPlanWebviewChannels = {
  /** 官网页购买成功后通知 App 刷新 entitlements 并关闭 webview。 */
  PurchaseComplete: "wbrand:coding-plan-purchase-complete",
} as const;
...
/**
 * 官网页 window.__wbrandLang__ 的取值，与 App IntlProvider 的 Locale 一致。
 ...
 * `wbrand-coding-plan-lang-change` CustomEvent，website 侧（wbrandBridge.onLangChange 或
...
export const InternalChannels = {
  DatabaseStartupState: "wbrand:database-startup-state",
  DatabaseStartupControl: "wbrand:database-startup-control",
  /** main → renderer 转发 MessagePort（通过 webContents.postMessage） */
  ServicePort: "wbrand:service-port",
  /** main → renderer 转发窗口 Host 的 scoped MessagePort */
  ScopedServicePort: "wbrand:scoped-service-port",
  /** renderer → main：scoped MessagePort 已注册，可安全切换 attachment */
  ScopedServicePortReady: "wbrand:scoped-service-port-ready",
  /** preload → renderer：主进程已确认系统通知展示，renderer 可播放提示音 */
  TaskNotificationSound: "wbrand:task-notification-sound",
} as const;

/** @deprecated `/ws` 已忽略该头；保留常量仅供旧客户端兼容。 */
export const WBRAND_RPC_CLIENT_MODE_HEADER = "x-wbrand-rpc-client-mode";
/** desktop 先经受保护 HTTP endpoint 申请，再在 `/ws/host` 握手时一次性消费。 */
export const WBRAND_RPC_HOST_CAPABILITY_HEADER = "x-wbrand-rpc-host-capability";
...
export const HostMessageTypes = {
  ...
  /** Main → Host：全局前台 WBrand 窗口派生的 producer focus fact。 */
...
export const HostResponseTypes = {
  ...
  /** host → main：声明一个 WBrand Agent session 当前归属该 host */
