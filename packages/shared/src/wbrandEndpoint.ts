...
declare const __WBRAND_ENDPOINT_ENV__: Record<string, string | undefined> | undefined;
export function pickProductEndpointEnv(
  env: Record<string, string | undefined>,
): Record<string, string> {
  const keys = [
    "WBRAND_BASE_URL",
    "WBRAND_ENDPOINT_ORIGIN",
...
export function readProductEndpointEnv(): Record<string, string | undefined> {
  return {
    ...(typeof __WBRAND_ENDPOINT_ENV__ === "undefined" ? {} : __WBRAND_ENDPOINT_ENV__),
    ...pickProductEndpointEnv(typeof process === "undefined" ? {} : process.env),
  };
}

export interface WBrandEndpointUrls {
  origin: string;
  apiBaseUrl: string;
  webShareCallbackUrl: string;
  wbrandPlanOpenAiBaseUrl: string;
  wbrandPlanAnthropicBaseUrl: string;
  wbrandPlanBillingCurrentUrl: string;
  wbrandPlanBillingBalanceUrl: string;
}

export interface RuntimeWBrandEndpointEnv {
  [key: string]: string | undefined;
  WBRAND_ENV?: string;
  WBRAND_BASE_URL?: string;
  WBRAND_ENDPOINT_ORIGIN?: string;
}
...
export interface RuntimeProductEndpointEnv
  extends RuntimeWBrandEndpointEnv, RuntimeBigModelApiEnv, RuntimeZaiEndpointEnv {}

export interface RuntimeProductEndpointConfig {
  wbrandEnv: WBrandEnv;
  wbrandEndpointOrigin: string;
  wbrandEndpointUrls: WBrandEndpointUrls;
...
export function normalizeWBrandEndpointOrigin(value: string): string {
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error("WBrand endpoint origin is empty");
  }

  const parsed = new URL(trimmed);
  if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
    throw new Error("WBrand endpoint origin must use http or https");
  }
...
export function isTrustedCodingPlanWebviewOrigin(
  ...
  try {
    const origin = normalizeWBrandEndpointOrigin(value);
    if (
      origin === DEFAULT_WBRAND_ENDPOINT_ORIGIN ||
      origin === resolveRuntimeWBrandEndpointOrigin()
    ) {
      return true;
    }
...
export function resolveWBrandEndpointOrigin(options?: {
  env?: WBrandEnv;
  envBaseOrigin?: string | null;
  overrideOrigin?: string | null;
}): string {
  const origin = options?.overrideOrigin?.trim() || options?.envBaseOrigin?.trim();
  return origin ? normalizeWBrandEndpointOrigin(origin) : DEFAULT_WBRAND_ENDPOINT_ORIGIN;
}

export function resolveRuntimeWBrandEnv(
  env: RuntimeWBrandEndpointEnv = readProductEndpointEnv(),
): WBrandEnv {
  // 产品身份仅用于既有展示与安装标识，不参与地址解析。
  return env.WBRAND_ENV?.trim().toLowerCase() === "test" ? "test" : "production";
}

export function resolveRuntimeWBrandEndpointOrigin(
  env: RuntimeWBrandEndpointEnv = readProductEndpointEnv(),
  options?: { overrideOrigin?: string | null },
): string {
  return resolveWBrandEndpointOrigin({
    envBaseOrigin:
      readRuntimeEnvValue(env, "WBRAND_BASE_URL") ??
      readRuntimeEnvValue(env, "WBRAND_ENDPOINT_ORIGIN"),
    overrideOrigin: options?.overrideOrigin,
  });
}

export function buildRuntimeWBrandEndpointUrls(
  env: RuntimeWBrandEndpointEnv = readProductEndpointEnv(),
): WBrandEndpointUrls {
  return buildWBrandEndpointUrls(resolveRuntimeWBrandEndpointOrigin(env));
}

export function buildRuntimeWBrandApiUrl(
  env: RuntimeWBrandEndpointEnv = readProductEndpointEnv(),
  path: string,
): string {
  const normalizedPath = path.startsWith("/") ? path : `/${path}`;
  return `${resolveRuntimeWBrandEndpointOrigin(env)}${normalizedPath}`;
}

export function resolveBigModelApiOrigin(
  env: RuntimeBigModelApiEnv = readProductEndpointEnv(),
): string {
  return normalizeWBrandEndpointOrigin(
...
export function buildZaiOAuthUrl(origin: string, path: string): string {
  const normalizedPath = path.startsWith("/") ? path : `/${path}`;
  return `${normalizeWBrandEndpointOrigin(origin)}${normalizedPath}`;
}
...
export function resolveRuntimeProductEndpointConfig(
  ...
  const wbrandEnv = resolveRuntimeWBrandEnv(env);
  const wbrandEndpointOrigin = resolveRuntimeWBrandEndpointOrigin(env);

  return {
    wbrandEnv,
    wbrandEndpointOrigin,
    wbrandEndpointUrls: buildWBrandEndpointUrls(wbrandEndpointOrigin),
...
export function buildWBrandEndpointUrls(origin: string): WBrandEndpointUrls {
  const normalizedOrigin = normalizeWBrandEndpointOrigin(origin);
  return {
    origin: normalizedOrigin,
    apiBaseUrl: `${normalizedOrigin}/api/v1`,
    webShareCallbackUrl: `${normalizedOrigin}/cn/share/callback`,
    wbrandPlanOpenAiBaseUrl: `${normalizedOrigin}/api/v1/wbrand-plan`,
    wbrandPlanAnthropicBaseUrl: `${normalizedOrigin}/api/v1/wbrand-plan/anthropic`,
    wbrandPlanBillingCurrentUrl: `${normalizedOrigin}/api/v1/wbrand-plan/billing/current`,
    wbrandPlanBillingBalanceUrl: `${normalizedOrigin}/api/v1/wbrand-plan/billing/balance`,
  };
}
...
export function rewriteWBrandEndpointUrl(
  ...
  const sourceOrigin = DEFAULT_WBRAND_ENDPOINT_ORIGIN;
  if (parsed.origin !== sourceOrigin) {
    return input;
  }

  const targetOrigin = normalizeWBrandEndpointOrigin(endpointOrigin);
