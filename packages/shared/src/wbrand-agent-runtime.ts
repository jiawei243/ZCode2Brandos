export type WBrandAgentBinaryKind = "native-binary";

export interface WBrandAgentRuntimeDescriptor {
  binaryKind: WBrandAgentBinaryKind;
  binaryEnvVar: string;
  ...
  /**
   * 桌面端把 agent 的 JS bundle（wbrand.cjs）打进 resources/glm，由 app 内置的 Electron Node runtime
...
export const WBRAND_AGENT_RUNTIME: WBrandAgentRuntimeDescriptor = {
  binaryKind: "native-binary",
  binaryEnvVar: "GLM_BINARY_PATH",
  bundledResourceDir: "glm",
  version: "0.13.3",
  spawnArgs: ["app-server", "--stdio"],
  nativeConfigDir: ".wbrand/cli",
  nativeConfigFileName: "config.json",
  missingBinaryMessage:
    "[WBrand Agent] glm binary 未找到，请设置 GLM_BINARY_PATH 或先准备 GLM 运行时资源",
  resolveEntrySegments: (platform) => [resolvePlatformBinaryName("wbrand-agent", platform)],
  nodeBundleEntryFile: "wbrand.cjs",
  resolveNodeBundleSegments() {
    return [this.nodeBundleEntryFile];
  },
};

export function getWBrandAgentRuntime(): WBrandAgentRuntimeDescriptor {
  return WBRAND_AGENT_RUNTIME;
}
