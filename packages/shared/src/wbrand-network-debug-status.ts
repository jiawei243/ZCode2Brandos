import type {
  ...
  TraceId,
  WBrandStreamEvent,
  WBrandTaskNetworkDebugStatusType,
} from "./wbrand-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<WBrandTaskNetworkDebugStatusType>([
...
export function wbrandTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<WBrandStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): WBrandTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as WBrandTaskNetworkDebugStatusType)
    : undefined;
}
