...
export type WorkspaceHookConfigFileKind = "wbrand.json" | ".wbrand/config.json" | "explicit";
...
function buildWorkspaceHookCandidatePaths(directories: readonly string[]): string[] {
  return directories.flatMap((directory) => [
    join(directory, "wbrand.json"),
    join(directory, ".wbrand", "config.json"),
  ]);
}
...
export function createWorkspaceHookSourceInput(input: {
  ...
  return {
    canonicalPath,
    baseDir: basename(configDirectory) === ".wbrand" ? dirname(configDirectory) : configDirectory,
    discoveryOrder: input.discoveryOrder,
    configFileKind: explicitProjectConfig
      ? "explicit"
      : basename(canonicalPath) === "wbrand.json"
        ? "wbrand.json"
        : ".wbrand/config.json",
    explicitProjectConfig,
    editable:
      !explicitProjectConfig &&
      canonicalPath === resolve(input.workingDirectory, ".wbrand", "config.json"),
    hooks: input.hooks,
  };
}
