import type { WBrandTaskMode } from "./wbrand-task-types-core.js";
...
/** 整条 automation 的生命周期。 */
export type WBrandAutomationLifecycleStatus = "active" | "completed" | "failed" | "paused";

/** 单条 automation 当前一轮的派发信息态（供 UI/诊断）。 */
export type WBrandAutomationDispatchStatus =
  | "idle"
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch";
...
export type WBrandAutomationLocationKind = "local" | "remote";

/** 触发来源：定时 or 立即运行。 */
export type WBrandAutomationTrigger = "schedule" | "manual";

/** 自定义重复规则；cronExpr 保留为兼容展示，调度以本字段为权威。 */
export interface WBrandAutomationScheduleRule {
 ...
 */
export type WBrandAutomationIntervalUnit = WBrandAutomationScheduleRule["unit"];

/** 单次 run 的派发结果（scheduler 权威，驱动重试）。skipped=错过触发窗口。 */
export type WBrandAutomationRunDispatchStatus =
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch"
  | "skipped";
...
export type WBrandAutomationRunOutcome = "running" | "succeeded" | "failed" | "stopped";

/** 一条定时任务定义 + 调度状态。 */
export interface WBrandAutomation {
  automationId: string;
  title: string;
  /** 5 段 cron，本地时区。 */
  cronExpr: string;
  ...
  targetTaskId?: string;
  locationKind: WBrandAutomationLocationKind;
  ...
  nextRunAt?: number;
  lastRunAt?: number;
  dispatchStatus: WBrandAutomationDispatchStatus;
...
export interface WBrandAutomationRunIdParts {
  automationId: string;
...
/**
 * runId 契约（见 WBrandAutomationRun.runId）的唯一解析入口；desktop host/scheduler 与
 * CLI telemetry 不各自按字符串猜格式。不符合两种声明格式的输入一律返回 null。
 */
export function parseAutomationRunId(runId: string): WBrandAutomationRunIdParts | null {
...
export interface WBrandAutomationRun {
...
export interface WBrandAutomationCreateParams {
  title: string;
  cronExpr: string;
   ...
   */
  intervalUnit?: WBrandAutomationIntervalUnit;
  /** 1–200 的整数间隔，必须与 intervalUnit 配对。 */
  interval?: number;
}

/** 编辑 automation 的可变字段。 */
export interface WBrandAutomationUpdateParams {
  title?: string;
  cronExpr?: string;
  prompt?: string;
