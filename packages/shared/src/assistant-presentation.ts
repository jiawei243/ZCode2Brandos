import {
  ...
  type WBrandAssistantMessagePart,
} from "./assistant-message-parts.js";

export interface WBrandAssistantPresentationToolCall {
  toolId: string;
...
export type WBrandAssistantPresentationBlock =
  | {
      type: "content";
      content: string;
    }
  ...
  | {
      type: "tool-call";
      toolCall: WBrandAssistantPresentationToolCall;
    };

export interface WBrandAssistantPresentation {
  messageParts: WBrandAssistantMessagePart[];
  blocks: WBrandAssistantPresentationBlock[];
  latestPart: Extract<WBrandAssistantPresentationBlock, { type: "content" }> | null;
  historyBlocks: WBrandAssistantPresentationBlock[];
}

export interface BuildWBrandAssistantPresentationOptions {
  content: string;
  thought?: string;
  toolCalls?: readonly WBrandAssistantPresentationToolCall[];
  parts?: readonly WBrandAssistantMessagePart[];
  streaming?: boolean;
  interrupted?: boolean;
  settling?: boolean;
}
...
function buildFallbackAssistantParts({
  ...
  thought,
  toolCalls,
}: Pick<BuildWBrandAssistantPresentationOptions, "content" | "thought" | "toolCalls">) {
...
export function buildWBrandAssistantPresentation({
  content,
  thought,
  toolCalls = [],
  parts,
  ...
  interrupted = false,
  settling = false,
}: BuildWBrandAssistantPresentationOptions): WBrandAssistantPresentation {
  const messageParts =
  ...
  const blocks: WBrandAssistantPresentationBlock[] = [];
  ...
  const latestContentPart =
    streaming || interrupted || settling
      ...
      : getLatestAssistantContentPart(
          blocks
            .filter(
              (block): block is Extract<WBrandAssistantPresentationBlock, { type: "content" }> =>
                block.type === "content",
            )
            .map((block) => ({ type: "content", content: block.content })),
        );
  let latestPart: Extract<WBrandAssistantPresentationBlock, { type: "content" }> | null = null;
    ...
    latestPart =
      latestBlockIndex >= 0
        ? (blocks[latestBlockIndex] as Extract<
            WBrandAssistantPresentationBlock,
            { type: "content" }
          >)
        : null;
  }
