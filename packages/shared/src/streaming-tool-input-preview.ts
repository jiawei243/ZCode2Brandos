export interface WBrandStreamingToolInputState {
  deltaCount?: number;
...
export interface WBrandStreamingToolInputPreview {
  complete: boolean;
  input: unknown;
  rawInput: string;
}

export type WBrandStreamingToolInputPreviewMode = "active-live" | "background-summary";

export const WBRAND_ACTIVE_STREAMING_TOOL_INPUT_EAGER_DELTA_COUNT = 1;
export const WBRAND_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS = 750;
export const WBRAND_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH = 8 * 1024;
export const WBRAND_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS = 1_000;
export const WBRAND_ACTIVE_STREAMING_TOOL_INPUT_TIME_BUDGET_MAX_RAW_INPUT =
  WBRAND_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH;
...
export function appendWBrandStreamingToolInputDelta(
  state: WBrandStreamingToolInputState | undefined,
  delta: string,
): WBrandStreamingToolInputState {
  return {
    ...state,
...
export function buildWBrandStreamingToolInputPreview(
  rawInput: string,
  completeInput?: unknown,
): WBrandStreamingToolInputPreview {
...
export function shouldMaterializeWBrandStreamingToolInputPreview(
  state: WBrandStreamingToolInputState,
  options: {
    mode?: WBrandStreamingToolInputPreviewMode;
    now?: number;
    toolName?: string;
  } = {},
): boolean {
  ...
  if (deltaCount <= WBRAND_ACTIVE_STREAMING_TOOL_INPUT_EAGER_DELTA_COUNT) {
    return true;
  }
  const lastPreviewAt = state.lastPreviewAt ?? 0;
  if (isWBrandFileStreamingToolInputPreviewTool(options.toolName)) {
    // 性能修复：Write/Edit 的半截 JSON 会触发全量内容恢复和行级 diff。
    // 大字节 chunk 不能绕过一秒窗口，否则模型输出越快，UI 反而更新越频繁。
    return (
      (options.now ?? Date.now()) - lastPreviewAt >=
      WBRAND_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    );
  }
  ...
  if (rawGrowth >= WBRAND_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH) {
    return true;
  }
  const intervalElapsed =
    (options.now ?? Date.now()) - lastPreviewAt >=
    WBRAND_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS;
  if (!intervalElapsed) {
    return false;
  }
  // 性能修复：大 Write/Edit 参数通常会被 provider 以 4KB 左右的慢速 chunk 推送。
  // 若只按时间预算，active 任务仍会每个 chunk 解析累计 JSON；超过小输入范围后改由 raw growth 控制。
  return state.rawInput.length <= WBRAND_ACTIVE_STREAMING_TOOL_INPUT_TIME_BUDGET_MAX_RAW_INPUT;
}

export function isWBrandFileStreamingToolInputPreviewTool(toolName?: string): boolean {
  const normalized = toolName?.trim().toLowerCase();
  return normalized === "write" || normalized === "edit";
}

export function markWBrandStreamingToolInputPreviewMaterialized(
  state: WBrandStreamingToolInputState,
  now = Date.now(),
): void {
