...
import { WBRAND_OFFICIAL_PLUGIN_MARKETPLACE_ID } from "../plugin-marketplaces.js";
...
/**
 * 扫描 `<plugins storage>/cache/wbrand-plugins-official/<name>/<version>/`。
...
export async function scanOfficialPluginCacheRoots(
  ...
  const cacheRoot = join(pluginStorageRoot, "cache", WBRAND_OFFICIAL_PLUGIN_MARKETPLACE_ID);
  let pluginEntries;
  try {
