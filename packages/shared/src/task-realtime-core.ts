...
import { z } from "zod";
import type { WBrandTaskMigrationSource, WBrandTaskMode } from "./wbrand-task-types-core.js";
import { wbrandAgentProviderSchema } from "./wbrand-agent-policy.js";
import { wbrandPermissionResponseSchema } from "./wbrand-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./wbrand-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./wbrand-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const wbrandTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly WBrandTaskMode[];
const wbrandTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly WBrandTaskMigrationSource[];
const wbrandTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 WBrandTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(wbrandTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: wbrandAgentProviderSchema.optional(),
  migrationSource: z.enum(wbrandTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: wbrandTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const wbrandPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const wbrandTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const wbrandTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  wbrandTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: wbrandTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: wbrandTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
