import type { WBrandPermissionRequest } from "./wbrand-task-types-core.js";
...
const FILE_PATH_KEYS = new Set([
  ...
  "paths",
  "file",
  // WBrand Agent edit 权限常把目标文件放在 file_path/filePath，漏掉会让权限预览只剩标题。
  "file_path",
  "filepath",
  "files",
  "filename",
...
function readPermissionInputSource(rawSource: unknown): unknown {
  ...
  // WBrand protocol 的 requestPermission schema 使用 input 承载工具参数；
...
export function getPermissionRequestPreview(
  request: Pick<WBrandPermissionRequest, "title" | "description" | "kind" | "raw">,
