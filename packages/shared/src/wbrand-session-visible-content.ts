...
// 消费者：wbrandTaskServiceAdapter/wbrandSessionProjection/wbrandTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromWBrandMessageParts 已迁
// wbrand-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { WBrandSessionGoal } from "./wbrand-protocol/index.js";
...
export function isWBrandGoalContinuationReminderText(text: string): boolean {
...
export function isWBrandGoalContinuationReminderMessage(message: WBrandMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isWBrandGoalContinuationReminderText(textFromWBrandMessageParts(message.parts)))
  );
}

export function isWBrandGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isWBrandGoalModelOnlyReminderMessage(message: WBrandMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isWBrandGoalContinuationReminderMessage(message) ||
      isWBrandGoalStateReminderText(textFromWBrandMessageParts(message.parts)))
  );
}

export function isWBrandModelOnlySyntheticUserMessage(message: WBrandMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isWBrandCompactSummaryMessage(message: WBrandMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getWBrandUserVisibleMessages(
  messages: readonly WBrandMessageWithParts[],
  _options: { target?: WBrandSessionGoal | null } = {},
): WBrandMessageWithParts[] {
  const visibleMessages: WBrandMessageWithParts[] = [];
  for (const message of messages) {
    if (isWBrandModelOnlySyntheticUserMessage(message) || isWBrandCompactSummaryMessage(message)) {
...
export function getWBrandGoalIterationByAssistantMessageId(
  messages: readonly WBrandMessageWithParts[],
  options: { maxGoalIteration?: number; target?: WBrandSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareWBrandMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isWBrandGoalContinuationReminderMessage(message)) {
...
export interface WBrandGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getWBrandGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly WBrandGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareWBrandMessagesByCreatedTime(
  left: WBrandMessageWithParts,
  right: WBrandMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: WBrandMessageWithParts, target: WBrandSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveWBrandVisibleSessionTitle(input: {
  title?: string;
  messages: readonly WBrandMessageWithParts[];
  target?: WBrandSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isWBrandGoalContinuationReminderText(normalizedTitle) &&
    !isWBrandGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isWBrandGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromWBrandMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
