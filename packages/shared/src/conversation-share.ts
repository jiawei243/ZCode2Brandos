 ...
 * 计数交给调用方转成「部分内容需要更新 WBrand 查看」的软提示，不能静默。
 */
...
const conversationShareArtifactDescriptorFields = {
  ...
  ref: z.string().regex(/^wbrand-artifact:\/\/share\/[A-Za-z0-9._~-]+$/u),
...
export type ConversationSharePreview = Omit<ConversationSharePreviewWire, "rows"> & {
  rows: ConversationRow[];
  /** 本端认不出、已跳过的行数；>0 时 UI 必须给「部分内容需要更新 WBrand 查看」软提示。 */
