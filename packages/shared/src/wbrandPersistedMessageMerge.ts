...
export function coalesceConsecutiveWBrandAssistants(
  messages: readonly WBrandPersistedMessage[],
): WBrandPersistedMessage[] {
  const merged: WBrandPersistedMessage[] = [];
...
function mergeAssistantPair(
  first: WBrandPersistedMessage,
  next: WBrandPersistedMessage,
): WBrandPersistedMessage {
  ...
  const shiftedNextParts: WBrandPersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
