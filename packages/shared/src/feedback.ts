...
export type FeedbackTicketFramework = "wbrand-agent";
...
export interface FeedbackDeviceInfo {
  ...
  osVersion?: string;
  osArch?: string;
  /** 提交反馈时的当前 Agent。单 WBrand Agent 模式下固定为 WBrand Agent。 */
  agentProvider?: string;
  /** 提交反馈时归一到反馈平台的框架标识，当前固定为 wbrand-agent。 */
...
export const DEFAULT_FEEDBACK_TICKET_FRAMEWORK: FeedbackTicketFramework = "wbrand-agent";

export const FEEDBACK_TICKET_FRAMEWORK_OPTIONS: {
  value: FeedbackTicketFramework;
  label: string;
}[] = [{ value: "wbrand-agent", label: "WBrand Agent" }];
