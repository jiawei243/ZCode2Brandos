...
import { z } from "zod";
import { wbrandProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { wbrandProviderSchema } from "./providers.js";
import { wbrandAgentProviderSchema } from "./wbrand-agent-policy.js";
...
} from "./wbrand-protocol/index.js";
import { wbrandTaskModeSchema } from "./wbrand-task-mode-schema.js";
import { PROTOCOL_V4_LIMITS } from "./wbrand-protocol-v4/core.js";
import { errorAttributionSchema } from "./wbrand-protocol-v4/snapshot.js";
import { sessionWorkflowActivitySchema } from "./wbrand-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { wbrandTaskModeSchema } from "./wbrand-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("wbrand-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  wbrandBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 WBrandTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { wbrandProviderSchema };

export const wbrandTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: wbrandProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = wbrandProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `wbrandProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: wbrandMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: wbrandToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: wbrandMcpTelemetryEventSchema,
  })
  .strict();
...
export const wbrandTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const wbrandPromptImageAttachmentSchema = z.object({
...
export const wbrandPromptVideoAttachmentSchema = z.object({
...
export const wbrandPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const wbrandPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const wbrandPromptAttachmentSchema = z.discriminatedUnion("kind", [
  wbrandPromptImageAttachmentSchema,
  wbrandPromptVideoAttachmentSchema,
  wbrandPromptPdfAttachmentSchema,
  wbrandPromptFileAttachmentSchema,
]);

export const wbrandPersistedToolCallSchema = z.object({
...
const wbrandPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const wbrandPersistedMessageSchema = z.object({
  ...
  attachments: z.array(wbrandPromptAttachmentSchema).optional(),
  tools: z.array(wbrandPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(wbrandPersistedMessagePartSchema).optional(),
...
export const wbrandTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const wbrandTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: wbrandTaskGoalStatusSchema,
...
export const wbrandTaskGoalChangedPatchSchema = z.object({
  action: wbrandTaskTargetChangedActionSchema,
  source: wbrandTaskTargetChangedSourceSchema,
  target: wbrandTaskGoalSchema.nullable(),
  previousTarget: wbrandTaskGoalSchema.nullable().optional(),
});

export const wbrandTaskTargetStatusSchema = wbrandTaskGoalStatusSchema;
export const wbrandTaskTargetSchema = wbrandTaskGoalSchema;
export const wbrandTaskTargetChangedPatchSchema = wbrandTaskGoalChangedPatchSchema;

export const wbrandTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: wbrandTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: wbrandAgentProviderSchema.optional(),
  migrationSource: wbrandTaskMigrationSourceSchema.optional(),
  ...
  status: wbrandTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: wbrandTaskGoalSchema.nullable().optional(),
});

export const wbrandTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const wbrandPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(wbrandTaskIndexEntrySchema),
});

const wbrandPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const wbrandPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(wbrandPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const wbrandPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const wbrandSessionFileSchema = z.object({
  meta: wbrandTaskMetaSchema,
  messages: z.array(wbrandPersistedMessageSchema),
  fileChanges: z.array(wbrandPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(wbrandPersistedTurnCheckpointSchema).optional(),
});
