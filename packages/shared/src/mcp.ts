/**
 * MCP (Model Context Protocol) types for WBrand
 * Based on the original Tauri implementation
 */

import type { SettingsDirectoryLocation } from "./settings-source.js";
import type { McpServerFailureKind } from "./wbrand-protocol/index.js";

// CUA official plugin 身份常量（port 自 feat；UI 设置面板 + bootstrap 复用以避免字面量漂移）。
export const WBRAND_CUA_OFFICIAL_PLUGIN_ID = "computer-use@wbrand-plugins-official";
// CUA server 身份串（port 自 feat mcp.ts）：server key = 模型可见工具前缀段（刻意不带 wbrand-）；
// namespace name = official plugin 运行时命名空间 plugin:<pluginId>:<serverKey>。
export const WBRAND_CUA_OFFICIAL_MCP_NAMESPACE_NAME = "plugin:computer-use:computer-use";
// 插件身份 env key：resolver（adapters/src/plugins/mcp.ts）权威写入 loaded.id，manifest/user env 不可覆盖。
// bootstrap + cli/plugin-host-command.ts 复用此常量识别 official wbrand-cua plugin server，避免字面量漂移。
export const WBRAND_PLUGIN_ID_ENV_KEY = "WBRAND_PLUGIN_ID";

export type McpSource = "mcp" | "wbrandagentmcp";
...
export interface McpConfig {
  ...
  wbrandagentmcp: CliMcpConfig;
}

export interface WBrandMcpServer {
  id: string;
  name: string;
  config: McpServerConfig;
  enabled: boolean;
...
export type WBrandAgentMcpServer =
  | {
      name: string;
      command: string;
      args: string[];
...
// wbrand-cua MCP server 识别的单一事实源。desktop 产品 broker resolver（@wbrand/services 的
// mcpBrokerInjection）与 CLI bootstrap（apps/wbrand-cli 的 mcp-config）两条注入入口必须用
// 完全一致的判定；否则同一 MCP 配置在不同入口行为不同，可能漏注入 product broker，让
// Python/uvx 自己持有 macOS TCC 权限（违反 fail-closed 边界）。改这里即同时改两条链路。
function wbrandCuaArgLeaf(value: string): string {
  // 先去掉结尾的路径分隔符再取叶子：`.../wbrand-cua/` 直接 split 会得到空串叶子 → 漏判 → fail-open。
  return (
    value
      .replace(/[\\/]+$/, "")
      .split(/[\\/]/)
...
// 单个候选串是否为 wbrand-cua 的包规格。PyPI 视 `_`/`-` 等价，故先把下划线归一成短横（wbrand_cua →
// wbrand-cua）；覆盖 uv/npm 的 `@version`、pip 的 `==version`、extras `[...]`、git 的 `.git`/`.git@`，
// 以及 `python -m wbrand_cua.server` 这种点号子模块（`wbrand-cua.<submodule>`）。fail-closed 边界宁可
// 过判也不漏判；仍不会误判 `wbrand-cua-proxy`（短横续接，不以 `.`/`@`/`[`/`==` 边界续接）。
function matchesWBrandCuaSpec(candidate: string): boolean {
  const c = candidate.replace(/_/g, "-");
  return (
    c === "wbrand-cua" ||
    c.startsWith("wbrand-cua[") ||
    c.startsWith("wbrand-cua@") ||
    c.startsWith("wbrand-cua==") ||
    // `.` 分支同时覆盖 `wbrand-cua.git` / `wbrand-cua.git@v1` 与 `wbrand-cua.server` 等 python 子模块。
    c.startsWith("wbrand-cua.")
  );
}

/**
 * MCP server 的 command 是否指向 wbrand-cua。用与 args 相同的包规格判定（并比对路径叶子），
 * 覆盖 `command: "wbrand-cua"`、`/opt/bin/wbrand-cua`，以及把包规格直接当 command 的写法
 * （`wbrand-cua@1.2.3` 等）。对 fail-closed 边界宁可过判也不漏判。
 */
export function isWBrandCuaMcpCommand(command: string): boolean {
  return matchesWBrandCuaSpec(command) || matchesWBrandCuaSpec(wbrandCuaArgLeaf(command));
}

/**
 * 单个 arg 是否为 wbrand-cua 的包规格。覆盖 `wbrand-cua`、`wbrand-cua[macos]`、`wbrand-cua@1.2.3`、
 * `wbrand-cua==1.2.3`、`wbrand_cua`，以及 git / 本地路径形态（`.../wbrand-cua`、`wbrand-cua.git`、
 * `git+https://.../wbrand-cua.git@v1`）。同时比对原始值与路径叶子，覆盖 `--from <path>`、`--from <git-url>`。
 */
export function isWBrandCuaMcpPackageArg(value: string): boolean {
  return matchesWBrandCuaSpec(value) || matchesWBrandCuaSpec(wbrandCuaArgLeaf(value));
}

export function convertToWBrandAgentMcpServer(
  name: string,
  config: McpServerConfig,
): WBrandAgentMcpServer | null {
  ...
  if (isStdio && config.command) {
    ...
    if (isWin32) {
      ...
      if ((lowerCmd === "cmd" || lowerCmd === "cmd.exe") && args[0] === "/c" && unwrappedCommand) {
        ...
        // 先显式取值并判空，既满足类型收窄，也避免把空命令传给 WBrand Agent。
