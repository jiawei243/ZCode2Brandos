...
// 透传到 JSON-RPC `error.data.code`；客户端读回时用 readWBrandAttachmentFaultCode。

export const WBRAND_ATTACHMENT_FAULT_CODES = {
  /** host 未实现 stat 能力。 */
...
export type WBrandAttachmentFaultCode =
  (typeof WBRAND_ATTACHMENT_FAULT_CODES)[keyof typeof WBRAND_ATTACHMENT_FAULT_CODES];

const KNOWN_FAULT_CODES = new Set<string>(Object.values(WBRAND_ATTACHMENT_FAULT_CODES));

export function isWBrandAttachmentFaultCode(value: unknown): value is WBrandAttachmentFaultCode {
 ...
 */
export class WBrandAttachmentFaultError extends Error {
  readonly code: WBrandAttachmentFaultCode;

  constructor(code: WBrandAttachmentFaultCode, options?: { cause?: unknown; message?: string }) {
    // message 默认就是 fault 码本身，旧版本客户端的文本匹配仍能命中。
    super(
      options?.message ?? code,
      options?.cause === undefined ? undefined : { cause: options.cause },
    );
    this.name = "WBrandAttachmentFaultError";
    this.code = code;
  }
}
...
/**
 * 从任意错误中读出附件 fault 码：
 * - 同进程抛出的 WBrandAttachmentFaultError / 带 code 的 Error；
...
export function readWBrandAttachmentFaultCode(
  ...
  if (candidate.data && typeof candidate.data === "object") {
    ...
    if (isWBrandAttachmentFaultCode(data.code)) return data.code;
  }
  // 兼容兜底：桌面端可能连接尚未带结构化 code 的旧 wbrand-cli。仅在消息「整体等于」
  // 某个 fault 码时命中，不做模糊匹配，避免把包装后的任意文本误判成确定分类。
  if (error instanceof Error && isWBrandAttachmentFaultCode(error.message)) return error.message;
  return undefined;
}
