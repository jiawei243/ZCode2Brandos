export type WBrandBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface WBrandBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: WBrandBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<WBrandBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = WBrandBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveWBrandBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): WBrandBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // WBrand Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseWBrandBackgroundTaskControlItems(
  value: unknown,
): WBrandBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, WBrandBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveWBrandBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: WBrandBackgroundTaskControlItem = {
      jobId,
...
export function isActiveWBrandBackgroundTaskControlItem(
  job: WBrandBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getWBrandBackgroundTaskControlItemElapsedMs(
  job: WBrandBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleWBrandBackgroundTaskControlItems(
  jobs: readonly WBrandBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<WBrandBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveWBrandBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getWBrandBackgroundTaskControlItemElapsedMs(job, now),
    }))
