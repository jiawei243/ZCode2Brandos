import type { WBrandApiRetryStatus } from "./wbrand-task-types-core.js";

export function normalizeWBrandApiRetryStatus(
  value: unknown,
...
export function wbrandApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): WBrandApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeWBrandApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = wbrandApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isWBrandModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function wbrandApiRetryFromStreamRecoveryPayload(
  value: unknown,
): WBrandApiRetryStatus | undefined {
  const record = asRecord(value);
