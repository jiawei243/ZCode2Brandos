...
import { readLegacyWBrandConfigProviders } from "../src/model-provider/legacyWBrandConfigProviderReader.js";
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "wbrand-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyWBrandConfigProviders();
    },
...
test("startup migrates published WBrand config into personal config without changing the source", async () => {
