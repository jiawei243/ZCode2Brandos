...
test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "wbrand-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "wbrand-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "wbrand-acp-snapshot-"));
  setDataBaseDir(dir);
...
test("current session recovery preserves Desktop and replayable projections", async () => {
  const snapshot = wbrandSessionStateSnapshotSchema.parse({
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "wbrand-provider-current-"));
  const runtime = createProviderConfigRuntime({
    wbrandBuiltinFilePath: fileURLToPath(
      new URL("../../../config/provider/wbrand-builtin.json", import.meta.url),
    ),
  ...
  try {
    ...
    assert.ok(config.wbrandBuiltinRevision);
