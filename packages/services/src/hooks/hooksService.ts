...
} from "@wbrand/shared/workspace-hook-discovery";
import { parseWorkspaceHookTrustStoreContent } from "@wbrand/shared/workspace-hook-trust-store-file";
...
const WBRAND_CONFIG_FILE = "config.json";
...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "wbrand") {
    return workspacePath ? join(baseDir, ".wbrand") : join(baseDir, ".wbrand", "cli");
  }
...
function getConfigPath(source: SettingsDirectorySource, workspacePath?: string): string {
  return join(
    ...
    source === "wbrand" ? WBRAND_CONFIG_FILE : SETTINGS_FILE,
  );
}
...
async function readUserWBrandSource(): Promise<WorkspaceHookSourceInput | undefined> {
  const path = getConfigPath("wbrand");
...
async function readPersistentWorkspaceHookTrustDigests(
  ...
  const userConfig = (await readJsonFile<Record<string, unknown>>(getConfigPath("wbrand"))) ?? {};
...
async function writeWBrandHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("wbrand", workspacePath);
  const existingConfig = (await readJsonFile<WBrandConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toWBrandHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".wbrand", "config.json");
  const userHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      (!hook.location || (hook.location.source === "wbrand" && hook.location.scope === "user")),
  );
  const projectHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      hook.location?.source === "wbrand" &&
      hook.location.scope === "project" &&
      (!hook.configuredState ||
        resolve(hook.configuredState.sourcePath) === currentProjectConfigPath),
  );
  await writeWBrandHooksConfig(undefined, userHooks);
  await writeWBrandHooksConfig(params.workspacePath, projectHooks);
}
