...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = WBRAND_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = WBRAND_COMMAND_AGENT_SOURCES;
...
const WBRAND_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "wbrandAgent",
  directorySource: "wbrand",
  userDirectorySegments: [".wbrand", "commands"],
  workspaceDirectorySegments: [".wbrand", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // WBrand Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .wbrand 抢在 workspace .agents 前面。
    for (const agentSource of agentSources) {
      const descriptors =
        agentSource === WBRAND_COMMAND_AGENT_SOURCE
    ...
    const pluginCommands =
      !params.agentSource || params.agentSource === WBRAND_COMMAND_AGENT_SOURCE
        ? await discoverPluginCommands(enabledOverrides)
        : [];

    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as WBrandCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(WBRAND_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        WBRAND_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, WBRAND_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.wbrand` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "wbrand" && discoveredCount > 0) {
      break;
    }
  }
}
