...
import { redactFeedbackText, WBRAND_VERSION, WBRAND_COMMIT } from "@wbrand/shared";
...
export async function createFeedbackDiagnosticArchive(options: {
  ...
  const path = join(outputDir, "wbrand-diagnostic-logs.zip");
  ...
  try {
    ...
    zip.addBuffer(
      Buffer.from(
        [
          "WBrand diagnostic logs",
          `timestamp: ${now.toISOString()}`,
          `appVersion: ${WBRAND_VERSION}`,
          `commit: ${WBRAND_COMMIT}`,
