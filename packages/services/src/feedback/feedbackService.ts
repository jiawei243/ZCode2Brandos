...
import type { ApiClient, FeedbackDeviceInfo } from "@wbrand/shared";
import {
...
function resolveApiBaseUrl(explicit?: string): string {
  return (
    explicit?.trim() ||
    process.env.WBRAND_FEEDBACK_API_BASE?.trim() ||
    buildRuntimeWBrandApiUrl(process.env, "/api/v1")
  );
}
...
function buildDeviceSnapshot(): FeedbackDeviceInfo {
  return {
    ...
    buildCommitId: WBRAND_COMMIT,
    buildTime: WBRAND_BUILD_TIME,
...
export function createFeedbackService(options: CreateFeedbackServiceOptions): IFeedbackService {
  ...
  async function getWbrandJwtToken(): Promise<string | undefined> {
    return (await options.credentialService.load(WBRAND_JWT_TOKEN_KEY))?.trim() || undefined;
  }

  async function hasWbrandJwtToken(): Promise<boolean> {
    return Boolean(await getWbrandJwtToken());
  }
  ...
  const httpClient = new FeedbackHttpClient({
    ...
    getAuthHeaders: async () => {
      ...
      const jwtToken = await getWbrandJwtToken();
      if (jwtToken) {
