import type { WBrandProvider, SkillsPromptContext, SkillsListResult } from "@wbrand/shared";
...
export interface ISkillsService {
  ...
  /** 将指定 skill 复制到通用目录（.wbrand/skills），成功后返回新 skill 的路径。 */
  copyToCommon(params: {
    workspacePath: string;
    workspaceIdentity?: string;
    skillId: string;
  }): Promise<{ newPath: string }>;
  /** 从通用目录中移除指定 skill（仅当 skill 位于 .wbrand/skills 时有效）。 */
   ...
   * 删除技能所在目录，仅允许命中 .wbrand/skills 或 .agents/skills 根，越界则拒绝。
   */
  deleteSkill(params: {
    workspacePath: string;
    workspaceIdentity?: string;
