...
import {
  ...
  type WBrandOffPeakTaskCreateParams,
} from "@wbrand/shared";
...
import type { ModelSelection, ModelSelectionValidation } from "@wbrand/provider";
...
function isValidCreateParams(params: WBrandOffPeakTaskCreateParams): boolean {
  return (
...
> {
  ...
  if (failureStage === "client_validation") {
    ...
    errorCode = error.biwBrand === undefined ? "" : String(error.biwBrand);
    if (error.biwBrand === 3101) errorCategory = "eligibility_3101";
    else if (error.biwBrand === 3103) errorCategory = "quota_3103";
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: WBrandOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: WBrandOffPeakTaskCreateParams = {
      ...params,
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<WBrandOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<WBrandOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<WBrandOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<WBrandOffPeakTask | null> {
  ...
  async list(): Promise<WBrandOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<WBrandOffPeakTask | null> {
  ...
  ): Promise<WBrandOffPeakTask[]> {
    const repaired: WBrandOffPeakTask[] = [];
  ...
  private async settleOne(task: WBrandOffPeakTask): Promise<void> {
