/* eslint-disable max-lines -- WBrand task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@wbrand/rpc";
import { ServiceChannels } from "@wbrand/shared";
import type { CommandPayloadMap } from "@wbrand/shared/wbrand-protocol-v4";
import { createServiceDescriptor } from "#src/descriptors.js";
import type {
  WBrandImportSessionsResult,
  WBrandImportableSessionCandidate,
  WBrandSessionCompactResult,
  WBrandSessionGoalAction,
  WBrandSessionGoalResult,
  WBrandTaskCreateResult,
  WBrandTaskMeta,
  WBrandStreamEvent,
  WBrandTaskMode,
  WBrandConfigOption,
  WBrandError,
  WBrandAssistantMessageFeedback,
  WBrandPromptAttachment,
  WBrandProvider,
  WBrandWorkspaceEvent,
  TraceId,
  WBrandSessionFile,
  WBrandAgentMcpServer,
  WBrandTaskSnapshot,
  WBrandTaskSnapshotBody,
  WBrandTaskSnapshotToolCallsSlice,
  WBrandTaskSnapshotRefContent,
  WBrandEnqueueTaskCommandResult,
  WBrandCancelTaskCommandResult,
  WBrandTaskClientMode,
  WBrandTaskTokenUsageResult,
...
import type {
  ...
  WBrandTaskListSortBy,
  WBrandTaskListWorkspaceScope,
...
export interface WBrandTaskSnapshotWithEtagResult {
  snapshot: WBrandTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface WBrandArchivedTaskDeletionResult {
...
export type WBrandModelTrajectoryContentPart =
...
export interface WBrandModelTrajectoryMessage {
  role: string;
  parts: WBrandModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface WBrandModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type WBrandModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface WBrandModelTrajectoryCallSource {
  kind: WBrandModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface WBrandModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: WBrandModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: WBrandModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: WBrandModelTrajectoryContentPart[];
    usage?: WBrandModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface WBrandModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 WBrand Agent 落盘 model-io）。 */
  available: boolean;
  records: WBrandModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface WBrandTaskTerminalOutcome {
  taskId: string;
...
export interface WBrandTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IWBrandTaskService — WBrand task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * WBrand Agent server 维护，新功能应优先走 IWBrandSessionService。
 */
export interface IWBrandTaskService {
  // ---- 生命周期 ----
  ...
  }): Promise<WBrandTaskCreateResult>;
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<WBrandEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: WBrandTaskClientMode;
  }): Promise<WBrandCancelTaskCommandResult>;
  ...
  }): Promise<WBrandSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<WBrandSessionGoalResult>;
   ...
   */
  listTaskList(params: WBrandTaskListQuery): Promise<WBrandTaskListResult>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<WBrandTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<WBrandTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<WBrandTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<WBrandTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<WBrandTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<WBrandConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: WBrandAssistantMessageFeedback | null;
  }): Promise<WBrandSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: WBrandTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.wbrand/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 WBrand Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<WBrandModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<WBrandTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 WBrand Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  deleteArchivedTasks(params: {
    ...
    workspaceIdentity?: string;
    taskIds: string[];
  }): Promise<WBrandArchivedTaskDeletionResult>;
  ...
  onDynamicWorkspaceEvent(
    workspace: string | WBrandWorkspaceEventSubscriptionParams,
  ): Event<WBrandWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<WBrandError>;
}

export const IWBrandTaskService = createServiceDescriptor<IWBrandTaskService>(
  ServiceChannels.WBrandTask,
);
