import type {
  WBrandAutomation,
  WBrandAutomationCreateParams,
  WBrandAutomationRun,
  WBrandAutomationScheduleRule,
  WBrandAutomationUpdateParams,
} from "@wbrand/shared";
import { resolveWorkspaceKey } from "@wbrand/shared";
...
function assertValidAutomationScheduleRule(rule: WBrandAutomationScheduleRule): void {
...
export class AutomationService {
  ...
  async create(params: WBrandAutomationCreateParams): Promise<WBrandAutomation> {
  ...
  async update(
    ...
    // 同时顺手修复历史上已经存在的同类脏数据。
    const normalizedParams: WBrandAutomationUpdateParams = hasIntervalCarrier
    ...
    const options: {
      ...
      lifecycleStatus?: WBrandAutomation["lifecycleStatus"];
      resetRetry?: boolean;
    } = {};
    ...
    const effectiveDirectScheduleRule = hasDirectScheduleRule
      ? directScheduleRule
      : carrierScheduleRule;
    const normalizedWithSchedule: WBrandAutomationUpdateParams =
  ...
  async runNow(
    automationId: string,
    scope?: AutomationWorkspaceScope,
  ): Promise<{ automation: WBrandAutomation; run: WBrandAutomationRun } | null> {
    const workspaceKey = resolveScopeKey(scope);
    const existing = await this.repo.get(automationId, workspaceKey);
    if (!existing) return null;
