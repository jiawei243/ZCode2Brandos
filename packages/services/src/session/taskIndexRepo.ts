...
import type {
  ...
  WBrandTaskListItem,
} from "#src/session/wbrandTaskListTypes.js";
import type {
  WBrandGroupedTaskRef,
  WBrandGroupedTaskView,
  WBrandGroupedTaskViewNode,
  WBrandGroupedTaskViewOrderInput,
  WBrandGroupedTaskViewQuery,
  WBrandGroupedTaskViewStructure,
  WBrandGroupedTaskViewStructureMember,
  WBrandGroupedTaskViewStructureTopOrder,
  WBrandGroupedTaskViewTopLevelNodeRef,
  WBrandTaskGroup,
  WBrandTaskGroupColor,
} from "#src/session/wbrandTaskListTypes.js";
...
function appendWBrandAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: WBrandTaskMeta["status"];
  lastError?: WBrandTaskMeta["lastError"];
  target?: WBrandTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY = "__wbrand_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: WBrandTaskGroupColor = "gray";
const WORKSPACE_BOOTSTRAP_TASK_GROUP_COLORS = [
  ...
  "blue",
  "purple",
] satisfies WBrandTaskGroupColor[];
...
function isTerminalTaskStatus(status: WBrandTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: WBrandTaskMeta | null,
  incomingMeta: WBrandTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): WBrandTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = wbrandTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as WBrandTaskMeta),
  ...
  return {
    ...
    mode: row.mode as WBrandTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === WBRAND_AGENT_PROVIDER ? WBRAND_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as WBrandTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as WBrandTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: WBrandTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@wbrand/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): WBrandTaskListItem {
...
function isTaskGroupColor(value: string): value is WBrandTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): WBrandTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): WBrandTaskGroupColor {
...
function groupedTopNodeOrderRef(node: WBrandGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: WBrandGroupedTaskViewNode,
  right: WBrandGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: WBrandTaskListItem, right: WBrandTaskListItem): number {
...
function compareGroupTasks(
  left: WBrandTaskListItem,
  right: WBrandTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendWBrandAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): WBrandTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: WBrandTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: WBrandTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 WBrand session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 WBrand 任务。
   ...
   */
  private ensureCronGroupMembership(meta: WBrandTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<WBrandTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: WBrandTaskMeta): Promise<WBrandTaskMeta> {
    await this.ensureReady();
  ...
  }): Promise<{ meta: WBrandTaskMeta; cleared: boolean }> {
    await this.ensureReady();
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<WBrandTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: WBrandTaskListQuery & { provider?: WBrandProvider },
  ): Promise<WBrandTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<WBrandTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: WBrandGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: WBrandGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: WBrandGroupedTaskViewQuery & { provider?: WBrandProvider },
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 WBrand Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendWBrandAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
  ...
  async queryGroupedTaskViewStructure(params: {
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: WBrandGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: WBrandGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: WBrandGroupedTaskViewOrderInput & { provider?: WBrandProvider },
  ): Promise<WBrandGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: WBrandGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 WBrand Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: WBrandGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: WBrandGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: WBrandGroupedTaskRef[] = [];
