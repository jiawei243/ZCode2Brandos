...
import type { WBrandImportSessionsResult, WBrandTaskMeta } from "@wbrand/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<WBrandTaskMeta>;
  onTaskImported: (meta: WBrandTaskMeta) => void;
}): Promise<WBrandImportSessionsResult> {
  const normalizedSessionIds = [
    ...new Set(params.sessionIds.map((item) => item.trim()).filter(Boolean)),
  ];
  const result: WBrandImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };
