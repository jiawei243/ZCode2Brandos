...
import type { WBrandImportableSessionCandidate } from "@wbrand/shared";
...
class ClaudeNativeSessionImportRepo {
  private getNativeProjectsRoots(): string[] {
    ...
    // 关键业务逻辑：扫描 Claude Code 原生历史目录 ~/.claude/projects，不是 wbrand 自己的数据目录。
    // 当 WBRAND_DATA_BASE_DIR 把 .wbrand 放到别处时，原生 .claude 往往仍在真实用户 HOME 下。
  ...
  async scanImportableSessions(params: {
    ...
    const candidates: WBrandImportableSessionCandidate[] = [];
  ...
  async findImportableSession(params: {
    workspacePath?: string;
    sessionId: string;
  }): Promise<WBrandImportableSessionCandidate | null> {
    const workspaceKey = params.workspacePath
      ? normalizePathForComparison(params.workspacePath)
      : null;
  ...
  async copySessionFileToWorkspace(params: {
    ...
    // 导入副本沿用历史目录布局 ~/.wbrand/v2/agent-config/claude/{workspaceHash}/projects；
