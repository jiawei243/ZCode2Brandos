...
export function buildSearchableTextFromMessages(messages: WBrandSessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function persistImportedClaudeTask(params: {
  ...
  const indexMeta: WBrandTaskMeta = {
    ...meta,
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.wbrand/v2/sessions/{hash}/{taskId}.json。现在真实 WBrand session 承担续聊，legacy snapshot
