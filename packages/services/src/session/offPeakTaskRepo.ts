...
/* off-peak 任务仓库：off_peak_tasks 的 sqlite schema、状态机守卫写入与调度认领。
   ...
   禁止往 automations 表或 WBrandAutomation 类型上加字段。 */
...
function rowToTask(row: OffPeakTaskRow): WBrandOffPeakTask {
  ...
  return {
    ...
    prompt: row.prompt,
    permissionMode: row.permission_mode as WBrandTaskMode,
    ...
    status: row.status as WBrandOffPeakTaskStatus,
...
function serializeOffPeakModelSelection(
  selection: NonNullable<WBrandOffPeakTask["modelSelection"]>,
): string {
...
export class OffPeakTaskRepo {
   ...
   */
  async create(
    params: WBrandOffPeakTaskCreateParams,
    options?: {
      /** 测试注入用；缺省 Date.now()。 */
      now?: number;
  ...
  async invalidateModelSelection(
    offPeakTaskId: string,
    modelSelection: NonNullable<WBrandOffPeakTask["modelSelection"]>,
    options?: { now?: number },
  ...
  async updateEditableFields(
    ...
    params: {
      ...
      modelSelection?: WBrandOffPeakTask["modelSelection"] | null;
    },
    options?: { now?: number },
  ): Promise<WBrandOffPeakTask | null> {
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<WBrandOffPeakTask[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: WBrandOffPeakTask[] = [];
  ...
  async listNonTerminal(): Promise<WBrandOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
  ...
  async listUnsettledTerminal(): Promise<WBrandOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
