...
import {
  ...
  type WBrandAutomationCreateParams,
  type WBrandAutomationDispatchStatus,
  type WBrandAutomationLifecycleStatus,
  type ModelSelection,
  type WBrandAutomationRun,
  type WBrandAutomationRunDispatchStatus,
  type WBrandAutomationRunOutcome,
  type WBrandAutomationTrigger,
  type WBrandAutomationUpdateParams,
} from "@wbrand/shared";
...
function rowToAutomation(row: AutomationRow): WBrandAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as WBrandAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as WBrandAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as WBrandAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): WBrandAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: WBrandAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): WBrandAutomation["mode"] | undefined {
  const parsed = wbrandTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!wbrandTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): WBrandAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as WBrandAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as WBrandAutomationRunDispatchStatus,
    outcome: (row.outcome as WBrandAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.wbrand/v2（历史脏数据
  ...
  async create(
    params: WBrandAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: WBrandAutomationLifecycleStatus },
  ): Promise<WBrandAutomation> {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<WBrandAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: WBrandAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: WBrandAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<WBrandAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<WBrandAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: WBrandAutomation[] = [];
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: WBrandAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<WBrandAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<WBrandAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()
