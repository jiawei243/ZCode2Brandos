...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["wbrand:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".wbrand", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".wbrand", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? WBRAND_VERSION;
  ...
  async function sendReport(
    ...
    if (!WBRAND_TELEMETRY_ENABLED || !WBRAND_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteWBrandEndpointUrl(
        WBRAND_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveWBrandEndpointOrigin?.()) ?? WBRAND_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildWBrandSourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? WBRAND_ENV,
