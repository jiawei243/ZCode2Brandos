...
import { getMediaPreviewFormat } from "@wbrand/shared";
import { packWorkspaceFileEntries } from "@wbrand/shared/workspaceFileEntriesCodec";
import type { IFileService, WorkspaceFileSearchParams } from "./file.js";
import { WORKSPACE_FILE_SEARCH_DISPLAY_CAP } from "@wbrand/shared/workspaceFileSearch";
...
const SCRATCH_WORKSPACE_ROOT_NAME = "WBrandProject";
 ...
 * 校验 rootPath + .wbrandignore 的 mtime/size 指纹；规则编辑后缓存失效，
 * 保持"编辑规则后下次使用生效"的契约。
 */
...
export function createFileService(options: CreateFileServiceOptions = {}): IFileService {
  ...
  // 全量扫描 + 打包（带 60s TTL / .wbrandignore 指纹缓存）。分块 RPC 共享同一份 packed。
