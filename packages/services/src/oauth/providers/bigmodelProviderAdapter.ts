...
interface BigModelWbrandTokenEnvelope {
  code?: number;
  msg?: string;
  data?: {
    token?: string | null;
...
export class BigModelProviderAdapter implements OAuthProviderAdapter {
  ...
  async exchangeToken(
    ...
    const tokenSet = await this.exchangeWbrandJwtToken(params, _context);
  ...
  private async exchangeWbrandJwtToken(
    params: OAuthCallbackParams,
    context: OAuthProviderContext,
  ): Promise<OAuthTokenSet> {
    log.info(undefined, "wbrand token request", {
      method: "POST",
    ...
    try {
      const payload = await readApiJson<BigModelWbrandTokenEnvelope>(
        this.apiClient,
      ...
      if (payload.code !== undefined && payload.code !== 0) {
        // BigModel 的一次性 authCode 现在只交给 wbrand token 路由。
        // 如果这里失败，不能继续保存半登录态，否则 Start Plan 仍会显示未连接。
        log.warn(undefined, "wbrand token business response rejected", {
          code: payload.code,
          msg: payload.msg,
        });
        throw new Error(
          payload.msg?.trim() || `BigModel wbrand token 交换失败（code: ${payload.code}）`,
        );
      }
      const wbrandJwtToken = payload.data?.token?.trim() || "";
      if (!wbrandJwtToken) {
        log.warn(undefined, "wbrand token response missing data.token", {
          code: payload.code,
          msg: payload.msg,
        });
        throw new Error("BigModel wbrand token 交换失败：响应缺少 data.token");
      }
      const accessToken = resolveBigModelBusinessAccessToken(payload);
      if (!accessToken) {
        // Coding Plan 付费套餐仍调用 bigmodel.cn 业务接口，只能使用
        // BigModel 业务 access token；wbrand JWT 只能写入 wbrandjwttoken 给 Start Plan 使用。
        // 如果继续把 wbrand JWT 写进 oauth:bigmodel:access_token，套餐预览会稳定报“令牌已过期”。
        log.warn(undefined, "wbrand token response missing bigmodel access token", {
          code: payload.code,
          msg: payload.msg,
        });
        throw new Error("BigModel wbrand token 交换失败：响应缺少 data.bigmodel.access_token");
      }
      ...
      // tokenByAuthCode 消费它；wbrand token 交换失败时直接中止登录并记录链路头。
      log.warn(undefined, "wbrand token request failed", {
  ...
  async fetchUserInfo(
    ...
    if (tokenSet.wbrandJwtToken && tokenSet.accessToken === tokenSet.wbrandJwtToken) {
      // 移除 tokenByAuthCode 后 callback 阶段没有 BigModel access token。
      // wbrand JWT 不能调用 bigmodel.cn 的 customer 接口，避免无意义的鉴权失败请求。
      return {
        id: "unknown",
        username: "user",
        displayName: "User",
...
function resolveBigModelBusinessAccessToken(payload: BigModelWbrandTokenEnvelope): string {
  return (
