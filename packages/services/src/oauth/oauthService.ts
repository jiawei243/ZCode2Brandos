...
export class OAuthService implements IOAuthService {
  ...
  async restoreCachedSessionState(): Promise<OAuthCachedSessionRestoreResult> {
    ...
    // 启动缓存恢复只需要检查共享 wbrand JWT；若通过 loadActiveTokenSet 连带读取
    ...
    if (wbrandJwtToken && resolveJwtExpiration(wbrandJwtToken, this.now()).kind === "expired") {
      serviceLog.info("cached session invalidated because wbrand JWT expired", {
    ...
    if (activeProvider === ZAI_PROVIDER_ID) {
      if (!wbrandJwtToken) {
        // sidebar 登录入口之前只看缓存 user_info，会把“缺少 wbrandjwttoken”的状态误判成已登录。
        // 这里补充 wbrandjwttoken 门槛，确保没有后端 JWT 时统一按未登录处理。
        log("restoreCachedSession skipped: missing wbrandjwttoken:", activeProvider);
        return { status: "signed-out" };
      }
    }
  ...
  private async invalidateExpiredCachedSession(
    ...
    const invalidated = await this.runSessionMutation(async () => {
      ...
      const currentJwt = (await this.credentialService.load(WBRAND_JWT_TOKEN_KEY))?.trim() ?? "";
      if (
  ...
  private async persistOAuthSession(
    ...
    if (inactiveProvider) {
      ...
      // 再保存当前 token；反序会让 clearProvider 误删共享的 wbrandjwttoken。
  ...
  async startOAuthWithPolling(provider: OAuthProviderId): Promise<OAuthStartResponse> {
    ...
    const initUrl = buildWBrandApiUrlFromEnv(this.env, "/api/v1/oauth/cli/init");
    ...
    this.pendingState = {
      ...
      polling: {
        ...
        pollIntervalMs,
        pollToken,
        pollUrl: buildWBrandApiUrlFromEnv(
          this.env,
  ...
  async pollPendingOAuth(): Promise<OAuthCallbackResult | null> {
    ...
    try {
      const result = await this.runPendingSessionCompletion(pending, async () => {
        ...
        const wbrandJwtToken = readTrimmedString(ready.token);
        const userId = readTrimmedString(user?.user_id);
        if (!wbrandJwtToken || !providerAccessToken || !userId) {
          throw new Error("OAuth flow 查询响应无效");
        }
