...
function getCachedProfileMigrationRetryAfter(profile: OAuthUserProfile): number | null {
  ...
  const retryAfter = (rawProfile as { wbrandProfileMigrationRetryAfter?: unknown })
    .wbrandProfileMigrationRetryAfter;
...
export function withProviderProfileSchema(
  ...
  delete nextRawProfile.wbrandProfileMigrationRetryAfter;

  return {
    ...profile,
    rawProfile: {
      ...nextRawProfile,
      wbrandProfileSchemaVersion: BIGMODEL_PROFILE_SCHEMA_VERSION,
    },
  };
}
...
function withBigModelProfileMigrationRetryAfter(
  ...
  return {
    ...
    rawProfile: {
      ...
      wbrandProfileMigrationRetryAfter: now + BIGMODEL_PROFILE_MIGRATION_RETRY_DELAY_MS,
    },
  };
}
...
export async function refreshLegacyBigModelCachedProfile(
  ...
  try {
    ...
    if (isBigModelUserInfoFallback(refreshedProfile)) {
      // 旧版本可能把 wbrand JWT 写进 BigModel access token。
