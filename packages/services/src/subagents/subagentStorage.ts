...
export async function resolveUserSubagentRoot(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveWBrandStorageRoot(options), "agents");
}

export function resolveWorkspaceSubagentRoot(workspacePath: string): string {
  return join(workspacePath, ".wbrand", "agents");
}

export async function resolveSubagentStateFile(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveWBrandStorageRoot(options), "v2", "agents-state.json");
}

export async function resolveWBrandStorageRoot(options?: SubagentStorageOptions): Promise<string> {
...
async function readUserCliConfig(
  ...
  try {
    const raw = await readFile(
      join(resolveUserHomeDir(options), ".wbrand", "cli", "config.json"),
      "utf8",
    );
