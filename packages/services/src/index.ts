...
// Conversation share 的具体实现依赖 Node 文件系统，只能从 @wbrand/services/node 引入；
...
// 若 value 导出 createOnboardingRecordService，会连带 fs/atomicFileUtils → @wbrand/shared/node →
...
// WBrand task wrapper service — task 列表/置顶/归档等 app 侧包装状态入口。
export { IWBrandTaskService } from "./session/wbrandTaskService.js";
export type {
  WBrandArchivedTaskDeletionResult,
  WBrandModelTrajectory,
  WBrandModelTrajectoryCallSource,
  WBrandModelTrajectoryCallSourceKind,
  WBrandModelTrajectoryContentPart,
  WBrandModelTrajectoryMessage,
  WBrandModelTrajectoryRecord,
  WBrandModelTrajectoryUsage,
  WBrandTaskListKind,
  WBrandTaskListQuery,
...
export type { WBrandTaskListItem } from "./session/wbrandTaskListTypes.js";
...
// WBrand agent service — IWBrandAgentService is both a type (interface) and value (descriptor)
export {
  IWBrandAgentService,
  type WBrandAgentLocalRuntimeChildProcesses,
  WBRAND_AGENT_RUNTIME_UNAVAILABLE_CODE,
} from "./wbrand-agent/wbrandAgent.js";
export {
  isWBrandAgentMcpStatusModeUnsupportedError,
  WBRAND_AGENT_MCP_STATUS_MODE_UNSUPPORTED_ERROR_CODE,
  WBrandAgentMcpStatusModeUnsupportedError,
} from "./wbrand-agent/wbrandAgentErrors.js";
export {
  createWBrandAgentConnectionScope,
  readTrustedWBrandAgentV4Connection,
} from "./wbrand-agent/wbrandAgentConnectionScope.js";
export type {
  WBrandAgentConnectionScope,
  WBrandAgentV4ClientMode,
  WBrandAgentV4ConnectionContext,
} from "./wbrand-agent/wbrandAgentConnectionScope.js";
export type {
  ...
  WBrandAgentStorageStartupSnapshot,
  WBrandAgentRuntimeLifecycleEvent,
  WBrandAgentRuntimePolicy,
  WBrandAgentReadSessionParams,
  WBrandAgentResumeSessionParams,
  WBrandAgentRunAutomationNowResult,
...
// WBrand session service — app-facing session facade without WBrand Agent naming.
export { IWBrandSessionService } from "./wbrand-session/wbrandSession.js";
export type {
  WBrandSessionCreateParams,
  WBrandSessionEventsParams,
  WBrandSessionInitializeResult,
  ...
  WBrandSessionSubscribeParams,
  WBrandTaskTarget,
  WBrandSessionWorkspaceTarget,
} from "./wbrand-session/wbrandSession.js";
...
// Storage（资源管理器「存储」tab）：数据类型在 @wbrand/shared；这里只导出服务接口与卷分组纯函数
...
// 设置页插件管理薄服务（UI 平台能力面不再直触 wbrandAgentService）
