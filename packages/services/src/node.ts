...
export {
  ...
  WBRAND_E2E_FS_FAULTS_ALLOW_ENV,
  WBRAND_E2E_FS_FAULTS_ENV,
...
export {
  ...
  WBRAND_WINDOWS_APP_INSTALL_DIR_ENV,
} from "./paths.js";
...
export { createWBrandAgentService } from "./wbrand-agent/wbrandAgentService.js";
export { createWBrandTaskServiceAdapter } from "./wbrand-agent/wbrandTaskServiceAdapter.js";
export { createWBrandSessionService } from "./wbrand-session/wbrandSessionService.js";
export {
  resolveDefaultWBrandAgentCommand,
  WBrandAgentProcessManager,
} from "./wbrand-agent/wbrandAgentProcessManager.js";
export type {
  WBrandAgentCommand,
  WBrandAgentCommandResolver,
  WBrandAgentCommandResolverContext,
  WBrandAgentProcessManagerOptions,
} from "./wbrand-agent/wbrandAgentProcessManager.js";
export { WBrandProtocolClient } from "./wbrand-agent/wbrandProtocolClient.js";
export type { WBrandProtocolTransport } from "./wbrand-agent/wbrandProtocolTransport.js";
export { WBrandStdioTransport } from "./wbrand-agent/wbrandStdioTransport.js";
export {
  getWBrandStdioTapDevLogDir,
  readWBrandStdioTapDevState,
  setWBrandStdioTapDevEnabled,
} from "./wbrand-agent/wbrandStdioTapDevConfig.js";
export type { WBrandStdioTapDevState } from "@wbrand/shared";
export {
...
import { IWBrandTaskService } from "./session/wbrandTaskService.js";
import { IWBrandAgentService } from "./wbrand-agent/wbrandAgent.js";
import type { CuaOperationStateReporter } from "./wbrand-agent/cuaOperationTurnTracker.js";
import { IWBrandSessionService } from "./wbrand-session/wbrandSession.js";
import {
...
import { createWBrandAgentService } from "./wbrand-agent/wbrandAgentService.js";
import type { WBrandAgentCommandResolver } from "./wbrand-agent/wbrandAgentProcessManager.js";
import { buildAgentTelemetrySpawnEnv } from "./wbrand-agent/agentTelemetryEnv.js";
import { resolveWBrandAgentPresentationSurface } from "./wbrand-agent/wbrandAgentPresentationSurface.js";
import { createWBrandTaskServiceAdapter } from "./wbrand-agent/wbrandTaskServiceAdapter.js";
import { createWBrandSessionService } from "./wbrand-session/wbrandSessionService.js";
import { createWBrandTaskIndexSyncer } from "./wbrand-agent/wbrandTaskIndexSyncer.js";
...
import { readLegacyWBrandConfigProviders } from "./model-provider/legacyWBrandConfigProviderReader.js";
...
import { fetchWBrandBuiltinRemoteRelease } from "./model-provider/wbrandBuiltinRemoteConfig.js";
import {
  createProviderRuntimeFromConfigRuntime,
...
import {
  ...
  isPotentialWBrandCuaAgentMcpServer,
...
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@wbrand/wbrand-cua/broker/helperConstants";
import { resolveBrokerSocketPath } from "@wbrand/wbrand-cua/broker/socketPath";
import {
  ...
  type BrowserCommand,
  isWBrandCuaMcpCommand,
  isWBrandCuaMcpPackageArg,
  isWBrandCuaInternalFeatureEnabled,
  WBRAND_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  type WBrandAutomation,
  type WBrandAutomationRun,
  getCapturedWBrandAgentTelemetryEnv,
  WBRAND_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV,
  ZAI_PROVIDER_ID,
  wbrandAccountAccessSchema,
  wbrandProviderAccountAccessSchema,
  WBRAND_VERSION,
  WBRAND_ENV,
  buildRuntimeWBrandApiUrl,
} from "@wbrand/shared";

// 这些 conversation-share 实现依赖 Node 文件系统；仅通过 @wbrand/services/node 暴露，
...
export function shouldEnableDefaultCuaProductHelper(
  ...
  // CUA 已随正式版默认开启（isWBrandCuaInternalFeatureEnabled 默认 ON，仅显式 0/false/off 关闭；2026-08 注释更正——旧注释称默认关闭已过期）。显式开启后 macOS 使用既有产品 Helper，Windows 使用安装包内 runtime；
  // 两端都保持按需启动。关闭时不创建 host、不探测资源、不产生子进程或权限提示。
  const env = options.env ?? process.env;
  if (!isWBrandCuaInternalFeatureEnabled(env)) return false;
  ...
  return {
    async resolveMcpServers(servers, context) {
      ...
      return options.isResolverCurrent?.(resolver, context) === false
        ? servers?.filter((server) => !isPotentialWBrandCuaAgentMcpServer(server))
        : resolved;
    },
    async restart() {
      ...
      if (!resolver) {
        throw new Error("WBrand Computer Use is not enabled (plugin off or not product mode).");
      }
...
export const WBRAND_CUA_BUNDLED_HELPER_APP_PATH_ENV = "WBRAND_CUA_BUNDLED_HELPER_APP_PATH";

export function resolveBundledCuaHelperAppPath(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const injectedPath = env[WBRAND_CUA_BUNDLED_HELPER_APP_PATH_ENV]?.trim();
  if (injectedPath) {
    return injectedPath;
  }
  const resourcesPath = (
...
export function hasGlobalCliWBrandCuaServer(env: NodeJS.ProcessEnv = process.env): boolean {
  const home = env.HOME?.trim() || homedir();
  const configPath = join(home, ".wbrand", "cli", "config.json");
  let parsed: unknown;
  try {
  ...
  return Object.entries(parsed.mcp.servers).some(([name, config]) =>
    isGlobalCliWBrandCuaServer(name, config),
  );
}

function isGlobalCliWBrandCuaServer(name: string, config: unknown): boolean {
  if (!isRecord(config)) return false;
  if (config.enabled === false) return false;
  if (typeof config.type === "string" && config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 与 desktop/services resolver 和 CLI bootstrap 共用 @wbrand/shared 的单一事实源，避免第三处
  // 判定漂移：git/.git/本地路径形态的 wbrand-cua 若这里漏判，全局 CLI env 注入不会带 broker
  // socket/token，agent 会回退成 Python/uvx 自己持有 macOS TCC（违反 product broker 边界）。
  if (typeof config.command === "string" && isWBrandCuaMcpCommand(config.command)) {
    return true;
  }
  return (
    Array.isArray(config.args) &&
    config.args.some((arg) => typeof arg === "string" && isWBrandCuaMcpPackageArg(arg))
  );
}
...
export async function buildCuaProductHelperAgentEnv(
  ...
  //
  // fail-closed 理由：CLI 全局配置里存在 wbrand-cua 时，返回空 env 会让 agent 按原始
  ...
  try {
    ...
    if (host.waitForTransport) {
      ...
      return {
        ...
        [WBRAND_CUA_PLUGIN_AUTHORITY_ENV_KEY]: transport.pluginAuthority,
      };
    }
    ...
    if (reserved && !hasCuaProductHelperAgentEnvUnavailable(host)) {
      ...
      return {
        ...
        [WBRAND_CUA_PLUGIN_AUTHORITY_ENV_KEY]: reserved.pluginAuthority,
      };
    }
    ...
    return {
      ...
      [WBRAND_CUA_PLUGIN_AUTHORITY_ENV_KEY]: handle.pluginAuthority,
    };
  } catch (error) {
    ...
    logger.warn(
      undefined,
      `Computer Use Helper broker_unavailable; disabling workspace wbrand-cua MCP server for this agent spawn (${cuaHelperStartErrorDetail(error)})`,
    );
    return {
...
export function createLocalServices(options: {
  ...
  wbrandAgentSpawnFallbackCwd?: string;
  /** desktop-attached remote server 从 Desktop Host 收到的一次性 Agent 网络配置。 */
  remoteAgentNetwork?: {
    httpProxy?: string;
    noProxy?: string;
  };
  /** 所属 Environment 的 WBrand Built-in Provider Config 物理路径。 */
  ...
  // 导致 bun 这类只在 shell profile 里追加的命令在 WBrand Agent/终端里不可见。
  // 这里在所有本地服务启动前统一修正运行时环境，并顺带把内置 rg 注入 PATH，
  // 让 WBrand Agent、终端、认证 runtime 共用同一套命令解析结果。
  initializeRuntimeProcessEnv(options?.runtimeProcessEnvPatch);

  const desktopContextPromptEnabledRaw =
    process.env[WBRAND_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]?.trim();
  ...
  const resolveCurrentWBrandEndpointOrigin = async () =>
    resolveRuntimeWBrandEndpointOrigin(process.env, {
      overrideOrigin: (await settingService.get()).wbrandEndpointOrigin,
    });
  ...
  const wbrandJwtLogoutHandlerRef: {
  ...
  const apiClient = createNodeApiClient({
    ...
    onWbrandJwtInvalid: (input, headers) => wbrandJwtLogoutHandlerRef.current?.(input, headers),
    isWbrandJwtRequest: (input, headers) =>
      isCurrentOAuthCredentialRequest({ input, headers, credentialService }),
    resolveWBrandEndpointOrigin: resolveCurrentWBrandEndpointOrigin,
  });
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    wbrandBuiltinFilePath: options.wbrandBuiltinProviderConfigFilePath,
    wbrandBuiltinEnvironment: {
      environmentConfigRoot: resolveAppConfigDir(),
      platform: clientConfigPlatform,
      appVersion: WBRAND_VERSION,
      resolveEndpointOrigin: resolveCurrentWBrandEndpointOrigin,
      onRefreshResult: (event) => {
        if (event.result === "updated")
          providerConfigLog.info(undefined, "WBrand Built-in CDN 配置已更新", event);
        else providerConfigLog.debug(undefined, "WBrand Built-in 刷新检查", event);
      },
      fetchRelease: (endpointOrigin, signal) =>
        fetchWBrandBuiltinRemoteRelease({
          apiClient,
          endpointOrigin,
          signal,
          appVersion: WBRAND_VERSION,
    ...
    onWBrandBuiltinRefreshError: (error) => {
      providerConfigLog.warn(undefined, "WBrand Built-in Config 远端刷新失败", { error });
    },
    ...
    // 已发布 config.json 保存的是 WBrand 用户配置；清理第三方 ACP 不能移除这条升级路径。
    // Repository 仅在新 Personal 配置不存在时导入，并保留旧文件以便回滚。
    readLegacyProviders: () => readLegacyWBrandConfigProviders(),
  });
  ...
  let providerConnectivityAgentService:
    | Pick<IWBrandAgentService, "testModelConnectivity">
    | undefined;
  ...
  // mcpSync/hooks 里引用 wbrandAgentService 的闭包是惰性调用，声明顺序不影响初始化。
  ...
  const mcpSyncService = createMcpSyncService({
    ...
    listMcpServerStatuses: (params) => wbrandAgentService.listMcpServerStatuses(params),
  });
  ...
  const hooksService = createHooksService({
    grantWorkspaceHookTrust: (params) => wbrandAgentService.grantWorkspaceHookTrust(params),
  });
  ...
  // createDefaultCuaProductHelper() 在 wbrandAgentService 存在之前就要组装 resolver，
  // 但"是否有活跃 turn"这个信号只有 wbrandAgentService 建好之后才能查询。用前向引用占位——resolver 真正
  ...
  const isCuaEnabledForContext = (context?: CuaProductMcpServerResolverContext): boolean =>
    // 保留 main 原有 gate 行为（避免回归）：dev/internal 特性开启时（WBRAND_CUA_DEV_MODE=1 或
    // WBRAND_CUA_PRODUCT_HELPER=1）即视为启用，不依赖 config.json 显式 enable——main 的 bootstrap
    // 用 isWBrandCuaInternalFeatureEnabled 门控 bundled plugin，与 feat 的 workspace enablement 不同。
    // 生产路径（dev mode off）回落到官方插件 workspace enablement 判定（与 feat 一致）。
    isWBrandCuaInternalFeatureEnabled(process.env) ||
    isOfficialCuaPluginEnabledForWorkspace({
      env: process.env,
  ...
  const createManagedDefaultCuaProductHelper = (
    ...
    const helper = createDefaultCuaProductHelper({
      // 转发活跃-turn 查询（前向引用，wbrandAgentService 建好后赋值）。
  ...
  //
  // dev 判定直接用上游的 isCuaLocalDevelopmentRuntime（@wbrand/wbrand-cua/broker/server，
  // 即本文件已经用来 import buildHelperOpenArgs 的那个 subpath，可正常导入）。
  //
  // 行为等价性（别误读成安全加固）：上游是 `COMPILED_LOCAL_DEVELOPMENT_RUNTIME &&
  // WBRAND_RUNTIME_ENV!=="production"`，而那个编译期常量只有 scripts/build-cua-helper-app.mjs
  // 会用 define 折叠（Helper bundle）；desktop host bundle 没有该 define，于是回退成
  // `process.env.NODE_ENV !== "production"` —— 正是复制品写的那一项。所以在**当前**打包形态下
  // 两者逐字等价，关门靠的是 WBRAND_RUNTIME_ENV=production（打包态显式注入且不传 NODE_ENV）。
  //
  // 换成上游的收益是消除漂移面：折叠点、因子个数与 fail-closed 方向都由上游一处决定，
  // 哪天 host bundle 也补上 __WBRAND_LOCAL_DEVELOPMENT_RUNTIME__ define（Helper 侧已经有），
  // 编译期门自动生效，不需要再回来改这里。
  ...
  const launchStandaloneCuaHelperForStatus = async (): Promise<string | null> => {
    ...
    const home = process.env.WBRAND_HOME?.trim() || join(homedir(), ".wbrand");
  ...
  const cuaPipSessionService = createCuaPipSessionService({
    ...
    resolveCredentials: async () => {
      ...
      // PiP 客户端以 role=presentation 声明，资格由 Helper 按对端（WBrand 主进程）签名 identifier 裁决。
  ...
  const cuaPermissionService: ICuaPermissionService = {
    async getStatus(
      ...
      if (
        ...
        return {
          available: false,
          reason: "WBrand Computer Use is not enabled (plugin off or not product mode).",
        };
      }
      ...
      if (!host || !host.running) {
        ...
        if (!stable) {
          return {
            available: false,
            reason:
              "WBrand Computer Use is not running; it will start automatically on first Computer Use use.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
        // standalone Helper 上直接查权限真值（身份模式，无 token）。
        try {
          const { callBrokerMethod } = await import("@wbrand/wbrand-cua/broker/helperHealth");
          ...
          return {
            available: false,
            reason: "WBrand Computer Use is starting up; retry in a moment.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
      }
      try {
        ...
        if (!helper || !isDefaultCuaProductHelperCurrent(helper)) {
          return {
            available: false,
            reason: "WBrand Computer Use lifecycle is disposed.",
          };
        }
    ...
    async restartHelper(
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Failed to restart WBrand Computer Use: ${
            error instanceof Error ? error.message : String(error)
          }`,
        };
      }
  ...
  const wbrandAgentService = createWBrandAgentService({
    ...
    commandResolver: options?.wbrandAgentCommandResolver,
    presentationSurface: resolveWBrandAgentPresentationSurface({
    ...
    // WBrandAgentProcessManager，导致 host 永远不向 main 上报 Agent spawn/exit，进程监控器
    // 因而看不到实际运行的 Agent，也无法验证只读到可写升级是否复用同一进程。
    processLifecycleReporter: options?.processLifecycleReporter,
    spawnFallbackCwd: options?.wbrandAgentSpawnFallbackCwd,
    ...
    // adapter 的 fetch wrapper。判定实现与 CLI 侧共用 @wbrand/shared 的同一份，避免分叉。
    // origin 解析复用 resolveCurrentWBrandEndpointOrigin——与闲时任务同口径（含 settings
    ...
    officialMcpTrustedOrigins: createOfficialMcpTrustedOriginRegistry({
      ...
      resolveWBrandApiOrigin: resolveCurrentWBrandEndpointOrigin,
    }),
    cuaOperationStateReporter: shouldEnableCuaOperationStateReporter({
      ...
      : undefined,
    // WBrand 只发布 turn/session 事实；面板 terminal policy 由 producer coordinator 决定。
    ...
    resolveSpawnEnv: async (context) => {
      ...
      if (!helper && cuaPluginEnabled && process.platform === "darwin") {
        ...
        cuaProductHelperEnv = {
          ...
          [WBRAND_CUA_PLUGIN_AUTHORITY_ENV_KEY]: randomBytes(16).toString("hex"),
        };
      ...
      const telemetryEnv = getCapturedWBrandAgentTelemetryEnv();
      ...
      return {
        ...
        ...buildAgentEndpointOriginEnv(await resolveCurrentWBrandEndpointOrigin()),
        // broker 凭据（socket/token）注入 agent spawn env，让内置 wbrand-cua plugin 的
        // computer-use MCP server 经 __wbrand-plugin-host 恢复 token 后连上 broker。
        ...
        ...createNodeProviderRuntimePathEnv({
          ...
          wbrandBuiltinFilePath: await providerConfigRuntime.resolveWBrandBuiltinActiveFilePath(),
    ...
    ...(isDesktopAttachedRemote
      ...
      : {
          ...
          resolveSessionRuntimePreferences: async (scope) => {
            ...
            const settings = await settingService.get();
            const modelContextBudgetStrategy = DEFAULT_WBRAND_MODEL_CONTEXT_BUDGET_STRATEGY;
            return {
  ...
  providerConnectivityAgentService = wbrandAgentService;
  ...
  hasActiveTurnRef = () => wbrandAgentService.hasActiveCuaOperationTurn();
  // desktop-continuous UI 直接订阅 wbrandSessionService，绕开 WBrand task adapter 的
  ...
  const wbrandTaskIndexSyncer = createWBrandTaskIndexSyncer({
    agentService: wbrandAgentService,
    taskIndexRepo,
  });
  ...
  const wbrandSessionService = createWBrandSessionService({
    agentService: wbrandAgentService,
  ...
  const gitCommitMessageGenerator = new GitCommitMessageGenerator({
    ...
    textGenerator: {
      async generateText(params) {
        return await wbrandAgentService.generateWorkspaceText({
  ...
  // task wrapper 由 WBrand task service adapter 提供；核心 session 状态由 WBrand agent server 维护。
  const wbrandTaskService = createWBrandTaskServiceAdapter({
    wbrandAgentService,
    taskIndexRepo,
    taskIndexSyncer: wbrandTaskIndexSyncer,
    settingService,
  ...
  const wbrandJwtLogoutLogger = createServiceLogger("wbrand-jwt-logout");
  wbrandJwtLogoutHandlerRef.current = (input, headers) => {
      ...
      .then((invalidated) => {
        ...
        if (invalidated) {
          void broadcastService.send({
            channel: WBRAND_JWT_INVALID_BROADCAST_CHANNEL,
            payload: {},
          });
        }
      })
      .catch((error) => {
        wbrandJwtLogoutLogger.warn("WBrand JWT logout failed", { error });
      });
  };
  ...
  const offPeakCredentialResolverDeps = {
    ...
    resolveAccountProvider: async () => {
      ...
      const staticAccess = wbrandProviderAccountAccessSchema.parse(config.access.toJSON());
      const access = await accountRequestAuthService.resolveAccessCurrent(staticAccess);
      if (!access) return null;
      return {
        providerId: provider.providerId,
        access: wbrandAccountAccessSchema.parse(access),
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    apiClient,
    baseUrl: buildRuntimeWBrandApiUrl(process.env, "/api/v1"),
    tokenProvider: async (): Promise<string | null> => {
      ...
      return tokenSet?.wbrandJwtToken ?? tokenSet?.accessToken ?? null;
    },
  });
  ...
  const services = new ServiceCollection()
    ...
    .register(IWBrandTaskService, wbrandTaskService)
    .register(IWBrandAgentService, wbrandAgentService)
    .register(IWBrandSessionService, wbrandSessionService)
    ...
    .register(
      ...
      createClientConfigService({
        ...
        resolveRequestContext: async () => ({
          endpointOrigin: await resolveCurrentWBrandEndpointOrigin(),
          appVersion: WBRAND_VERSION,
      ...
      (() => {
        ...
        const offPeakTaskService = new OffPeakTaskService({
          ...
          stopRunningTask: async (params) => {
            await wbrandTaskService.stopGeneration({
        ...
        // 回写前向引用，供 wbrandAgentService 的 offPeak/create、offPeak/list 协议 handler 调用。
    ...
    .register(IPluginManagementService, createPluginManagementService({ wbrandAgentService }))
...
/** 仅给同一事件账号返回当前 WBrand JWT；不缓存、不修改登录凭据。 */
export function createTelemetryAuthorizationLoader(
  ...
  return async (userId) => {
    ...
    try {
      ...
      const jwt = (await credentialService.load("wbrandjwttoken"))?.trim();
...
): () => Promise<import("@wbrand/shared").OAuthLoginAttribution | null> {
...
export function disposeServiceResources(services: ServiceCollection): void {
  ...
  const disposableServices = [
    ...
    services.getOptional(IWBrandTaskService),
    services.getOptional(IWBrandAgentService),
    services.getOptional(IWBrandSessionService),
...
export async function disposeServiceResourcesAndWait(services: ServiceCollection): Promise<void> {
  ...
  // 旧的同步 dispose 会在 host 退出时丢掉强杀 timer，导致 wbrand-cli/app-server 变成孤儿进程。
  const disposableServices = [
    services.getOptional(ITerminalService),
    services.getOptional(IWBrandTaskService),
    services.getOptional(IWBrandAgentService),
