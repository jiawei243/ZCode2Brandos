import type { AppSettings } from "@wbrand/shared";
...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "wbrandEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.wbrandEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedWBrandEndpointOrigin = normalizedPatch.wbrandEndpointOrigin.trim();
    normalizedPatch.wbrandEndpointOrigin =
      trimmedWBrandEndpointOrigin.length > 0 ? trimmedWBrandEndpointOrigin : undefined;
  }
