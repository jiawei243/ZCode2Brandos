import type {
  ...
  ModelSelection,
  WBrandSessionMode,
  WBrandSessionSettingsState,
  WBrandSessionStateSnapshot,
  WBrandTaskModeInfo,
} from "@wbrand/shared";
import {
  formatModelPickerValue as formatSharedModelSelection,
  getWBrandAgentAvailableModes as getSharedWBrandAgentAvailableModes,
  normalizeAvailableWBrandMode as normalizeSharedAvailableWBrandMode,
...
function resolveTaskMetaModelSelectionFromSnapshot(
  snapshot: Pick<WBrandSessionStateSnapshot, "messages" | "settings">,
): ModelSelection | undefined {
...
export function normalizeAvailableWBrandMode(mode: WBrandSessionMode): string {
  return normalizeSharedAvailableWBrandMode(mode);
}
