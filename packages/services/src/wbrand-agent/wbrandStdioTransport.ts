...
import { wbrandProtocolMessageSchema } from "@wbrand/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  WBrandProtocolTransport,
  WBrandProtocolTransportClosedEvent,
} from "./wbrandProtocolTransport.js";
import {
...
interface WBrandStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("wbrand-agent-process-tree");

export class WBrandStdioTransport implements WBrandProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<WBrandProtocolMessage>();
  private readonly closeEmitter = new Emitter<WBrandProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: WBrandStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // WBrand Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: WBrandProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("WBrand agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.WBRAND_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = wbrandProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // wbrand-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: WBrandProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;
