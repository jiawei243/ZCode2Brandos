...
class WBrandV4CommandRejectedError extends Error {
  readonly code = "WBRAND_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "WBrandV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new WBrandV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new WBrandV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
