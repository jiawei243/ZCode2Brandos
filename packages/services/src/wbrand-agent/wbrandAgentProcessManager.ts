import { resolveWBrandAgentSpawnCwd } from "#src/wbrand-agent/wbrandAgentSpawnCwd.js";
import type { WBrandAgentStorageStartupSnapshot } from "#src/wbrand-agent/wbrandAgent.js";
/* eslint-disable max-lines -- wbrandAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseWBrandProcessDiagnostic,
  WBRAND_AGENT_LIFECYCLE_LOG_MARKER,
  WBRAND_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  WBRAND_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  WBRAND_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@wbrand/shared/process-diagnostic";
import {
  WBRAND_AGENT_RUNTIME,
  WBRAND_AGENT_PROVIDER,
  WBRAND_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveWBrandRuntimeEnv,
  sanitizeWBrandRuntimeEnv,
} from "@wbrand/shared";
import {
  findWBrandAgentRuntimeBinary,
  findWBrandAgentRuntimeNodeBundle,
...
import { WBrandProtocolClient } from "./wbrandProtocolClient.js";
import { WBrandStdioTransport } from "./wbrandStdioTransport.js";
import { readWBrandStdioTapDevState } from "./wbrandStdioTapDevConfig.js";
import type { WBrandAgentPresentationSurface } from "./wbrandAgentPresentationSurface.js";
...
export interface WBrandAgentCommand {
...
export interface WBrandAgentCommandResolverContext {
  presentationSurface?: WBrandAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type WBrandAgentCommandResolver = (
  context: WBrandAgentCommandResolverContext,
) => Promise<WBrandAgentCommand | null> | WBrandAgentCommand | null;

export interface WBrandAgentProcessManagerOptions {
  commandResolver?: WBrandAgentCommandResolver;
  presentationSurface?: WBrandAgentPresentationSurface;
...
interface ManagedWBrandAgentProcess {
  ...
  runtimeIdentity: WBrandAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.WBRAND_E2E_ARTIFACT_DIR?.trim();
  if (env.WBRAND_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "wbrand-e2e-coverage-preload.cjs");
 ...
 */
interface WBrandAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: WBrandAgentRuntimeIdentity;
}

export interface WBrandAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: WBrandAgentRuntimeIdentity;
...
interface WBrandAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface WBrandAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("wbrand-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("WBRAND_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildWBrandAgentSpawnPreflight(
  command: WBrandAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<WBrandAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveWBrandAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceWBrandAgentCommand(
  context: WBrandAgentCommandResolverContext,
): WBrandAgentCommand | null {
  const distEntrypoint = findUpward("apps/wbrand-cli/packages/cli/dist/wbrand.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.WBRAND_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "wbrand.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 wbrand-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/wbrand-cli/packages/cli/src/main.ts");
...
function resolveDeployedWBrandAgentBinaryCommand(
  context: WBrandAgentCommandResolverContext,
): WBrandAgentCommand | null {
  // 旧 resolver 只识别 WBRAND_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 wbrand-server.cjs 单文件部署到 ~/.wbrand/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 wbrand-agent 已经部署到 ~/.wbrand/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "WBrand agent server command is not configured"。
  // 这里复用 findWBrandAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.wbrand/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findWBrandAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: WBRAND_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeWBrandAgentCommand(
  context: WBrandAgentCommandResolverContext,
): WBrandAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 wbrand-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 wbrand.cjs，
  ...
  const bundlePath = findWBrandAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...WBRAND_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultWBrandAgentCommand(
  context: WBrandAgentCommandResolverContext,
): WBrandAgentCommand | null {
  const command = process.env.WBRAND_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.WBRAND_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.WBRAND_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 wbrand.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceWBrandAgentCommand(context) ??
    resolveElectronRuntimeWBrandAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedWBrandAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: WBrandAgentCommand | null,
  presentationSurface: WBrandAgentCommandResolverContext["presentationSurface"],
): WBrandAgentCommand | null {
...
function wrapWBrandAgentCommandWithStdioTapDevProxy(
  command: WBrandAgentCommand,
  workspaceKey: string,
): WBrandAgentCommand {
  const tapState = readWBrandStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/wbrand-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("WBrand stdio tap proxy enabled but script not found");
    return command;
  }
...
export class WBrandAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedWBrandAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedWBrandAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<WBrandProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: WBrandAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): WBrandAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: WBrandAgentCommandResolver;
  private readonly presentationSurface: WBrandAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: WBrandAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: WBrandAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<WBrandAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<WBrandAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 WBrandAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: WBrandAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultWBrandAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("WBrand agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedWBrandAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedWBrandAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("WBrand agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private reportRuntimeUnavailable(managed: ManagedWBrandAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedWBrandAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("WBrand agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("WBrand agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`WBrand agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<WBrandProtocolClient> {
    if (this.disposed) {
      throw new Error("WBrand agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("WBrand agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("WBrand agent process start wait completed", {
        workspaceKey,
  ...
  getExistingClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): WBrandProtocolClient | undefined {
    const managed = this.processesByWorkspaceKey.get(resolveWorkspaceKey(params));
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "WBrand agent server command is not configured. Set WBRAND_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("WBrand agent process start was cancelled.");
    }
    const effectiveCommand = wrapWBrandAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("WBrand agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("WBrand agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildWBrandAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 WBRAND_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 WBrand 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveWBrandRuntimeEnv(process.env);
    log("WBrand agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeWBrandRuntimeEnv(process.env),
        [WBRAND_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new WBrandStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseWBrandProcessDiagnostic(line);
    ...
    const client = new WBrandProtocolClient(transport, {
    ...
    const runtimeIdentity: WBrandAgentRuntimeIdentity = {
    ...
    const managed: ManagedWBrandAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("WBrand agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `WBrand agent process error${this.processLifecycleReporter?.onError ? ` ${WBRAND_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("WBrand agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `WBrand agent process exited unexpectedly${this.processLifecycleReporter ? ` ${WBRAND_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "WBrand agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("WBrand agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("WBrand agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<WBrandAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("WBrand agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "WBRAND_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("WBrand agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("WBrand agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，wbrand-cli 会残留为孤儿进程。
