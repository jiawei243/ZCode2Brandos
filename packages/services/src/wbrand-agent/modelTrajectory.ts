...
} from "#src/session/wbrandTaskService.js";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import {
  readTrajectoryFileTail,
  resolveModelIODirs,
  sanitizeSessionSegment,
} from "#src/wbrand-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/wbrand-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.wbrand/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - WBrand Agent 把 taskId 当作 sessionId（见 wbrandTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): WBrandModelTrajectoryRecord {
  ...
  const mapped: WBrandModelTrajectoryRecord = {
...
function mapMessages(value: unknown): WBrandModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): WBrandModelTrajectoryContentPart {
...
function mapUsage(value: unknown): WBrandModelTrajectoryUsage | undefined {
