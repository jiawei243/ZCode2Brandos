import type { WBrandSessionEvent } from "@wbrand/shared";
import type { WBrandAgentServiceEvent } from "#src/wbrand-agent/wbrandAgent.js";
...
type SessionServiceEvent = Extract<WBrandAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: WBrandAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: WBrandSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: WBrandSessionEvent,
  next: WBrandSessionEvent,
): WBrandSessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: WBrandAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
