/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 WBrand session 的协议适配。 */
...
import {
  coalesceConsecutiveWBrandAssistants,
  createSessionTraceId,
  decodeCustomModelValue,
  deriveWBrandTaskStatusFromSessionSnapshot,
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  generateTraceId,
  getWBrandGoalActiveIterationCount,
  getWBrandGoalIterationByAssistantMessageId,
  getWBrandUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeWBrandApiRetryStatus,
  attachWBrandBackgroundTaskNotificationToRaw,
  collectWBrandBackgroundTaskNotificationsByToolUseId,
  mergeWBrandBackgroundTaskControlItems,
  parseWBrandBackgroundTaskControlItems,
  parseWBrandBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveWBrandVisibleSessionTitle,
  textFromWBrandMessageParts,
  WBRAND_AGENT_PROVIDER,
  wbrandBackgroundTaskNotificationToolUpdateStatus,
  appendWBrandStreamingToolInputDelta,
  buildWBrandStreamingToolInputPreview,
  createWBrandToolProjectionMemory,
  finalizeWBrandToolProjectionInput,
  forgetWBrandToolProjectionMetadata,
  isWBrandModelRetryRecoveryProgressPayload,
  markWBrandStreamingToolInputPreviewMaterialized,
  resolveWBrandToolProjectionMetadata,
  wbrandApiRetryFromModelNetworkStatusPayload,
  wbrandApiRetryFromStreamRecoveryPayload,
  wbrandTaskNetworkDebugStatusFromPayload,
  shouldMaterializeWBrandStreamingToolInputPreview,
  type WBrandApiRetryStatus,
  type WBrandAssistantMessageFeedback,
  type WBrandBackgroundTaskNotificationInfo,
  type WBrandBackgroundTaskControlItem,
  type WBrandBackgroundTurnAttribution,
  type WBrandCancelTaskCommandResult,
  type WBrandConfigOption,
  type WBrandEnqueueTaskCommandResult,
  type WBrandError,
  type WBrandGoalVerificationTimelineMeta,
  type WBrandImportSessionsResult,
  type WBrandImportableSessionCandidate,
  type WBrandUsage,
  type WBrandPersistedMessage,
  type WBrandPersistedMessagePart,
  type WBrandPersistedToolCall,
  type WBrandPromptAttachment,
  type WBrandProvider,
  type WBrandSessionFile,
  type WBrandTaskGoal,
  type WBrandTaskGoalStats,
  type WBrandTaskMode,
  type WBrandPlanStep,
  type WBrandSlashCommand,
  type WBrandStreamEvent,
  type WBrandTaskCreateResult,
  type WBrandTaskMeta,
  type WBrandTaskClientMode,
  type WBrandTaskRuntimeCommand,
  type WBrandTaskSnapshot,
  type WBrandTaskSnapshotBody,
  type WBrandTaskSnapshotRefContent,
  type WBrandTaskSnapshotToolCallsSlice,
  type WBrandTaskTokenUsageResult,
  type WBrandTodoGroup,
  type WBrandTurnSteerCommandKind,
  type WBrandTurnSteerSource,
  type WBrandWorkspaceEvent,
  type WBrandWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  wbrandContextUsageBreakdownSchema,
  wbrandSessionSettingsStateSchema,
  type WBrandDeliveryKind,
  type WBrandMessagePart,
  type WBrandMessageWithParts,
  type ModelSelection,
  type WBrandPermissionOption,
  type WBrandPermissionRequestParams,
  type WBrandPermissionRequest,
  type WBrandSessionEvent,
  type WBrandSessionMode,
  type WBrandSessionSettingsState,
  type WBrandSessionStateSnapshot,
  type WBrandStateUpdatedNotification,
  type WBrandContextCompactionTimelineMeta,
  type WBrandTimelineMeta,
  type WBrandTimelineStatus,
  type WBrandTimelineTrigger,
  type WBrandToolProjectionMemory,
  type WBrandUserInputRequestParams,
  type WBrandUserInputResponse,
  type WBrandAgentMcpServer,
} from "@wbrand/shared";
import type {
  WBrandTaskListQuery,
  WBrandTaskListResult,
  WBrandWorkspaceEventSubscriptionParams,
  IWBrandTaskService,
  WBrandArchivedTaskDeletionResult,
  WBrandTaskReadyOutcome,
  WBrandTaskTerminalOutcome,
} from "../session/wbrandTaskService.js";
...
import type {
  WBrandTaskIndexReadyEvent,
  WBrandTaskIndexSyncer,
  WBrandTaskIndexTerminalEvent,
} from "./wbrandTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@wbrand/shared/wbrand-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./wbrandV4HostCommand.js";
...
import {
  ...
  getWBrandAgentAvailableModes,
  normalizeAvailableWBrandMode,
  settingsToConfigOptions,
} from "./wbrandConfigOptions.js";
...
interface CreateWBrandTaskServiceAdapterOptions {
  wbrandAgentService: IWBrandAgentService;
  ...
  taskIndexSyncer: WBrandTaskIndexSyncer;
...
type WorkspaceEventInput = string | WBrandWorkspaceEventSubscriptionParams;
type WBrandSendPromptRuntimeCommand = Extract<WBrandTaskRuntimeCommand, { type: "send_prompt" }>;
type WBrandTerminalStreamEvent =
  | Extract<WBrandStreamEvent, { type: "task_complete" }>
  | Extract<WBrandStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: WBrandProvider = WBRAND_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: WBrandSlashCommand[] = [];
const logger = createServiceLogger("wbrand-task-service");
...
function formatWBrandAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveWBrandAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.WBRAND_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".wbrand", "cli", "log");
  return join(logDir, `wbrand-${formatWBrandAgentLogDate(now)}.jsonl`);
}

export function createWBrandTaskServiceAdapter(
  options: CreateWBrandTaskServiceAdapterOptions,
): IWBrandTaskService {
  const errorEmitter = new Emitter<WBrandError>();
  const taskEmitters = new Map<string, Emitter<WBrandStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<WBrandStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, WBrandTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, WBrandApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, WBrandBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, WBrandToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`WBrand task service adapter does not support IWBrandTaskService.${name} yet.`),
      {
        code: "WBRAND_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: WBrandBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: WBrandAgentMcpServer[] | undefined,
  ): Promise<WBrandAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    // WBrand task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 WBrand session 事件回投 WBrand Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "WBrand task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.wbrandAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.wbrandAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "WBrand task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "WBrand task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: WBrandTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: WBrandSendPromptRuntimeCommand,
  ): WBrandSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: WBrandSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: WBrandSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "WBrand task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: WBrandTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is WBrandSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "WBrand task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "WBrand task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "WBrand task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<WBrandStreamEvent, { type: "tool_call_update" }>["status"],
  ): WBrandPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: WBrandStreamEvent): void {
    ...
    const nextTool: WBrandPersistedToolCall =
  ...
  function persistedToolId(tool: WBrandPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: WBrandPersistedToolCall | undefined,
    liveTool: WBrandPersistedToolCall,
  ): WBrandPersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly WBrandPersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly WBrandPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly WBrandPersistedMessagePart[] | undefined,
    toolIndex: number,
  ...
  function updateSnapshotMessageTool(
    message: WBrandPersistedMessage,
    toolIndex: number,
    tool: WBrandPersistedToolCall,
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    meta: WBrandTaskMeta,
    messages: WBrandPersistedMessage[],
  ): WBrandPersistedMessage[] {
    ...
    if (changed) {
      logger.debug(undefined, "WBrand snapshot 合并 live tool projection", {
        event: "wbrand_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`WBrand session target is not loaded: ${taskId}`), {
        code: "WBRAND_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): WBrandToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createWBrandToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<WBrandWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<WBrandStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<WBrandStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<WBrandStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: WBrandStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: WBrandTaskMeta | undefined,
    reason: WBrandWorkspaceTaskListChanged["reason"],
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.wbrandAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.wbrandAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 WBrandSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.wbrandAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.wbrandAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: WBrandSessionStateSnapshot): WBrandTaskMeta {
    const target = {
    ...
    return applyOverlayToMeta(
      {
        ...
        mode: fromWBrandMode(snapshot.session.mode),
        ...
        status: deriveWBrandTaskStatusFromSessionSnapshot(snapshot),
        ...
        target: snapshot.projection.target
          ? fromWBrandGoal(snapshot.projection.target)
  ...
  function rememberIndexedTaskMeta(meta: WBrandTaskMeta): WBrandTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 WBrand session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: WBrandTaskMeta): Promise<WBrandTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 WBrand session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: WBrandStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: WBrandStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToWBrand(
    ...
    const goalIterationByAssistantMessageId = getWBrandGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectWBrandBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveWBrandAssistants(
              getWBrandUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: WBrandPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<WBrandStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseWBrandBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: WBrandAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 WBrand snapshot 到 task index 失败", error);
      });
      const snapshotEvent: WBrandStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: WBrandTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: WBrandTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeWBrandAgentServiceAndWait(): Promise<void> {
    const agentService = options.wbrandAgentService as IWBrandAgentService & {
  ...
  const service: IWBrandTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.wbrandAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.wbrandAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<WBrandTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: WBrandSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.wbrandAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.wbrandAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.wbrandAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        // 迁移属 v4 生命周期收口。
        snapshot = await options.wbrandAgentService.createSession({
          ...target,
          sessionTraceId: createSessionTraceId(),
          mode: toWBrandMode(params.mode),
          model: requestedSelection,
          thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<WBrandEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: WBrandTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<WBrandEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<WBrandCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "WBrand task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "WBrand task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "WBrand task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "WBrand task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "WBrand task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.wbrandAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[wbrand-task-service] goalSession start", {
      ...
      const result = await options.wbrandAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[wbrand-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[wbrand-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.wbrandAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<WBrandTaskMeta> {
    ...
    async listTasks(params): Promise<WBrandTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<WBrandTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: WBrandTaskListQuery): Promise<WBrandTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<WBrandTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<WBrandTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<WBrandTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<WBrandTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const wbrandSnapshot = limitTaskSnapshotMessages(
        snapshotToWBrand(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(wbrandSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[wbrand-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(wbrandSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...wbrandSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[wbrand-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[wbrand-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[wbrand-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<WBrandTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<WBrandTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<WBrandTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<WBrandTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<WBrandConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.wbrandAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<WBrandSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as WBrandAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<WBrandImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<WBrandImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.wbrandAgentService.createSession({
          ...
          // 导入后的任务必须是真实 WBrand session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toWBrandMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<WBrandConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.wbrandAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toWBrandMode(params.value as WBrandTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<WBrandConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.wbrandAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<WBrandConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveWBrandAgentCurrentLogFilePath();
      // 返回 WBrand Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // WBrand Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[WBrandTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<WBrandTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 WBrand Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.wbrandAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.wbrand-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<WBrandArchivedTaskDeletionResult> {
      const result: WBrandArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<WBrandTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[WBrandTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[WBrandTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[WBrandTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[WBrandTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[WBrandTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<WBrandTaskMeta> {
    ...
    async setTaskUnread(params): Promise<WBrandTaskMeta> {
    ...
    async archiveTask(params): Promise<WBrandTaskMeta> {
    ...
    async unarchiveTask(params): Promise<WBrandTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<WBrandTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<WBrandStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<WBrandTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<WBrandTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<WBrandStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 WBrandStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.wbrandAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toWBrandDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<WBrandWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.wbrandAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 wbrand-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeWBrandAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: WBrandTaskMeta, overlay: TaskOverlay): WBrandTaskMeta {
  return {
    ...meta,
...
function deriveTitleFromSnapshot(snapshot: WBrandSessionStateSnapshot): string {
  return resolveWBrandVisibleSessionTitle({
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<WBrandTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: WBrandTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 wbrand-cli。
...
function toWBrandMode(mode: WBrandTaskMode | undefined): WBrandSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function fromWBrandMode(mode: WBrandSessionMode): WBrandTaskMode {
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `wbrand-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<WBrandPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<WBrandGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `wbrand-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `wbrand-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: WBrandPersistedMessage[],
  timelineMessages: WBrandPersistedMessage[],
): WBrandPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: WBrandSessionStateSnapshot): number {
  ...
  return getWBrandGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toWBrandDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<WBrandStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseWBrandBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = wbrandBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachWBrandBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: WBrandMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, WBrandBackgroundTaskNotificationInfo>,
): WBrandPersistedMessage {
  const tools: WBrandPersistedToolCall[] = [];
  const parts: WBrandPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: WBrandTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly WBrandMessagePart[],
): WBrandPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<WBrandMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is WBrandPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<WBrandMessagePart, { type: "file" }>,
): WBrandPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<WBrandMessagePart, { type: "text" }>,
): WBrandTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<WBrandMessagePart, { type: "compaction" }>,
): WBrandTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<WBrandMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, WBrandBackgroundTaskNotificationInfo>,
): WBrandPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // WBrand Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachWBrandBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? wbrandBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly WBrandMessagePart[]): string {
  return textFromWBrandMessageParts(parts);
}
...
function fromWBrandGoal(goal: unknown): WBrandTaskGoal {
  ...
  return {
    ...
    status: isWBrandTaskGoalStatus(status) ? status : "active",
...
function isWBrandTaskGoalStatus(status: string | undefined): status is WBrandTaskGoal["status"] {
  return (
    status === "active" ||
    status === "paused" ||
...
function sessionTodosToPlanSteps(
  todos: WBrandSessionStateSnapshot["todos"],
): WBrandPlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: WBrandSessionStateSnapshot["goalStats"],
): WBrandTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: WBrandSessionStateSnapshot["todoGroups"],
): WBrandTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = wbrandSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableWBrandMode(parsedSettings.data.mode.current),
      availableModes: getWBrandAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: WBrandToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, WBrandBackgroundTaskControlItem[]>,
  ...
  // WBrand Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 WBrand prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: WBrandStreamEvent = {
  ...
  if (event.type === "turn.completed") {
    const events: WBrandStreamEvent[] = [];
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "WBrand compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "WBrand session failed",
...
): Extract<WBrandStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isWBrandModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendWBrandStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeWBrandStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildWBrandStreamingToolInputPreview(state.rawInput);
    markWBrandStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildWBrandStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeWBrandToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "WBrand streaming tool input projected", {
    ...details,
    event: "wbrand.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<WBrandStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<WBrandMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): WBrandContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<WBrandStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveWBrandToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeWBrandToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: WBrandStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // WBrand Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 WBrand Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetWBrandToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // WBrand Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: WBrandPermissionRequestParams,
): WBrandPermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: WBrandSessionStateSnapshot["projection"]["pendingPermissions"][number],
): WBrandPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: WBrandUserInputRequestParams,
): WBrandStreamEvent {
  const questions =
...
): Extract<WBrandStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: WBrandSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<WBrandStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: WBrandSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<WBrandStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, WBrandBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: WBrandBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): WBrandBackgroundTaskControlItem[] | null {
  const parsedJobs = parseWBrandBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeWBrandBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): WBrandPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as WBrandPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<WBrandStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = wbrandTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromWBrandGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromWBrandGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseWBrandBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<WBrandStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): WBrandApiRetryStatus | null | undefined {
  if ("apiRetry" in payload) {
    return normalizeWBrandApiRetryStatus(payload.apiRetry);
  }

  const runtimeRetry = normalizeWBrandApiRetryStatus(asRecord(payload.runtime).apiRetry);
  ...
  return (
    wbrandApiRetryFromStreamRecoveryPayload(payload) ??
    wbrandApiRetryFromModelNetworkStatusPayload(payload)
  );
}

function recordAgentModelNetworkTelemetry(event: WBrandSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `wbrand_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<WBrandStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: WBrandSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: WBrandSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // WBrand Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = wbrandContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): WBrandUsage | undefined {
...
function timelineStatusValue(value: unknown): WBrandTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): WBrandTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): WBrandTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): WBrandTurnSteerCommandKind | undefined {
