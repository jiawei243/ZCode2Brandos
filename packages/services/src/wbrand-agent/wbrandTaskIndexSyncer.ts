...
import { repairSubagentTaskIndex } from "#src/wbrand-agent/repairSubagentTaskIndex.js";
import {
  deriveWBrandTaskStatusFromSessionSnapshot,
  generateTraceId,
  getWBrandUserVisibleMessages,
  isWBrandGoalContinuationReminderText,
  isWBrandModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@wbrand/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { WBrandWorkspaceEventSubscriptionParams } from "#src/session/wbrandTaskListTypes.js";
import type {
  IWBrandAgentService,
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./wbrandConfigOptions.js";

const logger = createServiceLogger("wbrand-task-index-syncer");
...
export interface WBrandTaskIndexTerminalEvent {
...
export interface WBrandTaskIndexReadyEvent {
  target: WBrandAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface WBrandTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: WBrandAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，wbrandSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: WBrandWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: WBrandWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: WBrandAgentSessionTarget, model: string): Promise<WBrandTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: WBrandTaskMeta | undefined,
    reason: WBrandWorkspaceTaskListChanged["reason"],
    options?: Pick<WBrandWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): WBrandWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): WBrandTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
export function createWBrandTaskIndexSyncer(
  options: CreateWBrandTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<WBrandWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<WBrandTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<WBrandTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === WBRAND_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<WBrandWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<WBrandWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: WBrandAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: WBrandAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 wbrandSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: WBrandAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 wbrandSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 wbrandSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 wbrandSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useWBrandConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 WBrandConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useWBrandConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: WBrandAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: WBrandSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isWBrandGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isWBrandModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: WBrandSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getWBrandUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<WBrandMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromWBrandGoal(
  goal: NonNullable<WBrandSessionStateSnapshot["projection"]["target"]>,
): WBrandTaskGoal {
  return {
