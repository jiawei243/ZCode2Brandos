...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@wbrand/rpc";
import {
...
export type WBrandAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface WBrandAgentV4ConnectionContext {
  connectionId: string;
  clientMode: WBrandAgentV4ClientMode;
  role?: "terminal-client" | "trusted-host-relay";
}

const TRUSTED_CONNECTION_FIELD = "__wbrandTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__wbrandTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: WBrandAgentV4ConnectionContext;
};

interface TrustedWBrandAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedWBrandAgentV4UnsubscribeRoute;
};

export function readTrustedWBrandAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedWBrandAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedWBrandAgentV4Connection(
  value: unknown,
): WBrandAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: WBrandAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedWBrandAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: WBrandAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: WBrandAgentWorkspaceTarget): WBrandAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: WBrandAgentWorkspaceTarget;
  connection: WBrandAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: WBrandAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: WBrandAgentV4ConnectionContext): HelloMessage {
...
export interface WBrandAgentConnectionScope {
  readonly service: IWBrandAgentService;
...
export function createWBrandAgentConnectionScope(
  base: IWBrandAgentService,
  context: WBrandAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): WBrandAgentV4ConnectionContext => {
    const downstream = readTrustedWBrandAgentV4Connection(params);
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies WBrandAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: WBrandAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedWBrandAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedWBrandAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: WBrandAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: WBrandAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: WBrandAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IWBrandAgentService> = {
