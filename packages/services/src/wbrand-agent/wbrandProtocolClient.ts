import { WBrandStorageStartupGate } from "#src/wbrand-agent/wbrandStorageStartupGate.js";
...
import type { V4Method } from "@wbrand/shared/wbrand-protocol-v4";
import type { z } from "zod";
import type { WBrandProtocolTransport } from "./wbrandProtocolTransport.js";

/** 客户端可发的方法名：旧 wbrandProtocolMethods + v4/*（并存，收敛为 v4）。 */
type WBrandProtocolClientMethod = WBrandProtocolMethod | V4Method;

interface WBrandProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface WBrandProtocolRequestTimeoutEvent {
  method: WBrandProtocolClientMethod;
  requestId: WBrandProtocolRequestId;
  timeoutMs: number;
}
...
interface WBrandProtocolClientRequestOptions {
...
const DEFAULT_WBRAND_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class WBrandProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "WBrandProtocolClientError";
  }
}

export class WBrandProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: WBrandProtocolClientMethod,
    readonly requestId: WBrandProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`WBrand Protocol request timed out: ${method}`);
    this.name = "WBrandProtocolRequestTimeoutError";
  }
}

export class WBrandProtocolClient implements IDisposable {
  readonly storageStartup: WBrandStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<WBrandProtocolNotification>();
  private readonly requestEmitter = new Emitter<WBrandProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<WBrandProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "WBrand Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: WBrandProtocolTransport,
    options?: WBrandProtocolClientOptions,
  ) {
    this.storageStartup = new WBrandStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_WBRAND_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`WBrand agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new WBrandProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: WBrandProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: WBrandProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: WBrandProtocolRequestId; result: unknown }).id,
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as WBrandProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as WBrandProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as WBrandProtocolNotification);
    }
  }

  private resolveResponse(id: WBrandProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`WBrand Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: WBrandProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("WBrand Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("WBrand Protocol client is disposed");
    }
  }
}
