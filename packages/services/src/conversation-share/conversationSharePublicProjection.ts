...
import type { ConversationShareArtifactDescriptor } from "@wbrand/shared";
import type { ArtifactRow, ConversationRow } from "@wbrand/shared/wbrand-protocol-v4";
...
function assertSafeString(value: string): void {
  if (/^wbrand-artifact:\/\//iu.test(value)) {
...
function isPublicArtifactRef(value: string): boolean {
  return /^wbrand-artifact:\/\/share\/[A-Za-z0-9._~-]+$/u.test(value);
}
  ...
  switch (row.kind) {
    ...
    case "artifact": {
      ...
      return {
        ...
        sha256: row.sha256,
        ref: `wbrand-artifact://share/${artifactVersionId}`,
        state: row.state,
      };
    }
    case "timelineMarker":
  ...
  for (const [index, row] of input.rows.entries()) {
    ...
    if (row.kind === "turnHeader") {
      ...
      if (
        ...
        row.ref !== `wbrand-artifact://share/${row.artifactVersionId}`
      ) {
        throwProjectionError(
          "invalid_conversation",
