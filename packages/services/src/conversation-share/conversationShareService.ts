...
import type { ConversationRow } from "@wbrand/shared/wbrand-protocol-v4";
import {
  PROTOCOL_V4_LIMITS,
  WBRAND_ATTACHMENT_FAULT_CODES,
  readWBrandAttachmentFaultCode,
...
import type { IWBrandSessionService } from "#src/wbrand-session/wbrandSession.js";
...
interface ConversationShareServiceOptions {
  wbrandAgentService: ConversationShareAgentService;
  ...
  wbrandSessionService?: Pick<IWBrandSessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^wbrand-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 wbrand-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readWBrandAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === WBRAND_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === WBRAND_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === WBRAND_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === WBRAND_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === WBRAND_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === WBRAND_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readWBrandAttachmentFaultCode(error);
  return (
    faultCode === WBRAND_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === WBRAND_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly wbrandAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly wbrandSessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.wbrandAgentService = options.wbrandAgentService;
    ...
    this.wbrandSessionService = options.wbrandSessionService;
    ...
    // 兜底写死生产站 https://wbrand.unew.cc/cn/share，于是测试环境（API base 走
    ...
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.WBRAND_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeWBrandEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".wbrand-share-imports.json");
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.wbrandAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".wbrand-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".wbrand-share-import.json");
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.wbrand-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.wbrandSessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .wbrand-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".wbrand-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.wbrandAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.wbrandAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      wbrandAgentService: agentService,
