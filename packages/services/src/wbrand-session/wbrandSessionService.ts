/* eslint-disable max-lines -- wbrandSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IWBrandAgentService } from "#src/wbrand-agent/wbrandAgent.js";
import type { WBrandTaskIndexSyncer } from "#src/wbrand-agent/wbrandTaskIndexSyncer.js";
...
import type {
  ...
  WBrandSessionEventsParams,
  WBrandSessionListParams,
  WBrandSessionMessagesParams,
  WBrandSessionReadParams,
  WBrandSessionReadWorkspacePresentationParams,
  WBrandSessionResumeParams,
  WBrandSessionSetModeParams,
  WBrandSessionSetModelParams,
  WBrandSessionSetThoughtLevelParams,
  WBrandTaskTarget,
  WBrandSessionWorkspaceTarget,
} from "#src/wbrand-session/wbrandSession.js";
import { formatModelPickerValue } from "#src/wbrand-agent/wbrandConfigOptions.js";
import { createWBrandSessionApiRetryRuntimeTracker } from "#src/wbrand-session/wbrandSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/wbrand-session/importedClaudeSessionRepair.js";
import { createWBrandDeferredDraftRegistry } from "#src/wbrand-session/wbrandSessionDraftRegistry.js";
...
interface CreateWBrandSessionServiceOptions {
...
export function createWBrandSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateWBrandSessionServiceOptions): IWBrandSessionService {
  const { withApiRetryRuntime } = createWBrandSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createWBrandDeferredDraftRegistry();

  function notifySyncer(
    target: WBrandTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: WBrandSessionStateSnapshot) {
  ...
  // wbrandSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[wbrand-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: WBrandSessionStateSnapshot,
    params: WBrandSessionResumeParams | WBrandSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends WBrandSessionCreateParams | WBrandSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: WBrandSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: WBrandSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: WBrandSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: WBrandSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[wbrand-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[wbrand-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 WBrand task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[wbrand-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: WBrandSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[wbrand-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[wbrand-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[wbrand-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: WBrandSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: WBrandSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[wbrand-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: WBrandSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: WBrandSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: WBrandTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[wbrand-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: WBrandTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: WBrandTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[wbrand-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: WBrandSessionSetModelParams) {
    ...
    async setThoughtLevel(params: WBrandSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: WBrandSessionSetModeParams) {
