...
import { resolveBigModelStartPlanWbrandJwt } from "./bigmodelStartPlanWbrandJwt.js";
import {
...
function buildZaiSubscriptionListUrl(): string {
  ...
  // availability 校验必须和 OAuth business login 共用同一套 WBRAND_ENV 域名分流，只允许 origin 不同。
...
async function resolveStartPlanAuthorization(
  ...
  if (provider.family === "bigmodel") {
    const wbrandJwtToken = await resolveBigModelStartPlanWbrandJwt({
      credentialService: context.credentialService,
      provider,
    });
    return {
      value: wbrandJwtToken ? `Bearer ${wbrandJwtToken}` : "",
      missingReason: "coding_plan_not_authenticated",
    };
  }

  const credentialJwt = await loadZaiProviderConnectionWbrandJwtToken(context);
...
async function loadZaiProviderConnectionWbrandJwtToken(
  context: CodingPlanAvailabilityContext,
): Promise<string> {
  return (await context.credentialService?.load(WBRAND_JWT_TOKEN_KEY))?.trim() || "";
}
...
function hasActiveStartPlan(plans: ZaiStartPlanPlan[] | undefined): boolean {
  return Boolean(
    plans?.some((plan) => {
      ...
      // billing/balance 的 plans 真实返回的是 `plan_id=wbrand-v3-start-plan`
      // 和 `name=WBrand V3 Start Plan`；只认 `name === "start plan"` 的话，
