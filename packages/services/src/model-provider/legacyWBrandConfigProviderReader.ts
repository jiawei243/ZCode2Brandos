/* oxlint-disable eslint(max-lines) -- 已发布旧 WBrand config.json 的多版 Provider 结构读取集中在同一边界。 */
...
function normalizeBigModelCodingPlanAnthropicBaseUrlForEnv(
  ...
  const fallbackBaseUrl =
    resolveRuntimeWBrandEnv(env) === "production"
  ...
  if (resolveRuntimeWBrandEnv(env) === "production") {
...
function getWBrandConfigFilePath(): string {
...
interface WBrandConfigFile {
  $schema?: string;
  provider?: Record<string, WBrandOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface WBrandOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, WBrandOpenCodeModelConfig>;
  wbrand?: WBrandProviderConfigExtension;
  [key: string]: unknown;
}

interface WBrandOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | WBrandReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  wbrand?: WBrandModelConfigExtension;
  [key: string]: unknown;
}

interface WBrandReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface WBrandProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface WBrandModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const wbrandModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const wbrandProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const wbrandReasoningConfigExtensionSchema = z
  .object({
...
const wbrandModelConfigExtensionSchema = z
  .object({
...
const wbrandOpenCodeModelConfigSchema: z.ZodType<WBrandOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), wbrandReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(wbrandModelProviderModalitySchema).optional(),
        output: z.array(wbrandModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    wbrand: wbrandModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const wbrandProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: wbrandProviderModelMappingsSchema.optional(),
...
const wbrandOpenCodeProviderConfigSchema: z.ZodType<WBrandOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), wbrandOpenCodeModelConfigSchema).optional(),
    wbrand: wbrandProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const wbrandConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), wbrandOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: WBrandOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: WBrandOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/wbrand 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolvePresetProviderKindFromRuntimeBaseUrl(
  ...
  if (path.includes("/api/anthropic") || path.includes("/wbrand-plan/anthropic")) {
    return "anthropic";
  }
  return undefined;
}
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.wbrand?.endpoints ?? {};
}

function stripWBrandPrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("wbrand"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: WBrandOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.wbrand?.reasoning) {
    return stripModelProviderReasoningPatches(model.wbrand.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const wbrand = options.preferOpenCodeFields ? undefined : model.wbrand;
  const wbrandExtensions = model.wbrand;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(wbrandExtensions?.priority),
    disabledReason: model.disabledReason ?? wbrand?.disabledReason,
    supportsTools: model.supportsTools ?? wbrand?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? wbrand?.supportsStructuredOutput,
    modified: model.modified ?? wbrandExtensions?.modified,
    deleted: model.deleted ?? wbrandExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  for (const deletedModelId of normalizeDeletedModelIds(wbrand?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? wbrand?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripWBrandPrefixedProviderMappings(provider.providerMappings ?? wbrand?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : wbrand?.enabled !== undefined
        ? { enabled: wbrand.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? wbrand?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? wbrand?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ? resolveModelProviderKindApiFormat(defaultKind)
      : (provider.apiFormat ?? wbrand?.apiFormat ?? resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? wbrand?.source)
      ? { source: provider.source ?? wbrand?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? wbrand?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? wbrand?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? wbrand?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? wbrand?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? wbrand?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? wbrand?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? wbrand?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? wbrand?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? wbrand?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? wbrand?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? wbrand?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? wbrand?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? wbrand?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? wbrand?.updatedAt) : undefined) ?? now,
  });
}

async function readRawWBrandConfigFile(): Promise<WBrandConfigFile | null> {
  const filePath = getWBrandConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = wbrandConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readWBrandConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawWBrandConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripWBrandPrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 WBrand Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyWBrandConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readWBrandConfigProviders();
