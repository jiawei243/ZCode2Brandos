...
// to @wbrand/wbrand-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @wbrand/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @wbrand/services root
...
// at compile time; Vite never resolves @wbrand/wbrand-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@wbrand/wbrand-cua/broker/ports";
