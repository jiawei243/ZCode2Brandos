...
const DEV_ROOT_ENV = "WBRAND_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@wbrand/wbrand-cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.wbrandCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid wbrandCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}
