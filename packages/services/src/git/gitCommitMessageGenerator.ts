...
interface GitCommitMessageCurrentModelProvider {
  ...
  }): Promise<WBrandWorkspaceGenerateTextParams["selection"] | null>;
}
...
interface GitCommitMessageTextGenerator {
  generateText(params: {
    ...
    prompt: string;
    querySource: string;
  }): Promise<{ text: string; selection: WBrandWorkspaceGenerateTextParams["selection"] }>;
}
...
export class GitCommitMessageGenerator {
  ...
  private async resolveCurrentModel(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<WBrandWorkspaceGenerateTextParams["selection"]> {
