...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@wbrand/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/wbrand/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "WBrand Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@wbrand.local",
    GIT_COMMITTER_NAME: "WBrand Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@wbrand.local",
  };
}
