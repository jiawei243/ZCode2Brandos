...
const WBRAND_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "wbrand",
  directorySource: "wbrand",
  userConfigDirSegments: [".wbrand", "cli"],
  workspaceConfigDirSegments: [".wbrand"],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserWbrandMcpConfigPath()));
    },
...
function getUserWbrandMcpConfigPath(): string {
  return buildUserConfigPath(WBRAND_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const wbrandRecords = await readUserMcpRecordsFromFile(WBRAND_MCP_DESCRIPTOR);
  if (wbrandRecords.length > 0) {
    return sortMcpRecords(wbrandRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(WBRAND_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeWBrandServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const wbrandServers = await readDirectoryServersFromFile(
    WBRAND_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (wbrandServers.length > 0) {
    return wbrandServers;
  }
...
async function writeWBrandServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(WBRAND_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, WBRAND_MCP_DESCRIPTOR.configKeyName, servers);
  await writeTextAtomic(filePath, `${JSON.stringify(next, null, 2)}\n`);
}

async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserWbrandMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserWbrandMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserWbrandMcpConfigPath();
  const current = (await readJsonObject(targetPath)) ?? {};
  const targetServers = readServerMapFromJson(current, WBRAND_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, WBRAND_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }
