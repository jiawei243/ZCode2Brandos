...
export interface HostResourceUsageAgent {
  ...
  children: readonly WBrandProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: WBrandProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatWBrandAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
