...
export function createSkillSyncService(options?: { maxArchiveBytes?: number }): ISkillSyncService {
  ...
  return {
    ...
    async listRemoteUserSkillStatuses(params): Promise<SkillSyncRemoteStatusResult> {
      const root = getUserWbrandSkillRoot();
    ...
    async checkRemoteUserSkillWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(getUserWbrandSkillRoot());
    },
...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const userWbrandSkillRoot = getUserWbrandSkillRoot();
  const rawWbrandCandidates = await collectUserSkillCandidatesInRoot(userWbrandSkillRoot);
  const wbrandDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userWbrandSkillRoot);
  const wbrandSkillNameKeys = new Set(
    rawWbrandCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const wbrandCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawWbrandCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !wbrandDirectoryNames.has(candidate.directoryName) &&
        !wbrandSkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...wbrandCandidates, ...agentsCandidates].sort((left, right) =>
...
async function collectUserSkillDirectoryPathByName(): Promise<Map<string, string>> {
  const result = await collectSkillDirectoryPathByName(getUserWbrandSkillRoot());
...
async function importArchive(
  ...
  const targetRoot = getUserWbrandSkillRoot();
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 .wbrand/skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 .wbrand 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();
