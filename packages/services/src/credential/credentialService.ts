...
async function readAll(credentialsFile = getCredentialsFile()): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read WBrand credentials: ${credentialsFile}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`WBrand credentials are corrupt: ${credentialsFile}`, { cause: error });
  }
}
