...
import { buildWBrandToolEnvPassthroughEnv, sanitizeWBrandRuntimeEnvInPlace } from "@wbrand/shared";
...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildWBrandToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildWBrandToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. WBrand/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 WBRAND_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeWBrandRuntimeEnvInPlace(process.env);
