...
import { resolveWBrandEndpointOrigin, pickProductEndpointEnv } from "@wbrand/shared/wbrandEndpoint";
...
function resolveWBrandEnv(value: string | undefined): "test" | "production" {
...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "wbrand:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 WBRAND_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const wbrandEnv = resolveWBrandEnv(env.WBRAND_ENV);
  // 安装包身份与后端环境分轴；renderer 用它决定是否展示更新入口。
  const wbrandProductFlavor = resolveDesktopProductFlavor({
    ...process.env,
    ...env,
    WBRAND_ENV: wbrandEnv,
  });
  const e2eCoverageEnabled =
    env.WBRAND_E2E_COVERAGE === "1" || process.env.WBRAND_E2E_COVERAGE === "1";
  const e2eStoreBridgeEnabled =
    env.VITE_WBRAND_E2E_STORE_BRIDGE === "1" || process.env.VITE_WBRAND_E2E_STORE_BRIDGE === "1";
  const wbrandEndpointOrigin = resolveWBrandEndpointOrigin({
    env: wbrandEnv,
    envBaseOrigin: env.WBRAND_BASE_URL ?? env.WBRAND_ENDPOINT_ORIGIN,
  });
  ...
  return {
    ...
    // 临时：5174 被本机其他项目的 vite dev server 占用，dev 调试改用 5274（还原时改回 5174）。
    server: { port: 5274, strictPort: true },
    define: {
      __WBRAND_ENDPOINT_ENV__: JSON.stringify(pickProductEndpointEnv(env)),
      __WBRAND_VERSION__: JSON.stringify(buildMetadata.appVersion),
      __WBRAND_COMMIT__: JSON.stringify(buildMetadata.buildCommitId),
      __WBRAND_BUILD_TIME__: JSON.stringify(buildMetadata.buildTime),
      __WBRAND_ENV__: JSON.stringify(wbrandEnv),
      __WBRAND_PRODUCT_FLAVOR__: JSON.stringify(wbrandProductFlavor),
      __WBRAND_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      "import.meta.env.VITE_WBRAND_BASE_URL": JSON.stringify(wbrandEndpointOrigin),
      // 兼容旧 renderer 读取名；新代码统一读 VITE_WBRAND_BASE_URL。
      "import.meta.env.VITE_WBRAND_ENDPOINT_ORIGIN": JSON.stringify(wbrandEndpointOrigin),
      ...
      // E2E store bridge 只能由 WDIO 专用变量打开，避免把 WBRAND_ENV=test 产品环境误当成测试运行态。
      "import.meta.env.VITE_WBRAND_E2E_STORE_BRIDGE": JSON.stringify(
