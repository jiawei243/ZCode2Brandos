...
export function createElectronRuntimeMirrorEnv(mirror) {
  return {
    WBRAND_ELECTRON_RUNTIME_MIRROR: mirror,
...
function resolveDefaultElectronBuilderBinariesMirror(env = process.env) {
  return env.WBRAND_DEPS_BASE_URL?.trim() || env.INTRANET_MACHINE_HOST?.trim()
...
环境变量:
  WBRAND_TARGET_OS              与 --os 等价
  WBRAND_TARGET_ARCH            与 --arch 等价
`);
}
...
function parseArgs(argv) {
  const options = {
    os: process.env.WBRAND_TARGET_OS ?? null,
    arch: process.env.WBRAND_TARGET_ARCH ?? null,
    skipPrepare: process.env.WBRAND_SKIP_PREPARE === "1",
    skipBuild: process.env.WBRAND_SKIP_BUILD === "1",
    dryRun: false,
    positionals: [],
  };
