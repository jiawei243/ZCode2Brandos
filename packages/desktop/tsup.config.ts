import { pickProductEndpointEnv } from "@wbrand/shared/wbrandEndpoint";
...
const { environment: wbrandEnv } = await loadBuiltinProviderConfig();
// 安装包身份与后端环境分轴：WBRAND_PREVIEW_IDENTITY=1 让生产后端的构建仍以 WBrand Preview 身份打包运行。
const wbrandProductFlavor = resolveDesktopProductFlavor({ ...process.env, WBRAND_ENV: wbrandEnv });
console.log(`[tsup] WBRAND_ENV=${wbrandEnv} WBRAND_PRODUCT_FLAVOR=${wbrandProductFlavor}`);

export function resolveDesktopTsupBundleSecurityOptions(
  runtimeEnv: Record<string, string | undefined> = process.env,
) {
  const isProduction = runtimeEnv.NODE_ENV === "production";
  const isE2ECoverageBuild = runtimeEnv.WBRAND_E2E_COVERAGE === "1";
  return {
...
function createSharedDefines() {
  return {
    ...
    // 缺失时 installer 抛 "Packaged WBrand is missing its embedded Computer Use Helper build identity"。
    // CI 构建时通过 WBRAND_CUA_HELPER_BUILD_ID env 注入；dev 为空串走兜底（dev helper 不走下载）。
    __WBRAND_CUA_HELPER_BUILD_ID__: JSON.stringify(
      process.env.WBRAND_CUA_HELPER_BUILD_ID?.trim() ?? "",
    ),
    // 客户端只有一个 CDN 配置，与发布端 OSS 目标列表分离。
    __WBRAND_CDN_BASE_URL__: JSON.stringify(env.WBRAND_CDN_BASE_URL?.trim() || ""),
  };
}
...
export default defineConfig([
  {
    ...
    entry: {
      ...
      "main/wbrandDataSizeWorker": "src/main/wbrandDataSizeWorker.ts",
    ...
    noExternal: [
      ...
      "@wbrand/provider",
      "@wbrand/provider-node",
      // services 已内联进 main，但其 producer import 曾被保留为裸包引用；
      // electron-builder 又会排除 node_modules/@wbrand，导致安装包启动即 ERR_MODULE_NOT_FOUND。
  ...
  },
  {
    ...
    platform: "node",
    target: "node22",
    // 与 host 同构：常驻 cron scheduler 进程复用 @wbrand/services（tasks-index + cron），
