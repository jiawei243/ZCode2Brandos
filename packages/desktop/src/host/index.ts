...
import {
  ...
  createWBrandAgentConnectionScope,
  type WBrandAgentV4ClientMode,
...
import {
  ...
  type WBrandAutomationRunOutcome,
  type ModelSelection,
} from "@wbrand/shared";
import {
...
import type {
  ...
  RemoteConnection,
} from "@wbrand/server/remote";
import type { RemoteTarget } from "@wbrand/shared";
...
import { createRemoteConnectionProgressContext } from "@wbrand/server/remote/remoteConnectionProgressContext.js";
...
const remoteMediaRangePreviewEnabled =
  process.env["WBRAND_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 wbrand-* title，方便系统进程列表过滤。
process.title = formatWBrandHostProcessName(process.env["WBRAND_PROCESS_LABEL"]);
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("wbrand-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 wbrand-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  wbrandTaskService: IWBrandTaskService;
  taskId: string;
  workspacePath: string;
  workspaceIdentity?: string;
}): Promise<number | undefined> {
  try {
    const snapshot = await params.wbrandTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  void params.wbrandTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.wbrandTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        wbrandTaskService: params.wbrandTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  if (!wbrandTaskService) {
    throw new Error("WBrand task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        wbrandTaskService.listDeletedTaskIds(workspaceScope),
        wbrandTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      const task = await wbrandTaskService.createTask({
        workspacePath: request.workspacePath,
        workspaceIdentity: request.workspaceIdentity,
        // 空 Session 沿用普通初始化；idle Selection 只在下方执行中注入。
        // 在此写入会让闲时轮结束后的普通消息继续使用无票的隐藏 Provider。
        mode: request.permissionMode as WBrandTaskMode,
    ...
    await wbrandTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const wbrandTaskService = targetServices.getOptional(IWBrandTaskService);
  if (!wbrandTaskService) {
    throw new Error("WBrand task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await wbrandTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isWBrandTaskMeta(value: unknown): value is WBrandTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteWBrandTaskService<T extends object>(
  ...
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: WBrandPromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: WBrandTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // wbrandSessionStore 的整条消费链词表都是 WBrandStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→WBrandStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 wbrand-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: WBrandTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: WBrandTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote WBrand task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote WBrand task ready event is not subscribable");
    }
  ...
  // remote workspace 的 WBrand Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: WBrandTaskMeta; started: boolean } | undefined;
...
function warmUpWBrandAgent(
  ...
  const wbrandSessionService = services.getOptional(IWBrandSessionService);
  if (!wbrandSessionService) {
    return;
  }
  void wbrandSessionService
    .initializeWorkspace({
      workspacePath,
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `WBrand agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `WBrand agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `WBrand agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`WBrand agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(IWBrandAgentService),
...
async function createWindowRemoteConnectionHandle(params: {
  ...
  const services = createRemoteWorkspaceServiceCollection({
    ...
    createReportingRemoteWBrandTaskService: (service) =>
      createReportingRemoteWBrandTaskService(service, {
  ...
  let disposed = false;
  // 远端 workspace 的 CLI 与 MCP 样本走与本地同一条路径：远端 wbrand-server → 本地 Host → main。
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(IWBrandTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(IWBrandTaskService),
        agentService: services.getOptional(IWBrandAgentService),
    ...
    const taskService = activeServices?.getOptional(IWBrandTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(IWBrandAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<IWBrandTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<IWBrandTaskService["archiveTask"]>[0]
            | Parameters<IWBrandTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<IWBrandTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<IWBrandTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<IWBrandTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<IWBrandTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: WBrandAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(IWBrandAgentService);
  const connectionScope = agentService
    ? createWBrandAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(IWBrandTaskService);
  if (taskService) {
    overrides.set(
      IWBrandTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(IWBrandAgentService.channelName, connectionScope.service);
  }
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as WBrandTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!wbrandTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "WBrand task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              wbrandAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              wbrandBuiltinProviderConfigFilePath: msg.wbrandBuiltinProviderConfigFilePath,
              ...
              // browser-use：agent 的 interaction/browserExecute 经 wbrandAgentService 转到这个 executor，
        ...
        const wbrandTaskService = services.getOptional(IWBrandTaskService);
        if (wbrandTaskService) {
          const reportingWBrandTaskService = createReportingRemoteWBrandTaskService(
            wbrandTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(IWBrandTaskService, reportingWBrandTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@wbrand/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 WBRAND_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: WBRAND_VERSION,
    // 远端 wbrand-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。
