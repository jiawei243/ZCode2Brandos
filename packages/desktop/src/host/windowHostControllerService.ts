...
import { matchesTaskListMembershipKind } from "@wbrand/shared/wbrand-protocol-v4";
import type {
...
interface ResolvedWindowHostControllerSource {
  ...
  taskService?: IWBrandTaskService;
...
function taskKey(task: Pick<WBrandTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: WBrandTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@wbrand/shared/wbrand-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: WBrandTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (scope: WBrandTaskListWorkspaceScope) => ResolvedWindowHostControllerSource | null;
  onSourceError?: (
    scope: WindowHostControllerSourceScope,
  ...
  const sourceTaskServices = new Map<string, IWBrandTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IWBrandTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IWBrandTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IWBrandAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  function resolveQuerySources(
    scopes: WBrandTaskListWorkspaceScope[],
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@wbrand/shared").WindowHostAttachmentScope;
