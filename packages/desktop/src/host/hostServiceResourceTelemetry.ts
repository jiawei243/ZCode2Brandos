...
import { IWBrandAgentService, type ServiceCollection } from "@wbrand/services";
 ...
 * 本机 CLI 是 local，远端 wbrand-server 上的 CLI 是 remote，样本自报的硬件维度由 main 覆盖全局默认值。
...
export function registerHostServiceResourceTelemetry(
  ...
  const agentService = options.services.getOptional(IWBrandAgentService);
  if (!agentService) {
    return NO_TELEMETRY;
  }
