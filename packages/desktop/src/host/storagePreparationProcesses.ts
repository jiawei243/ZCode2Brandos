...
import {
  ...
  wbrandStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@wbrand/shared";
import { resolveDefaultWBrandAgentCommand } from "@wbrand/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultWBrandAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = wbrandStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);
