...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  createReportingRemoteWBrandTaskService: <T extends object>(service: T) => T;
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    tokenProvider: async () =>
      (await localCredentialService.load(WBRAND_JWT_TOKEN_KEY))?.trim() || null,
  });
  const conversationShareService = new ConversationShareService({
    wbrandAgentService: params.connectionServices.wbrandAgentService,
  ...
  const reportingRemoteWBrandTaskService = params.createReportingRemoteWBrandTaskService(
    params.connectionServices.wbrandTaskService,
  );
  ...
  const remoteWBrandTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteWBrandTaskService,
  );
  const remoteWBrandSessionService = params.createRemotePromptAttachmentSessionService(
    params.connectionServices.wbrandSessionService,
  );
  ...
  params.connectionServices.wbrandAgentService.onDynamicSessionRuntimePreferencesRequest()(
    (request) => {
      const startedAt = Date.now();
      const requestContext = {
        event: "wbrand_protocol.runtime_preferences.host_request_received",
      ...
      void (async () => {
        ...
        await params.connectionServices.wbrandAgentService.respondSessionRuntimePreferences({
          requestId: request.requestId,
          resolution,
        });
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、WBrand Agent 仍来自远端，
  ...
  const services = new ServiceCollection()
    ...
    .register(IWBrandTaskService, remoteWBrandTaskService)
    .register(IWBrandAgentService, params.connectionServices.wbrandAgentService)
    .register(IWBrandSessionService, remoteWBrandSessionService)
