...
type PatchedIpcRenderer = IpcRenderer & { __wbrandArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __wbrandArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__wbrandArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__wbrandArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__wbrandArmsIpcPatched) {
    return;
  }
  ...
  patched.__wbrandArmsIpcPatched = true;
}
