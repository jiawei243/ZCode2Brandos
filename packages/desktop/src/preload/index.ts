...
contextBridge.exposeInMainWorld("__WBRAND_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  formatWBrandRendererProcessName,
...
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__wbrandFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatWBrandRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 wbrand-* title，便于按窗口角色筛选。
 ...
 */
contextBridge.exposeInMainWorld("wbrand", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@wbrand/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  reportLocalTtftBatch: (batch: import("@wbrand/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.wbrand/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@wbrand/shared").ChromeBrowserDataImportOptions) =>
  ...
  getWBrandStdioTapDevState: (): Promise<WBrandStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetWBrandStdioTapDevState),
