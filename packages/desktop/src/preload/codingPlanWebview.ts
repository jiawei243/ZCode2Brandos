...
// - 在官网页主世界挂 window.wbrandBridge，暴露三个能力：
...
// - getLang 读 main world 的 window.__wbrandLang__（由 App executeJavaScript 注入）；
//   onLangChange 在 main world 监听 wbrand-coding-plan-lang-change CustomEvent
...
const PUBLIC_BRIDGE_KEY = "wbrandBridge";
const NATIVE_BRIDGE_KEY = "__wbrandCodingPlanWebviewNativeBridge__";
const LANG_VAR = "__wbrandLang__";
const LANG_CHANGE_EVENT = "wbrand-coding-plan-lang-change";
const REPORT_CONTEXT_VAR = "__wbrandReportContext__";
  ...
  try {
    ...
    if (
      !isTrustedCodingPlanWebviewOrigin(url.origin, {
        e2eStoreBridgeEnabled: process.env.VITE_WBRAND_E2E_STORE_BRIDGE === "1",
      })
    ) {
      return false;
    }
    ...
    // 该页支付成功后仍需 wbrandBridge.notifyPurchaseComplete 通知 App 刷新模型设置。
    ...
    notifyPurchaseComplete(payload: NotifyPurchaseCompletePayload) {
      try {
        ipcRenderer.sendToHost(CodingPlanWebviewChannels.PurchaseComplete, {
          ...
          timestamp: Date.now(),
        } satisfies import("@wbrand/shared").CodingPlanPurchaseCompletePayload);
      } catch {
  ...
  contextBridge.executeInMainWorld({
    func: (
      ...
      const bridge = {
        ...
        // App locale 变化时用 executeJavaScript 派发 wbrand-coding-plan-lang-change 事件。
