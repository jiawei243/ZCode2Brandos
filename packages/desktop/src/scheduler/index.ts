...
function resolveScheduledAt(automation: WBrandAutomation, now: number): number {
...
async function handleClaimed(automation: WBrandAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: WBrandAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: WBrandOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: WBrandAutomationTrigger =
