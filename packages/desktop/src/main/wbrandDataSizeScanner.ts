...
export type WBrandDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type WBrandDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: WBrandDataSizePartialReason }
);

export interface WBrandDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface WBrandDataSizeScanOptions extends WBrandDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("WBrand data size scan aborted", "AbortError");
}
 ...
 */
export async function scanWBrandDataDirectory(
  options: WBrandDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<WBrandDataSizePartialReason, "io_error"> | null = null;
...
export function isWBrandDataSizeScanResult(value: unknown): value is WBrandDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<WBrandDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
