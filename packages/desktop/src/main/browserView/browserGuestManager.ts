...
export class BrowserGuestManager {
  ...
  attachGuest(
    ...
    if (guest.getType() !== "webview") {
      ...
      // CDP/Runtime 输入会直接操作 WBrand composer。IAB 只允许真实 <webview> guest fail closed。
      this.log?.(
  ...
  private async installRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    await guest.executeJavaScript(`(() => {
      const id = "__wbrand_browser_recording_cursor";
      ...
      window["__wbrandBrowserRecordingCursorCleanup"]?.();
      ...
      window["__wbrandBrowserRecordingCursorCleanup"] = () => {
        ...
        cursor.remove();
        delete window["__wbrandBrowserRecordingCursorCleanup"];
      };
    })()`);
  }
  ...
  private async removeRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    ...
    await guest
      .executeJavaScript('window["__wbrandBrowserRecordingCursorCleanup"]?.()')
      .catch(() => undefined);
  }
  ...
  private finalizeTabs(
    ...
    // 产品语义：IAB tab 在当前 WBrand 进程内默认持久。keep 是状态标记集合，不是清理白名单；
