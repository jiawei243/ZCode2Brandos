...
import { WBRAND_AGENT_LIFECYCLE_LOG_MARKER } from "@wbrand/shared/process-diagnostic";
import {
...
function hasProductExecutable(
  ...
  // Linux Preview 使用 wbrand-preview。两者都必须精确匹配，不能放宽成前缀以免混入 helper dump。
...
const armsRumEnv = mapWBrandEnvToArmsRumEnv(desktopRuntimeEnv);
...
function startArmsRum(): Promise<void> {
  return armsRum
          ...
          (payload?.events ?? []).filter(
            (event) =>
              !(
                ...
                event.message.includes(WBRAND_AGENT_LIFECYCLE_LOG_MARKER)
              ),
          ),
    ...
    .then(() => {
      logger.info(`[arms] electron initialized env=${armsRumEnv} version=${WBRAND_VERSION}`);
    })
    .catch((error) => {
...
export const armsInitPromise: Promise<void> =
  WBRAND_TELEMETRY_ENABLED && WBRAND_ARMS_RUM_ENDPOINT ? startArmsRum() : Promise.resolve();
