...
function buildElectronManifestUrl(options: {
  ...
  const url = options.manifestUrl?.trim()
    ...
    : new URL(ELECTRON_MANIFEST_API_PATH, normalizeWBrandEndpointOrigin(options.endpointOrigin));
...
export class ManifestUpdateProvider extends Provider<UpdateInfo> {
  ...
  private resolveBaseUrl = new URL(DEFAULT_WBRAND_ENDPOINT_ORIGIN);
  ...
  constructor(
    ...
    this.resolveBaseUrl = new URL(
      normalizeWBrandEndpointOrigin(options.endpointOrigin ?? DEFAULT_WBRAND_ENDPOINT_ORIGIN),
    );
  }
  ...
  override async getLatestVersion(): Promise<UpdateInfo> {
    ...
    return {
      ...
      wbrandReleaseChannel: releaseChannel,
    } as UpdateInfo;
  }
  ...
  private async resolveEndpointOrigin(): Promise<string> {
    const resolved =
      ...
      DEFAULT_WBRAND_ENDPOINT_ORIGIN;
    return normalizeWBrandEndpointOrigin(resolved);
  }
