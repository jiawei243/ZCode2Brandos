...
import { readWBrandStdioTapDevState } from "@wbrand/services/node";
...
import {
  HELP_TOGGLE_WBRAND_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_WBRAND_ENDPOINT_PRODUCTION_MENU_ID = "help.wbrand-endpoint.production";
...
export function updateWBrandStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_WBRAND_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readWBrandStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  wbrandEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readWBrandStdioTapDevState();
      ...
      ? [
          {
            ...
            submenu: [
              ...
              ...(WBRAND_PRODUCT_FLAVOR === "production"
                ? [
                    {
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_WBRAND_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleWBrandStdioTap),
        ...
        ...(WBRAND_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpWBrandEndpoint),
                submenu: [
                  {
                    id: HELP_WBRAND_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpWBrandEndpointProduction),
                    type: "radio" as const,
                    checked: (options.wbrandEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetWBrandEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpWBrandEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetWBrandEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpWBrandEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetWBrandEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        wbrandEndpointSelection: options.wbrandEndpointSelection,
