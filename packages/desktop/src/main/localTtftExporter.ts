...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@wbrand/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "wbrand-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "wbrand.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "wbrand.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@wbrand/local-ttft", "1");
  const duration = meter.createHistogram("wbrand.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("wbrand.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("wbrand.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "wbrand.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("wbrand.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("wbrand.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("wbrand.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("wbrand.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("wbrand.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("wbrand.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("wbrand.local_ttft.records");
  const drops = meter.createCounter("wbrand.local_ttft.dropped");
