...
export { WBRAND_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV };
...
export function createElectronDesktopContextPromptConfigFetcher(options: {
  ...
  return async (signal) => {
    ...
    const url = new URL(`${buildWBrandEndpointUrls(endpointOrigin).origin}/api/v1/client/configs`);
    ...
    return await new Promise<unknown>((resolve, reject) => {
      ...
      const sourceHeaders = buildWBrandSourceHeadersFromContext({
