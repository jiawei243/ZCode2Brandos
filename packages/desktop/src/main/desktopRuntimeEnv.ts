...
import type { ConnectOptions } from "@wbrand/server/remote";
import { listSSHConfigAliasesFromLocalConfig } from "@wbrand/services/node";
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@wbrand/wbrand-cua/broker/helperConstants";
import {
  WBRAND_APP_VERSION_ENV,
  WBRAND_AGENT_RUNTIME,
  WBRAND_DYNAMIC_WORKFLOW_MODE_ENV,
  WBRAND_ENV,
  WBRAND_PRODUCT_FLAVOR,
  WBRAND_RUNTIME_ENV_KEY,
  WBRAND_VERSION,
  buildWBrandToolEnvPassthroughEnv,
...
export const desktopRuntimeEnv: WBrandRuntimeEnv = isLocalDevelopmentRuntime
  ? "development"
  : "production";
// 身份看编译期 flavor 而不是 WBRAND_ENV：WBRAND_PREVIEW_IDENTITY=1 的生产后端构建同样是 Preview，
// 需要独立的应用名、Electron 数据目录和 Helper 安装子目录才能与正式版并排运行。
const isPreviewPackagedRuntime = !isLocalDevelopmentRuntime && WBRAND_PRODUCT_FLAVOR === "preview";
...
// e2e 运行的是生产构建，默认会和本机正式版 WBrand 共用 app name / userData，
// 触发 Electron 单实例锁后只激活已有窗口，Chromedriver 无法接管测试进程。
// 这里允许测试显式隔离运行时身份，正常桌面/远控路径保持原来的默认值。
export const runtimeApplicationName =
  readRuntimeEnvOverride("WBRAND_DESKTOP_APPLICATION_NAME") ??
  (isLocalDevelopmentRuntime ? "WBrand Dev" : isPreviewPackagedRuntime ? "WBrand Preview" : "WBrand");
// Electron 的 app.getPath("home") 不一定跟随测试进程里的 HOME 覆盖。
// e2e 默认工作区依赖 home 路径，因此提供显式覆盖，避免测试写到开发者真实 ~/WBrandProject。
export const runtimeHomePath = readRuntimeEnvOverride("WBRAND_DESKTOP_HOME_DIR");
...
export const shouldUseElectronDefaultUserDataPath = isTruthyRuntimeEnvOverride(
  "WBRAND_DESKTOP_USE_ELECTRON_DEFAULT_USER_DATA",
);
export const runtimeUserDataPath =
  readRuntimeEnvOverride("WBRAND_DESKTOP_USER_DATA_DIR") ??
...
export const runtimeSessionDataPath =
  readRuntimeEnvOverride("WBRAND_DESKTOP_SESSION_DATA_DIR") ??
...
export async function isDockerDaemonAvailable(): Promise<boolean> {
  const { isDockerAvailable } = await import("@wbrand/server/remote");
  return isDockerAvailable();
}

export async function listAvailableWSLDistros() {
  const { listWSLDistros } = await import("@wbrand/server/remote");
  return listWSLDistros();
}

export async function listAvailableDockerContainers() {
  const { listDockerContainers } = await import("@wbrand/server/remote");
...
export function loadHostProcessEnvFromLocalFiles(): Record<string, string> {
  if (isElectronAppPackaged()) {
    ...
    return { WBRAND_TELEMETRY_RUNTIME_DISTRIBUTION: "packaged" };
  }
  ...
  return applySelectedWBrandEnvLinks(merged);
}
...
function resolveAvailableDevelopmentMockCdnDir(): string | undefined {
  ...
  const releaseDir = join(mockCdnDir, "releases", WBRAND_VERSION);
...
function shouldUseRemoteCdnInDevelopment(localEnv: LocalRuntimeEnv = {}): boolean {
  return isTruthyEnvFlag(resolveEnvValue("WBRAND_DEV_REMOTE_ASSET_USE_CDN", localEnv));
}
...
function resolveRemoteCdnBaseUrls(
  ...
  const raw = resolveEnvValue("WBRAND_REMOTE_ASSET_CDN_BASE_URL", localEnv);
...
export function resolveWBrandEndpointEnvBaseOrigin(
  localEnv: LocalRuntimeEnv = {},
): string | undefined {
  const buildEnv = readProductEndpointEnv();
  // main 进程临时验证更新服务时不会重新写 .env，命令行传入的 endpoint 必须优先于本地文件。
  return (
    process.env["WBRAND_BASE_URL"]?.trim() ||
    process.env["WBRAND_ENDPOINT_ORIGIN"]?.trim() ||
    localEnv.WBRAND_BASE_URL?.trim() ||
    localEnv.WBRAND_ENDPOINT_ORIGIN?.trim() ||
    buildEnv.WBRAND_BASE_URL?.trim() ||
    buildEnv.WBRAND_ENDPOINT_ORIGIN?.trim() ||
    undefined
  );
}
...
function applySelectedWBrandEnvLinks(env: Record<string, string>): Record<string, string> {
  const endpointEnv = {
    ...readProductEndpointEnv(),
    ...env,
    WBRAND_ENV,
  ...
  return {
    ...
    ...env,
    WBRAND_BASE_URL: env.WBRAND_BASE_URL ?? resolveRuntimeWBrandEndpointOrigin(endpointEnv),
...
function resolveHostProcessNodeEnv(): WBrandRuntimeEnv {
  return desktopRuntimeEnv;
}

function resolveRemoteAssetCacheDir(localEnv: LocalRuntimeEnv = {}): string {
  const overrideCacheDir = resolveEnvValue("WBRAND_REMOTE_ASSET_CACHE_DIR", localEnv);
  if (overrideCacheDir) {
...
function resolveBundledWBrandAgentBinaryPath(): string | undefined {
  const runtime = WBRAND_AGENT_RUNTIME;
...
export function resolveBundledGlmBinaryPath(): string | undefined {
  return resolveBundledWBrandAgentBinaryPath();
}
...
function resolveHostProcessBinaryEnv(
  ...
  // WBrand Agent 与 app 协议适配强绑定版本，生产包必须优先使用随包携带的固定 runtime。
...
function resolveDynamicWorkflowModeHostEnv(options: {
  ...
  if (!options.isPackaged) {
    ...
    return mode ? { [WBRAND_DYNAMIC_WORKFLOW_MODE_ENV]: mode } : {};
  }
  if (options.isPreview) {
    return { [WBRAND_DYNAMIC_WORKFLOW_MODE_ENV]: "alwaysOn" };
  }
  return {};
}
...
export function buildHostProcessEnv(hostProcessLocalEnv: Record<string, string>) {
    ...
    process.platform !== "darwin"
      ...
      : packagedDesktop
        ...
        : // Truthy grammar must match the producer's isUnsignedHelperLocalDevRequested
          ...
          ["1", "true", "on"].includes(
              rawInheritedEnv.WBRAND_CUA_HELPER_ALLOW_UNSIGNED_LOCAL?.trim().toLowerCase() ?? "",
            )
          ? rawInheritedEnv.WBRAND_CUA_BUNDLED_HELPER_APP_PATH?.trim() ||
            join(
              rawInheritedEnv.WBRAND_HOME?.trim() || join(homedir(), ".wbrand"),
              "computer-use",
              "dev",
              DEV_HELPER_APP_NAME,
            )
  ...
  const agentTelemetryEnv = readWBrandAgentTelemetryEnv(rawInheritedEnv);
  ...
  const inheritedEnv = applySelectedWBrandEnvLinks({
    ...sanitizeWBrandRuntimeEnv(rawInheritedEnv),
    ...buildWBrandToolEnvPassthroughEnv(rawInheritedEnv),
  });
  ...
  if (packagedDesktop) {
    delete inheritedEnv.WBRAND_CUA_HELPER_ALLOW_UNSIGNED_LOCAL;
  }
  const dynamicWorkflowModeHostEnv = resolveDynamicWorkflowModeHostEnv({
    inheritedValue: rawInheritedEnv[WBRAND_DYNAMIC_WORKFLOW_MODE_ENV],
  ...
  delete inheritedEnv[WBRAND_DYNAMIC_WORKFLOW_MODE_ENV];
  ...
  return {
    ...
    ...agentTelemetryEnv,
    // WBrand 运行时不再使用 NODE_ENV；它会被用户 shell、包管理器和测试框架复用。
    // 这里显式下发 WBRAND_RUNTIME_ENV，并在继承环境里清掉 NODE_ENV，避免 host/agent/Bash 被污染。
    [WBRAND_RUNTIME_ENV_KEY]: resolveHostProcessNodeEnv(),
    // 显式注入编译期产品身份，保证主进程与 host 的身份语义一致；地址独立解析。
    // inheritedEnv 从 .env 通用变量补齐 WBrand/ZAI 链接，未覆盖时统一使用线上默认值。
    WBRAND_ENV,
    // Preview 与生产版共享任务、配置和凭据，但不同版本的 Helper 不能互相覆盖或触发降级保护。
    // 只隔离 computer-use 下的运行组件，不改写 WBRAND_HOME / WBRAND_DATA_BASE_DIR 业务数据根。
    ...(isPreviewPackagedRuntime ? { WBRAND_CUA_HELPER_INSTALL_VARIANT: "preview" } : {}),
    ...
    [WBRAND_APP_VERSION_ENV]: WBRAND_VERSION,
    ...(dataBaseDir !== homedir() ? { WBRAND_DATA_BASE_DIR: dataBaseDir } : {}),
    ...(windowsAppInstallDir ? { [WBRAND_WINDOWS_APP_INSTALL_DIR_ENV]: windowsAppInstallDir } : {}),
    ...(bundledCuaHelperAppPath
      ? { [WBRAND_CUA_BUNDLED_HELPER_APP_PATH_ENV]: bundledCuaHelperAppPath }
      : {}),
    ...(resolvedGlmBinaryPath ? { GLM_BINARY_PATH: resolvedGlmBinaryPath } : {}),
    ...(resolvedLarkCliBinaryPath ? { WBRAND_LARK_CLI_BINARY: resolvedLarkCliBinaryPath } : {}),
  };
}
