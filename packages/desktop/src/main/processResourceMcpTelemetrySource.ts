...
interface StoredSample {
  sample: WBrandMcpResourceSample;
...
function purgeExpired(now: number): void {
  for (const [key, entry] of latest) {
    if (now - entry.receivedAt > WBRAND_MCP_RESOURCE_SAMPLE_INTERVAL_MS * 2) latest.delete(key);
  }
}
...
export function ingestMcpResourceSamples(
  ...
  const parsed = wbrandMcpResourceSamplesSchema.safeParse(raw);
  ...
  sample(context) {
    ...
    for (const [key, group] of groupSamples()) {
      recordExternalAppResourceSample({
        ...
        intervalMs: WBRAND_MCP_RESOURCE_SAMPLE_INTERVAL_MS,
