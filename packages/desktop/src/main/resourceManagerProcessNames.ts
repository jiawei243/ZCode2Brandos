...
import { formatWBrandRendererProcessName } from "@wbrand/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatWBrandRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatWBrandRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatWBrandRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
