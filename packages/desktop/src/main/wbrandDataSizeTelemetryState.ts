...
export interface WBrandDataSizeTelemetryState {
  lastReportedAt?: number;
  reportReservedAt?: number;
}

export async function readWBrandDataSizeTelemetryState(
  stateFile: string,
): Promise<WBrandDataSizeTelemetryState | null> {
  try {
    const parsed: unknown = JSON.parse(await readFile(stateFile, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      throw new TypeError("Invalid WBrand data size telemetry state");
    }
    const record = parsed as Record<string, unknown>;
    const state: WBrandDataSizeTelemetryState = {};
    if (record.lastReportedAt !== undefined) {
      if (typeof record.lastReportedAt !== "number" || !Number.isFinite(record.lastReportedAt)) {
        throw new TypeError("Invalid lastReportedAt in WBrand data size telemetry state");
      }
    ...
    if (record.reportReservedAt !== undefined) {
      if (
        ...
        throw new TypeError("Invalid reportReservedAt in WBrand data size telemetry state");
      }
...
export async function writeWBrandDataSizeTelemetryState(
  stateFile: string,
  state: WBrandDataSizeTelemetryState,
): Promise<void> {
