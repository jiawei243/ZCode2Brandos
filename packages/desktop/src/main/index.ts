...
import {
  ...
  DEFAULT_LOCALE,
  WBRAND_VERSION,
  WBRAND_TELEMETRY_ENABLED,
  WBRAND_ARMS_RUM_ENDPOINT,
...
import {
  ...
  updateWBrandStdioTapDevMenuState,
...
import { resolveWBrandBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveWBrandEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopWBrandDataSizeTelemetry,
  stopDesktopWBrandDataSizeTelemetry,
} from "./desktopWBrandDataSizeTelemetry.js";
...
import { mapWBrandEnvToArmsRumEnv } from "@wbrand/shared";
import {
...
// 会和开发态已打开的 WBrand Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.WBRAND_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".wbrand", "v2", "setting.json");
...
async function resolveCurrentWBrandEndpointOrigin() {
  return resolveWBrandEndpointOrigin({
    env: WBRAND_ENV,
    envBaseOrigin: resolveWBrandEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).wbrandEndpointOrigin,
  });
}
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 WBrand 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
const readHelpConfig = createDesktopHelpConfigReader({
  appVersion: WBRAND_VERSION || app.getVersion(),
  deviceMid,
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: WBRAND_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    WBRAND_TELEMETRY_ENABLED && WBRAND_ARMS_RUM_ENDPOINT
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，wbrand-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.WBRAND_E2E_RUN_ID?.trim()) {
...
function shouldConfirmAppQuit() {
  ...
  return WBRAND_ENV === "production" && getRunningAgentSessionCount() > 0;
}

function confirmAppQuit(originWindow?: BrowserWindow | null) {
  if (!shouldConfirmAppQuit()) {
    logger.info(`[app-quit] quit confirmation skipped in ${WBRAND_ENV}`);
    return true;
  }
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onWBrandEndpointChanged: handleWBrandEndpointChanged,
    wbrandEndpointEnvBaseOrigin: resolveWBrandEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveWBrandEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (WBRAND_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentWBrandEndpointOrigin();
  if (origin === DEFAULT_WBRAND_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleWBrandEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveWBrandEndpointSelection(), mainSettingService.get()]).then(
    ([wbrandEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        wbrandEndpointSelection,
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          wbrandBuiltinProviderConfigFilePath: resolveWBrandBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(WBRAND_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 WBrand 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: WBRAND_PRODUCT_FLAVOR === "production",
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapWBrandEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (WBRAND_TELEMETRY_ENABLED && WBRAND_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: WBRAND_VERSION,
  ...
  registerDesktopWBrandDataSizeTelemetry({
    ...
    isWBrandBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getWBrandDataRootDir(),
    stateFile: join(app.getPath("userData"), "wbrand-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // 本地未打包 dev 构建（app.isPackaged === false）必须跳过远端强制升级 gate。
  // 原因：force-update gate 只看 WBRAND_ENV === "production"，但 dev 构建（如 dev:desktop:cua
  ...
  const forceUpdateGuardResult =
    WBRAND_PRODUCT_FLAVOR === "production" && !skipForceUpdateForLocalDevRuntime
