...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@wbrand/shared";

import type { WBrandDataSizeScanResult } from "./wbrandDataSizeScanner.js";
import {
  readWBrandDataSizeTelemetryState,
  writeWBrandDataSizeTelemetryState,
  type WBrandDataSizeTelemetryState,
} from "./wbrandDataSizeTelemetryState.js";
import { scanWBrandDataDirectoryInWorker } from "./wbrandDataSizeWorkerClient.js";

export type { WBrandDataSizeTelemetryState } from "./wbrandDataSizeTelemetryState.js";

const WBRAND_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const WBRAND_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const WBRAND_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const WBRAND_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const WBRAND_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const WBRAND_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const WBRAND_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const WBRAND_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const WBRAND_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const WBRAND_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const WBRAND_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const WBRAND_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface WBrandDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: WBrandDataSizeTelemetryTiming = {
  abortedRetryMs: WBRAND_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: WBRAND_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: WBRAND_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: WBRAND_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: WBRAND_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: WBRAND_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: WBRAND_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: WBRAND_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: WBRAND_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: WBRAND_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface WBrandDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface WBrandDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isWBrandBusy: () => boolean;
  logger: WBrandDataSizeTelemetryLogger;
  readState: () => Promise<WBrandDataSizeTelemetryState | null>;
  report: (result: WBrandDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<WBrandDataSizeScanResult>;
  timing?: Partial<WBrandDataSizeTelemetryTiming>;
  writeState: (state: WBrandDataSizeTelemetryState) => Promise<void>;
}
...
function buildWBrandDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: WBrandDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_wbrand_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      schema_version: 1,
      wbrand_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / WBRAND_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createWBrandDataSizeTelemetryScheduler(
  dependencies: WBrandDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: WBrandDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isWBrandBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[wbrand-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<WBrandDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: WBrandDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[wbrand-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("WBrand data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("WBrand data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[wbrand-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[wbrand-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: WBrandDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[wbrand-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createWBrandDataSizeTelemetryScheduler> | null = null;

export function registerDesktopWBrandDataSizeTelemetry(options: {
  context: Parameters<typeof buildWBrandDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isWBrandBusy: () => boolean;
  logger: WBrandDataSizeTelemetryLogger;
  ...
  stopDesktopWBrandDataSizeTelemetry();
  desktopScheduler = createWBrandDataSizeTelemetryScheduler({
    ...
    isWBrandBusy: options.isWBrandBusy,
    logger: options.logger,
    readState: () => readWBrandDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildWBrandDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanWBrandDataDirectoryInWorker(
        {
          maxDurationMs: WBRAND_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: WBRAND_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeWBrandDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[wbrand-data-size] scheduler start failed", error);
  });
}

export function stopDesktopWBrandDataSizeTelemetry(): void {
