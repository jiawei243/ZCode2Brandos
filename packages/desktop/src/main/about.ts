...
const ABOUT_APPLICATION_NAME = "WBrand Desktop App";
...
> = {
  "zh-CN": {
    aboutTitle: "关于 WBrand",
    versionLabel: "版本",
    okButtonLabel: "确定",
    optimizedForAppleSilicon: "已针对 Apple Silicon 优化。",
    copyright: (year) => `版权所有 © ${year} WBrand。`,
  },
  "en-US": {
    aboutTitle: "About WBrand",
    versionLabel: "version",
    okButtonLabel: "OK",
    optimizedForAppleSilicon: "Optimized for Apple Silicon.",
    copyright: (year) => `Copyright © ${year} WBrand.`,
  },
};
...
export function createAboutSnapshot(options: AboutSnapshotOptions = {}): AboutSnapshot {
  ...
  return {
    appVersion: normalizeValue(options.appVersion ?? buildMetadata?.appVersion ?? WBRAND_VERSION),
    buildCommitId: normalizeValue(buildMetadata?.buildCommitId ?? WBRAND_COMMIT),
    buildTime: normalizeValue(buildMetadata?.buildTime ?? WBRAND_BUILD_TIME),
    environment: normalizeValue(options.environment ?? WBRAND_ENV),
