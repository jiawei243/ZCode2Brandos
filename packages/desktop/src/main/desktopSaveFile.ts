...
import type { SaveFileRequest, SaveFileResult } from "@wbrand/shared";
import { PlatformChannels } from "@wbrand/shared";
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "wbrand-save-file-"));
