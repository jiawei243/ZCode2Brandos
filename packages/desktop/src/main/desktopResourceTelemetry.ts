...
function resolveDefaultReportIntervalMs(): number {
  ...
  if (process.env.WBRAND_ENV === "test" && process.env.WBRAND_E2E_RUN_ID?.trim()) {
    return 60_000;
  }
  return 300_000;
}
...
export function ingestToolExecResource(
  ...
  const parsed = wbrandToolExecResourceSchema.safeParse(raw);
...
function auditDisabledAgentMetricProbe(logger: ResourceLogger | undefined): void {
  if (
    ...
    process.env.WBRAND_ENV !== "test" ||
    !process.env.WBRAND_E2E_RUNTIME_LOG_DIR?.trim()
  ) {
    return;
  }
