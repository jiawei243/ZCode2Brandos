...
import {
  ...
  resolveWBrandEndpointOrigin,
} from "@wbrand/shared";
import { readWBrandStdioTapDevState, setWBrandStdioTapDevEnabled } from "@wbrand/services/node";
...
export const HELP_TOGGLE_WBRAND_STDIO_TAP_MENU_ID = "help.toggle-wbrand-stdio-tap";
const WBRAND_ENDPOINT_PROMPT_WIDTH = 460;
const WBRAND_ENDPOINT_PROMPT_HEIGHT = 210;
const CODING_PLAN_WEBVIEW_PARTITION = "persist:wbrand-coding-plan";
...
async function clearAllDataAndRelaunch(options: {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: "确定要清除所有数据吗？",
    detail:
      "将删除 ~/.wbrand/v2（配置、凭据、日志）和浏览器缓存（localStorage）。操作不可恢复，清除后应用将自动重启。",
  });
  if (response !== 1) {
    return;
  }
  ...
  try {
    ...
    options.logger.info("[clear-all-data] deleted ~/.wbrand/v2");
  } catch (error) {
    options.logger.error("[clear-all-data] failed to delete ~/.wbrand/v2:", error);
  }
...
async function promptCustomWBrandEndpoint(
  targetWindow: BrowserWindow | null | undefined,
  currentValue: string,
): Promise<string | undefined> {
  return showWBrandEndpointPromptWindow({
    currentValue,
...
function buildWBrandEndpointPromptHtml(currentValue: string): string {
  const value = escapeHtmlAttribute(currentValue);
  return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>WBrand Endpoint</title>
    <style>
  ...
  <body>
    <form id="form">
      <label for="endpoint">WBrand endpoint origin</label>
    ...
    <script>
      ...
      const submit = (value) => { document.title = "wbrand-endpoint-submit:" + encodeURIComponent(value); };
      document.getElementById("form").addEventListener("submit", (event) => {
        event.preventDefault();
        submit(input.value);
      });
      document.getElementById("cancel").addEventListener("click", () => {
        document.title = "wbrand-endpoint-cancel";
      });
      input.focus();
      input.select();
    </script>
...
function showWBrandEndpointPromptWindow(options: {
  currentValue: string;
  ...
  return new Promise((resolve) => {
    ...
    const promptWindow = new BrowserWindow({
      width: WBRAND_ENDPOINT_PROMPT_WIDTH,
      height: WBRAND_ENDPOINT_PROMPT_HEIGHT,
      parent: options.parentWindow,
      modal: Boolean(options.parentWindow),
      resizable: false,
      minimizable: false,
      maximizable: false,
      title: "WBrand Endpoint",
      webPreferences: {
    ...
    promptWindow.on("page-title-updated", (event, title) => {
      if (title === "wbrand-endpoint-cancel") {
        event.preventDefault();
        finish(undefined);
        return;
      }
      if (!title.startsWith("wbrand-endpoint-submit:")) {
        return;
      }
      event.preventDefault();
      finish(decodeURIComponent(title.slice("wbrand-endpoint-submit:".length)));
    });
    ...
    void promptWindow.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        buildWBrandEndpointPromptHtml(options.currentValue),
      )}`,
    );
  });
}

async function setWBrandEndpointOverride(options: {
  value: string | undefined;
  settingService: { update(patch: { wbrandEndpointOrigin?: string | undefined }): Promise<void> };
  onWBrandEndpointChanged: () => Promise<void> | void;
  logger: { warn: (...args: unknown[]) => void };
}) {
  if (WBRAND_ENV === "production") {
    return;
  }
  const normalized = options.value ? normalizeWBrandEndpointOrigin(options.value) : undefined;
  await options.settingService.update({ wbrandEndpointOrigin: normalized });
  await options.onWBrandEndpointChanged();
}
...
function toggleWBrandStdioTapDevProxy(options: {
  logger: { info: (...args: unknown[]) => void };
  updateWBrandStdioTapDevMenuState: () => void;
}) {
  const current = readWBrandStdioTapDevState();
  const next = setWBrandStdioTapDevEnabled(!current.enabled);
  options.updateWBrandStdioTapDevMenuState();
...
function resolveChangelogUrl(
  ...
  const origin = buildWBrandEndpointUrls(endpointOrigin).origin;
...
async function resolveCurrentWBrandEndpointOrigin(settingService: {
  get(): Promise<{ wbrandEndpointOrigin?: string }>;
  envBaseOrigin?: string | null;
}): Promise<string> {
  const settings = await settingService.get();
  return resolveWBrandEndpointOrigin({
    env: WBRAND_ENV,
    envBaseOrigin: settingService.envBaseOrigin,
    overrideOrigin: settings.wbrandEndpointOrigin,
  });
}
...
export async function executeDesktopCommand(options: {
  ...
  settingService: {
    get(): Promise<Pick<AppSettings, "wbrandEndpointOrigin" | "desktopZoomLevel">>;
    update(
      patch: Partial<Pick<AppSettings, "wbrandEndpointOrigin" | "desktopZoomLevel">>,
    ): Promise<void>;
  };
  wbrandEndpointEnvBaseOrigin?: string | null;
  credentialsDir: string;
  ...
  switch (options.command) {
    ...
    case DesktopCommandIds.OpenChangelog:
      await openChangelog(
        ...
        await resolveCurrentWBrandEndpointOrigin({
          ...options.settingService,
          envBaseOrigin: options.wbrandEndpointEnvBaseOrigin,
        }),
      );
      return;
    case DesktopCommandIds.CheckForUpdates:
      // 按产品身份而不是后端环境放行：生产后端的 Preview 同样没有更新器。
      if (WBRAND_PRODUCT_FLAVOR === "production") {
    ...
    case DesktopCommandIds.OpenResourceManager:
      openResourceManager();
      return;
    case DesktopCommandIds.ToggleWBrandStdioTapDevProxy:
      toggleWBrandStdioTapDevProxy({
        logger: options.logger,
        updateWBrandStdioTapDevMenuState: options.updateWBrandStdioTapDevMenuState,
      });
      return;
    case DesktopCommandIds.SetWBrandEndpointProduction:
      await setWBrandEndpointOverride({
        value: DEFAULT_WBRAND_ENDPOINT_ORIGIN,
        settingService: options.settingService,
        onWBrandEndpointChanged: options.onWBrandEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetWBrandEndpointTest:
      await setWBrandEndpointOverride({
        value: options.wbrandEndpointEnvBaseOrigin ?? resolveRuntimeWBrandEndpointOrigin(),
        settingService: options.settingService,
        onWBrandEndpointChanged: options.onWBrandEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetWBrandEndpointCustom: {
      const current =
        (await options.settingService.get()).wbrandEndpointOrigin ?? DEFAULT_WBRAND_ENDPOINT_ORIGIN;
      const value = await promptCustomWBrandEndpoint(targetWindow, current);
      if (!value) {
        return;
      }
      try {
    ...
    case DesktopCommandIds.ResetWBrandEndpoint:
      await setWBrandEndpointOverride({
        value: undefined,
        settingService: options.settingService,
        onWBrandEndpointChanged: options.onWBrandEndpointChanged,
