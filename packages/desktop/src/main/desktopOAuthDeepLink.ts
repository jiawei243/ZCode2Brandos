...
export function resolveExternalWorkspaceOpenDialogCopy(
  ...
  if (locale === "zh-CN") {
    return {
      buttons: ["打开文件夹", "取消"],
      title: "打开外部 WBrand 链接？",
      message: "是否在 WBrand 中打开此文件夹？",
  ...
  return {
    ...
    title: "Open external WBrand link?",
    message: "Open this folder in WBrand?",
    detail: (path) =>
...
export function handleDeepLink(
  ...
  if (isWorkspaceOpenUrl(parsedUrl)) {
    ...
    const targetWindow = options.resolveApplicationWindow
      ? options.resolveApplicationWindow()
      : (BrowserWindow.getFocusedWindow() ?? BrowserWindow.getAllWindows()[0] ?? null);
    // wbrand://workspace/open 来自浏览器/IM 等外部应用，不能等同于用户在
    // WBrand 内部选择目录；确认必须发生在 statSync 之前，避免项目配置被静默信任。
    if (
  ...
  if (isShareImportUrl(parsedUrl)) {
    ...
    // 否则多窗口时导入会投递给先 ready 的 renderer，写入错误 workspace 的 .wbrand-share。
    const targetWindow = options.resolveApplicationWindow
      ? options.resolveApplicationWindow()
      : (BrowserWindow.getFocusedWindow() ?? BrowserWindow.getAllWindows()[0] ?? null);
    if (targetWindow) {
