...
function getWBrandDataDir() {
  return getAppConfigDir();
}

function getWBrandCliDir() {
  return join(homedir(), ".wbrand", "cli");
}

function getWBrandCliLogDir() {
  return join(getWBrandCliDir(), "log");
}

/**
 * Computer Use Helper 的运行目录。macOS 上 Helper 由 LaunchServices 启动，stderr 被系统丢弃，
 * 所以它把生命周期与后台输入诊断 tee 到 `<socket>.exit.log`（见 wbrand-cua
 * helperExitLogPathFor）。同目录下还有 `.tokens` broker 凭据，收集时必须按文件名白名单。
 */
function getCuaHelperRunDir() {
  return join(homedir(), ".wbrand", "computer-use", "run");
}
...
function isExcludedRelativePath(relativePath: string): boolean {
  ...
  // 过去显式收集 ~/.wbrand/cli/debug 会把这类上下文带进手动导出和反馈完整日志，这里按目录段统一跳过。
  ...
  // ~/.wbrand/v2/dev 保存 stdio-traffic 等高频协议流，真实机器上会累计到 GB 级。
...
function shouldApplyLogExportRetention(archivePath: string): boolean {
  ...
  if (
    ...
    normalizedArchivePath.startsWith(".wbrand/cli/log/")
  ) {
    return true;
  }
...
async function createLogArchiveArtifacts(
  ...
  const wbrandCliLogDir = getWBrandCliLogDir();
  // GLM / wbrand-cli 的运行日志写在 ~/.wbrand/cli/log，不在应用主数据目录 ~/.wbrand/v2 下。
  // 如果导出日志只扫描 v2，定位 agent CLI 启动、协议或崩溃问题时会缺少最关键的原生侧日志。
  await collectLogArchiveFilesFromDirectory(
    wbrandCliLogDir,
    posix.join(".wbrand", "cli", "log"),
    visitedDirs,
    files,
  );

  const wbrandCliDir = getWBrandCliDir();
  // 排查 agent CLI 问题还需要它的运行配置与模型 IO 轨迹。
  // config.json 是当前生效配置；rollout 是 model-io 调用轨迹，
  // 二者都不在 ~/.wbrand/cli/log 下，需要额外收集才能完整还原现场。
  await collectLogArchiveFile(
    join(wbrandCliDir, "config.json"),
    posix.join(".wbrand", "cli", "config.json"),
    files,
  );
  await collectLogArchiveFilesFromDirectory(
    join(wbrandCliDir, "rollout"),
    posix.join(".wbrand", "cli", "rollout"),
    visitedDirs,
    files,
  );
  ...
  // ~/.wbrand/computer-use/run/<socket>.exit.log，既不在 app data 也不在 ~/.wbrand/cli 下。
  // 同目录下有 .tokens broker 凭据，因此按文件名白名单只收 *.exit.log，不递归该目录。
  await collectLogArchiveFilesByName(
    getCuaHelperRunDir(),
    posix.join(".wbrand", "computer-use", "run"),
...
export async function createFeedbackLogArchiveFromExportLogs(
  ...
  return createFeedbackDiagnosticArchive({
    sources: [
      ...
      { directory: getWBrandCliLogDir(), archivePrefix: ".wbrand/cli/log" },
      {
        directory: getCuaHelperRunDir(),
        archivePrefix: ".wbrand/computer-use/run",
        exitLogsOnly: true,
      },
    ],
...
export async function exportLogs(
  ...
  try {
    ...
    const getSourceDir = dependencies.getWBrandDataDir ?? getWBrandDataDir;
    ...
    const exportBaseName = `wbrand-logs-${timestamp}`;
