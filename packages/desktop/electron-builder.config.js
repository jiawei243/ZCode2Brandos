...
const desktopProductIdentity = resolveDesktopProductIdentity({
  ...process.env,
  WBRAND_ENV: builtinProviderConfig.environment,
});
...
const shouldEnableMacSigning =
  process.env.WBRAND_ENABLE_MAC_SIGN === "1" && Boolean(macSigningIdentity);
...
const desktopDistDir = process.env.WBRAND_DESKTOP_DIST_DIR || "dist";
...
const REQUIRED_ASAR_RUNTIME_MODULES = [
  ...
  "pngjs",
  // @wbrand/services 的代理连通性探测会动态 require("undici") 取 ProxyAgent。
...
const WINDOWS_INSTALL_MANIFEST_NAME = ".wbrand-install-manifest";
...
function resolveElectronDownloadMirror(env = process.env) {
  const existingMirror =
    env.WBRAND_ELECTRON_RUNTIME_MIRROR ||
...
if (
  ...
  process.env.WBRAND_ENABLE_MAC_SIGN === "1" &&
  !macSigningIdentity
) {
  throw new Error(
    "WBrand Preview macOS packaging requires APPLE_SIGNING_IDENTITY or CSC_NAME when WBRAND_ENABLE_MAC_SIGN=1",
  );
}
...
function resolveAppAsarPath(context) {
  if (context.electronPlatformName === "darwin") {
    const appName = `${context.packager?.appInfo?.productFilename ?? "WBrand"}.app`;
...
async function injectHoistedRuntimeModulesIntoAsar(context) {
  ...
  const stagingDir = mkdtempSync(resolve(tmpdir(), "wbrand-app-asar-"));
  try {
...
export default {
  ...
  extraMetadata: {
    ...
    homepage: "https://wbrand.unew.cc",
    author: {
      name: "WBrand",
      email: "dev@wbrand.unew.cc",
    },
  },
  ...
  extraResources: [
      ...
      ? [
          {
            ...
            // 只是不吸附，所以这里不做存在性断言。
            from: "resources/macos-window-bounds/wbrand-window-bounds",
            to: "macos-window-bounds/wbrand-window-bounds",
          },
        ]
      : []),
    {
    ...
    {
      // Provider Registry 的 WBrand Built-in Config 是静态 Provider/Model 事实的唯一内置来源。
      // 显式随包发布，避免正式 Host 回退到旧 Catalog/Preset hardcode。
      from: builtinProviderConfig.sourcePath,
      to: "config/provider/wbrand-builtin.json",
    },
    {
      ...
      : []),
    {
      ...
      // 桌面端内置的是 agent 的 JS bundle（glm/wbrand.cjs，由 prepare:agent-bundle 生成），
      // Host 进程用 app 自带的 Electron Node runtime（ELECTRON_RUN_AS_NODE）执行 `wbrand.cjs app-server --stdio`，
  ...
  protocols: [
    {
      ...
      // 展示名跟随安装包身份；scheme 仍保持 wbrand，因此两个应用中最后注册者会成为默认 handler。
  ...
  linux: {
    ...
    // desktop 包名是 scoped package（@wbrand/desktop），electron-builder 默认会把
    // Linux executable/Icon 推成 @wbranddesktop。部分桌面环境无法按这个 icon name 命中
    // hicolor 图标，最终回退成系统齿轮。这里固定成稳定的小写名称，让 Icon=wbrand
    // 与 /usr/share/icons/hicolor/*/apps/wbrand.png 保持一致。
    executableName: desktopProductIdentity.linuxExecutableName,
    category: "Development",
    maintainer: "WBrand <dev@wbrand.unew.cc>",
  },
  deb: {
