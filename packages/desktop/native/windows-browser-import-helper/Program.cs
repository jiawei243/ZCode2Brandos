...
{
    private const string Protocol = "WBRAND_BROWSER_IMPORT_V1";
        ...
        try
        {
            if (HasArgument(args, "--version"))
            {
                ...
                Console.Out.WriteLine(
                    "WBRAND_BROWSER_IMPORT_HELPER\t" + HelperVersion + "\t" + GetProcessArchitecture() +
    ...
    private static void RunBrokerCore()
    {
        ...
        try
        {
            ...
            pipeName = "wbrand-browser-import-broker-" + Guid.NewGuid().ToString("N");
            token = CreateToken();
    ...
    private static void RunTemporarySystemService(
        ...
        int brokerPid)
    {
        string serviceName = "WBrandBrowserImport_" + Guid.NewGuid().ToString("N");
        string pipeName = "wbrand-browser-import-system-" + Guid.NewGuid().ToString("N");
