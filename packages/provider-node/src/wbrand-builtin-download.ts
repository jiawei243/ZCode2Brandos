import { z } from "zod";
import { decodeWBrandBuiltinRelease, type WBrandBuiltinRelease } from "./wbrand-builtin-release.js";
...
export interface WBrandBuiltinDownloadOptions {
...
export async function downloadWBrandBuiltinRelease(
  options: WBrandBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeWBrandBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`WBrand Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
