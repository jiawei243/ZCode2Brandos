...
import {
  decodeWBrandBuiltinRelease,
  encodeWBrandBuiltinRelease,
  serializeWBrandBuiltinRelease,
  type WBrandBuiltinRelease,
} from "./wbrand-builtin-release.js";

export interface NodeWBrandBuiltinProviderConfigSourceOptions {
  readonly bundledFilePath: string;
  readonly activeFilePath?: string;
  readonly watch?: boolean;
}

export type ApplyWBrandBuiltinReleaseResult = "updated" | "unchanged" | "stale";

/** Bundled、Active/LKG 与 Remote 共用同一 Release，并最终发布为现有 Config Snapshot。 */
export class NodeWBrandBuiltinProviderConfigSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #bundledFilePath: string;
  readonly #activeFilePath: string;
  ...
  constructor(options: NodeWBrandBuiltinProviderConfigSourceOptions) {
    const bundledFilePath = options.bundledFilePath.trim();
    if (!bundledFilePath) throw new Error("WBrand Built-in bundledFilePath 不能为空");
  ...
  async read(): Promise<ProviderConfigLayerSnapshot> {
    ...
    let release: WBrandBuiltinRelease;
    try {
  ...
  async applyRemoteRelease(release: WBrandBuiltinRelease): Promise<ApplyWBrandBuiltinReleaseResult> {
    ...
    const result = await withFileLock(this.#activeFilePath, async () => {
      ...
      if (release.revision === current.revision) {
        if (serializeWBrandBuiltinRelease(release) === serializeWBrandBuiltinRelease(current)) {
          return "unchanged" as const;
        }
        throw new Error(`WBrand Built-in 相同 revision ${release.revision} 对应不同内容`);
      }
  ...
  async #readAndMaterializeLocked(): Promise<WBrandBuiltinRelease> {
  ...
  async #writeActiveLocked(release: WBrandBuiltinRelease): Promise<void> {
    this.#assertNotDisposed();
    await atomicWritePrivateTextFile(
      this.#activeFilePath,
      JSON.stringify(encodeWBrandBuiltinRelease(release), null, 2),
    );
  }
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("NodeWBrandBuiltinProviderConfigSource 已 dispose");
  }
}

export function createNodeWBrandBuiltinProviderConfigSource(
  options: NodeWBrandBuiltinProviderConfigSourceOptions,
): NodeWBrandBuiltinProviderConfigSource {
  return new NodeWBrandBuiltinProviderConfigSource(options);
}

interface ReleaseCandidate {
  readonly release?: WBrandBuiltinRelease;
  readonly error?: unknown;
}

async function readReleaseCandidate(filePath: string): Promise<ReleaseCandidate | null> {
  try {
    return { release: decodeWBrandBuiltinRelease(JSON.parse(await readFile(filePath, "utf8"))) };
  } catch (error) {
...
function selectReleaseCandidate(
...
): WBrandBuiltinRelease {
  if (
    bundled?.release &&
    active?.release &&
    bundled.release.revision === active.release.revision &&
    serializeWBrandBuiltinRelease(bundled.release) !== serializeWBrandBuiltinRelease(active.release)
  ) {
  ...
  const valid = [bundled?.release, active?.release].filter(
    (candidate): candidate is WBrandBuiltinRelease => candidate !== undefined,
  );
  if (valid.length === 0) {
    throw new AggregateError(
      [bundled?.error, active?.error].filter((error) => error !== undefined),
      "Bundled 与 Active WBrand Built-in Release 均不可用",
    );
  }
...
function snapshotFromRelease(
  ...
  return Object.freeze({
    revision: `wbrand-builtin:${release.revision}:${sourceKey}`,
...
function signatureOf(release: WBrandBuiltinRelease): string {
  return `${release.revision}:${serializeWBrandBuiltinRelease(release)}`;
}
