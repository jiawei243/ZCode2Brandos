...
import type { NodeWBrandBuiltinProviderConfigSource } from "./wbrand-builtin-provider-config-source.js";
import type { WBrandBuiltinRelease } from "./wbrand-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type WBrandBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface WBrandBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: WBrandBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface WBrandBuiltinRefreshEvent {
  readonly result: WBrandBuiltinRefreshResult;
...
export class WBrandBuiltinRemoteSynchronizer {
  readonly #options: WBrandBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<WBrandBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: WBrandBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<WBrandBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<WBrandBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("WBrand Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: WBrandBuiltinRefreshEvent): void {
