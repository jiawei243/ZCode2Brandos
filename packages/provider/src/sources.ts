...
export interface ProviderConfigSnapshot {
  ...
  readonly wbrandBuiltinRevision: string;
  readonly personalRevision: string;
  readonly wbrandBuiltinProviders: ProviderConfigMap;
  readonly wbrandBuiltinProviderTemplates: ProviderTemplateMap;
  readonly personalProviders: ProviderConfigMap;
  readonly wbrandBuiltinModelRules: ModelConfigRules;
...
export function createFailClosedAccountProviderConfigSnapshot(
  ...
  const unentitledProviders = new ProviderConfigMap(
    config.wbrandBuiltinProviders.entries().flatMap(([providerId, provider]) =>
  ...
  return createAccountProviderConfigSnapshot(config.wbrandBuiltinRevision, unentitledProviders);
}

export function createAccountProviderConfigSnapshot(
  basedOnWBrandBuiltinRevision: string,
  providers: ProviderConfigMap,
  states?: AccountProviderStates,
): AccountProviderConfigSnapshot {
  return Object.freeze({
    revision: `account:${JSON.stringify([basedOnWBrandBuiltinRevision, providers.toJSON(), states])}`,
...
const EMPTY_ACCOUNT_PROVIDER_CONFIG_SNAPSHOT: AccountProviderConfigSnapshot = Object.freeze({
  ...
  basedOnWBrandBuiltinRevision: "uninitialized",
