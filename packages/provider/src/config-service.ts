...
import type { ModelSelection } from "@wbrand/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly wbrandBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#wbrandBuiltinSource = dependencies.wbrandBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#wbrandBuiltinSource.onDidChange((reason) => this.#emit(`wbrandBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [wbrandBuiltin, personal] = await Promise.all([
      this.#wbrandBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([wbrandBuiltin.revision, personal.revision]),
  ...
  async savePersonalProviderOverlay(
    ...
    const wbrandBuiltin = await this.#wbrandBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = wbrandBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = wbrandBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 WBrand Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(wbrandBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = wbrandBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? wbrandBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...wbrandBuiltin.providers.keys(), ...current.providers.keys()]);
      const providerId = nextPersonalProviderId(occupied, templateId);
      createdProviderId = providerId;
      const effectiveProviders = resolvePersonalProviderBaselines(wbrandBuiltin, current.providers);
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        wbrandBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(wbrandBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(wbrandBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(wbrandBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(wbrandBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#wbrandBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _wbrandBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  wbrandBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    wbrandBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(wbrandBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? wbrandBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return wbrandBuiltin.providers.overlay(personal);
}
