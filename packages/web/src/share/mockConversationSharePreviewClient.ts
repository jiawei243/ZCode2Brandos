import type { ConversationShareAccessMode, ConversationSharePreview } from "@wbrand/shared";
  ...
  return {
    ...
    rows: [
      ...
      // 没有样例数据就只能靠猜。
      {
        ...
        sizeBytes: 172_974,
        sha256: "c".repeat(64),
        ref: "wbrand-artifact://share/mock-artifact-1",
        state: "current",
      },
    ],
    artifacts: [
  ...
  async getPreview(shareCode: string, accessToken?: string): Promise<ConversationSharePreview> {
    ...
    if (shareCode === "mock-outdated-client") {
      throw new ConversationSharePreviewClientError({
        ...
        message: "Share requires a newer WBrand",
        status: 200,
      });
    }
