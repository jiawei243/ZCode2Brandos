...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    ...
    const wbrandJwtToken = tokenSet.wbrandJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (wbrandJwtToken) {
      this.localStorage.setItem(WBRAND_JWT_TOKEN_KEY, wbrandJwtToken);
    } else {
      this.localStorage.removeItem(WBRAND_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const wbrandJwtToken = this.localStorage.getItem(WBRAND_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(wbrandJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(wbrandJwtToken, this.now()).kind === "expired") {
  ...
  loadWBrandJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(WBRAND_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(WBRAND_JWT_TOKEN_KEY) ||
