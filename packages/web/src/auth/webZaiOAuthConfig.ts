...
function createWebZaiOAuthConfig(env: WebImportMetaEnv = {}): WebZaiOAuthConfig {
  ...
  const wbrandEndpointUrls = buildWBrandEndpointUrls(
    env.VITE_WBRAND_BASE_URL?.trim() ||
      env.VITE_WBRAND_ENDPOINT_ORIGIN?.trim() ||
      DEFAULT_WBRAND_ENDPOINT_ORIGIN,
  );
  ...
  return {
    ...
    // BigModel 用 appId 而不是 client_id，且默认值就是桌面端在用的 "wbrand"。
    bigmodelAppId: env.VITE_BIGMODEL_OAUTH_APP_ID?.trim() || "wbrand",
    redirectUri: wbrandEndpointUrls.webShareCallbackUrl,
    shareRedirectUri: wbrandEndpointUrls.webShareCallbackUrl,
    ...(devOrigin ? { devOrigin } : {}),
