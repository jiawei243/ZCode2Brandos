...
import { connectViaWebSocket } from "@wbrand/client";
...
import type { IPlatformService, RemoteTarget, ServerRemoteInfo } from "@wbrand/shared";
import { WEB_DEFAULT_THEME, resolveWebInitialTheme } from "./webThemeSeed.js";

function resolveWebThemePreference(defaultTheme: Theme = WEB_DEFAULT_THEME): Theme {
  const saved = localStorage.getItem("wbrand-theme");
...
async function renderConversationSharePage(): Promise<void> {
  ...
  document.title = routeLocale === "zh-CN" ? "WBrand 会话分享" : "WBrand Conversation Share";
  ...
  const endpointOrigin =
    import.meta.env.VITE_WBRAND_BASE_URL?.trim().replace(/\/+$/u, "") || window.location.origin;
  const mockMode =
  ...
  const getMockToken = () =>
    mockMode && window.sessionStorage.getItem("wbrand:share:mock-auth") === "owner"
      ? "mock-owner-token"
      : null;
  const onLogout = () => {
    if (mockMode) {
      window.sessionStorage.removeItem("wbrand:share:mock-auth");
  ...
  root.render(
    <ConversationShareLandingLoader
      shareCode={shareCode}
      client={client}
      getAccessToken={() => getMockToken() ?? webAuthService.getWBrandJwtToken()}
      onLogin={(provider) => {
        if (mockMode) {
          window.sessionStorage.setItem("wbrand:share:mock-auth", "owner");
          window.location.reload();
          return;
        }
...
async function bootstrapWebApp() {
  ...
  try {
    ...
    document.title = "WBrand - Web + Server";
