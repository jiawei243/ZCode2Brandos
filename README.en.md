...
<div align="center">
  <img src="public/logo/icons/1024x1024.png" alt="WBrand" width="128" height="128" />
</div>
<p align="center">
...
The Agent CLI and runtime source code lives in [apps/wbrand-cli/](apps/wbrand-cli/) as a regular directory included when you clone this repository. No separate checkout or Git submodule initialization is required.
...
Set `WBRAND_DATA_BASE_DIR` to use a separate development data directory. For example, on macOS / Linux:

```bash
WBRAND_DATA_BASE_DIR="$HOME/.wbrand-dev-home" pnpm dev:desktop:test
```
...
WBRAND_SERVER_WORKSPACE=/path/to/project pnpm dev:web
```
...
After changing Agent source code, run `pnpm --filter @wbrand/cli... build` and restart the service. To validate the complete distribution, extract and run it as described under Packaging → WBrand CLI distribution below.

### WBrand CLI distribution

The command-line distribution includes the TUI, Web client, and Agent behind one `wbrand` command. With no arguments it starts the TUI; a leading `--web` starts Web mode; all other arguments go to the existing Agent CLI. Both modes run locally without Electron.
...
wbrand --web --workspace /path/to/project --port 3030 --no-open
...
When starting the general Web service's HTTP entry directly, configure API/WebSocket authentication with `WBRAND_SERVER_AUTH_TOKEN`. When creating the service programmatically, use the `authToken` option.

See Packaging below for build instructions. `pnpm build:wbrand` only creates the distribution; it does not replace an existing `wbrand` on `PATH`. If the command still points to an older installation or another checkout, check it with `command -v wbrand` on macOS / Linux or `where.exe wbrand` on Windows.
...
```bash
pnpm --filter @wbrand/cli dev --help
pnpm --filter @wbrand/cli dev

# Build the CLI and its workspace dependencies
pnpm --filter @wbrand/cli... build
node apps/wbrand-cli/packages/cli/dist/wbrand.cjs --help
```

This entry runs the Agent CLI directly and does not handle the distribution's `--web` switch. Use `pnpm dev:web` for Web development, or the extracted `bin/wbrand.mjs` shown below to test the unified command.
...
Run `pnpm build:wbrand` to build the CLI/TUI, backend, and Web client, collect the TUI native libraries, workers, and runtime dependencies, then assemble the distribution. Running the distribution still requires Node.js; use the version specified in `mise.toml`.

Before packaging, set the download base URL with `WBRAND_DIST_BASE_URL` in `.env`, `.env.local`, or the process environment, or pass it through `--base-url`. The URL below is a placeholder; replace it with your hosting URL when publishing:

```bash
pnpm build:wbrand --base-url https://downloads.example.com/wbrand/

# When WBRAND_DIST_BASE_URL is already configured
pnpm build:wbrand

# Repackage existing Agent, backend, and Web build outputs
pnpm build:wbrand --skip-build

# Show options for the version, output directory, and more
pnpm build:wbrand --help
```

The version defaults to the root `package.json` version. Output is written to `dist/wbrand/`:

- `releases/<version>/wbrand-<version>.tar.gz`: runtime package.
- `releases/<version>/sha256.txt`: checksum file.
- `latest.json` and `install.sh`: version index and installer.

Upload the entire directory to the configured download base URL. The installer downloads the runtime package from that URL, installs it to `~/.wbrand/runtime` by default, and creates the `wbrand` command in `~/.local/bin`. Override these directories with `WBRAND_DIST_HOME` and `WBRAND_DIST_BIN_DIR`, respectively.
...
```bash
wbrand_version=$(node -p "require('./dist/wbrand/latest.json').version")
mkdir -p dist/wbrand/debug
tar -xzf "dist/wbrand/releases/$wbrand_version/wbrand-$wbrand_version.tar.gz" \
  -C dist/wbrand/debug
# Start the TUI by default
node dist/wbrand/debug/wbrand/bin/wbrand.mjs

# Start Web mode
node dist/wbrand/debug/wbrand/bin/wbrand.mjs --web \
