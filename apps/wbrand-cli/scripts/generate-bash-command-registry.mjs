...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "wbrand-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/wbrand-cli registry:generate`.",
      );
    }
  } finally {
