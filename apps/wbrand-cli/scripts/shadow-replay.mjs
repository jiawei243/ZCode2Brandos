// 影子重放对账（交付 / 测试基建）。
// 用途：把本机真实 CLI 库（默认 ~/.wbrand/cli/db/db.sqlite）的历史会话全量喂给
...
// 运行前先构建：pnpm -C apps/wbrand-cli build（脚本从各包 dist 导入）。
//
// 用法：
...
const [{ createSqliteSessionStore }, hydration, projectionModule, contracts] =
  await Promise.all([
    ...
    pkg("bootstrap/dist/wbrand-protocol-v4/transcript-hydration.js"),
    pkg("bootstrap/dist/wbrand-protocol-v4/product-projection.js"),
...
const sourceDbPath = args.db ?? join(homedir(), ".wbrand", "cli", "db", "db.sqlite");
const limit = args.limit ? Number(args.limit) : Infinity;

let dbPath = sourceDbPath;
if (!args["no-copy"]) {
  const tempDir = mkdtempSync(join(tmpdir(), "wbrand-shadow-replay-"));
