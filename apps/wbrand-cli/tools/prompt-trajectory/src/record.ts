...
interface WBrandBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createWBrandApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface WBrandContractsModule {
...
interface WBrandProviderModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadWBrandModules();
    ...
    const app = await bootstrap.createWBrandApp({
...
async function loadWBrandModules(): Promise<{
  bootstrap: WBrandBootstrapModule;
  contracts: WBrandContractsModule;
  provider: WBrandProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<WBrandBootstrapModule>,
      import(contractsPath) as Promise<WBrandContractsModule>,
      import(providerConfigPath) as Promise<Omit<WBrandProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<WBrandProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built wbrand-cli packages. Run `pnpm --filter @wbrand/bootstrap^... build && pnpm --filter @wbrand/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
