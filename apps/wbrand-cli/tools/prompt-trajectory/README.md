# @wbrand/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting wbrand-cli prompt assembly.
...
```bash
pnpm --filter @wbrand/bootstrap^... build
pnpm --filter @wbrand/bootstrap build

pnpm --filter @wbrand/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/wbrand-prompt-trajectory/basic-live

pnpm --filter @wbrand/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @wbrand/prompt-trajectory derive -- \
  --out /tmp/wbrand-prompt-trajectory/basic-live

pnpm --filter @wbrand/prompt-trajectory model-io -- \
  --input ~/.wbrand/cli/debug/model-io-<session>.jsonl \
  --out /tmp/wbrand-prompt-trajectory/model-io-session
```
...
uses the same wbrand model config resolution as the CLI. The upstream request is
...
`model-io` reads a real WBrand `model-io-*.jsonl` file and turns the main
