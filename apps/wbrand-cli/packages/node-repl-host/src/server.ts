...
import { JsInputJsonSchema } from "@wbrand/contracts/tools/node-repl";
// 值导入必须走 @wbrand/core/repl 这条深路径：barrel 会把 core 的整张图拖进 bundle
// （tool handlers → @wbrand/dynamic-workflow → typescript，实测 21.7MB 且求值即崩
...
} from "@wbrand/core/repl";
import { createComputerUseRuntime, type ComputerUseRuntime } from "@wbrand/wbrand-cua";
import { z } from "zod";
...
const WORKER_KIND = "wbrand-node-repl-call";
export const NODE_REPL_MCP_PROCESS_TITLE = "wbrand-node-repl-mcp";
const pluginRoot = process.env.WBRAND_PLUGIN_ROOT ?? process.cwd();
...
const cuaDocumentationRoot = resolve(
  process.env.WBRAND_CUA_PLUGIN_ROOT ?? pluginRoot,
  "docs",
);
...
function buildRequestMeta(meta: Record<string, unknown> | undefined): NodeReplRequestMeta {
  const parsed = requestContextSchema.safeParse(meta?.["com.wbrand/request-context"]);
  // 安全边界：顶层 MCP _meta 是第三方可扩展字段，不能成为 WBrand session 路由凭据。
...
export function captureComputerUseRuntimeFromEnvironment(
  ...
  const socketPath = env.WBRAND_CUA_PERMISSION_BROKER_SOCKET?.trim();
  if (!socketPath) return undefined;
  return createComputerUseRuntime({
    brokerSocketPath: socketPath,
    refreshMarkerPath: env.WBRAND_CUA_PERMISSION_BROKER_REFRESH_MARKER?.trim(),
  });
}
