import {
  WBRAND_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY,
  WBRAND_MCP_NODE_REPL_CUA_APP_META_KEY,
} from "@wbrand/contracts/mcp";
import { isOfficialCuaImageRefText } from "@wbrand/wbrand-cua/frame-contract";
import { CUA_APP_ASSOCIATIONS_META_KEY } from "@wbrand/wbrand-cua/host-display-contract";
import type { NodeReplRunResult } from "@wbrand/core/repl";
 ...
 * （wbrand-cua 的 frame-pixel-latest-only 测试即此契约），旧帧对模型没有使用价值。
...
export function toMcpRunResult(run: NodeReplRunResult): CallToolResult {
  ...
  delete responseMeta[WBRAND_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
  ...
  delete responseMeta[WBRAND_MCP_NODE_REPL_CUA_APP_META_KEY];
  if (run.cuaApp) responseMeta[WBRAND_MCP_NODE_REPL_CUA_APP_META_KEY] = run.cuaApp;
      ...
      ? {
          _meta: {
            ...
            ...((run.images?.length ?? 0) > 0 || structuredContent.some((block) => block.type === "image")
              ? { "wbrand/nodeReplEmittedImage": true }
              : {}),
