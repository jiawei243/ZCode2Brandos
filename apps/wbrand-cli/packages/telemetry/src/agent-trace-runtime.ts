...
import type {
  ...
  ToolTraceStart,
} from "@wbrand/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "wbrand.agent_turn.turn_number": integer(input.turnNumber),
            "wbrand.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "wbrand.agent_step.step_id": safeId(input.stepId),
            "wbrand.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "wbrand.execution.tool_call_id": safeId(input.toolCallId),
            "wbrand.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "wbrand.context_compaction.trigger": safeEnum(input.trigger),
            "wbrand.context_compaction.phase": safeEnum(input.phase),
            "wbrand.context_compaction.model_mode": safeEnum(input.modelMode),
            "wbrand.context_compaction.outer_attempt": integer(input.outerAttempt),
            "wbrand.context_compaction.max_attempts": integer(input.maxAttempts),
            "wbrand.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "wbrand.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "wbrand.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "wbrand.context_compaction.token_source": safeEnum(input.tokenSource),
            "wbrand.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "wbrand.detached_operation.operation": safeEnum(input.operation),
            "wbrand.detached_operation.execution_kind": safeEnum(input.executionKind),
            "wbrand.detached_operation.trigger": safeEnum(input.trigger),
            "wbrand.detached_operation.target_kind": safeEnum(input.targetKind),
            "wbrand.detached_operation.goal_iteration": integer(input.goalIteration),
            "wbrand.detached_operation.chunk_index": integer(input.chunkIndex),
            "wbrand.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "wbrand.execution.logical_call_id": safeId(input.logicalCallId),
            "wbrand.model_call.operation": safeEnum(input.operation),
            "wbrand.model_call.streaming": input.streaming,
            "wbrand.model_call.model_role": safeEnum(input.modelRole),
            "wbrand.model_call.requested_provider_id": safeString(input.requested.providerId, 128),
            "wbrand.model_call.requested_model": safeString(input.requested.requestedModel, 128),
            "wbrand.model_call.reasoning_capability": safeEnum(input.requested.reasoning.capability),
            "wbrand.model_call.reasoning_requested_state": safeEnum(
              input.requested.reasoning.requestedState,
            ),
            "wbrand.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "wbrand.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "wbrand.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "wbrand.model_call.call_cause": safeEnum(input.callCause),
            "wbrand.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "wbrand.execution.tool_call_id": safeId(parent.toolCallId),
            "wbrand.command_execution.safe_name": safeString(input.safeName, 128),
            "wbrand.command_execution.category": safeEnum(input.category),
            "wbrand.command_execution.command_count": integer(input.commandCount),
            "wbrand.command_execution.shell_kind": safeEnum(input.shellKind),
            "wbrand.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "wbrand.execution.tool_call_id": safeId(parent.toolCallId),
            "wbrand.execution.logical_call_id": parentWriter.logicalCallId,
            "wbrand.execution.model_operation": parentWriter.operation,
            "wbrand.execution.model_role": parentWriter.modelRole,
            "wbrand.model_attempt.request_id": safeId(input.requestId),
            "wbrand.model_attempt.attempt_number": integer(input.attemptNumber),
            "wbrand.model_attempt.max_attempts": integer(input.maxAttempts),
            "wbrand.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "wbrand.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "wbrand.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "wbrand.model_attempt.provider_id": safeString(target.providerId, 128),
            "wbrand.model_attempt.provider_kind": safeEnum(target.providerKind),
            "wbrand.model_attempt.provider_origin": safeString(target.providerOrigin),
            "wbrand.model_attempt.provider_route": safeString(target.providerRoute),
            "wbrand.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "wbrand.model_attempt.transport": safeEnum(input.transport),
            "wbrand.model_attempt.api_operation": safeEnum(input.apiOperation),
            "wbrand.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "wbrand.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "wbrand.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "wbrand.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "wbrand.model_attempt.reasoning_requested_budget_tokens": integer(
...
class TurnWriter extends TrackedBaseWriter implements AgentTurnSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() => this.setAttribute("wbrand.agent_turn.result_type", resultType));
  }
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("wbrand.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("wbrand.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("wbrand.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("wbrand.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("wbrand.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("wbrand.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("wbrand.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("wbrand.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("wbrand.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("wbrand.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("wbrand.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("wbrand.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("wbrand.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("wbrand.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("wbrand.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("wbrand.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("wbrand.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("wbrand.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "wbrand.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("wbrand.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "wbrand.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "wbrand.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "wbrand.model_attempt.reasoning_tokens");
  }

  setCacheReadTokens(tokens: number): void {
    this.setUsage("cacheReadTokens", "cache_read", tokens, "wbrand.model_attempt.cache_read_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "wbrand.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("wbrand.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("wbrand.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("wbrand.model_attempt.provider_error_code", safeString(code, 128));
  }

  setProviderErrorMessage(message: string): void {
    this.setAttribute("wbrand.model_attempt.provider_error_message", sanitizeErrorMessage(message));
  }

  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("wbrand.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("wbrand.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("wbrand.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("wbrand.model_attempt.time_to_first_text_ms", elapsed);
