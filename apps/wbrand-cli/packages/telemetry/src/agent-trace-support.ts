...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@wbrand/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 WBrand Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "wbrand.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "wbrand.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "wbrand.execution.turn_id": safeId(correlation.turnId),
    "wbrand.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "wbrand.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "wbrand.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "wbrand.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "wbrand.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "wbrand.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "wbrand.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
