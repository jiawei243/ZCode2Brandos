...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@wbrand/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@wbrand/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("wbrand.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("wbrand.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "wbrand.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "wbrand.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("wbrand.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("wbrand.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "wbrand.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("wbrand.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("wbrand.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("wbrand.model.attempt.tokens"),
    cardinalityView("wbrand.model.attempt.stream_stall.count"),
    cardinalityView("wbrand.telemetry.creation_drop.count"),
    cardinalityView("wbrand.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "wbrand.build.commit_id": resource.buildCommitId,
    "wbrand.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "wbrand.product.version": resource.productVersion,
    "wbrand.runtime.distribution": resource.runtimeDistribution,
    "wbrand.runtime.surface": resource.runtimeSurface,
    "wbrand.telemetry.schema_owner": "cli",
    "wbrand.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
