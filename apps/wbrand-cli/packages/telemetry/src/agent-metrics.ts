...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "wbrand.agent.turn.duration"),
      agent_step: duration(meter, "wbrand.agent.step.duration"),
      tool_execution: duration(meter, "wbrand.tool.execution.duration"),
      command_execution: duration(meter, "wbrand.command.execution.duration"),
      context_compaction: duration(meter, "wbrand.context.compaction.duration"),
      detached_operation: duration(meter, "wbrand.detached.operation.duration"),
      model_call: duration(meter, "wbrand.model.call.duration"),
      model_attempt: duration(meter, "wbrand.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("wbrand.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("wbrand.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "wbrand.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "wbrand.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "wbrand.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("wbrand.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "wbrand.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "wbrand.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("wbrand.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("wbrand.telemetry.abandoned.count", {
