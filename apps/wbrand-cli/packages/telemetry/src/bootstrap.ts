...
import type { ModelStatusSink } from "@wbrand/contracts/model";
...
export async function prepareModelTelemetryEnv(
  ...
  if (!resolveOtlpTraceEndpoint(env) || isExplicitlyDisabled(env.WBRAND_MODEL_TELEMETRY_ENABLED)) {
    return env;
  }
  const existingInstallationId = normalizeTelemetryDeviceMid(env.WBRAND_TELEMETRY_DEVICE_MID);
  const installationId =
    existingInstallationId ?? (await resolveStandaloneDeviceMid(env.WBRAND_HOME?.trim()));
  const preparedEnv = installationId ? { ...env, WBRAND_TELEMETRY_DEVICE_MID: installationId } : env;
...
async function createPreparedOwner(
  ...
  const resource: TelemetryResourceContext = {
    buildCommitId: normalizeBuildCommitId(options.buildCommitId ?? env.WBRAND_BUILD_COMMIT_ID),
    cliVersion: normalizeVersion(options.cliVersion),
    deploymentEnvironment:
      env.WBRAND_ENV ??
      env.WBRAND_RUNTIME_ENV ??
      resourceAttributesFromEnv(env)["deployment.environment.name"],
    installationId: normalizeTelemetryDeviceMid(env.WBRAND_TELEMETRY_DEVICE_MID),
    productVersion: normalizeVersion(options.productVersion ?? env.WBRAND_APP_VERSION),
    runtimeDistribution:
      options.runtimeDistribution ??
      resolveRuntimeDistribution(env.WBRAND_TELEMETRY_RUNTIME_DISTRIBUTION),
    runtimeSurface:
      options.runtimeSurface ?? resolveRuntimeSurface(env.WBRAND_TELEMETRY_RUNTIME_SURFACE),
    serviceInstanceId: randomUUID(),
    serviceName: env.OTEL_SERVICE_NAME?.trim() || "wbrand-cli-agent",
  };
  try {
...
function resolveTelemetryIdentity(env: EnvRecord): TelemetryIdentitySnapshot {
  const userSubjectId = normalizeTelemetryId(env.WBRAND_TELEMETRY_USER_SUBJECT_ID);
  const configuredState = env.WBRAND_TELEMETRY_IDENTITY_STATE?.trim();
  const identityState =
    ...
    configuredState === "unknown"
      ...
      : userSubjectId
        ? "authenticated"
        : normalizeTelemetryDeviceMid(env.WBRAND_TELEMETRY_DEVICE_MID)
          ? "anonymous"
          : "unknown";
...
async function resolveStandaloneDeviceMid(
  wbrandHome: string | undefined,
): Promise<string | undefined> {
  const stateFile = wbrandHome
    ? join(wbrandHome, "v2", "telemetry-state.json")
    : join(homedir(), ".wbrand", "v2", "telemetry-state.json");
