...
import { createNodeCustomCommandAdapter } from "@wbrand/adapters/commands";
import type {
...
export interface ListWBrandCustomCommandsOptions {
...
export interface InspectWBrandCustomCommandOptions extends ListWBrandCustomCommandsOptions {
  name: string;
}

export interface WBrandCustomCommandInspection {
  command: CustomCommandContent;
  diagnostics: CustomCommandDiagnostic[];
}

export async function listWBrandCustomCommands(
...
export async function inspectWBrandCustomCommand(
  options: InspectWBrandCustomCommandOptions,
): Promise<WBrandCustomCommandInspection> {
  const discovery = createCustomCommandDiscovery(options);
...
export async function loadWBrandCustomCommand(
  options: InspectWBrandCustomCommandOptions,
...
function createCustomCommandDiscovery(options: ListWBrandCustomCommandsOptions) {
