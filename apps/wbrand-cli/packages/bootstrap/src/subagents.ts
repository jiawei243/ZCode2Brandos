...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@wbrand/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@wbrand/core";
import type { Logger, PluginMetadata } from "@wbrand/contracts";
import {
  createAgentStateId,
...
interface LoadWBrandAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadWBrandAgentProfilesResult {
...
export async function loadWBrandAgentProfiles(
  input: LoadWBrandAgentProfilesInput,
): Promise<LoadWBrandAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".wbrand", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .wbrand/agents/*.md 是仓库内容，不能通过 frontmatter
