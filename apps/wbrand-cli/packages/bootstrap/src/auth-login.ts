import {
  ...
  createSharedWBrandCredentialStore,
  createCliOAuthClient,
  createCliOAuthPollToken,
  openUrlInBrowser,
  SHARED_WBRAND_CREDENTIAL_KEYS,
  type BrowserOpenResult,
  type SharedWBrandCredentialStore,
  type CliOAuthClient,
  type CliOAuthInitData,
  type CliOAuthPollData,
  type CliOAuthUser,
} from "@wbrand/adapters";
import { createConfig } from "@wbrand/adapters/config";
import { createNodeHttpClientAdapter } from "@wbrand/adapters/http";
import type { EnvRecord } from "@wbrand/adapters/model";
import { buildWBrandEndpointUrls, resolveRuntimeWBrandEndpointOrigin } from "@wbrand/shared";
import {
...
export interface LoginWBrandCliOptions {
  providerId?: CodingPlanProviderId;
  abortSignal?: AbortSignal;
  apiKeyResolver?: ReturnType<typeof createCodingPlanApiKeyResolver>;
  baseUrl?: string;
  credentialStore?: SharedWBrandCredentialStore;
  env?: EnvRecord;
...
export interface LoginWBrandCliResult {
...
export type LoginBigmodelCodingPlanOptions = Omit<LoginWBrandCliOptions, "providerId">;
export type LoginBigmodelCodingPlanResult = LoginWBrandCliResult & { providerId: "bigmodel" };
...
export interface LogoutWBrandCliOptions {
  credentialStore?: SharedWBrandCredentialStore;
  env?: EnvRecord;
}

export interface LogoutWBrandCliResult {
  credentialsPath: string;
}
...
export async function hasConfiguredStandaloneCodingPlan(
  ...
  const credentialStore =
    options.credentialStore ?? createSharedWBrandCredentialStore({ env: options.env });
  return hasStandaloneCodingPlanAccess(credentialStore, options.env ?? process.env);
}

export class WBrandCliLoginError extends Error {
  readonly code:
    | "auth_failed"
    | "auth_timeout"
  ...
  constructor(
    code: WBrandCliLoginError["code"],
    message: string,
    options: { cause?: unknown } = {},
  ) {
    super(message, options);
    this.name = "WBrandCliLoginError";
    this.code = code;
  }
}

export async function loginWBrandCli(
  options: LoginWBrandCliOptions = {},
): Promise<LoginWBrandCliResult> {
  ...
  const timeoutError = () =>
    new WBrandCliLoginError("auth_timeout", "Authorization timed out. Please retry login.");
  ...
  try {
    ...
    const credentialStore = options.credentialStore ?? createSharedWBrandCredentialStore({ env });
    ...
    const readyData = await pollUntilReady({
      ...
      createError: (code) =>
        code === "auth_timeout"
          ? timeoutError()
          : new WBrandCliLoginError(code, "Authorization failed. Please retry login."),
    });
    ...
    try {
      if (providerId === "zai") {
        ...
        await credentialStore.saveMany({
          [SHARED_WBRAND_CREDENTIAL_KEYS.activeProvider]: providerId,
          [SHARED_WBRAND_CREDENTIAL_KEYS.wbrandJwtToken]: readyData.token,
          [SHARED_WBRAND_CREDENTIAL_KEYS.bigmodelAccessToken]: readyData.accessToken,
          ...(readyData.refreshToken
            ? { [SHARED_WBRAND_CREDENTIAL_KEYS.bigmodelRefreshToken]: readyData.refreshToken }
            : {}),
          [SHARED_WBRAND_CREDENTIAL_KEYS.bigmodelUserInfo]: JSON.stringify({
    ...
    try {
      ...
      throw new WBrandCliLoginError(
        "config_update_failed",
        "Login succeeded but updating WBrand config failed.",
        { cause: error },
      );
    }
    return {
...
export async function loginBigmodelCodingPlan(
  ...
  return {
    ...(await loginWBrandCli({ ...options, providerId: "bigmodel" })),
    providerId: "bigmodel",
  };
}
...
export async function configureCodingPlanApiKey(
  ...
  if (!apiKey) {
    throw new WBrandCliLoginError("config_update_failed", "API key must not be empty.");
  }
  const credentialStore =
    options.credentialStore ?? createSharedWBrandCredentialStore({ env: options.env });
...
export async function logoutWBrandCli(
  options: LogoutWBrandCliOptions = {},
): Promise<LogoutWBrandCliResult> {
  const credentialStore =
    options.credentialStore ?? createSharedWBrandCredentialStore({ env: options.env });
  ...
  const keys = [
    ...Object.values(SHARED_WBRAND_CREDENTIAL_KEYS),
    ...identityKeys,
    ...dynamicApiKeyKeys,
  ];
...
async function persistStandaloneCodingPlanConnection(input: {
  ...
  readonly credentialStore: SharedWBrandCredentialStore;
  ...
  const path =
    ...
    input.env[WBRAND_PERSONAL_PROVIDER_CONFIG_FILE_ENV]?.trim() ??
...
function createOAuthClient(options: LoginWBrandCliOptions, env: EnvRecord): CliOAuthClient {
  return createCliOAuthClient({
    baseUrl:
      options.baseUrl ?? buildWBrandEndpointUrls(resolveCliWBrandEndpointOrigin(env)).apiBaseUrl,
    providerId: options.providerId ?? "zai",
    httpClient: options.httpClient ?? createDefaultHttpClient(env),
  });
}

function resolveCliWBrandEndpointOrigin(env: EnvRecord): string {
  return resolveRuntimeWBrandEndpointOrigin(env);
}
