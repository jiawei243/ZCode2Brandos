...
import { WBRAND_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@wbrand/contracts";
import { WBRAND_CUA_OFFICIAL_PLUGIN_ID, isWBrandCuaInternalFeatureEnabled } from "@wbrand/shared";
...
export interface ResolveWBrandPluginsOptions {
...
export interface ListWBrandPluginsOptions extends ResolveWBrandPluginsOptions {}

export interface SetWBrandPluginEnabledOptions extends ResolveWBrandPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetWBrandPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface WBrandMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface WBrandAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface WBrandInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface WBrandPluginsOverviewData {
  marketplaces: WBrandMarketplaceSummaryData[];
  availablePlugins: WBrandAvailablePluginData[];
  installedPlugins: WBrandInstalledPluginData[];
  restorableBuiltins: WBrandAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface WBrandMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: WBrandMarketplaceSummaryData[];
}

export interface AddWBrandMarketplaceOptions extends ResolveWBrandPluginsOptions {
...
export interface RemoveWBrandMarketplaceOptions extends ResolveWBrandPluginsOptions {
  marketplace: string;
}

export interface UpdateWBrandMarketplaceOptions extends ResolveWBrandPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallWBrandMarketplacePluginOptions extends ResolveWBrandPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallWBrandMarketplacePluginOptions extends ResolveWBrandPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`wbrand plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateWBrandMarketplacePluginOptions extends ResolveWBrandPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateWBrandPluginPathOptions extends ResolveWBrandPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface WBrandPluginUpdateData extends WBrandPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveWBrandPluginsOptions {
  pluginId: string;
}

interface ConfigureWBrandPluginOptions extends ResolveWBrandPluginsOptions {
...
interface ResetWBrandPluginConfigOptions extends ResolveWBrandPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateWBrandPluginOptions extends ResolveWBrandPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeWBrandPluginOptions extends ResolveWBrandPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface WBrandPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== WBRAND_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveWBrandPlugins(options: ResolveWBrandPluginsOptions = {}): PluginLoadOutcome {
...
export function getWBrandPluginsOverview(
  options: ResolveWBrandPluginsOptions = {},
  ...
  // wbrand-plugins-official 的内置与 CDN 分片已在 adapter 层合并为唯一 canonical manifest。
  const catalogs: Array<{
    summary: WBrandMarketplaceSummaryData;
  ...
  const restorableBuiltins: WBrandAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isWBrandCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
      marketplace: WBRAND_OFFICIAL_PLUGIN_MARKETPLACE,
      version: def.version,
      installed: false,
      ...(listing ? { listing } : {}),
    };
...
export function listWBrandPlugins(options: ListWBrandPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveWBrandPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setWBrandPluginEnabled(
  options: SetWBrandPluginEnabledOptions,
): Promise<SetWBrandPluginEnabledResult> {
...
export async function addWBrandPluginMarketplace(
  options: AddWBrandMarketplaceOptions,
): Promise<WBrandMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeWBrandPluginMarketplace(
  options: RemoveWBrandMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateWBrandPluginMarketplace(
  options: UpdateWBrandMarketplaceOptions,
): Promise<WBrandMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installWBrandMarketplacePlugin(
  options: InstallWBrandMarketplacePluginOptions,
): Promise<WBrandPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === WBRAND_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === WBRAND_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallWBrandMarketplacePlugin(
  options: UninstallWBrandMarketplacePluginOptions,
): Promise<WBrandInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 wbrand-plugins-official id 空间，且其缓存
...
/**
 * `wbrand plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateWBrandMarketplacePlugin(
  options: UpdateWBrandMarketplacePluginOptions,
): Promise<WBrandPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateWBrandPluginMarketplace({
    ...options,
  ...
  const installed = await installWBrandMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `wbrand plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateWBrandPluginPath(
  options: ValidateWBrandPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const wbrandCuaPluginId = WBRAND_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === wbrandCuaPluginId &&
    !isWBrandCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires WBRAND_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureWBrandPlugin(options: ConfigureWBrandPluginOptions): Promise<void> {
...
export async function resetWBrandPluginConfig(
  options: ResetWBrandPluginConfigOptions,
...
export async function validateWBrandPlugin(
  options: ValidateWBrandPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeWBrandPlugin(
  options: DescribeWBrandPluginOptions,
...
function resolvePluginContext(options: ResolveWBrandPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallWBrandMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveWBrandPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.wbrand/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".wbrand", "config.json");
