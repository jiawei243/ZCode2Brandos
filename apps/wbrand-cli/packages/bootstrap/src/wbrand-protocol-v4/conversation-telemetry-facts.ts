...
import { getModelUsageTotalTokens, SessionEventType } from "@wbrand/contracts";
import { parseAutomationRunId } from "@wbrand/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const wbrand = recordValue(meta.wbrand);
  return (
    ...
    optionalString(wbrand.parentToolCallId) ??
    optionalString(wbrand.parentToolUseId)
  );
}
