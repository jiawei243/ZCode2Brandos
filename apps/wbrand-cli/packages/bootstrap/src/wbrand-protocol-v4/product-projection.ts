import { PERMISSION_FULL_ACCESS_OPTION_ID } from "@wbrand/shared/wbrand-protocol-v4";
...
import { verdictWorkspaceHookReviewRequest } from "@wbrand/shared/workspace-hook-review-monotonicity";
import {
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  isWBrandModelRetryRecoveryProgressPayload,
  isWBrandFileStreamingToolInputPreviewTool,
  parseWBrandBackgroundTaskNotificationText,
  resolveWBrandBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseWBrandBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = wbrandBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isWBrandFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < WBRAND_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveWBrandBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @wbrand/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。
