...
import { localTtftNow, localTtftFactsSchema } from "@wbrand/shared/wbrand-protocol-v4";
import {
...
import { SessionEventType, isFileSystemPortError } from "@wbrand/contracts";
import type { WBrandWorkspaceRef } from "@wbrand/shared";
import { extractMarkdownArtifactImageRefs } from "@wbrand/shared";
import type {
  CommandAck,
  AttachmentRef,
  CommandEnvelope,
...
export interface V4GatewayHost {
  ...
  emitLocalTtftFacts?(facts: import("@wbrand/shared").LocalTtftFacts): void;
...
function artifactRefBelongsToSession(ref: string, sessionId: string): boolean {
  return ref.startsWith(`wbrand-artifact://${encodeURIComponent(sessionId)}/`);
}
...
function toShareStatFault(error: unknown): unknown {
  if (readWBrandAttachmentFaultCode(error)) return error;
  if (isFileSystemPortError(error) && MISSING_ATTACHMENT_FS_CODES.has(error.code)) {
    return new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.shareStatNotFound, {
      cause: error,
    });
  }
  return error;
...
export class ConversationV4Gateway {
   ...
   * effect 之前，而 CLI 严格串行派发请求（`wbrand-protocol/transport.ts`，只有 session/stop
  ...
  async conversationAttachmentRead(
    ...
    if (!this.host.readSessionAttachment) {
      throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.readUnsupported);
    }
    const existingReady = this.readyFlights.get(params.sessionId);
    const publisher = existingReady
      ? await existingReady
    ...
    if (row?.kind !== "userInput") {
      throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized);
    }
  ...
  async conversationAttachmentStat(
    ...
    if (row?.kind !== "userInput") {
      throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    }
    const attachment = row.attachments?.[params.attachmentIndex];
    if (!attachment || (attachment.ref !== params.ref && attachment.previewRef !== params.ref)) {
      throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    ...
    if (result.totalBytes > PROTOCOL_V4_LIMITS.attachmentStatMaxBytes) {
      throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.shareStatTooLarge);
    }
    ...
    const payload = this.host.readSessionAttachment!(sessionId, {
      ...
      .then((result) => {
        ...
        if (
          ...
          throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.previewNotMedia);
        }
        if (result.bytes.byteLength > maxBytes) {
          throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.previewTooLarge);
        }
        const current = this.binaryReadCache.get(key);
        if (current) {
  ...
  private emitReservation<F extends RoutedTopicFrame>(
    ...
    if (
      ...
      const observations: import("@wbrand/shared").LocalTtftFacts[] = [];
