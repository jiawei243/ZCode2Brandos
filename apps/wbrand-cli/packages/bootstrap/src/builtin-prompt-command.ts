...
interface ResolveWBrandBuiltinPromptCommandOptions {
  workingDirectory?: string;
}

export function resolveWBrandBuiltinPromptCommand(
  input: string,
  options: ResolveWBrandBuiltinPromptCommandOptions = {},
): string | undefined {
...
function buildInitAgentsPrompt(params: {
  ...
  return [
    "You are running WBrand's built-in /init command.",
    "",
    "Your task is to create or update a concise workspace instruction file for future WBrand agents.",
    "",
    "Target:",
    `- Workspace directory: ${params.workingDirectory}`,
    `- Instruction file: ${params.targetPath}`,
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".wbrand", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.wbrand/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .wbrand/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
    ...
    "6. Include only project-specific facts future WBrand agents would otherwise miss.",
