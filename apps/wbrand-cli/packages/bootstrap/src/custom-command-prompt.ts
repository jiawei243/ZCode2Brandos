...
import { loadWBrandCustomCommand, type ListWBrandCustomCommandsOptions } from "./custom-commands.js";
import { expandCustomCommandShellSyntax } from "./custom-command-shell-expansion.js";
import { isReservedWBrandSlashCommandName } from "./slash-command-surface.js";

const CUSTOM_COMMAND_NOT_FOUND_PATTERN = /not found/i;
const PROMPT_CUSTOM_COMMAND_PATTERN = /^\/([^\s]+)(?:\s+([\s\S]*))?$/;

interface ResolveWBrandCustomCommandPromptOptions extends ListWBrandCustomCommandsOptions {
  /**
   * 本会话不可寻址的自定义命令名（小写）。与 `isReservedWBrandSlashCommandName` 是同一个
   * 概念的两个来源：那个是全局保留字，这个是调用方按会话下发的门禁，目前唯一使用者是
   * 动态工作流灰度关闭时的 `workflow`。
   * 刻意按**命令名**而不是禁用路径表达：`workflow` 来自随 CLI 打包的 wbrand-guide 插件，
...
export async function resolveWBrandCustomCommandPrompt(
  input: string,
  options: ResolveWBrandCustomCommandPromptOptions = {},
): Promise<string | undefined> {
  const invocation = parsePromptCustomCommandInvocation(input);
  if (!invocation || isReservedWBrandSlashCommandName(invocation.name)) {
    return undefined;
  }
  ...
  try {
    const command = await loadWBrandCustomCommand({
      ...options,
