...
import {
  getCapturedWBrandCuaBrokerCredentials,
  isWBrandCuaMcpCommand,
  isWBrandCuaMcpPackageArg,
  WBRAND_CUA_BROKER_SOCKET_ENV_KEY,
  WBRAND_CUA_NODE_REPL_HOST_ENV_KEY,
  WBRAND_CUA_OFFICIAL_PLUGIN_ID,
  WBRAND_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  WBRAND_PLUGIN_ID_ENV_KEY,
} from "@wbrand/shared";

export { WBRAND_CUA_BROKER_SOCKET_ENV_KEY as WBRAND_CUA_BROKER_SOCKET_ENV } from "@wbrand/shared";
// CLI 入口会先清理 broker 凭据；shared node_repl 的可信配置随后从进程内捕获快照恢复它们。
function resolveWBrandCuaBrokerSocket(): string | undefined {
  // captured 优先；运行时残留的 stale socket 不能覆盖可信快照。
  return (
    getCapturedWBrandCuaBrokerCredentials().socket ||
    process.env[WBRAND_CUA_BROKER_SOCKET_ENV_KEY]?.trim()
  );
}

function resolveWBrandCuaBrokerToken(): string | undefined {
  return undefined;
}

const NODE_REPL_SERVER_NAME = "node_repl";
const REFRESH_MARKER_ENV = "WBRAND_CUA_PERMISSION_BROKER_REFRESH_MARKER";
    ...
    Object.entries(pluginServers)
      .filter(
        ([name, config]) =>
          ...
          config.type === "stdio" &&
          config.env?.[WBRAND_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() ===
            WBRAND_CUA_OFFICIAL_PLUGIN_ID,
      )
      .map(([name]) => name),
  );
...
export async function listMcpServerStatuses(
  ...
  for (const [name, config] of Object.entries(servers)) {
    // 迁移后旧的 wbrand-cua MCP 只是历史配置，不再进入状态投影；CUA
...
export function omitMcpServers(
  ...
  return injectWBrandCuaBrokerMcpServers(
    kept,
    resolveWBrandCuaBrokerSocket(),
    resolveWBrandCuaBrokerToken(),
    trustedOfficialCuaServerNames,
  );
}

function injectWBrandCuaBrokerMcpServers(
  servers: Record<string, McpServerConfig>,
...
function injectCuaCredentialsIntoNodeRepl(
  ...
  const captured = getCapturedWBrandCuaBrokerCredentials();
  ...
  return {
    ...
    env: {
      ...config.env,
      [WBRAND_CUA_BROKER_SOCKET_ENV_KEY]: socketPath,
      ...(refreshMarker ? { [REFRESH_MARKER_ENV]: refreshMarker } : {}),
      ...(pluginAuthority ? { [WBRAND_CUA_PLUGIN_AUTHORITY_ENV_KEY]: pluginAuthority } : {}),
      [WBRAND_CUA_NODE_REPL_HOST_ENV_KEY]: "1",
      [WBRAND_PLUGIN_ID_ENV_KEY]: WBRAND_CUA_OFFICIAL_PLUGIN_ID,
    },
  };
}

function isWBrandCuaStdioServer(
  name: string,
  config: McpServerConfig,
): config is McpStdioServerConfig {
  if (config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 内置 official wbrand-cua plugin 的 MCP server 走 __wbrand-plugin-host，command 是 Helper
  // (非 wbrand-cua)、args 是 [wbrand.cjs, __wbrand-plugin-host, server.js]（非 wbrand-cua package arg），
  // 上面的 name/command/args 三条都匹配不到。_plugin id 由 adapters resolver 权威写入 env
  // （manifest/user env 不可覆盖），用它识别 official plugin server。
  if (
    config.env?.[WBRAND_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() === WBRAND_CUA_OFFICIAL_PLUGIN_ID
  ) {
    return true;
  }
  // 判定与 desktop/services 共用 @wbrand/shared 的单一事实源，避免两条注入入口漂移。
  if (isWBrandCuaMcpCommand(config.command)) return true;
  return (config.args ?? []).some(isWBrandCuaMcpPackageArg);
}
...
function isRetiredCuaMcpServer(name: string, config: McpServerConfig | undefined): boolean {
  ...
  return (
    name !== NODE_REPL_SERVER_NAME && config !== undefined && isWBrandCuaStdioServer(name, config)
  );
}
