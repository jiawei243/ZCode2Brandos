...
import {
  WBRAND_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type WBrandProcessResourceSample,
} from "@wbrand/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const WBRAND_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = WBRAND_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateWBrandProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: WBrandProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: WBrandProcessResourceSample["platform"];
  arch?: WBrandProcessResourceSample["arch"];
...
export interface WBrandProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createWBrandProcessResourceSampler(
  options: CreateWBrandProcessResourceSamplerOptions,
): WBrandProcessResourceSampler {
  const platform = options.platform ?? (process.platform as WBrandProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as WBrandProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: WBrandProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, WBRAND_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
