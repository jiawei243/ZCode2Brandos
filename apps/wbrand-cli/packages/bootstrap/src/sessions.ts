...
import { openStartupSqliteSessionStore } from "@wbrand/adapters/storage";
import type { SessionInfo } from "@wbrand/contracts";
import type { ListWBrandSessionsOptions, ResolveLatestSessionOptions } from "./app/types.js";
...
export async function listWBrandSessions(
  options: ListWBrandSessionsOptions = {},
): Promise<SessionInfo[]> {
  const ownsStore = options.sessionStore === undefined;
  let sessionStore = options.sessionStore;
  if (!sessionStore) {
