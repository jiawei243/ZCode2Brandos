...
import type { SharedWBrandCredentialStore } from "@wbrand/adapters/auth";
import type { ProviderRuntimeHeadersPort } from "@wbrand/core";
import {
...
async function readStandaloneCodingPlanCatalog(
  ...
  if (config)
    return {
      wbrandBuiltinRevision: config.revision,
  ...
  const filePath = env[WBRAND_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  if (!filePath) {
    throw new Error(`${WBRAND_BUILTIN_PROVIDER_CONFIG_FILE_ENV} is required for login`);
  }
...
export async function resolveStandaloneCodingPlanProvider(
  ...
  if (matches.length !== 1) {
    throw new Error(
      `WBrand Built-in Config 必须为 ${family} 声明唯一 Individual Coding Plan Provider`,
    );
  }
  return matches[0]!;
}
...
export async function readStandaloneAccountProviderConfigSnapshot(
  credentialStore: Pick<SharedWBrandCredentialStore, "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
  config?: Pick<ProviderConfigLayerSnapshot, "revision" | "providers">,
  ...
  return createAccountProviderConfigSnapshot(catalog.wbrandBuiltinRevision, providers);
}
...
export function createStandaloneProviderRuntimeHeadersPort(
  credentialStore: Pick<SharedWBrandCredentialStore, "load" | "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
