import { updateUiLocaleInFileConfig, type ConfigResult } from "@wbrand/adapters/config";
import type { AgentRuntime } from "@wbrand/core";
import { resolveLocale } from "@wbrand/i18n";
import { normalizeModelSelection, type ModelSelection } from "@wbrand/provider";
import {
...
import type { PrepareUserExecutionBoundary, WBrandApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<WBrandApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<WBrandApp["updateTargetStatus"]>
      >,
