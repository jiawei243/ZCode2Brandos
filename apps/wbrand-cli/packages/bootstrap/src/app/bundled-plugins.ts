...
import { writeBundledOfficialMarketplacePartitionSync } from "@wbrand/adapters";
import { WBRAND_OFFICIAL_PLUGIN_MARKETPLACE, type Logger } from "@wbrand/contracts";
import { isWBrandCuaInternalFeatureEnabled, WBRAND_CUA_OFFICIAL_PLUGIN_ID } from "@wbrand/shared";
import {
...
const OFFICIAL_PLUGIN_MARKETPLACE = WBRAND_OFFICIAL_PLUGIN_MARKETPLACE;
const SEA_PLUGIN_ASSET_PREFIX = "wbrand-official-plugins/";
const SEA_PLUGIN_MANIFEST_ASSET_KEY = `${SEA_PLUGIN_ASSET_PREFIX}manifest.json`;
const SEED_MARKER_FILE = ".wbrand-plugin-seed.json";
    ...
    if (plugin.missingSeedPaths.length > 0) {
      warnCacheDegraded(input.logger, {
        error: Object.assign(
          ...
          { code: "WBRAND_PLUGIN_SEED_INCOMPLETE" },
        ),
...
export function resolveOfficialPluginRoots(input: {
  ...
  // wbrand-cua 内置 plugin 默认不启用，由 feature flag 控制加载。在 seed/discovery 层门控
  // （而非只隐藏某个 UI 面），这样开关关闭时用户无法经 plugin 列表/marketplace/MCP 设置/CLI 命令看到它。
  if (!isWBrandCuaInternalFeatureEnabled(input.env ?? process.env)) {
    suppressedBuiltins.add(WBRAND_CUA_OFFICIAL_PLUGIN_ID);
  }
  ...
  const fallbackRoots = failedSeeds.flatMap((definition) => {
    ...
    if (`${definition.name}@${OFFICIAL_PLUGIN_MARKETPLACE}` === WBRAND_CUA_OFFICIAL_PLUGIN_ID) {
      return [];
    }
...
function resolveFilesystemPluginRoot(definition: OfficialPluginDefinition): string | undefined {
  for (const baseDir of candidateBaseDirs()) {
    for (const relativePath of definition.rootCandidates) {
      ...
      if (existsSync(join(rootPath, ".wbrand-plugin", "plugin.json"))) return rootPath;
    }
  }
  return undefined;
}
...
function readSeedPluginDescription(
  ...
  const manifestFile = plugin.files.find((file) => file.path === ".wbrand-plugin/plugin.json");
...
function isSeedUsable(targetRoot: string, definition: OfficialPluginDefinition): boolean {
  try {
    const manifest = JSON.parse(
      readFileSync(join(targetRoot, ".wbrand-plugin", "plugin.json"), "utf8"),
    ) as { name?: unknown };
...
function candidateBaseDirs(): string[] {
  // Electron app-server 运行在 resources/glm/wbrand.cjs，官方插件资源也随桌面包
