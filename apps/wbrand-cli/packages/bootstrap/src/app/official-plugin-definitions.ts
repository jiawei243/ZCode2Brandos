import { WBRAND_OFFICIAL_PLUGIN_MARKETPLACE } from "@wbrand/contracts";
...
export const OFFICIAL_BROWSER_USE_PLUGIN_ID = `${OFFICIAL_BROWSER_USE_PLUGIN_NAME}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`;
/**
...
export const OFFICIAL_NODE_REPL_HOST_PLUGIN_ID = `${OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`;
const OFFICIAL_CUA_PLUGIN_NAME = "computer-use";
export const OFFICIAL_CUA_PLUGIN_ID = `${OFFICIAL_CUA_PLUGIN_NAME}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`;

export interface OfficialPluginDefinition {
  // 内容型 plugin (无 MCP server / 无系统依赖) 可以设为 true,
  // 这样用户首次 `/skill <name>` 就能用,不必先 `wbrand plugins enable`。
...
// wbrand-guide 原本没有 requiredSeedPaths，seed 丢文件时会静默装出一个
// 没有 /workflow 命令的插件——症状是命令不存在，没有任何诊断。commands/ 与技能正文都钉住。
const OFFICIAL_WBRAND_GUIDE_REQUIRED_SEED_PATHS = [
...
export const OFFICIAL_PLUGIN_DEFINITIONS: readonly OfficialPluginDefinition[] = [
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: {
        "zh-CN": "操作 WBrand 内置浏览器，检查网页并验证交互。",
      },
    },
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: {
        "zh-CN": "开发、校验 WBrand 插件，完成本地 dev 市场安装、试用与更新。",
      },
    },
    rootCandidates: [
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: { "zh-CN": "创建、编辑和验证可复用的 WBrand 技能。" },
    },
    name: "skill-creator",
    rootCandidates: [
  ...
  },
  {
    ...
    // 让用户/agent 开箱即用地拿到 WBrand 配置指南、自诊断技能与 dynamic workflow 编写指南。
    defaultEnabled: true,
    listing: {
      author: ZAI_AUTHOR,
      category: "utilities",
      displayName: "WBrand Guide",
      displayName_i18n: { "zh-CN": "WBrand 使用指南" },
      icon: `${OFFICIAL_PLUGIN_ASSETS_BASE_URL}/wbrand-guide/icon.png`,
      description_i18n: {
        "zh-CN": "提供 WBrand 配置指南与插件、技能、MCP、命令和钩子诊断。",
      },
      examplePrompts: [
        "How do I configure MCP servers in WBrand?",
        "Diagnose my current WBrand setup",
      ],
      examplePrompts_i18n: {
        "zh-CN": ["WBrand 里怎么配置 MCP 服务器？", "帮我诊断当前的 WBrand 配置"],
      },
    },
    name: "wbrand-guide",
    requiredSeedPaths: OFFICIAL_WBRAND_GUIDE_REQUIRED_SEED_PATHS,
    rootCandidates: [
      "packages/wbrand-guide-plugin",
      "../wbrand-guide-plugin",
      "../../wbrand-guide-plugin",
      "../../../wbrand-guide-plugin",
    ],
  ...
  },
  {
    ...
    // isWBrandCuaInternalFeatureEnabled（打包层默认 true）与输入框入口 hidden 默认值的联动语义。
    name: "computer-use",
    hostMcpServerNames: ["node_repl"],
    // 用户露出名统一为「Computer Use / 电脑控制」。包名与 producer 仓库仍保持 wbrand-cua，
    ...
    listing: {
      ...
      // 插件更名为 computer-use 后，CDN 图标仍发布在 wbrand-cua 目录；沿用资源路径避免 404。
      icon: `${OFFICIAL_PLUGIN_ASSETS_BASE_URL}/wbrand-cua/icon.png`,
    },
    rootCandidates: [
      "packages/wbrand-cua-plugin",
      "../wbrand-cua-plugin",
      "../../wbrand-cua-plugin",
      "../../../wbrand-cua-plugin",
    ],
    ...
    // 这里的 version 追踪上游 wbrand-cua runtime 版本，使插件 UI 展示、缓存路径、
...
// 注意: 任何解析 plugin 的入口 (CLI 子命令 resolveWBrandPlugins、应用启动 resolveStartupPlugins)
...
export const DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS: ReadonlySet<string> = new Set(
  OFFICIAL_PLUGIN_DEFINITIONS.filter((definition) => definition.defaultEnabled).map(
    (definition) => `${definition.name}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`,
  ),
);
...
export function resolveOfficialPluginHostMcpServerNames(pluginId: string): string[] {
  const definition = OFFICIAL_PLUGIN_DEFINITIONS.find(
    (candidate) => `${candidate.name}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}` === pluginId,
  );
