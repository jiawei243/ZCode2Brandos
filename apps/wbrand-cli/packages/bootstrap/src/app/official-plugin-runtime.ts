...
import {
  WBRAND_OFFICIAL_PLUGIN_MARKETPLACE,
  WBRAND_PLUGIN_HOST_COMMAND,
...
export function createBundledMcpRuntimeConfig(input: {
  ...
  return {
    ...
    env: {
      ...input.env,
      // 桌面打包态 process.execPath 是 WBrand Helper；缺少 Node 模式会误进 Electron main。
...
export function writeOfficialPluginRuntimeManifest(input: OfficialRuntimeManifestInput): void {
  const manifestPath = join(input.rootPath, ".wbrand-plugin", "plugin.json");
  ...
  // 把 seed 流程整个阻断，连带 listWBrandSkills 拉不出 plugin skill。
  if (manifest.mcpServers === undefined) return;
  const mcpServers = asRecord(manifest.mcpServers);
  const hostPrefixArgs = officialPluginHostPrefixArgs();
  if (!hostPrefixArgs) return;

  // 保留对其他历史 official plugin MCP 的通用重写；wbrand-cua 当前是 skill/SDK-only，
  ...
  for (const [serverKey, serverRaw] of Object.entries(mcpServers)) {
    ...
    mcpServer.env = {
      ...mcpServerEnv,
      // 桌面打包态的 process.execPath 是 WBrand Helper。
      ...
      [WBRAND_PLUGIN_ID_ENV_KEY]: `${input.pluginName}@${WBRAND_OFFICIAL_PLUGIN_MARKETPLACE}`,
    };
...
export function officialPluginHostPrefixArgs(): string[] | undefined {
  if (isSeaRuntime()) return [WBRAND_PLUGIN_HOST_COMMAND];

  const entrypoint = process.argv[1];
  if (!entrypoint) return undefined;

  return [...process.execArgv, resolve(entrypoint), WBRAND_PLUGIN_HOST_COMMAND];
}
