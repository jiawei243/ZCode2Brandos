...
import { detectLocale, resolveLocale } from "@wbrand/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@wbrand/contracts";
import type { WBrandAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.WBRAND_MESSAGE_ENABLED === "1" || env.WBRAND_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: WBrandAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: WBrandAppOptions): SupportedLocale | undefined {
