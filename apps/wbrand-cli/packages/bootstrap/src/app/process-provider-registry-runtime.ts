...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    ...
    readonly onBuiltinRefreshResult?: (event: WBrandBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 WBrand Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedWBrandCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[WBRAND_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          wbrandBuiltinFilePath: bundledFile,
          wbrandBuiltinActiveFilePath: paths.wbrandBuiltinFilePath,
          wbrandBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.wbrandBuiltinFilePath),
              "wbrand-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeWBrandEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadWBrandBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: WBRAND_VERSION,
                platform: resolveWBrandBuiltinClientPlatform(),
      ...
      : {}),
    onWBrandBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckWBrandBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.wbrandBuiltinRevision !== account.basedOnWBrandBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  readonly basedOnWBrandBuiltinRevision: string;
  ...
  const basedOnWBrandBuiltinRevision = input.basedOnWBrandBuiltinRevision.trim();
  if (!basedOnWBrandBuiltinRevision) {
