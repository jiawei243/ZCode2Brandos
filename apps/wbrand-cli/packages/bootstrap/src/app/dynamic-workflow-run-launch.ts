...
import {
  ...
  CoreErrorType,
  WBRAND_DWF_CHILD_COMMAND,
  type CreateSessionTaskLinkInput,
  type DynamicWorkflowRunEvent,
  type DynamicWorkflowRunProgressPayload,
  type SessionId,
} from "@wbrand/contracts";
import type { AgentRuntime } from "@wbrand/core";
import { parseModelPickerValue, type ModelSelection } from "@wbrand/shared/model-selection";
import {
  ...
  type RunSettlement,
  type ValidateFn,
} from "@wbrand/dynamic-workflow";
import { runWorkflowScript } from "@wbrand/dynamic-workflow-runtime";
...
export function launchDynamicWorkflowRun(
  ...
  return runWorkflowScript({
    ...
    makeDriver,
    // 入口文件写不进项目 `.wbrand/` 时 harness 回落到 OS 临时目录并报一声——run 照常启动，
 ...
 * `@wbrand/dynamic-workflow` 与 node 内建），既拿不到 contracts 的子命令常量，也不该知道
...
export function dynamicWorkflowChildSpawn(
  ...
  return isSea ? { argsPrefix: [WBRAND_DWF_CHILD_COMMAND] } : undefined;
}
 ...
 * `wbrand-protocol-v4/product-projection.ts` 的 `applyWorkflowRunEvent` 逐种类归约，其
