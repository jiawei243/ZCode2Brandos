...
} from "@wbrand/adapters/storage";
import { createNodeLoggerFactory } from "@wbrand/adapters/logging";
import { createConfig, resolvePath } from "@wbrand/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@wbrand/adapters/exec";
import { createNodeFileSystemAdapter } from "@wbrand/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@wbrand/adapters/http";
import { createJimpImageProcessorAdapter } from "@wbrand/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@wbrand/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@wbrand/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@wbrand/adapters/context";
import { createNodeSkillAdapter } from "@wbrand/adapters/skills";
import { createMcpAdapter } from "@wbrand/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@wbrand/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveWBrandRuntimeEnv } from "@wbrand/shared";
import {
...
import { resolveWBrandCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveWBrandBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadWBrandAgentProfiles } from "../subagents.js";
...
export async function createWBrandApp(options: WBrandAppOptions): Promise<WBrandApp> {
  if (!options?.providerRegistry) {
    throw new Error("createWBrandApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("wbrand").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveWBrandRuntimeEnv(options.env ?? process.env) === "development",
    );
    const wbrandSubagentProfileOutcome = await loadWBrandAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const wbrandSubagentProfiles = wbrandSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: wbrandSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: wbrandSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...wbrandSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          wbrandSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("WBrand runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("WBrand runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).WBRAND_MAILBOX_ROOT ?? "~/.wbrand/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveWBrandBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveWBrandCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("wbrand-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("wbrand-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new WBrandAttachmentFaultError(WBRAND_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("WBrand app startup failed", error, {
