...
import type { BrowserControlPort, Logger, McpServerConfig, TraceContext } from "@wbrand/contracts";
import {
...
export function createNodeReplBrowserBroker(input: {
  ...
  return {
    ...
    token,
    close: async () => {
      // createWBrandApp 是同步工厂，极短生命周期下 close 可能先于 listen 回调；
