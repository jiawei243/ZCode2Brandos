import type { WBrandToolExecResource, BackgroundBashOutputResult } from "@wbrand/shared";
import type { AiSdkModelAdapter } from "@wbrand/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { WBrandModelOption } from "@wbrand/shared";
import type { EffectiveModelSelectionResult } from "@wbrand/shared/model-selection";
export type { WBrandModelOption } from "@wbrand/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { WBrandInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@wbrand/contracts";
...
export type WBrandAppRuntimeConfigInput = AgentRuntimeConfig;

export interface WBrandAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: WBrandAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: WBrandToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@wbrand/contracts").TurnBackgroundAttribution;
...
export interface WBrandPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface WBrandPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: WBrandInstalledPluginData | null;
}
...
export interface WBrandApp {
  ...
  getCurrentModelOption?(): WBrandModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): WBrandModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@wbrand/shared").WBrandSessionSubagentsResult>;
  ...
  listModels(): WBrandModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<WBrandPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<WBrandPluginUninstallResult>;
  /**
...
export interface RunWBrandProtocolAgentOptions {
...
export interface ListWBrandSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
