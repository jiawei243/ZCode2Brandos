...
import type { PluginLoadOutcome } from "@wbrand/contracts";
...
import type {
  WBrandApp,
  WBrandAppOptions,
  WBrandPluginSetResult,
  WBrandPluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listWBrandPlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<WBrandPluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<WBrandPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === WBRAND_OFFICIAL_PLUGIN_MARKETPLACE) {
