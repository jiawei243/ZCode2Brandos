...
import type { SkillRoot } from "@wbrand/contracts";

/**
 * 动态工作流灰度门在 App 装配层的两处减法。
 * 工具面的减法在 core 的 registerBuiltInTools，`/` 目录的减法在 wbrand-protocol/slash-commands.ts；
 * 这里只放「命令展开」和「技能发现」这两项需要 bootstrap 侧常量/路径推导的。
 */

/** 灰度关闭时不允许展开的自定义命令名；`workflow` 由 wbrand-guide 内置插件提供。 */
export const DYNAMIC_WORKFLOW_GATED_COMMAND_NAMES: readonly string[] = ["workflow"];

/** wbrand-guide 插件里工作流编写指南技能的目录名（skills/<dir>/SKILL.md）。 */
 ...
 * （config.json 的 `skill.<path>.enable=false` 走的也是它），而 wbrand-guide 的其余技能
 * （配置指南、各种自诊断）与灰度无关，必须继续可见——整根 root 拿掉会连坐它们。
 *
 * 为什么不按 pluginId 精确匹配 wbrand-guide：插件 id 形如 `<name>@<marketplace>`，随安装
