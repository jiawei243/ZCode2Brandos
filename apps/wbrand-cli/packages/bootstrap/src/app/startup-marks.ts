import type { ConfigResult } from "@wbrand/adapters/config";
import { discoverNodePluginsSync } from "@wbrand/adapters/plugins";
import type { AgentRuntimeConfig } from "@wbrand/core";
import type { Logger, McpServerConfig, PluginLoadOutcome } from "@wbrand/contracts";
...
import type { WBrandAppOptions } from "./types.js";
...
export function resolveStartupPlugins(input: {
  ...
  logger?: Logger;
  options: Pick<WBrandAppOptions, "officialPluginRoots" | "pluginStorageRoot">;
  ...
  const pluginOutcome = discoverNodePluginsSync({
    ...
    // (resolveWBrandPlugins) 和应用启动路径都要传，否则 `/skill skill-creator` 在会话里
    // 报 "Skill not found: skill-creator"，但 `wbrand plugins list` 却显示它是 enabled。
    officialPluginsEnabledByDefault: DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS,
    storageRoot: pluginStorageRoot,
    workingDirectory: input.workingDirectory,
  });
  input.startupTimer.mark("WBrand plugins resolved", {
    context: {
...
export function startAppStartup(input: {
  ...
  input.startupTimer.start("WBrand app startup started", {
    context: {
...
export function markConfigurationLoaded(input: {
  ...
  input.startupTimer.mark("WBrand app configuration loaded", {
    context: {
...
export function markStorageAdaptersInitialized(input: {
  ...
  input.startupTimer.mark("WBrand storage adapters initialized", {
    context: {
...
export function markMcpAdapterInitialized(input: {
  ...
  input.startupTimer.mark("WBrand MCP adapter initialized", {
    context: {
...
export function markRuntimeConstructed(input: {
  ...
  input.startupTimer.mark("WBrand runtime constructed", {
    context: {
      hasInjectedModelAdapter: input.hasInjectedModelAdapter,
...
export function completeAppStartup(input: {
  ...
  input.startupTimer.complete("WBrand app startup completed", {
    context: {
      sessionId: input.sessionId,
      workingDirectory: input.workingDirectory,
    },
