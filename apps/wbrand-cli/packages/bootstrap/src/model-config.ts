...
export function createRuntimeAiSdkModelExecutionConfig(
  ...
  return {
    defaultHeaders: buildCliWBrandSourceHeaders(env, options),
    env,
    ...(network ? { network } : {}),
  };
}
...
function buildCliWBrandSourceHeaders(
  env: EnvRecord,
  ...
  return {
    "HTTP-Referer": resolveRuntimeWBrandEndpointOrigin(env),
    "User-Agent": `WBrand/${appVersion ?? "unknown"}`,
    ...(appVersion ? { "X-WBrand-App-Version": appVersion } : {}),
    "X-Title": `Z Code@${sourceTitle}`,
    "X-Release-Channel": resolveRuntimeWBrandEnv(env),
...
function resolveAppVersionForHeaders(
  ...
  return normalizePrintableHeaderValue(env[WBRAND_APP_VERSION_ENV] ?? options.appVersion);
}
