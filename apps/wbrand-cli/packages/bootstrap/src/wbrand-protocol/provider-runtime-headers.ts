...
import {
  ...
  wbrandProviderRuntimeHeadersResponseSchema,
  type WBrandWorkspaceRef,
} from "@wbrand/shared";
import type { WBrandAppOptions } from "../app/types.js";
import {
...
): NonNullable<WBrandAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          wbrandProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: wbrandProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
      ...
      // wbrand-plan 的账号鉴权材料是每次请求刷新的运行时配置，
