...
import {
  ...
  wbrandUserInputResponseSchema,
  type WBrandPermissionOption,
  type WBrandPermissionResponse,
  type WBrandUserInputQuestion,
...
import type { WBrandProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        wbrandProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → WBrandPermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: WBrandPermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 wbrandPermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        wbrandProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): WBrandUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): WBrandUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): WBrandUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): WBrandUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "wbrand_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
