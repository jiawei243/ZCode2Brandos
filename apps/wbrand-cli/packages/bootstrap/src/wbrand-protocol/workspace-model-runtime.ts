...
import { createInMemorySessionEventStore } from "@wbrand/adapters/storage";
import type { ModelSelection } from "@wbrand/contracts";
import {
  wbrandProviderTestModelConnectivityParamsSchema,
  wbrandWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(wbrandWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(wbrandProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceWBrandApp(
  context: WBrandProtocolAgentServerContext,
  ...
  return context.deps.createWBrandApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createWBrandApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<WBrandProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: WBrandProtocolAgentServerContext,
  record: Pick<WBrandProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
