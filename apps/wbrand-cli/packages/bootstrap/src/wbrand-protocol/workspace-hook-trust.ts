...
import { workspaceHookPolicySchema } from "@wbrand/contracts";
import type { WorkspaceHookPolicyProvider } from "@wbrand/core";
import {
  wbrandWorkspaceHookTrustGrantParamsSchema,
  wbrandWorkspaceHookTrustGrantReasonCodeSchema,
  wbrandWorkspaceHookTrustGrantResultSchema,
  type WBrandWorkspaceHookTrustGrantReasonCode,
  type WBrandWorkspaceHookTrustGrantResult,
} from "@wbrand/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = wbrandWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return wbrandWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): WBrandWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): WBrandWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = wbrandWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
