import type { WBrandMessagePart, WBrandMessageWithParts, WBrandToolState } from "@wbrand/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@wbrand/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): WBrandMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): WBrandMessagePart {
  const base = {
...
function mapToolState(state: ToolState): WBrandToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // WBrand protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
