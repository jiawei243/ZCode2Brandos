...
import {
  ...
  wbrandAutomationCheckTaskBindingResultSchema,
  wbrandAutomationCreateResultSchema,
  wbrandAutomationDeleteResultSchema,
  wbrandAutomationListResultSchema,
  wbrandAutomationUpdateResultSchema,
  wbrandProtocolMethods,
  type WBrandAutomationProtocol,
} from "@wbrand/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => WBrandProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              wbrandProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              wbrandProtocolMethods.automationList,
              {},
              wbrandAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          wbrandProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        wbrandProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        wbrandProtocolMethods.automationDelete,
        { automationId: input.id },
        wbrandAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: WBrandAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: WBrandAutomationProtocol): CronAutomation {
  return {
