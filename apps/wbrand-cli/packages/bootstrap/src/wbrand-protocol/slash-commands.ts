import { BUILTIN_WBRAND_SLASH_COMMAND_HELP_ENTRIES, type WBrandSlashCommand } from "@wbrand/shared";
import {
  listWBrandCustomCommands,
  type ListWBrandCustomCommandsOptions,
} from "../custom-commands.js";
import {
  APP_PROTOCOL_APP_ONLY_BUILTIN_SLASH_COMMANDS,
  APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES,
  isReservedWBrandSlashCommandName,
} from "../slash-command-surface.js";

/**
 * `workflow` 是 wbrand-guide 内置插件的自定义命令，随 CLI 打包，不受用户 commandOverrides
 * 影响；灰度关闭时只能在装配目录时按名剔除。
 */
const DYNAMIC_WORKFLOW_SLASH_COMMAND_NAME = "workflow";

export interface ListProtocolSlashCommandsOptions extends ListWBrandCustomCommandsOptions {
  /**
   * 动态工作流灰度门。**只有显式 false
...
export async function listProtocolSlashCommands(
...
): Promise<WBrandSlashCommand[]> {
  const builtins = listAppProtocolBuiltinSlashCommands();
  let customCommands: Awaited<ReturnType<typeof listWBrandCustomCommands>>["commands"] = [];
  try {
    const outcome = await listWBrandCustomCommands(options);
  ...
  return pinWorkflowAfterGoal([
    ...
    ...customCommands
      ...
      .filter((command) => !isReservedWBrandSlashCommandName(command.name))
 ...
 * `workflow` 是 wbrand-guide 内置插件的自定义命令，
 * 按发现顺序会沉在 custom 段末尾；产品要求它与 `goal` 一样作为「开启一段工作」的入口，
 * 紧随 goal 之后。
 * 只调顺序：来源、去重与 reserved 规则不变；任一方缺席时保持原序。
 */
function pinWorkflowAfterGoal(commands: WBrandSlashCommand[]): WBrandSlashCommand[] {
...
function listAppProtocolBuiltinSlashCommands(): WBrandSlashCommand[] {
  const sharedBuiltins = APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES.flatMap((name) => {
    const command = BUILTIN_WBRAND_SLASH_COMMAND_HELP_ENTRIES.find((entry) => entry.name === name);
    if (!command) return [];
    return [
      {
