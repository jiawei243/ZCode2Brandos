...
import type { McpTelemetryTracker } from "@wbrand/adapters";
import type { WorkspaceHookPolicyProvider } from "@wbrand/core";
import type { AccountProviderConfigSnapshot } from "@wbrand/provider";
import {
  wbrandProtocolErrorCodes,
  type WBrandDeliveryKind,
  type ModelSelection,
  type WBrandModelContextBudgetStrategy,
  type WBrandProtocolMessage,
  type WBrandProtocolMethod,
  type WBrandProtocolNotification,
  type WBrandProtocolRequest,
  type WBrandProtocolRequestId,
  type WBrandProtocolTrace,
  type WBrandSessionMode,
  type WBrandSessionPersistence,
  type WBrandWorkspaceRef,
} from "@wbrand/shared";
import type { WBrandApp, WBrandAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../wbrand-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../wbrand-protocol-v4/v4-gateway.js";
...
export interface WBrandProtocolAgentDependencies {
  createWBrandApp(options?: Omit<WBrandAppOptions, "providerRegistry">): WBrandApp | Promise<WBrandApp>;
  /**
...
export type WBrandProtocolAgentResolvedDependencies = WBrandProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface WBrandProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface WBrandProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface WBrandProtocolSessionRecord {
  app: WBrandApp;
  ...
  protocolEventSequences: Map<string, WBrandProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, WBrandProtocolToolInputTransmissionState>;
...
export interface WBrandProtocolClientRequestOptions {
  signal?: AbortSignal;
  timeoutMs?: number;
...
export interface WBrandProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: WBrandProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: WBrandProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: WBrandProtocolMessage): message is WBrandProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: WBrandProtocolMessage,
): message is WBrandProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: WBrandProtocolMessage,
): message is { id: WBrandProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: WBrandProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): WBrandProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: WBrandDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("WBrand Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "wbrand_protocol.session.require_missing",
...
export function createProtocolLogger(deps: WBrandProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("wbrand").child({
    module: "bootstrap.wbrand_protocol",
  });
}
