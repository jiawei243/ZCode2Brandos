...
import type { WBrandSessionEndedSubagent, WBrandSessionRunningSubagent } from "@wbrand/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: WBrandSessionRunningSubagent[];
  ended: WBrandSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): WBrandSessionRunningSubagent["status"] | undefined {
...
): WBrandSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): WBrandSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: WBrandSessionRunningSubagent[] = [];
  const ended: WBrandSessionEndedSubagent[] = [];
...
function encodeCursor(item: WBrandSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly WBrandSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: WBrandSessionEndedSubagent[]; nextCursor?: string } {
