import {
  ...
  type WBrandActiveToolCall,
  type WBrandContextUsageBreakdownItem,
  type WBrandDeliveryKind,
  type WBrandPendingPermission,
  type WBrandSessionContextUsage,
  type WBrandSessionEvent,
  type WBrandSessionGoal,
  type WBrandSessionGoalVerification,
  type WBrandSessionGoalVerificationTimeline,
  type WBrandSessionGoalStats,
  type WBrandSessionInfo,
  type WBrandSessionKind,
  type WBrandSessionProjection,
  type WBrandSessionRuntimeState,
  type WBrandSessionSettingsState,
  type WBrandSessionStateSnapshot,
  type WBrandSessionTodoGroup,
...
import type { WBrandApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: WBRAND_PROTOCOL_NAME,
      version: WBRAND_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<WBrandApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 wbrand-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("wbrand-artifact://") ? artifactUri : undefined;
}
...
): Promise<WBrandSessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<WBrandApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as WBrandSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is WBrandSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = wbrandApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const wbrandMeta = asRecord(meta.wbrand);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = wbrandApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): WBrandSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<WBrandApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: WBrandDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): WBrandSessionContextUsage | undefined {
...
): WBrandSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: WBrandSessionContextUsage["cache"] | undefined,
): WBrandSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): WBrandSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): WBrandPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 wbrandPendingPermissionSchema（shared/src/wbrand-protocol/index.ts:1139）和
  // wbrandPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 wbrandPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): WBrandActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): WBrandSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): WBrandSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): WBrandSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, WBrandSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, WBrandSessionTodoGroup>,
  input: {
  ...
  const group: WBrandSessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): WBrandSessionEvent["type"] {
  switch (type) {
