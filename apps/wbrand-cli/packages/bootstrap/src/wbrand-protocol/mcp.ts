...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@wbrand/contracts";
import {
  wbrandMcpListParamsSchema,
  wbrandMcpListResultSchema,
  type WBrandMcpListResult,
} from "@wbrand/shared";
import {
...
export async function listMcpServers(
  ...
  const params = parseParams(wbrandMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.wbrand_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.wbrand` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return wbrandMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return wbrandMcpListResultSchema.parse({
