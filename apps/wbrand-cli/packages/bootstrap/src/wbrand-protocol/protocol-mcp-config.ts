import type { McpServerConfig } from "@wbrand/contracts";
import type { WBrandProtocolMcpServer } from "@wbrand/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: WBrandProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // WBrand Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
