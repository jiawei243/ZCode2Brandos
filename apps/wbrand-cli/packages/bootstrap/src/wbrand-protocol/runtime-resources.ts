...
import type { WBrandProtocolAgentDependencies } from "./server-types.js";

/** 持有 app 资源而非业务 session；临时 workspace app、尚未登记的 app 也归此 owner。 */
export class ProtocolRuntimeResources {
  private stopping = false;
  private readonly apps = new Set<WBrandApp>();
  private closePromise?: Promise<void>;

  constructor(private readonly factory: WBrandProtocolAgentDependencies["createWBrandApp"]) {}

  readonly create: WBrandProtocolAgentDependencies["createWBrandApp"] = async (options) => {
    this.assertServing();
  ...
  assertServing(): void {
    if (this.stopping) throw new Error("WBrand Protocol runtime is stopping");
  }
  ...
  private async stopApp(app: WBrandApp): Promise<void> {
    try {
