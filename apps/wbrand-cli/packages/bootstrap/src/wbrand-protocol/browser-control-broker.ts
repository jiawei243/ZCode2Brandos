...
import type { BrowserControlPort, TraceContext } from "@wbrand/contracts";
import {
  wbrandBrowserExecuteResultSchema,
  wbrandBrowserListResultSchema,
 ...
 * WBrand Protocol 的 interaction/browserExecute 反向请求，由 app（host→main WebContentsView/CDP）
 * 执行并返回结果。与 permission broker 并列注入（server-operations 的 createWorkspaceWBrandApp options）。
 */
...
): WBrandProtocolClientRequestOptions {
  return {
    ...(signal ? { signal } : {}),
