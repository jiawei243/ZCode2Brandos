/* eslint-disable max-lines -- WBrand Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../wbrand-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@wbrand/adapters/exec";
import { inputIntentMetadata } from "../wbrand-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@wbrand/core";
import {
  CoreErrorType,
  ...
  type UsageStorePort,
  type WorkspaceId,
} from "@wbrand/contracts";
import {
  DEFAULT_WBRAND_MODEL_CONTEXT_BUDGET_STRATEGY,
  WBRAND_SESSION_RUNTIME_PREFERENCES_REQUEST_TIMEOUT_MS,
  wbrandProtocolErrorCodes,
  wbrandProtocolMethods,
  wbrandSessionCancelBackgroundTaskParamsSchema,
  wbrandSessionCompactParamsSchema,
  wbrandSessionCloseParamsSchema,
  wbrandSessionCreateParamsSchema,
  wbrandSessionEventsParamsSchema,
  wbrandSessionForkParamsSchema,
  wbrandSessionGoalParamsSchema,
  wbrandSessionListParamsSchema,
  wbrandSessionMessagesParamsSchema,
  wbrandSessionReadParamsSchema,
  wbrandSessionRuntimePreferencesResultSchema,
  wbrandSessionResumeParamsSchema,
  wbrandSessionSendParamsSchema,
  wbrandSessionSetModeParamsSchema,
  wbrandSessionSetModelParamsSchema,
  wbrandSessionSetThoughtLevelParamsSchema,
  wbrandSessionStopParamsSchema,
  wbrandSessionSubscribeParamsSchema,
  wbrandSessionSubagentsParamsSchema,
  wbrandTaskTokenUsageParamsSchema,
  wbrandUsageStatsParamsSchema,
  wbrandWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type WBrandSessionCreateParams,
  type WBrandDeliveryKind,
  type IntegratedTerminalShellSelection,
  type WBrandSessionRuntimePreferencesScope,
  type WBrandSessionRuntimePreferencesResult,
  type WBrandModelContextBudgetStrategy,
  type WBrandProtocolTrace,
  type WBrandSessionEvent,
  type WBrandSessionHistoryTarget,
  type WBrandSessionResumeParams,
  type WBrandSessionPersistence,
...
import {
  ...
  type WBrandProtocolSessionRecord,
  type WBrandProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceWBrandApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../wbrand-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<WBrandProtocolSessionRecord["app"]["readTarget"]>>>
>;

type WBrandSessionRecordParams = (
  | WBrandSessionCreateParams
  | (WBrandSessionResumeParams & {
      mode?: WBrandSessionCreateParams["mode"];
      model?: WBrandSessionCreateParams["model"];
      parentSessionId?: WBrandSessionCreateParams["parentSessionId"];
      thoughtLevel?: WBrandSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: WBrandSessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: WBrandModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: WBrandProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<WBrandProtocolSessionRecord["app"], "listThoughtLevels">,
...
function getProtocolToolInputTransmissionState(
  record: WBrandProtocolSessionRecord,
  deliveryKind?: WBrandDeliveryKind,
): WBrandProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: WBrandProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const wbrand = asRecord(meta.wbrand);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(wbrand.parentToolUseId) ??
    stringValue(wbrand.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: WBrandSessionEvent,
  toolInputTransmissions: WBrandProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
...
): WBrandSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("WBrand Protocol session event filtered", {
      event: "wbrand_protocol.session_event.filtered",
  ...
  // tool_input_delta 等高频模型中间态。WBrand Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<WBrandSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: WBrandSessionEvent[] = [];
  const toolInputTransmissions: WBrandProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("WBrand Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "wbrand_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<WBrandSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<WBrandProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<WBrandSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: WBrandProtocolSessionRecord;
  sessionId: SessionId;
  createParams: WBrandSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("WBrand Protocol imported history persisted", {
    event: "wbrand_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: WBrandProtocolAgentServerContext,
  rawParams: unknown,
  trace: WBrandProtocolTrace | undefined,
  project: (
    record: WBrandProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(wbrandSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("WBrand Protocol session/create started", {
    event: "wbrand_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("WBrand Protocol session/create skipped unsupported thought level", {
          event: "wbrand_protocol.session_create.thought_level_skipped",
          module: "bootstrap.wbrand_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 WBrand session。
    ...
    context.logger?.info("WBrand Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "wbrand_protocol.session_create.completed",
    ...
    context.logger?.warn("WBrand Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "wbrand_protocol.session_create.failed",
      module: "bootstrap.wbrand_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: WBrandProtocolAgentServerContext,
  params: WBrandSessionResumeParams,
  ...
  context.logger?.debug("WBrand Protocol session resume started", {
    activeBeforeWait,
    event: "wbrand_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("WBrand Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "wbrand_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.wbrand_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      wbrandProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("WBrand Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "wbrand_protocol.session.resume_completed",
    module: "bootstrap.wbrand_protocol",
...
export async function resumeSession(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "wbrand_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "wbrand_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.wbrand_protocol",
      sessionId: session.id,
...
export async function listSessions(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(wbrandSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("WBrand Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "wbrand_protocol.session.persisted_missing",
      module: "bootstrap.wbrand_protocol",
...
export async function getUsageStats(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(wbrandSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("WBrand Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("WBrand Protocol session/send accepted", {
...
export async function compactSession(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("WBrand Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("WBrand Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("WBrand Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("WBrand Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("WBrand Protocol goal paused"));
    }
...
export async function forkSession(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: WBrandProtocolAgentServerContext,
  record: WBrandProtocolSessionRecord,
  fork: Awaited<ReturnType<WBrandProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: WBrandProtocolAgentServerContext,
  parentRecord: WBrandProtocolSessionRecord,
  forkRecord: WBrandProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("WBrand Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("WBrand Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("WBrand Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("WBrand Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("WBrand Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("WBrand Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("WBrand Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("WBrand Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(wbrandSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("WBrand Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(wbrandSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: WBrandProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(wbrandSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: WBrandSessionPersistence,
  expectedPersistence?: WBrandSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(wbrandWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceWBrandApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: WBrandProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: WBrandProtocolAgentServerContext,
  _record: WBrandProtocolSessionRecord,
...
async function resolveForkTarget(
  context: WBrandProtocolAgentServerContext,
  record: WBrandProtocolSessionRecord,
  target: WBrandSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(wbrandTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: WBrandProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: wbrandProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "wbrand_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "wbrand_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // WBrandProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("WBrand Protocol runtime preferences request started", {
    event: "wbrand_protocol.runtime_preferences.request_started",
    module: "bootstrap.wbrand_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      wbrandProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      wbrandSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("WBrand Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "wbrand_protocol.runtime_preferences.response_received",
      module: "bootstrap.wbrand_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "wbrand_protocol.runtime_preferences.request_failed",
      module: "bootstrap.wbrand_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("WBrand Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("WBrand Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "WBrand Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_WBRAND_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: WBrandProtocolAgentServerContext,
  params: WBrandSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: WBrandProtocolTrace,
): Promise<WBrandProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("WBrand Protocol createRecord MCP config", {
    event: "wbrand_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: WBrandProtocolSessionRecord | undefined;
  const app = await createWorkspaceWBrandApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.wbrand/.agents 的 enabled MCP，
      ...
      // WBrand Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // WBrand Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: WBrandProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): WBrandSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isWBrandSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isWBrandSessionMode(
  value: unknown,
): value is NonNullable<WBrandSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("WBrand Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "wbrand_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "wbrand_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "wbrand_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "wbrand_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "wbrand_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: WBrandStateUpdatedNotification = {
    patch,
    reason,
