...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@wbrand/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  wbrandProtocolNotifications,
  wbrandStorageStartupStateSchema,
  type WBrandStorageStartupState,
} from "@wbrand/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: WBrandStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = wbrandStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: wbrandProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { wbrandStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@wbrand/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = wbrandStoragePathReadySchema.parse(JSON.parse(line));
