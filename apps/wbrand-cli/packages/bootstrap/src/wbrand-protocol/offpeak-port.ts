import type { OffPeakPort, OffPeakTaskSummary } from "@wbrand/contracts";
import {
  wbrandOffPeakCreateResultSchema,
  wbrandOffPeakListResultSchema,
  wbrandProtocolMethods,
  type WBrandOffPeakTaskProtocolSnapshot,
} from "@wbrand/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            wbrandProtocolMethods.offPeakList,
            {},
            wbrandOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        wbrandProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: WBrandOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
