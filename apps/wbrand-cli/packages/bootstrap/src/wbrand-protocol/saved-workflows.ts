...
// 解析器与序列化器只从 @wbrand/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.wbrand/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@wbrand/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  WBRAND_WORKFLOWS_RUNS_MAX_LIMIT,
  wbrandWorkflowsDeleteParamsSchema,
  wbrandWorkflowsGetParamsSchema,
  wbrandWorkflowsListParamsSchema,
  wbrandWorkflowsMoveParamsSchema,
  wbrandWorkflowsRunsParamsSchema,
  wbrandWorkflowsUpdateMetaParamsSchema,
  type WBrandSavedWorkflowRun,
  type WBrandSavedWorkflowScope,
  type WBrandWorkflowsDeleteResult,
  type WBrandWorkflowsGetResult,
  type WBrandWorkflowsListResult,
  type WBrandWorkflowsMoveResult,
  type WBrandWorkflowsRunsResult,
  type WBrandWorkflowsUpdateMetaResult,
} from "@wbrand/shared";
import type { JournalStorePort } from "@wbrand/dynamic-workflow";
...
function scopeOf(params: { scope?: WBrandSavedWorkflowScope }): WBrandSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  const params = parseParams(wbrandWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: WBrandProtocolAgentServerContext,
  rawParams: unknown,
): Promise<WBrandWorkflowsGetResult> {
  const params = parseParams(wbrandWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(wbrandWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(wbrandWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(wbrandWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(WBRAND_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: WBrandSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, WBrandSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, WBrandSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: WBrandProtocolAgentServerContext,
  rawParams: unknown,
): Promise<WBrandWorkflowsMoveResult> {
  const params = parseParams(wbrandWorkflowsMoveParamsSchema, rawParams);
