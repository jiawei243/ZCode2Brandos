...
import {
  ...
  wbrandPluginsReferenceCatalogParamsSchema,
  wbrandPluginsResolveSuggestedReferenceParamsSchema,
  type WBrandPluginReferenceCatalogEntry,
  type WBrandPluginDiagnostic as SharedPluginDiagnostic,
  type WBrandPluginsReferenceCatalogResult,
  type WBrandPluginsResolveSuggestedReferenceResult,
} from "@wbrand/shared";
import type { PluginReferenceCatalogEntry } from "@wbrand/contracts";
import { buildPluginReferenceCatalog } from "@wbrand/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(wbrandPluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = WBRAND_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<WBrandPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(wbrandPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 wbrand-plugins-official 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveWBrandPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: wbrandProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateWBrandPluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 wbrand-plugins-official 超时（10000 ms）");
  ...
  const overview = getWBrandPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getWBrandPluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): WBrandPluginReferenceCatalogEntry {
