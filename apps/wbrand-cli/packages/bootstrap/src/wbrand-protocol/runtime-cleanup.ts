import type { Logger, McpPort } from "@wbrand/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@wbrand/adapters/mcp";
import type { SqliteSessionStore } from "@wbrand/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@wbrand/telemetry";
...
import type { WBrandProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<WBrandProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<WBrandProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`WBrand Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `wbrand_protocol.${resource}.shutdown.failed`,
