...
import {
  createWBrandProcessResourceSampler,
  type WBrandProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: WBrandProtocolAgentServer,
  send: (message: WBrandProtocolNotification) => void,
  logger: Logger,
): WBrandProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createWBrandProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: wbrandProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "wbrand_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
