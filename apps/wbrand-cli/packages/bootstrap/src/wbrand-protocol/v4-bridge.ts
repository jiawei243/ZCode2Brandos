...
// - 命令执行 = V4CommandExecutor（wbrand-protocol-v4/commands/，原生直驱 core）；
...
import {
  ...
  type WBrandSessionContextUsage,
  type WBrandWorkspaceRef,
} from "@wbrand/shared";
import { createExternalTurnFaultError } from "@wbrand/core";
import {
  V4_NOTIFICATIONS,
...
import { V4CommandExecutor } from "../wbrand-protocol-v4/commands/executor.js";
import { V4QueuePromotionLeaseUnavailableError } from "../wbrand-protocol-v4/commands/handlers/queue.js";
import { V4CapabilityUnsupportedError } from "../wbrand-protocol-v4/commands/handlers/interaction-background.js";
import {
  buildColdFileChangeSummaries,
  readConversationFileChangesFromEvents,
} from "../wbrand-protocol-v4/cold-file-change-summaries.js";
import {
  loadPersistedConversationMaterialization,
  mergeColdConversationEvents,
} from "../wbrand-protocol-v4/cold-event-merge.js";
import { lookupGlobalCreateSessionCommand } from "../wbrand-protocol-v4/create-session-command-fact.js";
import type { V4CommandCoreHost } from "../wbrand-protocol-v4/commands/types.js";
import type {
  ConversationRowTargetResolution,
  SessionUsageSeed,
} from "../wbrand-protocol-v4/product-projection.js";
import { PersistentCommandIndex } from "../wbrand-protocol-v4/persistent-command-index.js";
import { queueItemIdForCommand } from "../wbrand-protocol-v4/command-inbox.js";
import { resolveStableForkTargetFromTranscript } from "../wbrand-protocol-v4/stable-fork-target.js";
import { shouldAutoDrainV4QueueHead } from "../wbrand-protocol-v4/queue-auto-drain.js";
import { persistAssistantFeedback } from "../wbrand-protocol-v4/assistant-feedback-persistence.js";
import {
...
} from "../wbrand-protocol-v4/persistent-command-facts.js";
import {
  ConversationV4Gateway,
  V4CommandNotImplementedError,
} from "../wbrand-protocol-v4/v4-gateway.js";
import {
...
import { HYDRATION_TRACE_ID } from "../wbrand-protocol-v4/projection-state.js";
...
import type {
  WBrandProtocolAgentServerContext,
...
function sessionUsageSeedFromRuntimeContextUsage(
  contextUsage: WBrandSessionContextUsage | undefined,
...
function cloneModelSelection(
  selection: ReturnType<WBrandProtocolSessionRecord["app"]["runtime"]["getSessionModelSelection"]>,
): ModelSelection | undefined {
...
async function replayDynamicWorkflowRunEvents(
  ...
  try {
    ...
    context.logger?.warn("v4 hydrate dynamic workflow replay failed", {
      ...
      event: "wbrand_protocol.v4.hydrate_workflow_replay_failed",
...
async function resolveConversationBackingRecord(
  context: WBrandProtocolAgentServerContext,
  sessionId: string,
): Promise<WBrandProtocolSessionRecord | undefined> {
...
export function createConversationV4Gateway(
  ...
  const log = createProtocolLogger(context.deps)?.child({
    module: "bootstrap.wbrand_protocol_v4_gateway",
  });
  ...
  const autoDrainV4QueueIfReady = async (record: WBrandProtocolSessionRecord): Promise<void> => {
  ...
  const scheduleAutoDrainRetry = (record: WBrandProtocolSessionRecord): void => {
  ...
  const coreHost: V4CommandCoreHost = {
    ...
    ensureModelReady: (record) =>
      ensureSessionModelAvailableForNextTurn(context, record as WBrandProtocolSessionRecord),
    ...
    afterLegacyStateMutation: async (record, reason) => {
      await afterStateMutation(context, record as WBrandProtocolSessionRecord, reason);
      await autoDrainV4QueueIfReady(record as WBrandProtocolSessionRecord);
    },
  ...
  const loadStoredSessionSummaries = async (
    ...
    try {
        ...
        try {
          ...
          if (claimedCount > 0) {
            context.logger?.info("legacy remote sessions claimed by task-index allowlist", {
              claimedCount,
              event: "wbrand_protocol.v4.sessions_index_legacy_remote_claimed",
              module: "bootstrap.wbrand_protocol",
              workspaceId,
            });
          }
          ...
          context.logger?.warn("legacy remote sessions claim failed; continuing strict load", {
            error: error instanceof Error ? error.message : String(error),
            event: "wbrand_protocol.v4.sessions_index_legacy_remote_claim_failed",
            module: "bootstrap.wbrand_protocol",
            workspaceId,
          });
        }
      ...
      context.logger?.warn("sessions-index stored summaries failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "wbrand_protocol.v4.sessions_index_stored_failed",
        module: "bootstrap.wbrand_protocol",
      });
      return [];
    }
  ...
  return new ConversationV4Gateway({
    ...
    resumePersistedSession: async (
      ...
      if (!persisted) {
        context.logger?.warn("WBrand Protocol v4 cold resume has no persisted session", {
          activeSessionCount: context.sessions.size,
          event: "wbrand_protocol.v4.resume_persisted_missing",
          module: "bootstrap.wbrand_protocol",
          sessionId,
        });
    ...
    loadPersistedEvents: async (sessionId, persistedMessages) => {
      ...
      if (!record) {
        ...
        context.logger?.warn("WBrand Protocol v4 hydrate has no active runtime", {
          activeSessionCount: context.sessions.size,
          event: "wbrand_protocol.v4.hydrate_runtime_missing",
          module: "bootstrap.wbrand_protocol",
                      ...
                      sessionEntries: (input) =>
                        store.sessionEntries!(input).catch((error) => {
                          context.logger?.warn("v4 hydrate session entries read failed", {
                            error: error instanceof Error ? error.message : String(error),
                            event: "wbrand_protocol.v4.hydrate_session_entries_failed",
                            module: "bootstrap.wbrand_protocol",
                          });
                          return [];
                        }),
      ...
      const fileChangeSummariesByMessageId = await buildColdFileChangeSummaries({
        ...
        onArtifactError: (messageId, error) =>
          context.logger?.warn("v4 cold file change artifact read failed", {
            error: error instanceof Error ? error.message : String(error),
            event: "wbrand_protocol.v4.hydrate_file_changes_failed",
            messageId,
            module: "bootstrap.wbrand_protocol",
            sessionId,
          }),
      ...
      for (const diagnostic of merged.diagnostics) {
        const fields = {
          ...diagnostic,
          event: "wbrand_protocol.v4.hydrate_three_source_merge",
          module: "bootstrap.wbrand_protocol",
          sessionId,
        };
        if (
    ...
    onError: (scope, error, errorContext) =>
      context.logger?.warn("WBrand Protocol v4 gateway error", {
        ...errorContext,
        error: error instanceof Error ? error.message : String(error),
        event: "wbrand_protocol.v4.gateway_error",
        module: "bootstrap.wbrand_protocol",
        scope,
      }),
  });
