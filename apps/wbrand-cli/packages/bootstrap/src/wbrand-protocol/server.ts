...
import {
  wbrandPluginsCancelOperationParamsSchema,
  wbrandProtocolMethods,
  wbrandWorkspaceCancelGenerateTextParamsSchema,
  wbrandWorkspaceHookTrustGrantParamsSchema,
} from "@wbrand/shared";
import type { BrowserControlPort } from "@wbrand/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@wbrand/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  WBrandProtocolError,
  WBrandProtocolMessage,
  WBrandProtocolMethod,
  WBrandProtocolNotification,
  WBrandProtocolRequest,
  WBrandProtocolRequestId,
  WBrandProtocolResponse,
} from "@wbrand/shared";
import {
...
} from "../wbrand-protocol-v4/interaction-registry.js";
...
import {
  ...
  parseParams,
  toProtocolError,
  type WBrandProtocolClientRequestOptions,
  type WBrandProtocolAgentDependencies,
  type WBrandProtocolAgentServerContext,
  type WBrandProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@wbrand/contracts";

export type { WBrandProtocolAgentDependencies, WBrandProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type WBrandProtocolOutboundMessage = WBrandProtocolNotification | WBrandProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, WBrandProtocolSessionRecord>;
}): Promise<void> {
...
interface WBrandProtocolPostResponseBatch {
  readonly messages: readonly WBrandProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class WBrandProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<WBrandProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: WBrandProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: WBrandProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    WBrandProtocolRequestId,
    WBrandProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: WBrandProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createWBrandApp);
    const resolvedDeps = {
      ...deps,
      createWBrandApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, WBrandProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: WBrandProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("WBrand Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "wbrand_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: WBrandProtocolRequestId): WBrandProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: WBrandProtocolRequestId): WBrandProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: WBrandProtocolMessage,
  ): Promise<WBrandProtocolError | WBrandProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("WBrand Protocol notification ignored", {
        event: "wbrand_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: WBrandProtocolRequest,
  ): Promise<WBrandProtocolError | WBrandProtocolResponse> {
  ...
  private async dispatchRequest(request: WBrandProtocolRequest) {
      ...
      case wbrandProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: wbrandWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? wbrandWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(wbrandPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(wbrandWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: WBrandProtocolRequestId, result: unknown): WBrandProtocolResponse {
    return { id, result };
  }

  private fail(
    id: WBrandProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): WBrandProtocolError {
  ...
  private requestClient<T>(
    method: WBrandProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: WBrandProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No WBrand Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: WBrandProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: WBrandProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
