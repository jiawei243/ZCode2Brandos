...
import { wbrandProtocolMessageSchema, wbrandProtocolMethods } from "@wbrand/shared";
import type { Logger } from "@wbrand/contracts";

type WBrandProtocolOutgoingMessage =
  | WBrandProtocolError
  | WBrandProtocolNotification
  | WBrandProtocolRequest
  | WBrandProtocolResponse;

type WBrandProtocolMessageHandler = (
  message: WBrandProtocolMessage,
) => Promise<WBrandProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface WBrandProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: WBrandProtocolMessageHandler;
  ...
  ) => readonly WBrandProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: WBrandProtocolRequestId) => {
    readonly messages: readonly WBrandProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class WBrandProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: WBrandProtocolNdjsonConnectionOptions) {
  ...
  send(message: WBrandProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("WBrand Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("WBrand Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): WBrandProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("WBrand Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "wbrand_protocol.parse.failed",
    ...
    const parsed = wbrandProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid WBrand Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: WBrandProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === wbrandProtocolMethods.sessionStop ||
        message.method === wbrandProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: WBrandProtocolMessage): Promise<void> {
    if (this.terminal) return;
