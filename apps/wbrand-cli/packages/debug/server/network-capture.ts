...
const traceHeaderNames = ["x-wbrand-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          WBRAND_HTTP_PROXY: url,
          WBRAND_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.wbrandCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.WBRAND_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.WBRAND_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.WBRAND_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.WBRAND_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.WBRAND_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
