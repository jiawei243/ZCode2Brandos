...
export const CREATE_WORKFLOW_ARGS_WITHOUT_PATH_ERROR =
  "`args` belongs to the `path` source: it carries values for the arguments a script file declares in its `/* wbrand-workflow` block. For a saved workflow pass `saved.args`; an inline `script` declares no arguments, so it takes none.";
...
const CreateWorkflowModelInputSchema = z
  .object({
    name: z
      ...
      .optional()
      .describe(
        "Short display label for this run, in the user's language (\"PR review\", \"代码评审\"). Always pass it for an inline script: it labels the run everywhere and names its draft file under .wbrand/workflow-drafts/. Defaults to the saved workflow's name when running a saved workflow.",
      ),
    script: z
      .string()
      .optional()
     ...
     * 只改一行再把同一个路径交回来。带 `/* wbrand-workflow` 块的文件按保存定义解析（块剥掉、
     * 正文当脚本、`args` 按块里的声明校验）。
     */
    path: z
      .string()
    ...
    args: z
      ...
      .optional()
      .describe(
        "Values for the arguments a `path` file declares in its `/* wbrand-workflow` block. Only with `path`; with `saved` use `saved.args`. Unknown keys, missing required values and type mismatches are rejected before anything runs.",
      ),
    /**
     * run 自己的并发上界。只压低、不抬高：
