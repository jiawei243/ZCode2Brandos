...
export const SAVE_WORKFLOW_SENTINEL_IN_SCRIPT_ERROR =
  "The `script` must be the workflow body only — it already starts with a `/* wbrand-workflow` metadata block. Pass the metadata through the `description` / `whenToUse` / `args` fields instead; the block is written for you.";
...
export const SaveWorkflowInputSchema = z
  .object({
    ...
    script_path: z
      ...
      .optional()
      .describe(
        "The file holding the script body, relative to the working directory or absolute — usually a draft a CreateWorkflow/AmendWorkflow result named. Provide this OR `script`, never both; it saves a working draft without re-emitting it. A `/* wbrand-workflow` block in that file is dropped: the metadata comes from this call's `description` / `whenToUse` / `args`.",
      ),
    // 作用域由**模型**说出，不由系统猜——没有默认值，每次都要判断。判据中性：脚本是否
    // 引用本仓库的东西？是 → project，否 → global。
    scope: SavedWorkflowScopeSchema.describe(
      'Where the workflow is saved. "project" when the script references this repository\'s files, commands, conventions or directory layout; "global" when it depends on nothing in this project and should be available from every project (saved under ~/.wbrand/workflows). Decide every time; there is no default.',
    ),
