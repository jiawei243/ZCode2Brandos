...
export const WBRAND_OFFICIAL_PLUGIN_MARKETPLACE = "wbrand-plugins-official";
export const WBRAND_INLINE_PLUGIN_MARKETPLACE = "inline";
export const WBRAND_PLUGIN_HOST_COMMAND = "__wbrand-plugin-host";
/**
 * 隐藏子命令：dynamic workflow 的沙箱子进程入口（`__wbrand-dwf-child <entry path>`；argv 末位是
 * harness 写好的入口文件路径，payload 不过命令行）。
 *
 * 与 {@link WBRAND_PLUGIN_HOST_COMMAND} 同族、同机制：SEA 单文件二进制不解释 Node CLI 旗标，
 ...
 */
export const WBRAND_DWF_CHILD_COMMAND = "__wbrand-dwf-child";

export function isOfficialMarketplaceId(id: string): boolean {
  return id === WBRAND_OFFICIAL_PLUGIN_MARKETPLACE;
}
...
export interface PluginDiscoverRequest {
  ...
  // 让用户不必先 `wbrand plugins enable` 就能用 (例如纯内容型的 skill-creator)。
