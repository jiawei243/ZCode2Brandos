import type { TuiSelection, TuiSubmitPrompt } from "@wbrand/tui";
...
export function buildLoginSelection(locale?: string): TuiSelection {
  const copy = getWBrandCopy(locale).tui.loginSetup;
  return {
...
export function loginSetupResponse(locale?: string): string {
  return getWBrandCopy(locale).tui.loginSetup.response;
}
