...
import { formatJson, type PresentationSurface } from "@wbrand/core";
import type { RunContext, GlobalOptions } from "@wbrand/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createWBrandApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createWBrandApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createWBrandApp ?? bootstrapModule?.createWBrandApp;
    if (!createApp) throw new Error("WBrand app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareWBrandTelemetryEnv ?? bootstrapModule?.prepareWBrandTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownWBrandTelemetry ?? bootstrapModule?.shutdownWBrandTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.WBRAND_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `wbrand -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveWBrandCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedWBrandSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listWBrandCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadWBrandCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: wbrand hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
