...
import type { PlaywrightChromiumModule } from "@wbrand/adapters/browser";
...
const ASSET_PREFIX = "wbrand-playwright-runtime/";
...
export async function loadCliPlaywrightChromium(): Promise<PlaywrightChromiumModule> {
  ...
  const require = createRequire(join(runtimeDirectory, "wbrand-playwright-loader.cjs"));
...
function cacheBaseDirectory(): string {
  ...
  if (platform() === "darwin" && home) {
    return join(home, "Library", "Caches", "wbrand", "sea-assets");
  }
  if (platform() === "win32") {
    return join(
