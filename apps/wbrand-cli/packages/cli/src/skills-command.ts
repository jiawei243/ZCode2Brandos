...
type BootstrapModule = typeof import("@wbrand/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listWBrandSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: wbrand skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listWBrandSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@wbrand/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectWBrandSkill;
...
function formatHumanSkillInspection(
  inspection: WBrandSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: WBrandSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
