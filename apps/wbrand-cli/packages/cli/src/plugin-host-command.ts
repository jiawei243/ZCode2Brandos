...
import { WBRAND_PLUGIN_HOST_COMMAND } from "@wbrand/contracts/plugins";
import {
  getCapturedWBrandCuaBrokerCredentials,
  WBRAND_CUA_BROKER_SOCKET_ENV_KEY,
  WBRAND_CUA_NODE_REPL_HOST_ENV_KEY,
} from "@wbrand/shared/runtime-env";
import { WBRAND_CUA_OFFICIAL_PLUGIN_ID, WBRAND_PLUGIN_ID_ENV_KEY } from "@wbrand/shared/mcp";
import type { RunContext } from "@wbrand/shared-types";

const HOST_USAGE = `${WBRAND_PLUGIN_HOST_COMMAND} <server-path> [-- <server-arg>...]`;
...
export function isPluginHostInvocation(argv: readonly string[]): boolean {
  return argv[0] === WBRAND_PLUGIN_HOST_COMMAND;
}

// __wbrand-plugin-host 在 agent 子进程里运行 official plugin 的 MCP server（server.js）。
...
// Agent 进程曾收到过 Helper 凭据，不能证明当前传入的 server 就是官方 wbrand-cua：
...
export async function runPluginHostCommand(ctx: RunContext, argv: string[]): Promise<number> {
  ...
  try {
    ...
    const capturedBrokerCredentials = getCapturedWBrandCuaBrokerCredentials();
    ...
    const originalBrokerSocket = process.env[WBRAND_CUA_BROKER_SOCKET_ENV_KEY];
    // shared node_repl 把同一凭据组恢复到环境，由 broker bridge 读取；旧的独立 CUA
    // MCP 不再拥有执行入口。
    process.argv = [process.execPath, serverPath, ...serverArgs];
    if (capturedBrokerCredentials.socket && process.env[WBRAND_CUA_NODE_REPL_HOST_ENV_KEY] === "1") {
      process.env[WBRAND_CUA_BROKER_SOCKET_ENV_KEY] = capturedBrokerCredentials.socket;
    }
    try {
      await module.main();
    } finally {
      ...
      if (originalBrokerSocket === undefined) {
        delete process.env[WBRAND_CUA_BROKER_SOCKET_ENV_KEY];
      } else {
        process.env[WBRAND_CUA_BROKER_SOCKET_ENV_KEY] = originalBrokerSocket;
      }
    }
...
type CapturedBrokerCredentials = ReturnType<typeof getCapturedWBrandCuaBrokerCredentials>;
...
function assertCapturedBrokerLaunchIsAuthorized(credentials: CapturedBrokerCredentials): void {
  ...
  const pluginId = process.env[WBRAND_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase();
  ...
  // 残留读取方只能靠 CLI 自己的 typecheck 发现（根 `pnpm typecheck` 不含 apps/wbrand-cli）。
  ...
  if (
    ...
    pluginId !== WBRAND_CUA_OFFICIAL_PLUGIN_ID ||
    process.env[WBRAND_CUA_NODE_REPL_HOST_ENV_KEY] !== "1"
  ) {
    throw new Error(
      "Captured WBrand CUA broker credentials may only launch the trusted shared node_repl host",
    );
  }
}
