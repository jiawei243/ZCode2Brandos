...
import {
  ...
  normalizeWBrandRuntimeEnv,
  sanitizeWBrandRuntimeEnv,
  sanitizeWBrandRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeWBrandRuntimeEnv(env),
    ...buildWBrandToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildWBrandToolEnvPassthroughEnv(env);
  sanitizeWBrandRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[WBRAND_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): WBrandRuntimeEnv {
  const explicit = normalizeWBrandRuntimeEnv(env[WBRAND_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.WBRAND_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.WBRAND_BETA === "1" || env.WBRAND_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])wbrand-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.WBRAND_STORAGE_DIR = join(homedir(), ".wbrand-beta");
}
