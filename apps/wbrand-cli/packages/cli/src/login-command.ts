...
export async function runLoginCommand(
  ...
  try {
    ...
    if (args.length > 1 || (providerId !== "zai" && providerId !== "bigmodel")) {
      throw new Error("Usage: wbrand login [zai|bigmodel] [--no-browser]");
    }
    ...
    const login = deps.loginWBrandCli ?? (await loadBootstrapModule()).loginWBrandCli;
...
export async function runLogoutCommand(
  ...
  try {
    ...
    const logout = deps.logoutWBrandCli ?? (await loadBootstrapModule()).logoutWBrandCli;
