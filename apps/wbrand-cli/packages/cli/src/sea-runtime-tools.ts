...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@wbrand/shared/runtime-tool-runtime";
...
const assetPrefix = "wbrand-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".wbrand-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.WBRAND_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".wbrand");
