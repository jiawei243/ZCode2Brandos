...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listWBrandCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: wbrand commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listWBrandCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@wbrand/bootstrap"));
  return (await bootstrap()).inspectWBrandCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: WBrandCustomCommandInspection,
  options: GlobalOptions,
): string {
