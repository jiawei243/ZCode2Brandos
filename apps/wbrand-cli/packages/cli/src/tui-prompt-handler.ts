import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@wbrand/adapters/config";
import type { SessionEvent } from "@wbrand/contracts";
import type { WBrandAppOptions } from "@wbrand/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@wbrand/i18n";
import type { TuiRequestPermission } from "@wbrand/tui";
import type { GlobalOptions } from "@wbrand/shared-types";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createWBrandApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<WBrandAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createWBrandApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createWBrandApp"]>>>;
    try {
