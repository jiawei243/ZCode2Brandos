...
// 品牌类型（SessionId）和 createWBrandApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createWBrandApp ?? bootstrapModule?.createWBrandApp;
  if (!createAppFactory) throw new Error("WBrand app factory is unavailable.");
  const prepareTelemetry =
    deps.prepareWBrandTelemetryEnv ?? bootstrapModule?.prepareWBrandTelemetryEnv;
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownWBrandTelemetry ?? bootstrapModule?.shutdownWBrandTelemetry;
  }
