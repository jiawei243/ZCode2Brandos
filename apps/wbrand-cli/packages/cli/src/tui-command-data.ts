import type { TuiSessionMetadata } from "@wbrand/tui";
...
export async function listSkillsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listWBrandSkillsForTui = deps.listSkills ?? (await loadBootstrapModule()).listWBrandSkills;
  return await listWBrandSkillsForTui({
    env,
    logger: deps.logger,
    workingDirectory,
  });
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listWBrandSessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listWBrandSessions;
  const sessions = await listWBrandSessionsForTui({
