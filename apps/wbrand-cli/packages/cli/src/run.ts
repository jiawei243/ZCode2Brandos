...
import { createNodeLoggerFactory } from "@wbrand/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@wbrand/core";
import { color, formatJson, supportsColor } from "@wbrand/core";
import { getWBrandCopy, isUiLocale, type UiLocale } from "@wbrand/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@wbrand/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getWBrandCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "wbrand_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("wbrand doctor", colors)}\n`);
...
const runWBrandProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 WBrand agent transport closed。
    ...
    const runProtocolAgent =
      deps.runWBrandProtocolAgent ?? (await loadBootstrapModule()).runWBrandProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("wbrand").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runWBrandProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
