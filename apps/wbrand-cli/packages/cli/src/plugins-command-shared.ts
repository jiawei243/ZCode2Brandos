...
import type { listWBrandPlugins } from "@wbrand/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@wbrand/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@wbrand/bootstrap");
export type PluginListOutcome = ReturnType<typeof listWBrandPlugins>;
 ...
 * 避免 `wbrand plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addWBrandPluginMarketplace",
  getPluginsOverview: "getWBrandPluginsOverview",
  installPlugin: "installWBrandMarketplacePlugin",
  listPlugins: "listWBrandPlugins",
  removeMarketplace: "removeWBrandPluginMarketplace",
  setPluginEnabled: "setWBrandPluginEnabled",
  uninstallPlugin: "uninstallWBrandMarketplacePlugin",
  updateMarketplace: "updateWBrandPluginMarketplace",
  updatePlugin: "updateWBrandMarketplacePlugin",
  validatePluginPath: "validateWBrandPluginPath",
...
/** `wbrand plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@wbrand/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by wbrand. Use: user, project");
  }
