...
import { createDenyPermissionBroker } from "@wbrand/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  WBrandAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `wbrandSessionEventTypeSchema`（`shared/src/wbrand-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `wbrandSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createWBrandApp` 注入点边界）。
 *
