...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@wbrand/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.WBRAND_STORAGE_DIR?.trim() || join(homedir(), ".wbrand");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "wbrand-clipboard-"));
