...
import type { SetWBrandPluginEnabledResult, WBrandPluginInstallData } from "@wbrand/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: wbrand plugins <command> [options]
...
Scopes: user (default), project. \`wbrand plugin\` is an alias of \`wbrand plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart wbrand to apply.\n`,
  );
...
function reportInstallOutcome(
  ctx: RunContext,
  options: GlobalOptions,
  result: WBrandPluginInstallData,
  pluginId: string,
): number {
  const failed = hasErrors(result.diagnostics);
  if (options.json) {
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetWBrandPluginEnabledResult; error?: string }> = [];
