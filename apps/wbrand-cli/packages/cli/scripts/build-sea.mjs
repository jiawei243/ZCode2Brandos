...
const cliBundle = resolve(dist, "wbrand.cjs");
const seaBlobForTarget = (target) => resolve(dist, `wbrand-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "wbrand-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/wbrand.cjs",
        output: `dist/wbrand-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "wbrand-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        WBRAND_STORAGE_DIR: storageRoot,
      };
      delete env.WBRAND_BFS_BINARY;
      delete env.WBRAND_RG_BINARY;
      delete env.WBRAND_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/wbrand.cjs. Run `pnpm build` first.");
  }
