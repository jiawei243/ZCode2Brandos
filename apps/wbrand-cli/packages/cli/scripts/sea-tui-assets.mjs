...
export const seaTuiAssetPrefix = "wbrand-tui-runtime/";
...
const workspacePackageParentDirectories = (root) => {
  ...
  return [
    ...
    // TUI runtime 闭包会经由 @wbrand/contracts 依赖仓库根的 @wbrand/shared；
    // 只扫描 apps/wbrand-cli 子 workspace 会把这个合法 workspace 误判为缺失。
    resolve(repositoryRoot, "packages"),
  ];
};

const runtimePackageNames = async ({ root, target, workspacePackageDirectories }) => {
  const tuiDirectory = workspacePackageDirectories.get("@wbrand/tui");
  if (!tuiDirectory) {
    throw new Error("Missing @wbrand/tui workspace package.");
  }
  const queue = [
    { fromDirectory: tuiDirectory, packageName: "@wbrand/tui" },
  ...
  while (queue.length > 0) {
    ...
    const assetPackagePath = placeRuntimePackage({
      ...
      packageDirectory,
      placements,
      fromAssetPath: entry.fromAssetPath ?? "node_modules/@wbrand/tui",
    });
    ...
    // SEA 缓存目录无法解析 pnpm workspace 链接；旧的 @wbrand 白名单漏掉 i18n
