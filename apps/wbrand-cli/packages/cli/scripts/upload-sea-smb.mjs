...
export const DEFAULT_DESTINATION_ROOT = resolve(DEFAULT_MOUNT_ROOT, "wbrand", "deps");
export const RELEASE_DIRECTORY_PREFIX = "wbrand-cli-";
...
const usage = `Usage:
  ...
  node packages/cli/scripts/upload-sea-smb.mjs --dest-root /Volumes/shared/wbrand/deps
...
Options:
  ...
  --dest-root <path>  Mounted SMB release root. Defaults to /Volumes/shared/wbrand/deps.
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing SEA binaries in ${distDir}: ${missing.join(", ")}. ` +
        "Run `pnpm --filter @wbrand/cli build:sea` first.",
    );
  }
