...
import type { SkillRoot, SkillSource } from "@wbrand/contracts";
...
export async function resolveDefaultSkillRoots(
  ...
  const includeWbrand = options.includeWbrandSkills ?? true;
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.wbrand` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.wbrand` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, WBRAND_DIR, SKILLS_DIR), scope, "wbrand", nextPriority()),
