...
import { acquireFileLock } from "@wbrand/shared/node";
import { WBRAND_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@wbrand/shared";
import type { SharedWBrandCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === WBRAND_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
