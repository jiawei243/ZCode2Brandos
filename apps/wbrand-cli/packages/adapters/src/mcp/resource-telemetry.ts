...
import { WBRAND_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type WBrandMcpResourceSample } from "@wbrand/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: WBrandMcpResourceSample["arch"];
  platform: WBrandMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: WBrandMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const intervalMs =
        previousAt === undefined
          ? WBRAND_MCP_RESOURCE_SAMPLE_INTERVAL_MS
          : Math.max(1, sampledAt - previousAt);
      const next = new Map<string, Map<number, number>>();
      const groups = new Map<string, WBrandMcpResourceSample>();
