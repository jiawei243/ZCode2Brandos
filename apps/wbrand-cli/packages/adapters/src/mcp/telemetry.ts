...
import type { WBrandMcpTelemetryEvent } from "@wbrand/shared";
import {
...
type McpTelemetryEvent = WBrandMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: WBrandMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: WBrandMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as WBrandMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as WBrandMcpTelemetryEvent["platform"]);
