...
import { sanitizeWBrandRuntimeEnv } from "@wbrand/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeWBrandRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.wbrand/server/node 启动，但登录环境 PATH 不含该目录，
