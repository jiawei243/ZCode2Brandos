...
import type { CustomCommandRoot, CustomCommandSource } from "@wbrand/contracts";

const COMMANDS_DIR = "commands";
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const PRIORITY_STEP = 10;
const WBRAND_DIR = ".wbrand";
...
export async function resolveDefaultCustomCommandRoots(
  ...
  const includeWbrand = options.includeWbrandCommands ?? true;
...
function commandRootsForBase(
  ...
  // 合并而不是 fallback：兼容 `.agents` 命令和原生 `.wbrand` 命令需要同时可见。
  // 同一级别 `.wbrand` 先扫描，命令同名时仍按“先到先赢”处理。
  return [
    root(join(baseDirectory, WBRAND_DIR, COMMANDS_DIR), scope, "wbrand", nextPriority()),
