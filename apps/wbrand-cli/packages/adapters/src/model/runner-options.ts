...
import type { ModelToolChoice } from "@wbrand/contracts";
...
/** wbrand-plan 业务码常只出现在 finish chunk 的 response.body，流式路径需显式开启。 */
...
export function createGenerateTextOptions(input: {
  ...
  return removeUndefined({
    ...
    // WBrand owns system-message construction in core/context. Keep AI SDK from
...
export function createStreamTextOptions(input: {
  ...
  return removeUndefined({
    ...
    // wbrand-plan 的业务码可能只在流式响应尾部 body 里，需保留 responseBody 供错误分类读取。
