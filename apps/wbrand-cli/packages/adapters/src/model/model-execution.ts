...
import type { RegistryProviderConfig } from "@wbrand/provider";
import { withOpenRouterAttributionHeaders } from "@wbrand/shared";
...
export function createProviderBusinessErrorFetch(
  ...
  return async (input, init) => {
    ...
    // wbrand-plan 安全校验拒绝（3007）等场景返回 HTTP 403 + JSON，但未必带
...
/**
 * 模型请求出口：官方 Coding Plan 端点经 WBrand 平台网关发送（做套餐权益校验等平台侧处理），
...
/** 从 HTTP JSON body 解析 wbrand-plan 等业务错误（供 failure-classifier 在 APICallError 路径复用）。 */
export function readProviderBusinessFailureFromBody(body: unknown):
  ...
  const providerCode =
    ...
    // 二次包装后的外层 code 是 WBrand 自己的 PROVIDER_BUSINESS_ERROR，
