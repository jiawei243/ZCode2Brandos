...
import type { McpServerConfig, RuntimeConfigPatch } from "@wbrand/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 WBrandConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".wbrand" ? dirname(configDirectory) : configDirectory;
}
