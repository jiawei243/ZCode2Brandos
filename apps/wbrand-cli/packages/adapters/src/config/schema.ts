/* eslint-disable max-lines -- wbrand-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
import type { RuntimeConfigPatch } from "@wbrand/contracts";
...
export const LEGACY_CUA_PLUGIN_ID = "wbrand-cua@wbrand-plugins-official";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@wbrand-plugins-official";
...
export const WBrandConfigFileSchema = z
  .object({
...
export type WBrandConfigFile = z.infer<typeof WBrandConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. WBrand does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；WBrand 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = WBrandConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: WBrandConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: WBrandConfigFile["plugins"],
...
function parseSkillsRuntimeConfig(
  skills: WBrandConfigFile["skills"],
