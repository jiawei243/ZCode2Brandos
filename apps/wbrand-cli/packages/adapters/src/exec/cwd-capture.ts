...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `wbrand-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__wbrand_status=$?",
    `if [ "$__wbrand_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__wbrand_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__wbrand_status=%ERRORLEVEL%"',
    `if "%__wbrand_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__wbrand_status%",
  ].join("\r\n");
}
