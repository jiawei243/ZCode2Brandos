...
import { windowsPathToGitBashPath } from "@wbrand/contracts";
import type { ExecutionShellDialect } from "@wbrand/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_WBRAND_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_WBRAND_SHELL_INIT_SNAPSHOT",
  ];
}
