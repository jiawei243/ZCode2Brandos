/*
 * Plugin `.mcp.json` 里 `auth: { type: "wbrand_official" }` 的严格解析与 provenance 生成。
 *
 * 单独成文件而不是留在 mcp.ts：官方鉴权的解析规则覆盖 http 与 stdio 两种传输方式，
 * 且与模板变量解析、传输层字段解析没有耦合——放一起只会让 mcp.ts 继续膨胀（它已到 max-lines 上限）。
 */
import type { McpOfficialProvenance, WBrandOfficialMcpAuthConfig } from "@wbrand/contracts";
 ...
 */
export function parseWBrandOfficialAuth(
  value: unknown,
  mcpKey: string,
): WBrandOfficialMcpAuthConfig | undefined {
  if (value === undefined) return undefined;
  if (!isRecord(value)) {
    throw new Error(`MCP server ${mcpKey}: auth must be an object`);
  }
  // 精确值匹配，区分大小写；wbrand-official、wbrand_official_auth 等别名一律拒绝。
  if (value.type !== "wbrand_official") {
  ...
  return { type: "wbrand_official", provider: "jwt_token" };
}
