...
import { findOfficialMcpReservedHeaders } from "@wbrand/shared";
import type {
  McpOAuthConfig,
  McpServerConfig,
...
import { WBRAND_PLUGIN_ID_ENV_KEY } from "@wbrand/shared";
import type { LoadedPlugin } from "./types.js";
import { isNotFoundError, isPluginOptionValue, isRecord, resolveInside } from "./helpers.js";
import { buildOfficialProvenance, parseWBrandOfficialAuth } from "./mcp-official-auth.js";
...
function resolveMcpServerConfig(
  ...
  // WBrand 官方市场同时包含随应用装载的 Builtin Plugin 与按需安装的 CDN Plugin；后者运行时
  // source 为 `cache`，因此必须按 marketplace 身份归类，不能只看 loader source。
  const source: McpServerRuntimeSource = {
    kind: context.loaded.marketplace === WBRAND_OFFICIAL_PLUGIN_MARKETPLACE ? "builtin" : "plugin",
  };

  // wbrand_official 允许 http 与 stdio，sse 出现即禁用该 MCP，不静默忽略——静默会让配置作者以为鉴权已生效。
  //
  // stdio 之所以能放开：请求由插件进程自己发出，身份头随每条出站协议消息的 _meta 下发
  // （见 adapters/src/mcp/index.ts）。sse 没有对应通道，继续拒绝。
  const officialAuth = parseWBrandOfficialAuth(server.auth, identity.mcpKey);
  ...
  if (type === "stdio") {
    ...
    const env = resolveStringRecord(
      {
        ...
        WBRAND_PLUGIN_DATA: context.dataPath,
        WBRAND_PLUGIN_ROOT: context.loaded.rootPath,
        WBRAND_PROJECT_DIR: context.workingDirectory,
    ...
    // registry，不是可序列化配置），不能让第三方伪造 official wbrand-cua 身份后获得只应定向注入给
    // 内置插件的 broker 凭据。manifest env spread 之后覆写，确保 user/manifest 无法覆盖。
    env[WBRAND_PLUGIN_ID_ENV_KEY] = context.loaded.id;
    return {
      type: "stdio",
