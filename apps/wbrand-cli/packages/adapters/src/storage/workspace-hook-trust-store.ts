...
import type { WorkspaceHookTrustRecord, WorkspaceHookTrustStoreFile } from "@wbrand/contracts";
import {
...
export async function resolveWorkspaceHookTrustStorePath(
  ...
  const userConfigPath = resolve(
    options.userConfigPath ?? join(home, ".wbrand", "cli", "config.json"),
  );
  ...
  const storageRoot = configured ? resolveTrustedUserPath(configured, home) : join(home, ".wbrand");
