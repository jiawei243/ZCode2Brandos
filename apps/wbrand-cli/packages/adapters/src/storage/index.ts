...
// 内存 event store 实现已下沉到 @wbrand/contracts，
// 这里保持 `@wbrand/adapters/storage` 的导出路径不变，避免调用方改 import。
export {
...
export class NodeToolArtifactStore implements ToolArtifactStorePort {
  ...
  async writeToolResultArtifact(
    ...
    return {
      id: artifactId,
      uri: `wbrand-artifact://${encodeURIComponent(request.sessionId)}/${encodeURIComponent(artifactId)}`,
      path,
...
function parseArtifactUri(uri: string): { artifactId: string; sessionId: string } {
  ...
  if (parsed.protocol !== "wbrand-artifact:") {
