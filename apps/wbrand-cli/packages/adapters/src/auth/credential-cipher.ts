...
const CREDENTIAL_SECRET_ENV_KEY = "WBRAND_CREDENTIAL_SECRET";

export interface WBrandCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface WBrandCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createWBrandCredentialCipher(
  options: WBrandCredentialCipherOptions = {},
): WBrandCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedWBrandCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedWBrandCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `wbrand-credential-fallback:${platform()}:${homedir()}:${username}`;
}
