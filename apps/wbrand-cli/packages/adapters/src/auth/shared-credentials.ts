...
import { atomicWritePrivateTextFile, backupCorruptFile, withFileLock } from "@wbrand/shared/node";
import { createWBrandCredentialCipher, type WBrandCredentialCipher } from "./credential-cipher.js";

const WBRAND_DATA_BASE_DIR_ENV_KEY = "WBRAND_DATA_BASE_DIR";
...
export const SHARED_WBRAND_CREDENTIAL_KEYS = {
  ...
  wbrandJwtToken: "wbrandjwttoken",
} as const;

export interface SharedWBrandCredentialStoreOptions {
  baseDir?: string;
  cipher?: WBrandCredentialCipher;
...
export interface SharedWBrandCredentialStore {
...
export function createSharedWBrandCredentialStore(
  options: SharedWBrandCredentialStoreOptions = {},
): SharedWBrandCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedWBrandCredentialsPath(options);
  const cipher = options.cipher ?? createWBrandCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.zaiAccessToken];
        delete rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.wbrandJwtToken];
        if (activeProvider === ZAI_PROVIDER_ID) {
          delete rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.zaiAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.wbrandJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_WBRAND_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedWBrandCredentialSync(
  key: string,
  options: SharedWBrandCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedWBrandCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createWBrandCredentialCipher({ env: options.env });
...
export function resolveSharedWBrandCredentialsPath(
  options: SharedWBrandCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[WBRAND_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".wbrand", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared WBrand credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared WBrand credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
