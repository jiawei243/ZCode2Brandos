...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@wbrand/contracts";
import { LogLevel, LogLevelName } from "@wbrand/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.WBRAND_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.WBRAND_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".wbrand", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeWBrandRuntimeEnv(env[WBRAND_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `wbrand-${formatLocalLogDate(new Date())}.jsonl`;
}
