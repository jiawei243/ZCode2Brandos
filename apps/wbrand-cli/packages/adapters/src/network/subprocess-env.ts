import {
  ...
  WBRAND_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readWBrandToolEnvPassthroughEnv,
} from "@wbrand/shared";
...
const EXPLICIT_CA_SOURCE_KEYS = [WBRAND_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [WBRAND_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, WBRAND_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readWBrandToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const wbrandProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [WBRAND_HTTP_PROXY_ENV_KEY], platform),
  );
  if (wbrandProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, wbrandProxy, platform);
    }
    return;
  }
}
