 ...
 * 本包**只**依赖 `@wbrand/dynamic-workflow` 与 node 内建——证明整条管线 app-free 可跑，
 * 绝不 import `@wbrand/core`/`@wbrand/contracts`/`@wbrand/bootstrap`/`@wbrand/adapters`。
 *
 * 时序（happy path）：
 *
 *   parent                         child(vm)
 *     │  write <cwd>/.wbrand/workflow-runs/<runId>.mjs（payload: lowered+args 内嵌）
 *     │  spawn(node <entry>)
...
export interface RunWorkflowOptions {
  ...
  /**
   * 非致命状况的上报口（今日只有一种：入口文件写不进项目 `.wbrand/`，回落到了 OS 临时目录）。
