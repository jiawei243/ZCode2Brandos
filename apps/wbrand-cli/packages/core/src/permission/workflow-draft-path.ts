...
import { WORKFLOW_DRAFTS_DIR } from "@wbrand/contracts";
...
/** `filePath` 是否落在 `<workingDirectory>/.wbrand/workflow-drafts/` 之内。 */
function isWorkflowDraftPath(input: WorkflowDraftPathInput): boolean {
  ...
  // 只看路径字符串，不看文件系统：`.wbrand/workflow-drafts-other/x.ts` 因此不会因为前缀
