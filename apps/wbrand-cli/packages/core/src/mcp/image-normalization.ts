...
} from "@wbrand/wbrand-cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@wbrand/wbrand-cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[WBRAND_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
