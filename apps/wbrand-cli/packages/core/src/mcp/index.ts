...
import {
  ...
  WBRAND_MCP_ERROR_PRESENTATION_MESSAGE_ONLY,
  WBRAND_MCP_ERROR_PRESENTATION_META_KEY,
  type JsonSchema,
  type McpPort,
...
import { WBRAND_CUA_OFFICIAL_MCP_NAMESPACE_NAME as WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME } from "@wbrand/shared";
import { OFFICIAL_CUA_FRAME_MODEL_CONTENT_PROTECTION } from "@wbrand/wbrand-cua/frame-contract";
...
const WBRAND_CUA_CANONICAL_MODEL_PREFIX = "mcp__computer-use__";
const WBRAND_CUA_PROVIDER_SPELLING_ALIAS_PREFIX = "mcp__computer_use__";
...
function toRegisteredMcpToolName(
  ...
  if (
    ...
    descriptor.serverName === WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME
  ) {
    // adapter 会把官方插件 serverName 命名空间化，descriptor.name 因而是
    // mcp__plugin_wbrand-cua_computer-use__*；直接沿用它会让 provider 约定的 computer-use
    // 工具永远不存在。可信门成立后仅投影模型可见名称，handler 仍用 descriptor 的原路由。
    return `${WBRAND_CUA_CANONICAL_MODEL_PREFIX}${toModelVisibleMcpNamePart(descriptor.toolName)}`;
  }
...
function createMcpToolEntry(
  ...
  const isCuaAppObservation = isWBrandCuaGetAppState(descriptor);
...
function officialCuaProviderSpellingAliases(
  ...
  if (
    !officialCuaAuthorityVerified ||
    descriptor.serverName !== WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME ||
    !name.startsWith(WBRAND_CUA_CANONICAL_MODEL_PREFIX)
  ) {
    return undefined;
  }
  const toolName = name.slice(WBRAND_CUA_CANONICAL_MODEL_PREFIX.length);
  return toolName.length > 0
    ? [`${WBRAND_CUA_PROVIDER_SPELLING_ALIAS_PREFIX}${toolName}`]
    : undefined;
}
...
function createModelFacingMcpInputSchema(
  ...
  // 原因：title 是 WBrand 给用户看的意图摘要，不属于上游 wbrand-cua 参数。只在模型 contract
...
function isWBrandCuaGetAppState(
  ...
  return (
    descriptor.serverName === WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME ||
    serverName === "wbrand-cua" ||
    serverName === "computer-use" ||
    (serverName.includes("wbrand-cua") && serverName.includes("computer-use"))
  );
}
...
function formatMcpToolResult(output: unknown): ModelMessageContent {
  ...
  const errorPresentation = output._meta?.[WBRAND_MCP_ERROR_PRESENTATION_META_KEY];
  return typeof errorPresentation === "string" &&
    errorPresentation === WBRAND_MCP_ERROR_PRESENTATION_MESSAGE_ONLY
    ? content
