...
import { parseOfficialMcpToolError, type OfficialMcpToolErrorCode } from "@wbrand/shared";
import {
  CUA_REQUEST_ACCESS_STATUS_META_KEY,
  cuaRequestAccessStatusSchema,
} from "@wbrand/wbrand-cua/request-access-contract";
 ...
 * 响应来自已校验 origin 的 WBrand 后端。stdio 官方 MCP 与第三方 MCP 塞同样的 payload 一律忽略：
...
function readCuaToolName(toolName: string): string | undefined {
  ...
  // 命名空间形式 `mcp__plugin_wbrand-cua_computer-use__<action>`。
 ...
 * 读取宿主写入的 CUA 目标应用身份。
 *
 * 只认 `wbrand/nodeReplCuaApp`：producer 自己的 `wbrand.cua/app-associations-v1` 也可能出现在
...
function readNodeReplCuaApp(output: Record<string, unknown>): NodeReplCuaAppDisplay | undefined {
  ...
  const parsed = nodeReplCuaAppDisplaySchema.safeParse(
    meta?.[WBRAND_MCP_NODE_REPL_CUA_APP_META_KEY],
  );
