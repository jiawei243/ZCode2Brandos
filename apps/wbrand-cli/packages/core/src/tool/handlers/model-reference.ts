...
import type { ModelCatalogEntry, ModelSelection } from "@wbrand/contracts";
import {
  WBRAND_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@wbrand/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(WBRAND_MODEL_REASONING_SEPARATOR, searchFrom);
