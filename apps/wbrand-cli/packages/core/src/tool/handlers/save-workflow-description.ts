...
import { FACADE_DTS } from "@wbrand/dynamic-workflow";
import { SAVED_WORKFLOW_GLOBAL_DIR, SAVED_WORKFLOW_PROJECT_DIR } from "@wbrand/contracts";
 ...
 * 远比用户宽松。一个会自作主张保存的 agent 会在几轮对话里往 `.wbrand/workflows` 里堆满
...
const FILE_FORMAT = [
  "File format:",
  "- The saved file is valid TypeScript: a `/* wbrand-workflow` block comment carrying YAML metadata, followed by the script verbatim.",
  ...
  "- `script_path` saves a working draft without re-emitting it: pass the file a CreateWorkflow or AmendWorkflow result named instead of `script`, and its body is what gets saved (a `/* wbrand-workflow` block in that file is dropped — the metadata comes from the fields here).",
