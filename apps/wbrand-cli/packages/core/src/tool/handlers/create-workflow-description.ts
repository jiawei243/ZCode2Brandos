...
import { SAVED_WORKFLOW_PROJECT_DIR, WORKFLOW_DRAFTS_DIR } from "@wbrand/contracts";
...
const SOURCES = [
  ...
  `- \`path\`: a script file on disk, relative to the working directory or absolute — normally the file a previous result named. Pass \`args\` alongside it when the file declares them in a \`/* wbrand-workflow\` block.`,
  "",
