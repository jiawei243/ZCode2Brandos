...
import type { SessionId } from "@wbrand/contracts";
...
/**
 * 每个 wbrand session 一个持久 NodeReplSession，跨多次 js 调用保持内存状态（globalThis）。
...
function isBrowserSurfaceSideEffect(command: import("@wbrand/contracts").BrowserCommand): boolean {
...
function isAutoScreenshotTriggerCommand(
  command: import("@wbrand/contracts").BrowserCommand,
): boolean {
  if (
