import { CompactTrigger } from "@wbrand/contracts";
import { ESTIMATED_TOKEN_CHAR_DIVISOR } from "@wbrand/shared";
import type {
...
import { modelMessageContentBlockToText, modelMessageContentToText } from "@wbrand/contracts";
...
function stringifyToolCallInputForTokenEstimate(input: unknown): string {
  try {
    ...
    // WBrand 的模型输入仍可能包含未知内容；异常输入不能让本地预算估算中断 compact。
