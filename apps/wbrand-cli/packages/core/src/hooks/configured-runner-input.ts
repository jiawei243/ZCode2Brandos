...
import { resolveWorkspaceHookTimeoutMs } from "@wbrand/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "wbrand-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 WBrand camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    WBRAND_PROJECT_DIR: input.cwd || workingDirectory,
    WBRAND_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      WBRAND_PLUGIN_DATA: plugin.dataPath,
      WBRAND_PLUGIN_ID: plugin.id,
      WBRAND_PLUGIN_NAME: plugin.name,
      WBRAND_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.WBRAND_PLUGIN_DATA = plugin.dataPath;
    replacements.WBRAND_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|WBRAND_PLUGIN_DATA|WBRAND_PLUGIN_ROOT|WBRAND_PROJECT_DIR|WBRAND_SESSION_ID|WBRAND_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "WBRAND_SKILL_DIR") {
