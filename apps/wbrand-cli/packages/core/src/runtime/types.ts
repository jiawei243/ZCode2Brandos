...
import type { WBrandProviderAccountAccess } from "@wbrand/shared";
...
export interface AgentRuntimeConfig {
  ...
  runtimeFeatures?: {
     ...
     * 由 bootstrap 根据 WBrand 官方插件启停推导，不由普通插件 manifest 自声明。
     */
    nodeRepl?: boolean;
    /**
  ...
  /**
   * 动态工作流灰度门：Host 判定后经
   * WBrand Protocol 下发，runtime 只消费。**缺席即开启**——TUI、headless `-p` 与
  ...
  agentName?: string; // Default: "wbrand-agent"
...
export interface ProviderRuntimeHeadersPort {
  ...
  refreshBeforeModelRequest(input: {
    accountAccess?: WBrandProviderAccountAccess;
    abortSignal?: AbortSignal;
    modelId: string;
    providerId: string;
...
export interface ExecuteToolsOptions {
  ...
  subagentModelOverride?: import("@wbrand/contracts").SubagentRunOptions["modelOverride"];
  model?: Model;
