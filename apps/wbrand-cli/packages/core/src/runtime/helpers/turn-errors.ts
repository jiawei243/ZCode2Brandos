...
const EXTERNAL_TURN_FAULT_MARKER = "wbrand.externalTurnFault";

interface ExternalTurnFaultError extends Error {
  code: string;
  wbrandTurnFault: typeof EXTERNAL_TURN_FAULT_MARKER;
}
...
export function createExternalTurnFaultError(code: string, message = code): Error {
  return Object.assign(new Error(message), {
    code,
    wbrandTurnFault: EXTERNAL_TURN_FAULT_MARKER as typeof EXTERNAL_TURN_FAULT_MARKER,
...
function findExternalTurnFault(error: unknown): ExternalTurnFaultError | null {
  ...
  for (let depth = 0; depth <= 6; depth += 1) {
    ...
    if (
      record.wbrandTurnFault === EXTERNAL_TURN_FAULT_MARKER &&
