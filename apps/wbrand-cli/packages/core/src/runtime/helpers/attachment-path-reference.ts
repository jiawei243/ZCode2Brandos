...
export function isDataOrArtifactUrl(content: string): boolean {
  return content.startsWith("data:") || content.startsWith("wbrand-artifact://");
}
...
function formatPathReferenceReason(reason: PathReferenceReason): string {
  ...
  if (reason === "video_too_large") return "the video is larger than the WBrand video input limit";
