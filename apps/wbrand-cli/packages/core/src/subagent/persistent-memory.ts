...
import type { FileSystemPort, Logger, TraceContext } from "@wbrand/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".wbrand", "agent-memory", agentKey)
    : join(workspace, ".wbrand", "agent-memory-local", agentKey);
}
