...
import {
  WBRAND_CUA_OFFICIAL_MCP_NAMESPACE_NAME as WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME,
  WBRAND_CUA_OFFICIAL_PLUGIN_ID,
} from "@wbrand/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@wbrand/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === WBRAND_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === WBRAND_CUA_OFFICIAL_PLUGIN_ID,
  );
