import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@wbrand/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getWBrandCopy(result.locale ?? input.fallback.locale).tui;
