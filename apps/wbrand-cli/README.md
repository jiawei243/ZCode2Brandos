...
- `npm run build` produces `dist/wbrand.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
wbrand plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.wbrand/cli/plugins`:
...
- `marketplaces/wbrand-plugins-official/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and WBrand Guide content plugins are default-enabled and appear as `browser-use@wbrand-plugins-official`, `document-skills@wbrand-plugins-official`, `skill-creator@wbrand-plugins-official`, and `wbrand-guide@wbrand-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@wbrand-plugins-official`, `android-emulator@wbrand-plugins-official`, and `restore-legacy-sessions@wbrand-plugins-official`, are discovered by wbrand but stay disabled until the user enables them.

```sh
wbrand plugins list
wbrand plugins enable ios-simulator
wbrand plugins disable browser-use
wbrand plugins enable restore-legacy-sessions
wbrand plugins disable ios-simulator
```
...
MCP config can live directly in `.wbrand-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current wbrand plugin surface:
...
Example `.wbrand-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${WBRAND_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${WBRAND_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${WBRAND_PLUGIN_DATA}",
...
Only environment variables with the `WBRAND_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.

### Recommended Layout

```txt
my-plugin/
  .wbrand-plugin/plugin.json
  .mcp.json
  skills/
    my-skill/SKILL.md
  commands/
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting wbrand-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

wbrand reads MCP servers from the main JSON config. The default user config path is `~/.wbrand/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits wbrand's environment plus any `env` overrides.
...
wbrand reads hooks from the same main JSON config file as MCP, usually `~/.wbrand/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
