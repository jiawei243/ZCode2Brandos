...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "wbrand-native-search-build-"));
  ...
  console.log("==> WBrand native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DWBRAND_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DWBRAND_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DWBRAND_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DWBRAND_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DWBRAND_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DWBRAND_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DWBRAND_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DWBRAND_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
