...
function resolveBootstrapWithRemoteEnv(baseEnv = process.env) {
  ...
  return {
    ...
    WBRAND_BOOTSTRAP_WITH_REMOTE: "1",
...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @wbrand/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@wbrand/rpc", "@wbrand/web", "@wbrand/formal-proof"]) {
...
runGit(["submodule", "update", "--init", "--recursive", "apps/wbrand-cli"]);
