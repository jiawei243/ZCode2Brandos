...
export async function generateThirdPartyNotices(root = repositoryRoot) {
  ...
  for (const record of copied) {
    ...
    for (const file of record.modifiedFiles ?? []) {
      ...
      if (!(await readFile(join(root, file), "utf8")).includes("Modified by WBrand:"))
  ...
  const sections = [
    ...
    ...patches.map(
      (item) =>
        `- ${item.package}: modified by WBrand; the changes are recorded in ${item.file} in the source repository.`,
    ),
