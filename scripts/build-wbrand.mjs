...
import {
  ...
  stageTuiRuntime,
} from "./wbrand-distribution/assets.mjs";
import { installScriptSource } from "./wbrand-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "wbrand");
const defaultBaseUrl = (await loadEndpointEnv()).WBRAND_DIST_BASE_URL?.trim() || "";
const packageDirName = "wbrand";
const usage = `Usage:
  pnpm build:wbrand
  node scripts/build-wbrand.mjs --skip-build
  node scripts/build-wbrand.mjs --version 3.3.3-dev.1
  node scripts/build-wbrand.mjs --out-dir dist/wbrand
  node scripts/build-wbrand.mjs --base-url http://host/wbrand/deps/wbrand/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/wbrand.
...
function run(command, args, options = {}) {
  console.log(`[wbrand] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[wbrand] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@wbrand/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@wbrand/server", "build"]);
  run("pnpm", ["--filter", "@wbrand/web", "build"]);
}

async function stageWBrandPackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(root, "apps", "wbrand-cli", "packages", "cli", "dist", "wbrand.cjs");
  const agentProvider = resolve(root, "apps/wbrand-cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "wbrand-builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "wbrand.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/wbrand-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "wbrand.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "wbrand.mjs");
  await cp(resolve(root, "scripts/wbrand-distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure WBRAND_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve WBrand version.");
  }
  ...
  console.log(`[wbrand] release directory: ${outDir}`);
  console.log(`[wbrand] tarball: ${tarball}`);
  console.log(`[wbrand] sha256: ${sha256}`);
}
