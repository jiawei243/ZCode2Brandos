...
export async function prepareNativeSearchTools({
  platform = process.env.WBRAND_TARGET_OS || process.platform,
  arch = process.env.WBRAND_TARGET_ARCH || process.arch,
  outputDir,
  dependenciesDir,
  prebuiltPlan,
} = {}) {
...
async function main() {
  await prepareNativeSearchTools({
    platform: readOption("platform") ?? process.env.WBRAND_TARGET_OS ?? process.platform,
    arch: readOption("arch") ?? process.env.WBRAND_TARGET_ARCH ?? process.arch,
