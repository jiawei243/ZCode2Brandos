...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.WBRAND_ENV !== undefined) value = parsed.WBRAND_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid WBRAND_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.WBRAND_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/wbrand-builtin.json",
  );
  try {
    ...
    const { decodeWBrandBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/wbrand-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeWBrandBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "wbrand-builtin.json"), config.content, "utf8");
  return config;
}
