  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          WBRAND_ENV: requestedEnv,
          WBRAND_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@wbrand/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@wbrand/desktop", "dev:runtime"]);
} catch (error) {
