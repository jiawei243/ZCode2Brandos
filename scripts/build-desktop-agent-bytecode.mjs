...
const defaultEntryPath = join(repoRoot, "apps/wbrand-cli/packages/cli/dist/wbrand.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.WBRAND_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "wbrand.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `wbrand.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `wbrand.bytecode-runtime-${runtimeHash}.cjs`;
