...
const agentEntry = join(root, "agent", "wbrand.cjs");

function usage() {
  return `Usage:
  wbrand --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  wbrand --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      WBRAND_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      WBRAND_AGENT_SERVER_COMMAND: process.execPath,
      WBRAND_SERVER_HOST: options.host,
      WBRAND_SERVER_WORKSPACE: options.workspace,
      WBRAND_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      WBRAND_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("WBrand Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: wbrand --web [options] (wbrand --web --help for details)\n");
    }
