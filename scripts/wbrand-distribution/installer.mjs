...
BASE_URL="\${WBRAND_DIST_BASE_URL:-${baseUrl}}"
INSTALL_DIR="\${WBRAND_DIST_HOME:-$HOME/.wbrand/runtime}"
BIN_DIR="\${WBRAND_DIST_BIN_DIR:-$HOME/.local/bin}"

need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "wbrand install requires $1" >&2
    exit 1
  fi
}
...
cat > "$BIN_DIR/wbrand" <<SH
#!/usr/bin/env sh
exec node "$INSTALL_DIR/current/bin/wbrand.mjs" "\\$@"
SH
chmod +x "$BIN_DIR/wbrand"

echo "WBrand $VERSION installed."
echo "Run: wbrand (TUI) or wbrand --web (Web)"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
